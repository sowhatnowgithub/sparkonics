...
class webSocketManager {
    ...
    this.socket.onmessage = (event) => {
      ...
      if (data["is_thrd"]) {
        ...
        if (thrdFind) {
          ...
          messagePanel
            .querySelector(".broadcasted-message-server")
            ?.parentNode?.insertBefore(
              messageDiv,
  ...
  retryWithBackoff() {
    ...
    const delay = Math.floor(
      Math.random() * Math.pow(2, this.retryCount) * 1000,
    );
...
//const endPoint = "https://dc229bef2caa.ngrok-free.app/ws";
...
thrdCreate.querySelector("button").addEventListener("click", (e) => {
  ...
  if (
    ...
    thrdCreate.querySelector(".thrd-create.error").innerHTML =
      "Please Enter the fields to create a thread";
  } else {
    thrdCreate.querySelector(".thrd-create.error").innerHTML = "";
...
document
  .querySelector(".user-name-input #set-user button")
  .addEventListener("click", (e) => {
    e.preventDefault();
    userSetName = document.querySelector(".set-username")?.value;
    document.querySelector(".set-username").value = "";
    document.querySelector(".thrd-header .userName").innerHTML =
