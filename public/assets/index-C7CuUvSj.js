(function () {
  const s = document.createElement("link").relList;
  if (s && s.supports && s.supports("modulepreload")) return;
  for (const d of document.querySelectorAll('link[rel="modulepreload"]')) f(d);
  new MutationObserver((d) => {
    for (const v of d)
      if (v.type === "childList")
        for (const A of v.addedNodes)
          A.tagName === "LINK" && A.rel === "modulepreload" && f(A);
  }).observe(document, { childList: !0, subtree: !0 });
  function o(d) {
    const v = {};
    return (
      d.integrity && (v.integrity = d.integrity),
      d.referrerPolicy && (v.referrerPolicy = d.referrerPolicy),
      d.crossOrigin === "use-credentials"
        ? (v.credentials = "include")
        : d.crossOrigin === "anonymous"
          ? (v.credentials = "omit")
          : (v.credentials = "same-origin"),
      v
    );
  }
  function f(d) {
    if (d.ep) return;
    d.ep = !0;
    const v = o(d);
    fetch(d.href, v);
  }
})();
var Af = { exports: {} },
  Mu = {};
/**
 * @license React
 * react-jsx-runtime.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var Cd;
function q0() {
  if (Cd) return Mu;
  Cd = 1;
  var i = Symbol.for("react.transitional.element"),
    s = Symbol.for("react.fragment");
  function o(f, d, v) {
    var A = null;
    if (
      (v !== void 0 && (A = "" + v),
      d.key !== void 0 && (A = "" + d.key),
      "key" in d)
    ) {
      v = {};
      for (var b in d) b !== "key" && (v[b] = d[b]);
    } else v = d;
    return (
      (d = v.ref),
      { $$typeof: i, type: f, key: A, ref: d !== void 0 ? d : null, props: v }
    );
  }
  return ((Mu.Fragment = s), (Mu.jsx = o), (Mu.jsxs = o), Mu);
}
var Hd;
function Y0() {
  return (Hd || ((Hd = 1), (Af.exports = q0())), Af.exports);
}
var C = Y0(),
  xf = { exports: {} },
  et = {};
/**
 * @license React
 * react.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var jd;
function w0() {
  if (jd) return et;
  jd = 1;
  var i = Symbol.for("react.transitional.element"),
    s = Symbol.for("react.portal"),
    o = Symbol.for("react.fragment"),
    f = Symbol.for("react.strict_mode"),
    d = Symbol.for("react.profiler"),
    v = Symbol.for("react.consumer"),
    A = Symbol.for("react.context"),
    b = Symbol.for("react.forward_ref"),
    p = Symbol.for("react.suspense"),
    h = Symbol.for("react.memo"),
    O = Symbol.for("react.lazy"),
    j = Symbol.iterator;
  function z(y) {
    return y === null || typeof y != "object"
      ? null
      : ((y = (j && y[j]) || y["@@iterator"]),
        typeof y == "function" ? y : null);
  }
  var G = {
      isMounted: function () {
        return !1;
      },
      enqueueForceUpdate: function () {},
      enqueueReplaceState: function () {},
      enqueueSetState: function () {},
    },
    M = Object.assign,
    Y = {};
  function Q(y, B, X) {
    ((this.props = y),
      (this.context = B),
      (this.refs = Y),
      (this.updater = X || G));
  }
  ((Q.prototype.isReactComponent = {}),
    (Q.prototype.setState = function (y, B) {
      if (typeof y != "object" && typeof y != "function" && y != null)
        throw Error(
          "takes an object of state variables to update or a function which returns an object of state variables.",
        );
      this.updater.enqueueSetState(this, y, B, "setState");
    }),
    (Q.prototype.forceUpdate = function (y) {
      this.updater.enqueueForceUpdate(this, y, "forceUpdate");
    }));
  function q() {}
  q.prototype = Q.prototype;
  function V(y, B, X) {
    ((this.props = y),
      (this.context = B),
      (this.refs = Y),
      (this.updater = X || G));
  }
  var P = (V.prototype = new q());
  ((P.constructor = V), M(P, Q.prototype), (P.isPureReactComponent = !0));
  var ct = Array.isArray,
    I = { H: null, A: null, T: null, S: null, V: null },
    _t = Object.prototype.hasOwnProperty;
  function xt(y, B, X, w, J, ft) {
    return (
      (X = ft.ref),
      { $$typeof: i, type: y, key: B, ref: X !== void 0 ? X : null, props: ft }
    );
  }
  function zt(y, B) {
    return xt(y.type, B, void 0, void 0, void 0, y.props);
  }
  function St(y) {
    return typeof y == "object" && y !== null && y.$$typeof === i;
  }
  function Jt(y) {
    var B = { "=": "=0", ":": "=2" };
    return (
      "$" +
      y.replace(/[=:]/g, function (X) {
        return B[X];
      })
    );
  }
  var oe = /\/+/g;
  function Xt(y, B) {
    return typeof y == "object" && y !== null && y.key != null
      ? Jt("" + y.key)
      : B.toString(36);
  }
  function Tl() {}
  function Al(y) {
    switch (y.status) {
      case "fulfilled":
        return y.value;
      case "rejected":
        throw y.reason;
      default:
        switch (
          (typeof y.status == "string"
            ? y.then(Tl, Tl)
            : ((y.status = "pending"),
              y.then(
                function (B) {
                  y.status === "pending" &&
                    ((y.status = "fulfilled"), (y.value = B));
                },
                function (B) {
                  y.status === "pending" &&
                    ((y.status = "rejected"), (y.reason = B));
                },
              )),
          y.status)
        ) {
          case "fulfilled":
            return y.value;
          case "rejected":
            throw y.reason;
        }
    }
    throw y;
  }
  function Qt(y, B, X, w, J) {
    var ft = typeof y;
    (ft === "undefined" || ft === "boolean") && (y = null);
    var tt = !1;
    if (y === null) tt = !0;
    else
      switch (ft) {
        case "bigint":
        case "string":
        case "number":
          tt = !0;
          break;
        case "object":
          switch (y.$$typeof) {
            case i:
            case s:
              tt = !0;
              break;
            case O:
              return ((tt = y._init), Qt(tt(y._payload), B, X, w, J));
          }
      }
    if (tt)
      return (
        (J = J(y)),
        (tt = w === "" ? "." + Xt(y, 0) : w),
        ct(J)
          ? ((X = ""),
            tt != null && (X = tt.replace(oe, "$&/") + "/"),
            Qt(J, B, X, "", function (Fe) {
              return Fe;
            }))
          : J != null &&
            (St(J) &&
              (J = zt(
                J,
                X +
                  (J.key == null || (y && y.key === J.key)
                    ? ""
                    : ("" + J.key).replace(oe, "$&/") + "/") +
                  tt,
              )),
            B.push(J)),
        1
      );
    tt = 0;
    var ee = w === "" ? "." : w + ":";
    if (ct(y))
      for (var bt = 0; bt < y.length; bt++)
        ((w = y[bt]), (ft = ee + Xt(w, bt)), (tt += Qt(w, B, X, ft, J)));
    else if (((bt = z(y)), typeof bt == "function"))
      for (y = bt.call(y), bt = 0; !(w = y.next()).done; )
        ((w = w.value), (ft = ee + Xt(w, bt++)), (tt += Qt(w, B, X, ft, J)));
    else if (ft === "object") {
      if (typeof y.then == "function") return Qt(Al(y), B, X, w, J);
      throw (
        (B = String(y)),
        Error(
          "Objects are not valid as a React child (found: " +
            (B === "[object Object]"
              ? "object with keys {" + Object.keys(y).join(", ") + "}"
              : B) +
            "). If you meant to render a collection of children, use an array instead.",
        )
      );
    }
    return tt;
  }
  function N(y, B, X) {
    if (y == null) return y;
    var w = [],
      J = 0;
    return (
      Qt(y, w, "", "", function (ft) {
        return B.call(X, ft, J++);
      }),
      w
    );
  }
  function L(y) {
    if (y._status === -1) {
      var B = y._result;
      ((B = B()),
        B.then(
          function (X) {
            (y._status === 0 || y._status === -1) &&
              ((y._status = 1), (y._result = X));
          },
          function (X) {
            (y._status === 0 || y._status === -1) &&
              ((y._status = 2), (y._result = X));
          },
        ),
        y._status === -1 && ((y._status = 0), (y._result = B)));
    }
    if (y._status === 1) return y._result.default;
    throw y._result;
  }
  var W =
    typeof reportError == "function"
      ? reportError
      : function (y) {
          if (
            typeof window == "object" &&
            typeof window.ErrorEvent == "function"
          ) {
            var B = new window.ErrorEvent("error", {
              bubbles: !0,
              cancelable: !0,
              message:
                typeof y == "object" &&
                y !== null &&
                typeof y.message == "string"
                  ? String(y.message)
                  : String(y),
              error: y,
            });
            if (!window.dispatchEvent(B)) return;
          } else if (
            typeof process == "object" &&
            typeof process.emit == "function"
          ) {
            process.emit("uncaughtException", y);
            return;
          }
          console.error(y);
        };
  function yt() {}
  return (
    (et.Children = {
      map: N,
      forEach: function (y, B, X) {
        N(
          y,
          function () {
            B.apply(this, arguments);
          },
          X,
        );
      },
      count: function (y) {
        var B = 0;
        return (
          N(y, function () {
            B++;
          }),
          B
        );
      },
      toArray: function (y) {
        return (
          N(y, function (B) {
            return B;
          }) || []
        );
      },
      only: function (y) {
        if (!St(y))
          throw Error(
            "React.Children.only expected to receive a single React element child.",
          );
        return y;
      },
    }),
    (et.Component = Q),
    (et.Fragment = o),
    (et.Profiler = d),
    (et.PureComponent = V),
    (et.StrictMode = f),
    (et.Suspense = p),
    (et.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = I),
    (et.__COMPILER_RUNTIME = {
      __proto__: null,
      c: function (y) {
        return I.H.useMemoCache(y);
      },
    }),
    (et.cache = function (y) {
      return function () {
        return y.apply(null, arguments);
      };
    }),
    (et.cloneElement = function (y, B, X) {
      if (y == null)
        throw Error(
          "The argument must be a React element, but you passed " + y + ".",
        );
      var w = M({}, y.props),
        J = y.key,
        ft = void 0;
      if (B != null)
        for (tt in (B.ref !== void 0 && (ft = void 0),
        B.key !== void 0 && (J = "" + B.key),
        B))
          !_t.call(B, tt) ||
            tt === "key" ||
            tt === "__self" ||
            tt === "__source" ||
            (tt === "ref" && B.ref === void 0) ||
            (w[tt] = B[tt]);
      var tt = arguments.length - 2;
      if (tt === 1) w.children = X;
      else if (1 < tt) {
        for (var ee = Array(tt), bt = 0; bt < tt; bt++)
          ee[bt] = arguments[bt + 2];
        w.children = ee;
      }
      return xt(y.type, J, void 0, void 0, ft, w);
    }),
    (et.createContext = function (y) {
      return (
        (y = {
          $$typeof: A,
          _currentValue: y,
          _currentValue2: y,
          _threadCount: 0,
          Provider: null,
          Consumer: null,
        }),
        (y.Provider = y),
        (y.Consumer = { $$typeof: v, _context: y }),
        y
      );
    }),
    (et.createElement = function (y, B, X) {
      var w,
        J = {},
        ft = null;
      if (B != null)
        for (w in (B.key !== void 0 && (ft = "" + B.key), B))
          _t.call(B, w) &&
            w !== "key" &&
            w !== "__self" &&
            w !== "__source" &&
            (J[w] = B[w]);
      var tt = arguments.length - 2;
      if (tt === 1) J.children = X;
      else if (1 < tt) {
        for (var ee = Array(tt), bt = 0; bt < tt; bt++)
          ee[bt] = arguments[bt + 2];
        J.children = ee;
      }
      if (y && y.defaultProps)
        for (w in ((tt = y.defaultProps), tt))
          J[w] === void 0 && (J[w] = tt[w]);
      return xt(y, ft, void 0, void 0, null, J);
    }),
    (et.createRef = function () {
      return { current: null };
    }),
    (et.forwardRef = function (y) {
      return { $$typeof: b, render: y };
    }),
    (et.isValidElement = St),
    (et.lazy = function (y) {
      return { $$typeof: O, _payload: { _status: -1, _result: y }, _init: L };
    }),
    (et.memo = function (y, B) {
      return { $$typeof: h, type: y, compare: B === void 0 ? null : B };
    }),
    (et.startTransition = function (y) {
      var B = I.T,
        X = {};
      I.T = X;
      try {
        var w = y(),
          J = I.S;
        (J !== null && J(X, w),
          typeof w == "object" &&
            w !== null &&
            typeof w.then == "function" &&
            w.then(yt, W));
      } catch (ft) {
        W(ft);
      } finally {
        I.T = B;
      }
    }),
    (et.unstable_useCacheRefresh = function () {
      return I.H.useCacheRefresh();
    }),
    (et.use = function (y) {
      return I.H.use(y);
    }),
    (et.useActionState = function (y, B, X) {
      return I.H.useActionState(y, B, X);
    }),
    (et.useCallback = function (y, B) {
      return I.H.useCallback(y, B);
    }),
    (et.useContext = function (y) {
      return I.H.useContext(y);
    }),
    (et.useDebugValue = function () {}),
    (et.useDeferredValue = function (y, B) {
      return I.H.useDeferredValue(y, B);
    }),
    (et.useEffect = function (y, B, X) {
      var w = I.H;
      if (typeof X == "function")
        throw Error(
          "useEffect CRUD overload is not enabled in this build of React.",
        );
      return w.useEffect(y, B);
    }),
    (et.useId = function () {
      return I.H.useId();
    }),
    (et.useImperativeHandle = function (y, B, X) {
      return I.H.useImperativeHandle(y, B, X);
    }),
    (et.useInsertionEffect = function (y, B) {
      return I.H.useInsertionEffect(y, B);
    }),
    (et.useLayoutEffect = function (y, B) {
      return I.H.useLayoutEffect(y, B);
    }),
    (et.useMemo = function (y, B) {
      return I.H.useMemo(y, B);
    }),
    (et.useOptimistic = function (y, B) {
      return I.H.useOptimistic(y, B);
    }),
    (et.useReducer = function (y, B, X) {
      return I.H.useReducer(y, B, X);
    }),
    (et.useRef = function (y) {
      return I.H.useRef(y);
    }),
    (et.useState = function (y) {
      return I.H.useState(y);
    }),
    (et.useSyncExternalStore = function (y, B, X) {
      return I.H.useSyncExternalStore(y, B, X);
    }),
    (et.useTransition = function () {
      return I.H.useTransition();
    }),
    (et.version = "19.1.0"),
    et
  );
}
var Bd;
function Hf() {
  return (Bd || ((Bd = 1), (xf.exports = w0())), xf.exports);
}
var D = Hf(),
  Rf = { exports: {} },
  Du = {},
  Of = { exports: {} },
  zf = {};
/**
 * @license React
 * scheduler.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var qd;
function L0() {
  return (
    qd ||
      ((qd = 1),
      (function (i) {
        function s(N, L) {
          var W = N.length;
          N.push(L);
          t: for (; 0 < W; ) {
            var yt = (W - 1) >>> 1,
              y = N[yt];
            if (0 < d(y, L)) ((N[yt] = L), (N[W] = y), (W = yt));
            else break t;
          }
        }
        function o(N) {
          return N.length === 0 ? null : N[0];
        }
        function f(N) {
          if (N.length === 0) return null;
          var L = N[0],
            W = N.pop();
          if (W !== L) {
            N[0] = W;
            t: for (var yt = 0, y = N.length, B = y >>> 1; yt < B; ) {
              var X = 2 * (yt + 1) - 1,
                w = N[X],
                J = X + 1,
                ft = N[J];
              if (0 > d(w, W))
                J < y && 0 > d(ft, w)
                  ? ((N[yt] = ft), (N[J] = W), (yt = J))
                  : ((N[yt] = w), (N[X] = W), (yt = X));
              else if (J < y && 0 > d(ft, W))
                ((N[yt] = ft), (N[J] = W), (yt = J));
              else break t;
            }
          }
          return L;
        }
        function d(N, L) {
          var W = N.sortIndex - L.sortIndex;
          return W !== 0 ? W : N.id - L.id;
        }
        if (
          ((i.unstable_now = void 0),
          typeof performance == "object" &&
            typeof performance.now == "function")
        ) {
          var v = performance;
          i.unstable_now = function () {
            return v.now();
          };
        } else {
          var A = Date,
            b = A.now();
          i.unstable_now = function () {
            return A.now() - b;
          };
        }
        var p = [],
          h = [],
          O = 1,
          j = null,
          z = 3,
          G = !1,
          M = !1,
          Y = !1,
          Q = !1,
          q = typeof setTimeout == "function" ? setTimeout : null,
          V = typeof clearTimeout == "function" ? clearTimeout : null,
          P = typeof setImmediate < "u" ? setImmediate : null;
        function ct(N) {
          for (var L = o(h); L !== null; ) {
            if (L.callback === null) f(h);
            else if (L.startTime <= N)
              (f(h), (L.sortIndex = L.expirationTime), s(p, L));
            else break;
            L = o(h);
          }
        }
        function I(N) {
          if (((Y = !1), ct(N), !M))
            if (o(p) !== null) ((M = !0), _t || ((_t = !0), Xt()));
            else {
              var L = o(h);
              L !== null && Qt(I, L.startTime - N);
            }
        }
        var _t = !1,
          xt = -1,
          zt = 5,
          St = -1;
        function Jt() {
          return Q ? !0 : !(i.unstable_now() - St < zt);
        }
        function oe() {
          if (((Q = !1), _t)) {
            var N = i.unstable_now();
            St = N;
            var L = !0;
            try {
              t: {
                ((M = !1), Y && ((Y = !1), V(xt), (xt = -1)), (G = !0));
                var W = z;
                try {
                  e: {
                    for (
                      ct(N), j = o(p);
                      j !== null && !(j.expirationTime > N && Jt());

                    ) {
                      var yt = j.callback;
                      if (typeof yt == "function") {
                        ((j.callback = null), (z = j.priorityLevel));
                        var y = yt(j.expirationTime <= N);
                        if (((N = i.unstable_now()), typeof y == "function")) {
                          ((j.callback = y), ct(N), (L = !0));
                          break e;
                        }
                        (j === o(p) && f(p), ct(N));
                      } else f(p);
                      j = o(p);
                    }
                    if (j !== null) L = !0;
                    else {
                      var B = o(h);
                      (B !== null && Qt(I, B.startTime - N), (L = !1));
                    }
                  }
                  break t;
                } finally {
                  ((j = null), (z = W), (G = !1));
                }
                L = void 0;
              }
            } finally {
              L ? Xt() : (_t = !1);
            }
          }
        }
        var Xt;
        if (typeof P == "function")
          Xt = function () {
            P(oe);
          };
        else if (typeof MessageChannel < "u") {
          var Tl = new MessageChannel(),
            Al = Tl.port2;
          ((Tl.port1.onmessage = oe),
            (Xt = function () {
              Al.postMessage(null);
            }));
        } else
          Xt = function () {
            q(oe, 0);
          };
        function Qt(N, L) {
          xt = q(function () {
            N(i.unstable_now());
          }, L);
        }
        ((i.unstable_IdlePriority = 5),
          (i.unstable_ImmediatePriority = 1),
          (i.unstable_LowPriority = 4),
          (i.unstable_NormalPriority = 3),
          (i.unstable_Profiling = null),
          (i.unstable_UserBlockingPriority = 2),
          (i.unstable_cancelCallback = function (N) {
            N.callback = null;
          }),
          (i.unstable_forceFrameRate = function (N) {
            0 > N || 125 < N
              ? console.error(
                  "forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported",
                )
              : (zt = 0 < N ? Math.floor(1e3 / N) : 5);
          }),
          (i.unstable_getCurrentPriorityLevel = function () {
            return z;
          }),
          (i.unstable_next = function (N) {
            switch (z) {
              case 1:
              case 2:
              case 3:
                var L = 3;
                break;
              default:
                L = z;
            }
            var W = z;
            z = L;
            try {
              return N();
            } finally {
              z = W;
            }
          }),
          (i.unstable_requestPaint = function () {
            Q = !0;
          }),
          (i.unstable_runWithPriority = function (N, L) {
            switch (N) {
              case 1:
              case 2:
              case 3:
              case 4:
              case 5:
                break;
              default:
                N = 3;
            }
            var W = z;
            z = N;
            try {
              return L();
            } finally {
              z = W;
            }
          }),
          (i.unstable_scheduleCallback = function (N, L, W) {
            var yt = i.unstable_now();
            switch (
              (typeof W == "object" && W !== null
                ? ((W = W.delay),
                  (W = typeof W == "number" && 0 < W ? yt + W : yt))
                : (W = yt),
              N)
            ) {
              case 1:
                var y = -1;
                break;
              case 2:
                y = 250;
                break;
              case 5:
                y = 1073741823;
                break;
              case 4:
                y = 1e4;
                break;
              default:
                y = 5e3;
            }
            return (
              (y = W + y),
              (N = {
                id: O++,
                callback: L,
                priorityLevel: N,
                startTime: W,
                expirationTime: y,
                sortIndex: -1,
              }),
              W > yt
                ? ((N.sortIndex = W),
                  s(h, N),
                  o(p) === null &&
                    N === o(h) &&
                    (Y ? (V(xt), (xt = -1)) : (Y = !0), Qt(I, W - yt)))
                : ((N.sortIndex = y),
                  s(p, N),
                  M || G || ((M = !0), _t || ((_t = !0), Xt()))),
              N
            );
          }),
          (i.unstable_shouldYield = Jt),
          (i.unstable_wrapCallback = function (N) {
            var L = z;
            return function () {
              var W = z;
              z = L;
              try {
                return N.apply(this, arguments);
              } finally {
                z = W;
              }
            };
          }));
      })(zf)),
    zf
  );
}
var Yd;
function G0() {
  return (Yd || ((Yd = 1), (Of.exports = L0())), Of.exports);
}
var Mf = { exports: {} },
  Kt = {};
/**
 * @license React
 * react-dom.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var wd;
function X0() {
  if (wd) return Kt;
  wd = 1;
  var i = Hf();
  function s(p) {
    var h = "https://react.dev/errors/" + p;
    if (1 < arguments.length) {
      h += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var O = 2; O < arguments.length; O++)
        h += "&args[]=" + encodeURIComponent(arguments[O]);
    }
    return (
      "Minified React error #" +
      p +
      "; visit " +
      h +
      " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
    );
  }
  function o() {}
  var f = {
      d: {
        f: o,
        r: function () {
          throw Error(s(522));
        },
        D: o,
        C: o,
        L: o,
        m: o,
        X: o,
        S: o,
        M: o,
      },
      p: 0,
      findDOMNode: null,
    },
    d = Symbol.for("react.portal");
  function v(p, h, O) {
    var j =
      3 < arguments.length && arguments[3] !== void 0 ? arguments[3] : null;
    return {
      $$typeof: d,
      key: j == null ? null : "" + j,
      children: p,
      containerInfo: h,
      implementation: O,
    };
  }
  var A = i.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
  function b(p, h) {
    if (p === "font") return "";
    if (typeof h == "string") return h === "use-credentials" ? h : "";
  }
  return (
    (Kt.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = f),
    (Kt.createPortal = function (p, h) {
      var O =
        2 < arguments.length && arguments[2] !== void 0 ? arguments[2] : null;
      if (!h || (h.nodeType !== 1 && h.nodeType !== 9 && h.nodeType !== 11))
        throw Error(s(299));
      return v(p, h, null, O);
    }),
    (Kt.flushSync = function (p) {
      var h = A.T,
        O = f.p;
      try {
        if (((A.T = null), (f.p = 2), p)) return p();
      } finally {
        ((A.T = h), (f.p = O), f.d.f());
      }
    }),
    (Kt.preconnect = function (p, h) {
      typeof p == "string" &&
        (h
          ? ((h = h.crossOrigin),
            (h =
              typeof h == "string"
                ? h === "use-credentials"
                  ? h
                  : ""
                : void 0))
          : (h = null),
        f.d.C(p, h));
    }),
    (Kt.prefetchDNS = function (p) {
      typeof p == "string" && f.d.D(p);
    }),
    (Kt.preinit = function (p, h) {
      if (typeof p == "string" && h && typeof h.as == "string") {
        var O = h.as,
          j = b(O, h.crossOrigin),
          z = typeof h.integrity == "string" ? h.integrity : void 0,
          G = typeof h.fetchPriority == "string" ? h.fetchPriority : void 0;
        O === "style"
          ? f.d.S(p, typeof h.precedence == "string" ? h.precedence : void 0, {
              crossOrigin: j,
              integrity: z,
              fetchPriority: G,
            })
          : O === "script" &&
            f.d.X(p, {
              crossOrigin: j,
              integrity: z,
              fetchPriority: G,
              nonce: typeof h.nonce == "string" ? h.nonce : void 0,
            });
      }
    }),
    (Kt.preinitModule = function (p, h) {
      if (typeof p == "string")
        if (typeof h == "object" && h !== null) {
          if (h.as == null || h.as === "script") {
            var O = b(h.as, h.crossOrigin);
            f.d.M(p, {
              crossOrigin: O,
              integrity: typeof h.integrity == "string" ? h.integrity : void 0,
              nonce: typeof h.nonce == "string" ? h.nonce : void 0,
            });
          }
        } else h == null && f.d.M(p);
    }),
    (Kt.preload = function (p, h) {
      if (
        typeof p == "string" &&
        typeof h == "object" &&
        h !== null &&
        typeof h.as == "string"
      ) {
        var O = h.as,
          j = b(O, h.crossOrigin);
        f.d.L(p, O, {
          crossOrigin: j,
          integrity: typeof h.integrity == "string" ? h.integrity : void 0,
          nonce: typeof h.nonce == "string" ? h.nonce : void 0,
          type: typeof h.type == "string" ? h.type : void 0,
          fetchPriority:
            typeof h.fetchPriority == "string" ? h.fetchPriority : void 0,
          referrerPolicy:
            typeof h.referrerPolicy == "string" ? h.referrerPolicy : void 0,
          imageSrcSet:
            typeof h.imageSrcSet == "string" ? h.imageSrcSet : void 0,
          imageSizes: typeof h.imageSizes == "string" ? h.imageSizes : void 0,
          media: typeof h.media == "string" ? h.media : void 0,
        });
      }
    }),
    (Kt.preloadModule = function (p, h) {
      if (typeof p == "string")
        if (h) {
          var O = b(h.as, h.crossOrigin);
          f.d.m(p, {
            as: typeof h.as == "string" && h.as !== "script" ? h.as : void 0,
            crossOrigin: O,
            integrity: typeof h.integrity == "string" ? h.integrity : void 0,
          });
        } else f.d.m(p);
    }),
    (Kt.requestFormReset = function (p) {
      f.d.r(p);
    }),
    (Kt.unstable_batchedUpdates = function (p, h) {
      return p(h);
    }),
    (Kt.useFormState = function (p, h, O) {
      return A.H.useFormState(p, h, O);
    }),
    (Kt.useFormStatus = function () {
      return A.H.useHostTransitionStatus();
    }),
    (Kt.version = "19.1.0"),
    Kt
  );
}
var Ld;
function Q0() {
  if (Ld) return Mf.exports;
  Ld = 1;
  function i() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(i);
      } catch (s) {
        console.error(s);
      }
  }
  return (i(), (Mf.exports = X0()), Mf.exports);
}
/**
 * @license React
 * react-dom-client.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var Gd;
function Z0() {
  if (Gd) return Du;
  Gd = 1;
  var i = G0(),
    s = Hf(),
    o = Q0();
  function f(t) {
    var e = "https://react.dev/errors/" + t;
    if (1 < arguments.length) {
      e += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var l = 2; l < arguments.length; l++)
        e += "&args[]=" + encodeURIComponent(arguments[l]);
    }
    return (
      "Minified React error #" +
      t +
      "; visit " +
      e +
      " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
    );
  }
  function d(t) {
    return !(!t || (t.nodeType !== 1 && t.nodeType !== 9 && t.nodeType !== 11));
  }
  function v(t) {
    var e = t,
      l = t;
    if (t.alternate) for (; e.return; ) e = e.return;
    else {
      t = e;
      do ((e = t), (e.flags & 4098) !== 0 && (l = e.return), (t = e.return));
      while (t);
    }
    return e.tag === 3 ? l : null;
  }
  function A(t) {
    if (t.tag === 13) {
      var e = t.memoizedState;
      if (
        (e === null && ((t = t.alternate), t !== null && (e = t.memoizedState)),
        e !== null)
      )
        return e.dehydrated;
    }
    return null;
  }
  function b(t) {
    if (v(t) !== t) throw Error(f(188));
  }
  function p(t) {
    var e = t.alternate;
    if (!e) {
      if (((e = v(t)), e === null)) throw Error(f(188));
      return e !== t ? null : t;
    }
    for (var l = t, a = e; ; ) {
      var u = l.return;
      if (u === null) break;
      var n = u.alternate;
      if (n === null) {
        if (((a = u.return), a !== null)) {
          l = a;
          continue;
        }
        break;
      }
      if (u.child === n.child) {
        for (n = u.child; n; ) {
          if (n === l) return (b(u), t);
          if (n === a) return (b(u), e);
          n = n.sibling;
        }
        throw Error(f(188));
      }
      if (l.return !== a.return) ((l = u), (a = n));
      else {
        for (var c = !1, r = u.child; r; ) {
          if (r === l) {
            ((c = !0), (l = u), (a = n));
            break;
          }
          if (r === a) {
            ((c = !0), (a = u), (l = n));
            break;
          }
          r = r.sibling;
        }
        if (!c) {
          for (r = n.child; r; ) {
            if (r === l) {
              ((c = !0), (l = n), (a = u));
              break;
            }
            if (r === a) {
              ((c = !0), (a = n), (l = u));
              break;
            }
            r = r.sibling;
          }
          if (!c) throw Error(f(189));
        }
      }
      if (l.alternate !== a) throw Error(f(190));
    }
    if (l.tag !== 3) throw Error(f(188));
    return l.stateNode.current === l ? t : e;
  }
  function h(t) {
    var e = t.tag;
    if (e === 5 || e === 26 || e === 27 || e === 6) return t;
    for (t = t.child; t !== null; ) {
      if (((e = h(t)), e !== null)) return e;
      t = t.sibling;
    }
    return null;
  }
  var O = Object.assign,
    j = Symbol.for("react.element"),
    z = Symbol.for("react.transitional.element"),
    G = Symbol.for("react.portal"),
    M = Symbol.for("react.fragment"),
    Y = Symbol.for("react.strict_mode"),
    Q = Symbol.for("react.profiler"),
    q = Symbol.for("react.provider"),
    V = Symbol.for("react.consumer"),
    P = Symbol.for("react.context"),
    ct = Symbol.for("react.forward_ref"),
    I = Symbol.for("react.suspense"),
    _t = Symbol.for("react.suspense_list"),
    xt = Symbol.for("react.memo"),
    zt = Symbol.for("react.lazy"),
    St = Symbol.for("react.activity"),
    Jt = Symbol.for("react.memo_cache_sentinel"),
    oe = Symbol.iterator;
  function Xt(t) {
    return t === null || typeof t != "object"
      ? null
      : ((t = (oe && t[oe]) || t["@@iterator"]),
        typeof t == "function" ? t : null);
  }
  var Tl = Symbol.for("react.client.reference");
  function Al(t) {
    if (t == null) return null;
    if (typeof t == "function")
      return t.$$typeof === Tl ? null : t.displayName || t.name || null;
    if (typeof t == "string") return t;
    switch (t) {
      case M:
        return "Fragment";
      case Q:
        return "Profiler";
      case Y:
        return "StrictMode";
      case I:
        return "Suspense";
      case _t:
        return "SuspenseList";
      case St:
        return "Activity";
    }
    if (typeof t == "object")
      switch (t.$$typeof) {
        case G:
          return "Portal";
        case P:
          return (t.displayName || "Context") + ".Provider";
        case V:
          return (t._context.displayName || "Context") + ".Consumer";
        case ct:
          var e = t.render;
          return (
            (t = t.displayName),
            t ||
              ((t = e.displayName || e.name || ""),
              (t = t !== "" ? "ForwardRef(" + t + ")" : "ForwardRef")),
            t
          );
        case xt:
          return (
            (e = t.displayName || null),
            e !== null ? e : Al(t.type) || "Memo"
          );
        case zt:
          ((e = t._payload), (t = t._init));
          try {
            return Al(t(e));
          } catch {}
      }
    return null;
  }
  var Qt = Array.isArray,
    N = s.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
    L = o.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
    W = { pending: !1, data: null, method: null, action: null },
    yt = [],
    y = -1;
  function B(t) {
    return { current: t };
  }
  function X(t) {
    0 > y || ((t.current = yt[y]), (yt[y] = null), y--);
  }
  function w(t, e) {
    (y++, (yt[y] = t.current), (t.current = e));
  }
  var J = B(null),
    ft = B(null),
    tt = B(null),
    ee = B(null);
  function bt(t, e) {
    switch ((w(tt, e), w(ft, t), w(J, null), e.nodeType)) {
      case 9:
      case 11:
        t = (t = e.documentElement) && (t = t.namespaceURI) ? cd(t) : 0;
        break;
      default:
        if (((t = e.tagName), (e = e.namespaceURI)))
          ((e = cd(e)), (t = fd(e, t)));
        else
          switch (t) {
            case "svg":
              t = 1;
              break;
            case "math":
              t = 2;
              break;
            default:
              t = 0;
          }
    }
    (X(J), w(J, t));
  }
  function Fe() {
    (X(J), X(ft), X(tt));
  }
  function ci(t) {
    t.memoizedState !== null && w(ee, t);
    var e = J.current,
      l = fd(e, t.type);
    e !== l && (w(ft, t), w(J, l));
  }
  function qu(t) {
    (ft.current === t && (X(J), X(ft)),
      ee.current === t && (X(ee), (Au._currentValue = W)));
  }
  var fi = Object.prototype.hasOwnProperty,
    ri = i.unstable_scheduleCallback,
    si = i.unstable_cancelCallback,
    vh = i.unstable_shouldYield,
    yh = i.unstable_requestPaint,
    Re = i.unstable_now,
    gh = i.unstable_getCurrentPriorityLevel,
    Lf = i.unstable_ImmediatePriority,
    Gf = i.unstable_UserBlockingPriority,
    Yu = i.unstable_NormalPriority,
    ph = i.unstable_LowPriority,
    Xf = i.unstable_IdlePriority,
    Sh = i.log,
    bh = i.unstable_setDisableYieldValue,
    Na = null,
    le = null;
  function Pe(t) {
    if (
      (typeof Sh == "function" && bh(t),
      le && typeof le.setStrictMode == "function")
    )
      try {
        le.setStrictMode(Na, t);
      } catch {}
  }
  var ae = Math.clz32 ? Math.clz32 : Ah,
    Eh = Math.log,
    Th = Math.LN2;
  function Ah(t) {
    return ((t >>>= 0), t === 0 ? 32 : (31 - ((Eh(t) / Th) | 0)) | 0);
  }
  var wu = 256,
    Lu = 4194304;
  function xl(t) {
    var e = t & 42;
    if (e !== 0) return e;
    switch (t & -t) {
      case 1:
        return 1;
      case 2:
        return 2;
      case 4:
        return 4;
      case 8:
        return 8;
      case 16:
        return 16;
      case 32:
        return 32;
      case 64:
        return 64;
      case 128:
        return 128;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return t & 4194048;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return t & 62914560;
      case 67108864:
        return 67108864;
      case 134217728:
        return 134217728;
      case 268435456:
        return 268435456;
      case 536870912:
        return 536870912;
      case 1073741824:
        return 0;
      default:
        return t;
    }
  }
  function Gu(t, e, l) {
    var a = t.pendingLanes;
    if (a === 0) return 0;
    var u = 0,
      n = t.suspendedLanes,
      c = t.pingedLanes;
    t = t.warmLanes;
    var r = a & 134217727;
    return (
      r !== 0
        ? ((a = r & ~n),
          a !== 0
            ? (u = xl(a))
            : ((c &= r),
              c !== 0
                ? (u = xl(c))
                : l || ((l = r & ~t), l !== 0 && (u = xl(l)))))
        : ((r = a & ~n),
          r !== 0
            ? (u = xl(r))
            : c !== 0
              ? (u = xl(c))
              : l || ((l = a & ~t), l !== 0 && (u = xl(l)))),
      u === 0
        ? 0
        : e !== 0 &&
            e !== u &&
            (e & n) === 0 &&
            ((n = u & -u),
            (l = e & -e),
            n >= l || (n === 32 && (l & 4194048) !== 0))
          ? e
          : u
    );
  }
  function Ua(t, e) {
    return (t.pendingLanes & ~(t.suspendedLanes & ~t.pingedLanes) & e) === 0;
  }
  function xh(t, e) {
    switch (t) {
      case 1:
      case 2:
      case 4:
      case 8:
      case 64:
        return e + 250;
      case 16:
      case 32:
      case 128:
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return e + 5e3;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return -1;
      case 67108864:
      case 134217728:
      case 268435456:
      case 536870912:
      case 1073741824:
        return -1;
      default:
        return -1;
    }
  }
  function Qf() {
    var t = wu;
    return ((wu <<= 1), (wu & 4194048) === 0 && (wu = 256), t);
  }
  function Zf() {
    var t = Lu;
    return ((Lu <<= 1), (Lu & 62914560) === 0 && (Lu = 4194304), t);
  }
  function oi(t) {
    for (var e = [], l = 0; 31 > l; l++) e.push(t);
    return e;
  }
  function Ca(t, e) {
    ((t.pendingLanes |= e),
      e !== 268435456 &&
        ((t.suspendedLanes = 0), (t.pingedLanes = 0), (t.warmLanes = 0)));
  }
  function Rh(t, e, l, a, u, n) {
    var c = t.pendingLanes;
    ((t.pendingLanes = l),
      (t.suspendedLanes = 0),
      (t.pingedLanes = 0),
      (t.warmLanes = 0),
      (t.expiredLanes &= l),
      (t.entangledLanes &= l),
      (t.errorRecoveryDisabledLanes &= l),
      (t.shellSuspendCounter = 0));
    var r = t.entanglements,
      m = t.expirationTimes,
      T = t.hiddenUpdates;
    for (l = c & ~l; 0 < l; ) {
      var _ = 31 - ae(l),
        H = 1 << _;
      ((r[_] = 0), (m[_] = -1));
      var x = T[_];
      if (x !== null)
        for (T[_] = null, _ = 0; _ < x.length; _++) {
          var R = x[_];
          R !== null && (R.lane &= -536870913);
        }
      l &= ~H;
    }
    (a !== 0 && Vf(t, a, 0),
      n !== 0 && u === 0 && t.tag !== 0 && (t.suspendedLanes |= n & ~(c & ~e)));
  }
  function Vf(t, e, l) {
    ((t.pendingLanes |= e), (t.suspendedLanes &= ~e));
    var a = 31 - ae(e);
    ((t.entangledLanes |= e),
      (t.entanglements[a] = t.entanglements[a] | 1073741824 | (l & 4194090)));
  }
  function Kf(t, e) {
    var l = (t.entangledLanes |= e);
    for (t = t.entanglements; l; ) {
      var a = 31 - ae(l),
        u = 1 << a;
      ((u & e) | (t[a] & e) && (t[a] |= e), (l &= ~u));
    }
  }
  function di(t) {
    switch (t) {
      case 2:
        t = 1;
        break;
      case 8:
        t = 4;
        break;
      case 32:
        t = 16;
        break;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        t = 128;
        break;
      case 268435456:
        t = 134217728;
        break;
      default:
        t = 0;
    }
    return t;
  }
  function hi(t) {
    return (
      (t &= -t),
      2 < t ? (8 < t ? ((t & 134217727) !== 0 ? 32 : 268435456) : 8) : 2
    );
  }
  function Jf() {
    var t = L.p;
    return t !== 0 ? t : ((t = window.event), t === void 0 ? 32 : zd(t.type));
  }
  function Oh(t, e) {
    var l = L.p;
    try {
      return ((L.p = t), e());
    } finally {
      L.p = l;
    }
  }
  var Ie = Math.random().toString(36).slice(2),
    Zt = "__reactFiber$" + Ie,
    Wt = "__reactProps$" + Ie,
    Ql = "__reactContainer$" + Ie,
    mi = "__reactEvents$" + Ie,
    zh = "__reactListeners$" + Ie,
    Mh = "__reactHandles$" + Ie,
    $f = "__reactResources$" + Ie,
    Ha = "__reactMarker$" + Ie;
  function vi(t) {
    (delete t[Zt], delete t[Wt], delete t[mi], delete t[zh], delete t[Mh]);
  }
  function Zl(t) {
    var e = t[Zt];
    if (e) return e;
    for (var l = t.parentNode; l; ) {
      if ((e = l[Ql] || l[Zt])) {
        if (
          ((l = e.alternate),
          e.child !== null || (l !== null && l.child !== null))
        )
          for (t = dd(t); t !== null; ) {
            if ((l = t[Zt])) return l;
            t = dd(t);
          }
        return e;
      }
      ((t = l), (l = t.parentNode));
    }
    return null;
  }
  function Vl(t) {
    if ((t = t[Zt] || t[Ql])) {
      var e = t.tag;
      if (e === 5 || e === 6 || e === 13 || e === 26 || e === 27 || e === 3)
        return t;
    }
    return null;
  }
  function ja(t) {
    var e = t.tag;
    if (e === 5 || e === 26 || e === 27 || e === 6) return t.stateNode;
    throw Error(f(33));
  }
  function Kl(t) {
    var e = t[$f];
    return (
      e ||
        (e = t[$f] =
          { hoistableStyles: new Map(), hoistableScripts: new Map() }),
      e
    );
  }
  function jt(t) {
    t[Ha] = !0;
  }
  var kf = new Set(),
    Wf = {};
  function Rl(t, e) {
    (Jl(t, e), Jl(t + "Capture", e));
  }
  function Jl(t, e) {
    for (Wf[t] = e, t = 0; t < e.length; t++) kf.add(e[t]);
  }
  var Dh = RegExp(
      "^[:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD][:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD\\-.0-9\\u00B7\\u0300-\\u036F\\u203F-\\u2040]*$",
    ),
    Ff = {},
    Pf = {};
  function _h(t) {
    return fi.call(Pf, t)
      ? !0
      : fi.call(Ff, t)
        ? !1
        : Dh.test(t)
          ? (Pf[t] = !0)
          : ((Ff[t] = !0), !1);
  }
  function Xu(t, e, l) {
    if (_h(e))
      if (l === null) t.removeAttribute(e);
      else {
        switch (typeof l) {
          case "undefined":
          case "function":
          case "symbol":
            t.removeAttribute(e);
            return;
          case "boolean":
            var a = e.toLowerCase().slice(0, 5);
            if (a !== "data-" && a !== "aria-") {
              t.removeAttribute(e);
              return;
            }
        }
        t.setAttribute(e, "" + l);
      }
  }
  function Qu(t, e, l) {
    if (l === null) t.removeAttribute(e);
    else {
      switch (typeof l) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          t.removeAttribute(e);
          return;
      }
      t.setAttribute(e, "" + l);
    }
  }
  function Ce(t, e, l, a) {
    if (a === null) t.removeAttribute(l);
    else {
      switch (typeof a) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          t.removeAttribute(l);
          return;
      }
      t.setAttributeNS(e, l, "" + a);
    }
  }
  var yi, If;
  function $l(t) {
    if (yi === void 0)
      try {
        throw Error();
      } catch (l) {
        var e = l.stack.trim().match(/\n( *(at )?)/);
        ((yi = (e && e[1]) || ""),
          (If =
            -1 <
            l.stack.indexOf(`
    at`)
              ? " (<anonymous>)"
              : -1 < l.stack.indexOf("@")
                ? "@unknown:0:0"
                : ""));
      }
    return (
      `
` +
      yi +
      t +
      If
    );
  }
  var gi = !1;
  function pi(t, e) {
    if (!t || gi) return "";
    gi = !0;
    var l = Error.prepareStackTrace;
    Error.prepareStackTrace = void 0;
    try {
      var a = {
        DetermineComponentFrameRoot: function () {
          try {
            if (e) {
              var H = function () {
                throw Error();
              };
              if (
                (Object.defineProperty(H.prototype, "props", {
                  set: function () {
                    throw Error();
                  },
                }),
                typeof Reflect == "object" && Reflect.construct)
              ) {
                try {
                  Reflect.construct(H, []);
                } catch (R) {
                  var x = R;
                }
                Reflect.construct(t, [], H);
              } else {
                try {
                  H.call();
                } catch (R) {
                  x = R;
                }
                t.call(H.prototype);
              }
            } else {
              try {
                throw Error();
              } catch (R) {
                x = R;
              }
              (H = t()) &&
                typeof H.catch == "function" &&
                H.catch(function () {});
            }
          } catch (R) {
            if (R && x && typeof R.stack == "string") return [R.stack, x.stack];
          }
          return [null, null];
        },
      };
      a.DetermineComponentFrameRoot.displayName = "DetermineComponentFrameRoot";
      var u = Object.getOwnPropertyDescriptor(
        a.DetermineComponentFrameRoot,
        "name",
      );
      u &&
        u.configurable &&
        Object.defineProperty(a.DetermineComponentFrameRoot, "name", {
          value: "DetermineComponentFrameRoot",
        });
      var n = a.DetermineComponentFrameRoot(),
        c = n[0],
        r = n[1];
      if (c && r) {
        var m = c.split(`
`),
          T = r.split(`
`);
        for (
          u = a = 0;
          a < m.length && !m[a].includes("DetermineComponentFrameRoot");

        )
          a++;
        for (; u < T.length && !T[u].includes("DetermineComponentFrameRoot"); )
          u++;
        if (a === m.length || u === T.length)
          for (
            a = m.length - 1, u = T.length - 1;
            1 <= a && 0 <= u && m[a] !== T[u];

          )
            u--;
        for (; 1 <= a && 0 <= u; a--, u--)
          if (m[a] !== T[u]) {
            if (a !== 1 || u !== 1)
              do
                if ((a--, u--, 0 > u || m[a] !== T[u])) {
                  var _ =
                    `
` + m[a].replace(" at new ", " at ");
                  return (
                    t.displayName &&
                      _.includes("<anonymous>") &&
                      (_ = _.replace("<anonymous>", t.displayName)),
                    _
                  );
                }
              while (1 <= a && 0 <= u);
            break;
          }
      }
    } finally {
      ((gi = !1), (Error.prepareStackTrace = l));
    }
    return (l = t ? t.displayName || t.name : "") ? $l(l) : "";
  }
  function Nh(t) {
    switch (t.tag) {
      case 26:
      case 27:
      case 5:
        return $l(t.type);
      case 16:
        return $l("Lazy");
      case 13:
        return $l("Suspense");
      case 19:
        return $l("SuspenseList");
      case 0:
      case 15:
        return pi(t.type, !1);
      case 11:
        return pi(t.type.render, !1);
      case 1:
        return pi(t.type, !0);
      case 31:
        return $l("Activity");
      default:
        return "";
    }
  }
  function tr(t) {
    try {
      var e = "";
      do ((e += Nh(t)), (t = t.return));
      while (t);
      return e;
    } catch (l) {
      return (
        `
Error generating stack: ` +
        l.message +
        `
` +
        l.stack
      );
    }
  }
  function de(t) {
    switch (typeof t) {
      case "bigint":
      case "boolean":
      case "number":
      case "string":
      case "undefined":
        return t;
      case "object":
        return t;
      default:
        return "";
    }
  }
  function er(t) {
    var e = t.type;
    return (
      (t = t.nodeName) &&
      t.toLowerCase() === "input" &&
      (e === "checkbox" || e === "radio")
    );
  }
  function Uh(t) {
    var e = er(t) ? "checked" : "value",
      l = Object.getOwnPropertyDescriptor(t.constructor.prototype, e),
      a = "" + t[e];
    if (
      !t.hasOwnProperty(e) &&
      typeof l < "u" &&
      typeof l.get == "function" &&
      typeof l.set == "function"
    ) {
      var u = l.get,
        n = l.set;
      return (
        Object.defineProperty(t, e, {
          configurable: !0,
          get: function () {
            return u.call(this);
          },
          set: function (c) {
            ((a = "" + c), n.call(this, c));
          },
        }),
        Object.defineProperty(t, e, { enumerable: l.enumerable }),
        {
          getValue: function () {
            return a;
          },
          setValue: function (c) {
            a = "" + c;
          },
          stopTracking: function () {
            ((t._valueTracker = null), delete t[e]);
          },
        }
      );
    }
  }
  function Zu(t) {
    t._valueTracker || (t._valueTracker = Uh(t));
  }
  function lr(t) {
    if (!t) return !1;
    var e = t._valueTracker;
    if (!e) return !0;
    var l = e.getValue(),
      a = "";
    return (
      t && (a = er(t) ? (t.checked ? "true" : "false") : t.value),
      (t = a),
      t !== l ? (e.setValue(t), !0) : !1
    );
  }
  function Vu(t) {
    if (
      ((t = t || (typeof document < "u" ? document : void 0)), typeof t > "u")
    )
      return null;
    try {
      return t.activeElement || t.body;
    } catch {
      return t.body;
    }
  }
  var Ch = /[\n"\\]/g;
  function he(t) {
    return t.replace(Ch, function (e) {
      return "\\" + e.charCodeAt(0).toString(16) + " ";
    });
  }
  function Si(t, e, l, a, u, n, c, r) {
    ((t.name = ""),
      c != null &&
      typeof c != "function" &&
      typeof c != "symbol" &&
      typeof c != "boolean"
        ? (t.type = c)
        : t.removeAttribute("type"),
      e != null
        ? c === "number"
          ? ((e === 0 && t.value === "") || t.value != e) &&
            (t.value = "" + de(e))
          : t.value !== "" + de(e) && (t.value = "" + de(e))
        : (c !== "submit" && c !== "reset") || t.removeAttribute("value"),
      e != null
        ? bi(t, c, de(e))
        : l != null
          ? bi(t, c, de(l))
          : a != null && t.removeAttribute("value"),
      u == null && n != null && (t.defaultChecked = !!n),
      u != null &&
        (t.checked = u && typeof u != "function" && typeof u != "symbol"),
      r != null &&
      typeof r != "function" &&
      typeof r != "symbol" &&
      typeof r != "boolean"
        ? (t.name = "" + de(r))
        : t.removeAttribute("name"));
  }
  function ar(t, e, l, a, u, n, c, r) {
    if (
      (n != null &&
        typeof n != "function" &&
        typeof n != "symbol" &&
        typeof n != "boolean" &&
        (t.type = n),
      e != null || l != null)
    ) {
      if (!((n !== "submit" && n !== "reset") || e != null)) return;
      ((l = l != null ? "" + de(l) : ""),
        (e = e != null ? "" + de(e) : l),
        r || e === t.value || (t.value = e),
        (t.defaultValue = e));
    }
    ((a = a ?? u),
      (a = typeof a != "function" && typeof a != "symbol" && !!a),
      (t.checked = r ? t.checked : !!a),
      (t.defaultChecked = !!a),
      c != null &&
        typeof c != "function" &&
        typeof c != "symbol" &&
        typeof c != "boolean" &&
        (t.name = c));
  }
  function bi(t, e, l) {
    (e === "number" && Vu(t.ownerDocument) === t) ||
      t.defaultValue === "" + l ||
      (t.defaultValue = "" + l);
  }
  function kl(t, e, l, a) {
    if (((t = t.options), e)) {
      e = {};
      for (var u = 0; u < l.length; u++) e["$" + l[u]] = !0;
      for (l = 0; l < t.length; l++)
        ((u = e.hasOwnProperty("$" + t[l].value)),
          t[l].selected !== u && (t[l].selected = u),
          u && a && (t[l].defaultSelected = !0));
    } else {
      for (l = "" + de(l), e = null, u = 0; u < t.length; u++) {
        if (t[u].value === l) {
          ((t[u].selected = !0), a && (t[u].defaultSelected = !0));
          return;
        }
        e !== null || t[u].disabled || (e = t[u]);
      }
      e !== null && (e.selected = !0);
    }
  }
  function ur(t, e, l) {
    if (
      e != null &&
      ((e = "" + de(e)), e !== t.value && (t.value = e), l == null)
    ) {
      t.defaultValue !== e && (t.defaultValue = e);
      return;
    }
    t.defaultValue = l != null ? "" + de(l) : "";
  }
  function nr(t, e, l, a) {
    if (e == null) {
      if (a != null) {
        if (l != null) throw Error(f(92));
        if (Qt(a)) {
          if (1 < a.length) throw Error(f(93));
          a = a[0];
        }
        l = a;
      }
      (l == null && (l = ""), (e = l));
    }
    ((l = de(e)),
      (t.defaultValue = l),
      (a = t.textContent),
      a === l && a !== "" && a !== null && (t.value = a));
  }
  function Wl(t, e) {
    if (e) {
      var l = t.firstChild;
      if (l && l === t.lastChild && l.nodeType === 3) {
        l.nodeValue = e;
        return;
      }
    }
    t.textContent = e;
  }
  var Hh = new Set(
    "animationIterationCount aspectRatio borderImageOutset borderImageSlice borderImageWidth boxFlex boxFlexGroup boxOrdinalGroup columnCount columns flex flexGrow flexPositive flexShrink flexNegative flexOrder gridArea gridRow gridRowEnd gridRowSpan gridRowStart gridColumn gridColumnEnd gridColumnSpan gridColumnStart fontWeight lineClamp lineHeight opacity order orphans scale tabSize widows zIndex zoom fillOpacity floodOpacity stopOpacity strokeDasharray strokeDashoffset strokeMiterlimit strokeOpacity strokeWidth MozAnimationIterationCount MozBoxFlex MozBoxFlexGroup MozLineClamp msAnimationIterationCount msFlex msZoom msFlexGrow msFlexNegative msFlexOrder msFlexPositive msFlexShrink msGridColumn msGridColumnSpan msGridRow msGridRowSpan WebkitAnimationIterationCount WebkitBoxFlex WebKitBoxFlexGroup WebkitBoxOrdinalGroup WebkitColumnCount WebkitColumns WebkitFlex WebkitFlexGrow WebkitFlexPositive WebkitFlexShrink WebkitLineClamp".split(
      " ",
    ),
  );
  function ir(t, e, l) {
    var a = e.indexOf("--") === 0;
    l == null || typeof l == "boolean" || l === ""
      ? a
        ? t.setProperty(e, "")
        : e === "float"
          ? (t.cssFloat = "")
          : (t[e] = "")
      : a
        ? t.setProperty(e, l)
        : typeof l != "number" || l === 0 || Hh.has(e)
          ? e === "float"
            ? (t.cssFloat = l)
            : (t[e] = ("" + l).trim())
          : (t[e] = l + "px");
  }
  function cr(t, e, l) {
    if (e != null && typeof e != "object") throw Error(f(62));
    if (((t = t.style), l != null)) {
      for (var a in l)
        !l.hasOwnProperty(a) ||
          (e != null && e.hasOwnProperty(a)) ||
          (a.indexOf("--") === 0
            ? t.setProperty(a, "")
            : a === "float"
              ? (t.cssFloat = "")
              : (t[a] = ""));
      for (var u in e)
        ((a = e[u]), e.hasOwnProperty(u) && l[u] !== a && ir(t, u, a));
    } else for (var n in e) e.hasOwnProperty(n) && ir(t, n, e[n]);
  }
  function Ei(t) {
    if (t.indexOf("-") === -1) return !1;
    switch (t) {
      case "annotation-xml":
      case "color-profile":
      case "font-face":
      case "font-face-src":
      case "font-face-uri":
      case "font-face-format":
      case "font-face-name":
      case "missing-glyph":
        return !1;
      default:
        return !0;
    }
  }
  var jh = new Map([
      ["acceptCharset", "accept-charset"],
      ["htmlFor", "for"],
      ["httpEquiv", "http-equiv"],
      ["crossOrigin", "crossorigin"],
      ["accentHeight", "accent-height"],
      ["alignmentBaseline", "alignment-baseline"],
      ["arabicForm", "arabic-form"],
      ["baselineShift", "baseline-shift"],
      ["capHeight", "cap-height"],
      ["clipPath", "clip-path"],
      ["clipRule", "clip-rule"],
      ["colorInterpolation", "color-interpolation"],
      ["colorInterpolationFilters", "color-interpolation-filters"],
      ["colorProfile", "color-profile"],
      ["colorRendering", "color-rendering"],
      ["dominantBaseline", "dominant-baseline"],
      ["enableBackground", "enable-background"],
      ["fillOpacity", "fill-opacity"],
      ["fillRule", "fill-rule"],
      ["floodColor", "flood-color"],
      ["floodOpacity", "flood-opacity"],
      ["fontFamily", "font-family"],
      ["fontSize", "font-size"],
      ["fontSizeAdjust", "font-size-adjust"],
      ["fontStretch", "font-stretch"],
      ["fontStyle", "font-style"],
      ["fontVariant", "font-variant"],
      ["fontWeight", "font-weight"],
      ["glyphName", "glyph-name"],
      ["glyphOrientationHorizontal", "glyph-orientation-horizontal"],
      ["glyphOrientationVertical", "glyph-orientation-vertical"],
      ["horizAdvX", "horiz-adv-x"],
      ["horizOriginX", "horiz-origin-x"],
      ["imageRendering", "image-rendering"],
      ["letterSpacing", "letter-spacing"],
      ["lightingColor", "lighting-color"],
      ["markerEnd", "marker-end"],
      ["markerMid", "marker-mid"],
      ["markerStart", "marker-start"],
      ["overlinePosition", "overline-position"],
      ["overlineThickness", "overline-thickness"],
      ["paintOrder", "paint-order"],
      ["panose-1", "panose-1"],
      ["pointerEvents", "pointer-events"],
      ["renderingIntent", "rendering-intent"],
      ["shapeRendering", "shape-rendering"],
      ["stopColor", "stop-color"],
      ["stopOpacity", "stop-opacity"],
      ["strikethroughPosition", "strikethrough-position"],
      ["strikethroughThickness", "strikethrough-thickness"],
      ["strokeDasharray", "stroke-dasharray"],
      ["strokeDashoffset", "stroke-dashoffset"],
      ["strokeLinecap", "stroke-linecap"],
      ["strokeLinejoin", "stroke-linejoin"],
      ["strokeMiterlimit", "stroke-miterlimit"],
      ["strokeOpacity", "stroke-opacity"],
      ["strokeWidth", "stroke-width"],
      ["textAnchor", "text-anchor"],
      ["textDecoration", "text-decoration"],
      ["textRendering", "text-rendering"],
      ["transformOrigin", "transform-origin"],
      ["underlinePosition", "underline-position"],
      ["underlineThickness", "underline-thickness"],
      ["unicodeBidi", "unicode-bidi"],
      ["unicodeRange", "unicode-range"],
      ["unitsPerEm", "units-per-em"],
      ["vAlphabetic", "v-alphabetic"],
      ["vHanging", "v-hanging"],
      ["vIdeographic", "v-ideographic"],
      ["vMathematical", "v-mathematical"],
      ["vectorEffect", "vector-effect"],
      ["vertAdvY", "vert-adv-y"],
      ["vertOriginX", "vert-origin-x"],
      ["vertOriginY", "vert-origin-y"],
      ["wordSpacing", "word-spacing"],
      ["writingMode", "writing-mode"],
      ["xmlnsXlink", "xmlns:xlink"],
      ["xHeight", "x-height"],
    ]),
    Bh =
      /^[\u0000-\u001F ]*j[\r\n\t]*a[\r\n\t]*v[\r\n\t]*a[\r\n\t]*s[\r\n\t]*c[\r\n\t]*r[\r\n\t]*i[\r\n\t]*p[\r\n\t]*t[\r\n\t]*:/i;
  function Ku(t) {
    return Bh.test("" + t)
      ? "javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')"
      : t;
  }
  var Ti = null;
  function Ai(t) {
    return (
      (t = t.target || t.srcElement || window),
      t.correspondingUseElement && (t = t.correspondingUseElement),
      t.nodeType === 3 ? t.parentNode : t
    );
  }
  var Fl = null,
    Pl = null;
  function fr(t) {
    var e = Vl(t);
    if (e && (t = e.stateNode)) {
      var l = t[Wt] || null;
      t: switch (((t = e.stateNode), e.type)) {
        case "input":
          if (
            (Si(
              t,
              l.value,
              l.defaultValue,
              l.defaultValue,
              l.checked,
              l.defaultChecked,
              l.type,
              l.name,
            ),
            (e = l.name),
            l.type === "radio" && e != null)
          ) {
            for (l = t; l.parentNode; ) l = l.parentNode;
            for (
              l = l.querySelectorAll(
                'input[name="' + he("" + e) + '"][type="radio"]',
              ),
                e = 0;
              e < l.length;
              e++
            ) {
              var a = l[e];
              if (a !== t && a.form === t.form) {
                var u = a[Wt] || null;
                if (!u) throw Error(f(90));
                Si(
                  a,
                  u.value,
                  u.defaultValue,
                  u.defaultValue,
                  u.checked,
                  u.defaultChecked,
                  u.type,
                  u.name,
                );
              }
            }
            for (e = 0; e < l.length; e++)
              ((a = l[e]), a.form === t.form && lr(a));
          }
          break t;
        case "textarea":
          ur(t, l.value, l.defaultValue);
          break t;
        case "select":
          ((e = l.value), e != null && kl(t, !!l.multiple, e, !1));
      }
    }
  }
  var xi = !1;
  function rr(t, e, l) {
    if (xi) return t(e, l);
    xi = !0;
    try {
      var a = t(e);
      return a;
    } finally {
      if (
        ((xi = !1),
        (Fl !== null || Pl !== null) &&
          (Un(), Fl && ((e = Fl), (t = Pl), (Pl = Fl = null), fr(e), t)))
      )
        for (e = 0; e < t.length; e++) fr(t[e]);
    }
  }
  function Ba(t, e) {
    var l = t.stateNode;
    if (l === null) return null;
    var a = l[Wt] || null;
    if (a === null) return null;
    l = a[e];
    t: switch (e) {
      case "onClick":
      case "onClickCapture":
      case "onDoubleClick":
      case "onDoubleClickCapture":
      case "onMouseDown":
      case "onMouseDownCapture":
      case "onMouseMove":
      case "onMouseMoveCapture":
      case "onMouseUp":
      case "onMouseUpCapture":
      case "onMouseEnter":
        ((a = !a.disabled) ||
          ((t = t.type),
          (a = !(
            t === "button" ||
            t === "input" ||
            t === "select" ||
            t === "textarea"
          ))),
          (t = !a));
        break t;
      default:
        t = !1;
    }
    if (t) return null;
    if (l && typeof l != "function") throw Error(f(231, e, typeof l));
    return l;
  }
  var He = !(
      typeof window > "u" ||
      typeof window.document > "u" ||
      typeof window.document.createElement > "u"
    ),
    Ri = !1;
  if (He)
    try {
      var qa = {};
      (Object.defineProperty(qa, "passive", {
        get: function () {
          Ri = !0;
        },
      }),
        window.addEventListener("test", qa, qa),
        window.removeEventListener("test", qa, qa));
    } catch {
      Ri = !1;
    }
  var tl = null,
    Oi = null,
    Ju = null;
  function sr() {
    if (Ju) return Ju;
    var t,
      e = Oi,
      l = e.length,
      a,
      u = "value" in tl ? tl.value : tl.textContent,
      n = u.length;
    for (t = 0; t < l && e[t] === u[t]; t++);
    var c = l - t;
    for (a = 1; a <= c && e[l - a] === u[n - a]; a++);
    return (Ju = u.slice(t, 1 < a ? 1 - a : void 0));
  }
  function $u(t) {
    var e = t.keyCode;
    return (
      "charCode" in t
        ? ((t = t.charCode), t === 0 && e === 13 && (t = 13))
        : (t = e),
      t === 10 && (t = 13),
      32 <= t || t === 13 ? t : 0
    );
  }
  function ku() {
    return !0;
  }
  function or() {
    return !1;
  }
  function Ft(t) {
    function e(l, a, u, n, c) {
      ((this._reactName = l),
        (this._targetInst = u),
        (this.type = a),
        (this.nativeEvent = n),
        (this.target = c),
        (this.currentTarget = null));
      for (var r in t)
        t.hasOwnProperty(r) && ((l = t[r]), (this[r] = l ? l(n) : n[r]));
      return (
        (this.isDefaultPrevented = (
          n.defaultPrevented != null ? n.defaultPrevented : n.returnValue === !1
        )
          ? ku
          : or),
        (this.isPropagationStopped = or),
        this
      );
    }
    return (
      O(e.prototype, {
        preventDefault: function () {
          this.defaultPrevented = !0;
          var l = this.nativeEvent;
          l &&
            (l.preventDefault
              ? l.preventDefault()
              : typeof l.returnValue != "unknown" && (l.returnValue = !1),
            (this.isDefaultPrevented = ku));
        },
        stopPropagation: function () {
          var l = this.nativeEvent;
          l &&
            (l.stopPropagation
              ? l.stopPropagation()
              : typeof l.cancelBubble != "unknown" && (l.cancelBubble = !0),
            (this.isPropagationStopped = ku));
        },
        persist: function () {},
        isPersistent: ku,
      }),
      e
    );
  }
  var Ol = {
      eventPhase: 0,
      bubbles: 0,
      cancelable: 0,
      timeStamp: function (t) {
        return t.timeStamp || Date.now();
      },
      defaultPrevented: 0,
      isTrusted: 0,
    },
    Wu = Ft(Ol),
    Ya = O({}, Ol, { view: 0, detail: 0 }),
    qh = Ft(Ya),
    zi,
    Mi,
    wa,
    Fu = O({}, Ya, {
      screenX: 0,
      screenY: 0,
      clientX: 0,
      clientY: 0,
      pageX: 0,
      pageY: 0,
      ctrlKey: 0,
      shiftKey: 0,
      altKey: 0,
      metaKey: 0,
      getModifierState: _i,
      button: 0,
      buttons: 0,
      relatedTarget: function (t) {
        return t.relatedTarget === void 0
          ? t.fromElement === t.srcElement
            ? t.toElement
            : t.fromElement
          : t.relatedTarget;
      },
      movementX: function (t) {
        return "movementX" in t
          ? t.movementX
          : (t !== wa &&
              (wa && t.type === "mousemove"
                ? ((zi = t.screenX - wa.screenX), (Mi = t.screenY - wa.screenY))
                : (Mi = zi = 0),
              (wa = t)),
            zi);
      },
      movementY: function (t) {
        return "movementY" in t ? t.movementY : Mi;
      },
    }),
    dr = Ft(Fu),
    Yh = O({}, Fu, { dataTransfer: 0 }),
    wh = Ft(Yh),
    Lh = O({}, Ya, { relatedTarget: 0 }),
    Di = Ft(Lh),
    Gh = O({}, Ol, { animationName: 0, elapsedTime: 0, pseudoElement: 0 }),
    Xh = Ft(Gh),
    Qh = O({}, Ol, {
      clipboardData: function (t) {
        return "clipboardData" in t ? t.clipboardData : window.clipboardData;
      },
    }),
    Zh = Ft(Qh),
    Vh = O({}, Ol, { data: 0 }),
    hr = Ft(Vh),
    Kh = {
      Esc: "Escape",
      Spacebar: " ",
      Left: "ArrowLeft",
      Up: "ArrowUp",
      Right: "ArrowRight",
      Down: "ArrowDown",
      Del: "Delete",
      Win: "OS",
      Menu: "ContextMenu",
      Apps: "ContextMenu",
      Scroll: "ScrollLock",
      MozPrintableKey: "Unidentified",
    },
    Jh = {
      8: "Backspace",
      9: "Tab",
      12: "Clear",
      13: "Enter",
      16: "Shift",
      17: "Control",
      18: "Alt",
      19: "Pause",
      20: "CapsLock",
      27: "Escape",
      32: " ",
      33: "PageUp",
      34: "PageDown",
      35: "End",
      36: "Home",
      37: "ArrowLeft",
      38: "ArrowUp",
      39: "ArrowRight",
      40: "ArrowDown",
      45: "Insert",
      46: "Delete",
      112: "F1",
      113: "F2",
      114: "F3",
      115: "F4",
      116: "F5",
      117: "F6",
      118: "F7",
      119: "F8",
      120: "F9",
      121: "F10",
      122: "F11",
      123: "F12",
      144: "NumLock",
      145: "ScrollLock",
      224: "Meta",
    },
    $h = {
      Alt: "altKey",
      Control: "ctrlKey",
      Meta: "metaKey",
      Shift: "shiftKey",
    };
  function kh(t) {
    var e = this.nativeEvent;
    return e.getModifierState
      ? e.getModifierState(t)
      : (t = $h[t])
        ? !!e[t]
        : !1;
  }
  function _i() {
    return kh;
  }
  var Wh = O({}, Ya, {
      key: function (t) {
        if (t.key) {
          var e = Kh[t.key] || t.key;
          if (e !== "Unidentified") return e;
        }
        return t.type === "keypress"
          ? ((t = $u(t)), t === 13 ? "Enter" : String.fromCharCode(t))
          : t.type === "keydown" || t.type === "keyup"
            ? Jh[t.keyCode] || "Unidentified"
            : "";
      },
      code: 0,
      location: 0,
      ctrlKey: 0,
      shiftKey: 0,
      altKey: 0,
      metaKey: 0,
      repeat: 0,
      locale: 0,
      getModifierState: _i,
      charCode: function (t) {
        return t.type === "keypress" ? $u(t) : 0;
      },
      keyCode: function (t) {
        return t.type === "keydown" || t.type === "keyup" ? t.keyCode : 0;
      },
      which: function (t) {
        return t.type === "keypress"
          ? $u(t)
          : t.type === "keydown" || t.type === "keyup"
            ? t.keyCode
            : 0;
      },
    }),
    Fh = Ft(Wh),
    Ph = O({}, Fu, {
      pointerId: 0,
      width: 0,
      height: 0,
      pressure: 0,
      tangentialPressure: 0,
      tiltX: 0,
      tiltY: 0,
      twist: 0,
      pointerType: 0,
      isPrimary: 0,
    }),
    mr = Ft(Ph),
    Ih = O({}, Ya, {
      touches: 0,
      targetTouches: 0,
      changedTouches: 0,
      altKey: 0,
      metaKey: 0,
      ctrlKey: 0,
      shiftKey: 0,
      getModifierState: _i,
    }),
    tm = Ft(Ih),
    em = O({}, Ol, { propertyName: 0, elapsedTime: 0, pseudoElement: 0 }),
    lm = Ft(em),
    am = O({}, Fu, {
      deltaX: function (t) {
        return "deltaX" in t
          ? t.deltaX
          : "wheelDeltaX" in t
            ? -t.wheelDeltaX
            : 0;
      },
      deltaY: function (t) {
        return "deltaY" in t
          ? t.deltaY
          : "wheelDeltaY" in t
            ? -t.wheelDeltaY
            : "wheelDelta" in t
              ? -t.wheelDelta
              : 0;
      },
      deltaZ: 0,
      deltaMode: 0,
    }),
    um = Ft(am),
    nm = O({}, Ol, { newState: 0, oldState: 0 }),
    im = Ft(nm),
    cm = [9, 13, 27, 32],
    Ni = He && "CompositionEvent" in window,
    La = null;
  He && "documentMode" in document && (La = document.documentMode);
  var fm = He && "TextEvent" in window && !La,
    vr = He && (!Ni || (La && 8 < La && 11 >= La)),
    yr = " ",
    gr = !1;
  function pr(t, e) {
    switch (t) {
      case "keyup":
        return cm.indexOf(e.keyCode) !== -1;
      case "keydown":
        return e.keyCode !== 229;
      case "keypress":
      case "mousedown":
      case "focusout":
        return !0;
      default:
        return !1;
    }
  }
  function Sr(t) {
    return (
      (t = t.detail),
      typeof t == "object" && "data" in t ? t.data : null
    );
  }
  var Il = !1;
  function rm(t, e) {
    switch (t) {
      case "compositionend":
        return Sr(e);
      case "keypress":
        return e.which !== 32 ? null : ((gr = !0), yr);
      case "textInput":
        return ((t = e.data), t === yr && gr ? null : t);
      default:
        return null;
    }
  }
  function sm(t, e) {
    if (Il)
      return t === "compositionend" || (!Ni && pr(t, e))
        ? ((t = sr()), (Ju = Oi = tl = null), (Il = !1), t)
        : null;
    switch (t) {
      case "paste":
        return null;
      case "keypress":
        if (!(e.ctrlKey || e.altKey || e.metaKey) || (e.ctrlKey && e.altKey)) {
          if (e.char && 1 < e.char.length) return e.char;
          if (e.which) return String.fromCharCode(e.which);
        }
        return null;
      case "compositionend":
        return vr && e.locale !== "ko" ? null : e.data;
      default:
        return null;
    }
  }
  var om = {
    color: !0,
    date: !0,
    datetime: !0,
    "datetime-local": !0,
    email: !0,
    month: !0,
    number: !0,
    password: !0,
    range: !0,
    search: !0,
    tel: !0,
    text: !0,
    time: !0,
    url: !0,
    week: !0,
  };
  function br(t) {
    var e = t && t.nodeName && t.nodeName.toLowerCase();
    return e === "input" ? !!om[t.type] : e === "textarea";
  }
  function Er(t, e, l, a) {
    (Fl ? (Pl ? Pl.push(a) : (Pl = [a])) : (Fl = a),
      (e = Yn(e, "onChange")),
      0 < e.length &&
        ((l = new Wu("onChange", "change", null, l, a)),
        t.push({ event: l, listeners: e })));
  }
  var Ga = null,
    Xa = null;
  function dm(t) {
    ld(t, 0);
  }
  function Pu(t) {
    var e = ja(t);
    if (lr(e)) return t;
  }
  function Tr(t, e) {
    if (t === "change") return e;
  }
  var Ar = !1;
  if (He) {
    var Ui;
    if (He) {
      var Ci = "oninput" in document;
      if (!Ci) {
        var xr = document.createElement("div");
        (xr.setAttribute("oninput", "return;"),
          (Ci = typeof xr.oninput == "function"));
      }
      Ui = Ci;
    } else Ui = !1;
    Ar = Ui && (!document.documentMode || 9 < document.documentMode);
  }
  function Rr() {
    Ga && (Ga.detachEvent("onpropertychange", Or), (Xa = Ga = null));
  }
  function Or(t) {
    if (t.propertyName === "value" && Pu(Xa)) {
      var e = [];
      (Er(e, Xa, t, Ai(t)), rr(dm, e));
    }
  }
  function hm(t, e, l) {
    t === "focusin"
      ? (Rr(), (Ga = e), (Xa = l), Ga.attachEvent("onpropertychange", Or))
      : t === "focusout" && Rr();
  }
  function mm(t) {
    if (t === "selectionchange" || t === "keyup" || t === "keydown")
      return Pu(Xa);
  }
  function vm(t, e) {
    if (t === "click") return Pu(e);
  }
  function ym(t, e) {
    if (t === "input" || t === "change") return Pu(e);
  }
  function gm(t, e) {
    return (t === e && (t !== 0 || 1 / t === 1 / e)) || (t !== t && e !== e);
  }
  var ue = typeof Object.is == "function" ? Object.is : gm;
  function Qa(t, e) {
    if (ue(t, e)) return !0;
    if (
      typeof t != "object" ||
      t === null ||
      typeof e != "object" ||
      e === null
    )
      return !1;
    var l = Object.keys(t),
      a = Object.keys(e);
    if (l.length !== a.length) return !1;
    for (a = 0; a < l.length; a++) {
      var u = l[a];
      if (!fi.call(e, u) || !ue(t[u], e[u])) return !1;
    }
    return !0;
  }
  function zr(t) {
    for (; t && t.firstChild; ) t = t.firstChild;
    return t;
  }
  function Mr(t, e) {
    var l = zr(t);
    t = 0;
    for (var a; l; ) {
      if (l.nodeType === 3) {
        if (((a = t + l.textContent.length), t <= e && a >= e))
          return { node: l, offset: e - t };
        t = a;
      }
      t: {
        for (; l; ) {
          if (l.nextSibling) {
            l = l.nextSibling;
            break t;
          }
          l = l.parentNode;
        }
        l = void 0;
      }
      l = zr(l);
    }
  }
  function Dr(t, e) {
    return t && e
      ? t === e
        ? !0
        : t && t.nodeType === 3
          ? !1
          : e && e.nodeType === 3
            ? Dr(t, e.parentNode)
            : "contains" in t
              ? t.contains(e)
              : t.compareDocumentPosition
                ? !!(t.compareDocumentPosition(e) & 16)
                : !1
      : !1;
  }
  function _r(t) {
    t =
      t != null &&
      t.ownerDocument != null &&
      t.ownerDocument.defaultView != null
        ? t.ownerDocument.defaultView
        : window;
    for (var e = Vu(t.document); e instanceof t.HTMLIFrameElement; ) {
      try {
        var l = typeof e.contentWindow.location.href == "string";
      } catch {
        l = !1;
      }
      if (l) t = e.contentWindow;
      else break;
      e = Vu(t.document);
    }
    return e;
  }
  function Hi(t) {
    var e = t && t.nodeName && t.nodeName.toLowerCase();
    return (
      e &&
      ((e === "input" &&
        (t.type === "text" ||
          t.type === "search" ||
          t.type === "tel" ||
          t.type === "url" ||
          t.type === "password")) ||
        e === "textarea" ||
        t.contentEditable === "true")
    );
  }
  var pm = He && "documentMode" in document && 11 >= document.documentMode,
    ta = null,
    ji = null,
    Za = null,
    Bi = !1;
  function Nr(t, e, l) {
    var a =
      l.window === l ? l.document : l.nodeType === 9 ? l : l.ownerDocument;
    Bi ||
      ta == null ||
      ta !== Vu(a) ||
      ((a = ta),
      "selectionStart" in a && Hi(a)
        ? (a = { start: a.selectionStart, end: a.selectionEnd })
        : ((a = (
            (a.ownerDocument && a.ownerDocument.defaultView) ||
            window
          ).getSelection()),
          (a = {
            anchorNode: a.anchorNode,
            anchorOffset: a.anchorOffset,
            focusNode: a.focusNode,
            focusOffset: a.focusOffset,
          })),
      (Za && Qa(Za, a)) ||
        ((Za = a),
        (a = Yn(ji, "onSelect")),
        0 < a.length &&
          ((e = new Wu("onSelect", "select", null, e, l)),
          t.push({ event: e, listeners: a }),
          (e.target = ta))));
  }
  function zl(t, e) {
    var l = {};
    return (
      (l[t.toLowerCase()] = e.toLowerCase()),
      (l["Webkit" + t] = "webkit" + e),
      (l["Moz" + t] = "moz" + e),
      l
    );
  }
  var ea = {
      animationend: zl("Animation", "AnimationEnd"),
      animationiteration: zl("Animation", "AnimationIteration"),
      animationstart: zl("Animation", "AnimationStart"),
      transitionrun: zl("Transition", "TransitionRun"),
      transitionstart: zl("Transition", "TransitionStart"),
      transitioncancel: zl("Transition", "TransitionCancel"),
      transitionend: zl("Transition", "TransitionEnd"),
    },
    qi = {},
    Ur = {};
  He &&
    ((Ur = document.createElement("div").style),
    "AnimationEvent" in window ||
      (delete ea.animationend.animation,
      delete ea.animationiteration.animation,
      delete ea.animationstart.animation),
    "TransitionEvent" in window || delete ea.transitionend.transition);
  function Ml(t) {
    if (qi[t]) return qi[t];
    if (!ea[t]) return t;
    var e = ea[t],
      l;
    for (l in e) if (e.hasOwnProperty(l) && l in Ur) return (qi[t] = e[l]);
    return t;
  }
  var Cr = Ml("animationend"),
    Hr = Ml("animationiteration"),
    jr = Ml("animationstart"),
    Sm = Ml("transitionrun"),
    bm = Ml("transitionstart"),
    Em = Ml("transitioncancel"),
    Br = Ml("transitionend"),
    qr = new Map(),
    Yi =
      "abort auxClick beforeToggle cancel canPlay canPlayThrough click close contextMenu copy cut drag dragEnd dragEnter dragExit dragLeave dragOver dragStart drop durationChange emptied encrypted ended error gotPointerCapture input invalid keyDown keyPress keyUp load loadedData loadedMetadata loadStart lostPointerCapture mouseDown mouseMove mouseOut mouseOver mouseUp paste pause play playing pointerCancel pointerDown pointerMove pointerOut pointerOver pointerUp progress rateChange reset resize seeked seeking stalled submit suspend timeUpdate touchCancel touchEnd touchStart volumeChange scroll toggle touchMove waiting wheel".split(
        " ",
      );
  Yi.push("scrollEnd");
  function Ee(t, e) {
    (qr.set(t, e), Rl(e, [t]));
  }
  var Yr = new WeakMap();
  function me(t, e) {
    if (typeof t == "object" && t !== null) {
      var l = Yr.get(t);
      return l !== void 0
        ? l
        : ((e = { value: t, source: e, stack: tr(e) }), Yr.set(t, e), e);
    }
    return { value: t, source: e, stack: tr(e) };
  }
  var ve = [],
    la = 0,
    wi = 0;
  function Iu() {
    for (var t = la, e = (wi = la = 0); e < t; ) {
      var l = ve[e];
      ve[e++] = null;
      var a = ve[e];
      ve[e++] = null;
      var u = ve[e];
      ve[e++] = null;
      var n = ve[e];
      if (((ve[e++] = null), a !== null && u !== null)) {
        var c = a.pending;
        (c === null ? (u.next = u) : ((u.next = c.next), (c.next = u)),
          (a.pending = u));
      }
      n !== 0 && wr(l, u, n);
    }
  }
  function tn(t, e, l, a) {
    ((ve[la++] = t),
      (ve[la++] = e),
      (ve[la++] = l),
      (ve[la++] = a),
      (wi |= a),
      (t.lanes |= a),
      (t = t.alternate),
      t !== null && (t.lanes |= a));
  }
  function Li(t, e, l, a) {
    return (tn(t, e, l, a), en(t));
  }
  function aa(t, e) {
    return (tn(t, null, null, e), en(t));
  }
  function wr(t, e, l) {
    t.lanes |= l;
    var a = t.alternate;
    a !== null && (a.lanes |= l);
    for (var u = !1, n = t.return; n !== null; )
      ((n.childLanes |= l),
        (a = n.alternate),
        a !== null && (a.childLanes |= l),
        n.tag === 22 &&
          ((t = n.stateNode), t === null || t._visibility & 1 || (u = !0)),
        (t = n),
        (n = n.return));
    return t.tag === 3
      ? ((n = t.stateNode),
        u &&
          e !== null &&
          ((u = 31 - ae(l)),
          (t = n.hiddenUpdates),
          (a = t[u]),
          a === null ? (t[u] = [e]) : a.push(e),
          (e.lane = l | 536870912)),
        n)
      : null;
  }
  function en(t) {
    if (50 < vu) throw ((vu = 0), (Kc = null), Error(f(185)));
    for (var e = t.return; e !== null; ) ((t = e), (e = t.return));
    return t.tag === 3 ? t.stateNode : null;
  }
  var ua = {};
  function Tm(t, e, l, a) {
    ((this.tag = t),
      (this.key = l),
      (this.sibling =
        this.child =
        this.return =
        this.stateNode =
        this.type =
        this.elementType =
          null),
      (this.index = 0),
      (this.refCleanup = this.ref = null),
      (this.pendingProps = e),
      (this.dependencies =
        this.memoizedState =
        this.updateQueue =
        this.memoizedProps =
          null),
      (this.mode = a),
      (this.subtreeFlags = this.flags = 0),
      (this.deletions = null),
      (this.childLanes = this.lanes = 0),
      (this.alternate = null));
  }
  function ne(t, e, l, a) {
    return new Tm(t, e, l, a);
  }
  function Gi(t) {
    return ((t = t.prototype), !(!t || !t.isReactComponent));
  }
  function je(t, e) {
    var l = t.alternate;
    return (
      l === null
        ? ((l = ne(t.tag, e, t.key, t.mode)),
          (l.elementType = t.elementType),
          (l.type = t.type),
          (l.stateNode = t.stateNode),
          (l.alternate = t),
          (t.alternate = l))
        : ((l.pendingProps = e),
          (l.type = t.type),
          (l.flags = 0),
          (l.subtreeFlags = 0),
          (l.deletions = null)),
      (l.flags = t.flags & 65011712),
      (l.childLanes = t.childLanes),
      (l.lanes = t.lanes),
      (l.child = t.child),
      (l.memoizedProps = t.memoizedProps),
      (l.memoizedState = t.memoizedState),
      (l.updateQueue = t.updateQueue),
      (e = t.dependencies),
      (l.dependencies =
        e === null ? null : { lanes: e.lanes, firstContext: e.firstContext }),
      (l.sibling = t.sibling),
      (l.index = t.index),
      (l.ref = t.ref),
      (l.refCleanup = t.refCleanup),
      l
    );
  }
  function Lr(t, e) {
    t.flags &= 65011714;
    var l = t.alternate;
    return (
      l === null
        ? ((t.childLanes = 0),
          (t.lanes = e),
          (t.child = null),
          (t.subtreeFlags = 0),
          (t.memoizedProps = null),
          (t.memoizedState = null),
          (t.updateQueue = null),
          (t.dependencies = null),
          (t.stateNode = null))
        : ((t.childLanes = l.childLanes),
          (t.lanes = l.lanes),
          (t.child = l.child),
          (t.subtreeFlags = 0),
          (t.deletions = null),
          (t.memoizedProps = l.memoizedProps),
          (t.memoizedState = l.memoizedState),
          (t.updateQueue = l.updateQueue),
          (t.type = l.type),
          (e = l.dependencies),
          (t.dependencies =
            e === null
              ? null
              : { lanes: e.lanes, firstContext: e.firstContext })),
      t
    );
  }
  function ln(t, e, l, a, u, n) {
    var c = 0;
    if (((a = t), typeof t == "function")) Gi(t) && (c = 1);
    else if (typeof t == "string")
      c = x0(t, l, J.current)
        ? 26
        : t === "html" || t === "head" || t === "body"
          ? 27
          : 5;
    else
      t: switch (t) {
        case St:
          return (
            (t = ne(31, l, e, u)),
            (t.elementType = St),
            (t.lanes = n),
            t
          );
        case M:
          return Dl(l.children, u, n, e);
        case Y:
          ((c = 8), (u |= 24));
          break;
        case Q:
          return (
            (t = ne(12, l, e, u | 2)),
            (t.elementType = Q),
            (t.lanes = n),
            t
          );
        case I:
          return ((t = ne(13, l, e, u)), (t.elementType = I), (t.lanes = n), t);
        case _t:
          return (
            (t = ne(19, l, e, u)),
            (t.elementType = _t),
            (t.lanes = n),
            t
          );
        default:
          if (typeof t == "object" && t !== null)
            switch (t.$$typeof) {
              case q:
              case P:
                c = 10;
                break t;
              case V:
                c = 9;
                break t;
              case ct:
                c = 11;
                break t;
              case xt:
                c = 14;
                break t;
              case zt:
                ((c = 16), (a = null));
                break t;
            }
          ((c = 29),
            (l = Error(f(130, t === null ? "null" : typeof t, ""))),
            (a = null));
      }
    return (
      (e = ne(c, l, e, u)),
      (e.elementType = t),
      (e.type = a),
      (e.lanes = n),
      e
    );
  }
  function Dl(t, e, l, a) {
    return ((t = ne(7, t, a, e)), (t.lanes = l), t);
  }
  function Xi(t, e, l) {
    return ((t = ne(6, t, null, e)), (t.lanes = l), t);
  }
  function Qi(t, e, l) {
    return (
      (e = ne(4, t.children !== null ? t.children : [], t.key, e)),
      (e.lanes = l),
      (e.stateNode = {
        containerInfo: t.containerInfo,
        pendingChildren: null,
        implementation: t.implementation,
      }),
      e
    );
  }
  var na = [],
    ia = 0,
    an = null,
    un = 0,
    ye = [],
    ge = 0,
    _l = null,
    Be = 1,
    qe = "";
  function Nl(t, e) {
    ((na[ia++] = un), (na[ia++] = an), (an = t), (un = e));
  }
  function Gr(t, e, l) {
    ((ye[ge++] = Be), (ye[ge++] = qe), (ye[ge++] = _l), (_l = t));
    var a = Be;
    t = qe;
    var u = 32 - ae(a) - 1;
    ((a &= ~(1 << u)), (l += 1));
    var n = 32 - ae(e) + u;
    if (30 < n) {
      var c = u - (u % 5);
      ((n = (a & ((1 << c) - 1)).toString(32)),
        (a >>= c),
        (u -= c),
        (Be = (1 << (32 - ae(e) + u)) | (l << u) | a),
        (qe = n + t));
    } else ((Be = (1 << n) | (l << u) | a), (qe = t));
  }
  function Zi(t) {
    t.return !== null && (Nl(t, 1), Gr(t, 1, 0));
  }
  function Vi(t) {
    for (; t === an; )
      ((an = na[--ia]), (na[ia] = null), (un = na[--ia]), (na[ia] = null));
    for (; t === _l; )
      ((_l = ye[--ge]),
        (ye[ge] = null),
        (qe = ye[--ge]),
        (ye[ge] = null),
        (Be = ye[--ge]),
        (ye[ge] = null));
  }
  var $t = null,
    Rt = null,
    st = !1,
    Ul = null,
    Oe = !1,
    Ki = Error(f(519));
  function Cl(t) {
    var e = Error(f(418, ""));
    throw (Ja(me(e, t)), Ki);
  }
  function Xr(t) {
    var e = t.stateNode,
      l = t.type,
      a = t.memoizedProps;
    switch (((e[Zt] = t), (e[Wt] = a), l)) {
      case "dialog":
        (nt("cancel", e), nt("close", e));
        break;
      case "iframe":
      case "object":
      case "embed":
        nt("load", e);
        break;
      case "video":
      case "audio":
        for (l = 0; l < gu.length; l++) nt(gu[l], e);
        break;
      case "source":
        nt("error", e);
        break;
      case "img":
      case "image":
      case "link":
        (nt("error", e), nt("load", e));
        break;
      case "details":
        nt("toggle", e);
        break;
      case "input":
        (nt("invalid", e),
          ar(
            e,
            a.value,
            a.defaultValue,
            a.checked,
            a.defaultChecked,
            a.type,
            a.name,
            !0,
          ),
          Zu(e));
        break;
      case "select":
        nt("invalid", e);
        break;
      case "textarea":
        (nt("invalid", e), nr(e, a.value, a.defaultValue, a.children), Zu(e));
    }
    ((l = a.children),
      (typeof l != "string" && typeof l != "number" && typeof l != "bigint") ||
      e.textContent === "" + l ||
      a.suppressHydrationWarning === !0 ||
      id(e.textContent, l)
        ? (a.popover != null && (nt("beforetoggle", e), nt("toggle", e)),
          a.onScroll != null && nt("scroll", e),
          a.onScrollEnd != null && nt("scrollend", e),
          a.onClick != null && (e.onclick = wn),
          (e = !0))
        : (e = !1),
      e || Cl(t));
  }
  function Qr(t) {
    for ($t = t.return; $t; )
      switch ($t.tag) {
        case 5:
        case 13:
          Oe = !1;
          return;
        case 27:
        case 3:
          Oe = !0;
          return;
        default:
          $t = $t.return;
      }
  }
  function Va(t) {
    if (t !== $t) return !1;
    if (!st) return (Qr(t), (st = !0), !1);
    var e = t.tag,
      l;
    if (
      ((l = e !== 3 && e !== 27) &&
        ((l = e === 5) &&
          ((l = t.type),
          (l =
            !(l !== "form" && l !== "button") || rf(t.type, t.memoizedProps))),
        (l = !l)),
      l && Rt && Cl(t),
      Qr(t),
      e === 13)
    ) {
      if (((t = t.memoizedState), (t = t !== null ? t.dehydrated : null), !t))
        throw Error(f(317));
      t: {
        for (t = t.nextSibling, e = 0; t; ) {
          if (t.nodeType === 8)
            if (((l = t.data), l === "/$")) {
              if (e === 0) {
                Rt = Ae(t.nextSibling);
                break t;
              }
              e--;
            } else (l !== "$" && l !== "$!" && l !== "$?") || e++;
          t = t.nextSibling;
        }
        Rt = null;
      }
    } else
      e === 27
        ? ((e = Rt), yl(t.type) ? ((t = hf), (hf = null), (Rt = t)) : (Rt = e))
        : (Rt = $t ? Ae(t.stateNode.nextSibling) : null);
    return !0;
  }
  function Ka() {
    ((Rt = $t = null), (st = !1));
  }
  function Zr() {
    var t = Ul;
    return (
      t !== null &&
        (te === null ? (te = t) : te.push.apply(te, t), (Ul = null)),
      t
    );
  }
  function Ja(t) {
    Ul === null ? (Ul = [t]) : Ul.push(t);
  }
  var Ji = B(null),
    Hl = null,
    Ye = null;
  function el(t, e, l) {
    (w(Ji, e._currentValue), (e._currentValue = l));
  }
  function we(t) {
    ((t._currentValue = Ji.current), X(Ji));
  }
  function $i(t, e, l) {
    for (; t !== null; ) {
      var a = t.alternate;
      if (
        ((t.childLanes & e) !== e
          ? ((t.childLanes |= e), a !== null && (a.childLanes |= e))
          : a !== null && (a.childLanes & e) !== e && (a.childLanes |= e),
        t === l)
      )
        break;
      t = t.return;
    }
  }
  function ki(t, e, l, a) {
    var u = t.child;
    for (u !== null && (u.return = t); u !== null; ) {
      var n = u.dependencies;
      if (n !== null) {
        var c = u.child;
        n = n.firstContext;
        t: for (; n !== null; ) {
          var r = n;
          n = u;
          for (var m = 0; m < e.length; m++)
            if (r.context === e[m]) {
              ((n.lanes |= l),
                (r = n.alternate),
                r !== null && (r.lanes |= l),
                $i(n.return, l, t),
                a || (c = null));
              break t;
            }
          n = r.next;
        }
      } else if (u.tag === 18) {
        if (((c = u.return), c === null)) throw Error(f(341));
        ((c.lanes |= l),
          (n = c.alternate),
          n !== null && (n.lanes |= l),
          $i(c, l, t),
          (c = null));
      } else c = u.child;
      if (c !== null) c.return = u;
      else
        for (c = u; c !== null; ) {
          if (c === t) {
            c = null;
            break;
          }
          if (((u = c.sibling), u !== null)) {
            ((u.return = c.return), (c = u));
            break;
          }
          c = c.return;
        }
      u = c;
    }
  }
  function $a(t, e, l, a) {
    t = null;
    for (var u = e, n = !1; u !== null; ) {
      if (!n) {
        if ((u.flags & 524288) !== 0) n = !0;
        else if ((u.flags & 262144) !== 0) break;
      }
      if (u.tag === 10) {
        var c = u.alternate;
        if (c === null) throw Error(f(387));
        if (((c = c.memoizedProps), c !== null)) {
          var r = u.type;
          ue(u.pendingProps.value, c.value) ||
            (t !== null ? t.push(r) : (t = [r]));
        }
      } else if (u === ee.current) {
        if (((c = u.alternate), c === null)) throw Error(f(387));
        c.memoizedState.memoizedState !== u.memoizedState.memoizedState &&
          (t !== null ? t.push(Au) : (t = [Au]));
      }
      u = u.return;
    }
    (t !== null && ki(e, t, l, a), (e.flags |= 262144));
  }
  function nn(t) {
    for (t = t.firstContext; t !== null; ) {
      if (!ue(t.context._currentValue, t.memoizedValue)) return !0;
      t = t.next;
    }
    return !1;
  }
  function jl(t) {
    ((Hl = t),
      (Ye = null),
      (t = t.dependencies),
      t !== null && (t.firstContext = null));
  }
  function Vt(t) {
    return Vr(Hl, t);
  }
  function cn(t, e) {
    return (Hl === null && jl(t), Vr(t, e));
  }
  function Vr(t, e) {
    var l = e._currentValue;
    if (((e = { context: e, memoizedValue: l, next: null }), Ye === null)) {
      if (t === null) throw Error(f(308));
      ((Ye = e),
        (t.dependencies = { lanes: 0, firstContext: e }),
        (t.flags |= 524288));
    } else Ye = Ye.next = e;
    return l;
  }
  var Am =
      typeof AbortController < "u"
        ? AbortController
        : function () {
            var t = [],
              e = (this.signal = {
                aborted: !1,
                addEventListener: function (l, a) {
                  t.push(a);
                },
              });
            this.abort = function () {
              ((e.aborted = !0),
                t.forEach(function (l) {
                  return l();
                }));
            };
          },
    xm = i.unstable_scheduleCallback,
    Rm = i.unstable_NormalPriority,
    Ct = {
      $$typeof: P,
      Consumer: null,
      Provider: null,
      _currentValue: null,
      _currentValue2: null,
      _threadCount: 0,
    };
  function Wi() {
    return { controller: new Am(), data: new Map(), refCount: 0 };
  }
  function ka(t) {
    (t.refCount--,
      t.refCount === 0 &&
        xm(Rm, function () {
          t.controller.abort();
        }));
  }
  var Wa = null,
    Fi = 0,
    ca = 0,
    fa = null;
  function Om(t, e) {
    if (Wa === null) {
      var l = (Wa = []);
      ((Fi = 0),
        (ca = Ic()),
        (fa = {
          status: "pending",
          value: void 0,
          then: function (a) {
            l.push(a);
          },
        }));
    }
    return (Fi++, e.then(Kr, Kr), e);
  }
  function Kr() {
    if (--Fi === 0 && Wa !== null) {
      fa !== null && (fa.status = "fulfilled");
      var t = Wa;
      ((Wa = null), (ca = 0), (fa = null));
      for (var e = 0; e < t.length; e++) (0, t[e])();
    }
  }
  function zm(t, e) {
    var l = [],
      a = {
        status: "pending",
        value: null,
        reason: null,
        then: function (u) {
          l.push(u);
        },
      };
    return (
      t.then(
        function () {
          ((a.status = "fulfilled"), (a.value = e));
          for (var u = 0; u < l.length; u++) (0, l[u])(e);
        },
        function (u) {
          for (a.status = "rejected", a.reason = u, u = 0; u < l.length; u++)
            (0, l[u])(void 0);
        },
      ),
      a
    );
  }
  var Jr = N.S;
  N.S = function (t, e) {
    (typeof e == "object" &&
      e !== null &&
      typeof e.then == "function" &&
      Om(t, e),
      Jr !== null && Jr(t, e));
  };
  var Bl = B(null);
  function Pi() {
    var t = Bl.current;
    return t !== null ? t : pt.pooledCache;
  }
  function fn(t, e) {
    e === null ? w(Bl, Bl.current) : w(Bl, e.pool);
  }
  function $r() {
    var t = Pi();
    return t === null ? null : { parent: Ct._currentValue, pool: t };
  }
  var Fa = Error(f(460)),
    kr = Error(f(474)),
    rn = Error(f(542)),
    Ii = { then: function () {} };
  function Wr(t) {
    return ((t = t.status), t === "fulfilled" || t === "rejected");
  }
  function sn() {}
  function Fr(t, e, l) {
    switch (
      ((l = t[l]),
      l === void 0 ? t.push(e) : l !== e && (e.then(sn, sn), (e = l)),
      e.status)
    ) {
      case "fulfilled":
        return e.value;
      case "rejected":
        throw ((t = e.reason), Ir(t), t);
      default:
        if (typeof e.status == "string") e.then(sn, sn);
        else {
          if (((t = pt), t !== null && 100 < t.shellSuspendCounter))
            throw Error(f(482));
          ((t = e),
            (t.status = "pending"),
            t.then(
              function (a) {
                if (e.status === "pending") {
                  var u = e;
                  ((u.status = "fulfilled"), (u.value = a));
                }
              },
              function (a) {
                if (e.status === "pending") {
                  var u = e;
                  ((u.status = "rejected"), (u.reason = a));
                }
              },
            ));
        }
        switch (e.status) {
          case "fulfilled":
            return e.value;
          case "rejected":
            throw ((t = e.reason), Ir(t), t);
        }
        throw ((Pa = e), Fa);
    }
  }
  var Pa = null;
  function Pr() {
    if (Pa === null) throw Error(f(459));
    var t = Pa;
    return ((Pa = null), t);
  }
  function Ir(t) {
    if (t === Fa || t === rn) throw Error(f(483));
  }
  var ll = !1;
  function tc(t) {
    t.updateQueue = {
      baseState: t.memoizedState,
      firstBaseUpdate: null,
      lastBaseUpdate: null,
      shared: { pending: null, lanes: 0, hiddenCallbacks: null },
      callbacks: null,
    };
  }
  function ec(t, e) {
    ((t = t.updateQueue),
      e.updateQueue === t &&
        (e.updateQueue = {
          baseState: t.baseState,
          firstBaseUpdate: t.firstBaseUpdate,
          lastBaseUpdate: t.lastBaseUpdate,
          shared: t.shared,
          callbacks: null,
        }));
  }
  function al(t) {
    return { lane: t, tag: 0, payload: null, callback: null, next: null };
  }
  function ul(t, e, l) {
    var a = t.updateQueue;
    if (a === null) return null;
    if (((a = a.shared), (ot & 2) !== 0)) {
      var u = a.pending;
      return (
        u === null ? (e.next = e) : ((e.next = u.next), (u.next = e)),
        (a.pending = e),
        (e = en(t)),
        wr(t, null, l),
        e
      );
    }
    return (tn(t, a, e, l), en(t));
  }
  function Ia(t, e, l) {
    if (
      ((e = e.updateQueue), e !== null && ((e = e.shared), (l & 4194048) !== 0))
    ) {
      var a = e.lanes;
      ((a &= t.pendingLanes), (l |= a), (e.lanes = l), Kf(t, l));
    }
  }
  function lc(t, e) {
    var l = t.updateQueue,
      a = t.alternate;
    if (a !== null && ((a = a.updateQueue), l === a)) {
      var u = null,
        n = null;
      if (((l = l.firstBaseUpdate), l !== null)) {
        do {
          var c = {
            lane: l.lane,
            tag: l.tag,
            payload: l.payload,
            callback: null,
            next: null,
          };
          (n === null ? (u = n = c) : (n = n.next = c), (l = l.next));
        } while (l !== null);
        n === null ? (u = n = e) : (n = n.next = e);
      } else u = n = e;
      ((l = {
        baseState: a.baseState,
        firstBaseUpdate: u,
        lastBaseUpdate: n,
        shared: a.shared,
        callbacks: a.callbacks,
      }),
        (t.updateQueue = l));
      return;
    }
    ((t = l.lastBaseUpdate),
      t === null ? (l.firstBaseUpdate = e) : (t.next = e),
      (l.lastBaseUpdate = e));
  }
  var ac = !1;
  function tu() {
    if (ac) {
      var t = fa;
      if (t !== null) throw t;
    }
  }
  function eu(t, e, l, a) {
    ac = !1;
    var u = t.updateQueue;
    ll = !1;
    var n = u.firstBaseUpdate,
      c = u.lastBaseUpdate,
      r = u.shared.pending;
    if (r !== null) {
      u.shared.pending = null;
      var m = r,
        T = m.next;
      ((m.next = null), c === null ? (n = T) : (c.next = T), (c = m));
      var _ = t.alternate;
      _ !== null &&
        ((_ = _.updateQueue),
        (r = _.lastBaseUpdate),
        r !== c &&
          (r === null ? (_.firstBaseUpdate = T) : (r.next = T),
          (_.lastBaseUpdate = m)));
    }
    if (n !== null) {
      var H = u.baseState;
      ((c = 0), (_ = T = m = null), (r = n));
      do {
        var x = r.lane & -536870913,
          R = x !== r.lane;
        if (R ? (it & x) === x : (a & x) === x) {
          (x !== 0 && x === ca && (ac = !0),
            _ !== null &&
              (_ = _.next =
                {
                  lane: 0,
                  tag: r.tag,
                  payload: r.payload,
                  callback: null,
                  next: null,
                }));
          t: {
            var F = t,
              $ = r;
            x = e;
            var vt = l;
            switch ($.tag) {
              case 1:
                if (((F = $.payload), typeof F == "function")) {
                  H = F.call(vt, H, x);
                  break t;
                }
                H = F;
                break t;
              case 3:
                F.flags = (F.flags & -65537) | 128;
              case 0:
                if (
                  ((F = $.payload),
                  (x = typeof F == "function" ? F.call(vt, H, x) : F),
                  x == null)
                )
                  break t;
                H = O({}, H, x);
                break t;
              case 2:
                ll = !0;
            }
          }
          ((x = r.callback),
            x !== null &&
              ((t.flags |= 64),
              R && (t.flags |= 8192),
              (R = u.callbacks),
              R === null ? (u.callbacks = [x]) : R.push(x)));
        } else
          ((R = {
            lane: x,
            tag: r.tag,
            payload: r.payload,
            callback: r.callback,
            next: null,
          }),
            _ === null ? ((T = _ = R), (m = H)) : (_ = _.next = R),
            (c |= x));
        if (((r = r.next), r === null)) {
          if (((r = u.shared.pending), r === null)) break;
          ((R = r),
            (r = R.next),
            (R.next = null),
            (u.lastBaseUpdate = R),
            (u.shared.pending = null));
        }
      } while (!0);
      (_ === null && (m = H),
        (u.baseState = m),
        (u.firstBaseUpdate = T),
        (u.lastBaseUpdate = _),
        n === null && (u.shared.lanes = 0),
        (dl |= c),
        (t.lanes = c),
        (t.memoizedState = H));
    }
  }
  function ts(t, e) {
    if (typeof t != "function") throw Error(f(191, t));
    t.call(e);
  }
  function es(t, e) {
    var l = t.callbacks;
    if (l !== null)
      for (t.callbacks = null, t = 0; t < l.length; t++) ts(l[t], e);
  }
  var ra = B(null),
    on = B(0);
  function ls(t, e) {
    ((t = Ke), w(on, t), w(ra, e), (Ke = t | e.baseLanes));
  }
  function uc() {
    (w(on, Ke), w(ra, ra.current));
  }
  function nc() {
    ((Ke = on.current), X(ra), X(on));
  }
  var nl = 0,
    lt = null,
    ht = null,
    Nt = null,
    dn = !1,
    sa = !1,
    ql = !1,
    hn = 0,
    lu = 0,
    oa = null,
    Mm = 0;
  function Mt() {
    throw Error(f(321));
  }
  function ic(t, e) {
    if (e === null) return !1;
    for (var l = 0; l < e.length && l < t.length; l++)
      if (!ue(t[l], e[l])) return !1;
    return !0;
  }
  function cc(t, e, l, a, u, n) {
    return (
      (nl = n),
      (lt = e),
      (e.memoizedState = null),
      (e.updateQueue = null),
      (e.lanes = 0),
      (N.H = t === null || t.memoizedState === null ? ws : Ls),
      (ql = !1),
      (n = l(a, u)),
      (ql = !1),
      sa && (n = us(e, l, a, u)),
      as(t),
      n
    );
  }
  function as(t) {
    N.H = Sn;
    var e = ht !== null && ht.next !== null;
    if (((nl = 0), (Nt = ht = lt = null), (dn = !1), (lu = 0), (oa = null), e))
      throw Error(f(300));
    t === null ||
      Bt ||
      ((t = t.dependencies), t !== null && nn(t) && (Bt = !0));
  }
  function us(t, e, l, a) {
    lt = t;
    var u = 0;
    do {
      if ((sa && (oa = null), (lu = 0), (sa = !1), 25 <= u))
        throw Error(f(301));
      if (((u += 1), (Nt = ht = null), t.updateQueue != null)) {
        var n = t.updateQueue;
        ((n.lastEffect = null),
          (n.events = null),
          (n.stores = null),
          n.memoCache != null && (n.memoCache.index = 0));
      }
      ((N.H = jm), (n = e(l, a)));
    } while (sa);
    return n;
  }
  function Dm() {
    var t = N.H,
      e = t.useState()[0];
    return (
      (e = typeof e.then == "function" ? au(e) : e),
      (t = t.useState()[0]),
      (ht !== null ? ht.memoizedState : null) !== t && (lt.flags |= 1024),
      e
    );
  }
  function fc() {
    var t = hn !== 0;
    return ((hn = 0), t);
  }
  function rc(t, e, l) {
    ((e.updateQueue = t.updateQueue), (e.flags &= -2053), (t.lanes &= ~l));
  }
  function sc(t) {
    if (dn) {
      for (t = t.memoizedState; t !== null; ) {
        var e = t.queue;
        (e !== null && (e.pending = null), (t = t.next));
      }
      dn = !1;
    }
    ((nl = 0), (Nt = ht = lt = null), (sa = !1), (lu = hn = 0), (oa = null));
  }
  function Pt() {
    var t = {
      memoizedState: null,
      baseState: null,
      baseQueue: null,
      queue: null,
      next: null,
    };
    return (Nt === null ? (lt.memoizedState = Nt = t) : (Nt = Nt.next = t), Nt);
  }
  function Ut() {
    if (ht === null) {
      var t = lt.alternate;
      t = t !== null ? t.memoizedState : null;
    } else t = ht.next;
    var e = Nt === null ? lt.memoizedState : Nt.next;
    if (e !== null) ((Nt = e), (ht = t));
    else {
      if (t === null)
        throw lt.alternate === null ? Error(f(467)) : Error(f(310));
      ((ht = t),
        (t = {
          memoizedState: ht.memoizedState,
          baseState: ht.baseState,
          baseQueue: ht.baseQueue,
          queue: ht.queue,
          next: null,
        }),
        Nt === null ? (lt.memoizedState = Nt = t) : (Nt = Nt.next = t));
    }
    return Nt;
  }
  function oc() {
    return { lastEffect: null, events: null, stores: null, memoCache: null };
  }
  function au(t) {
    var e = lu;
    return (
      (lu += 1),
      oa === null && (oa = []),
      (t = Fr(oa, t, e)),
      (e = lt),
      (Nt === null ? e.memoizedState : Nt.next) === null &&
        ((e = e.alternate),
        (N.H = e === null || e.memoizedState === null ? ws : Ls)),
      t
    );
  }
  function mn(t) {
    if (t !== null && typeof t == "object") {
      if (typeof t.then == "function") return au(t);
      if (t.$$typeof === P) return Vt(t);
    }
    throw Error(f(438, String(t)));
  }
  function dc(t) {
    var e = null,
      l = lt.updateQueue;
    if ((l !== null && (e = l.memoCache), e == null)) {
      var a = lt.alternate;
      a !== null &&
        ((a = a.updateQueue),
        a !== null &&
          ((a = a.memoCache),
          a != null &&
            (e = {
              data: a.data.map(function (u) {
                return u.slice();
              }),
              index: 0,
            })));
    }
    if (
      (e == null && (e = { data: [], index: 0 }),
      l === null && ((l = oc()), (lt.updateQueue = l)),
      (l.memoCache = e),
      (l = e.data[e.index]),
      l === void 0)
    )
      for (l = e.data[e.index] = Array(t), a = 0; a < t; a++) l[a] = Jt;
    return (e.index++, l);
  }
  function Le(t, e) {
    return typeof e == "function" ? e(t) : e;
  }
  function vn(t) {
    var e = Ut();
    return hc(e, ht, t);
  }
  function hc(t, e, l) {
    var a = t.queue;
    if (a === null) throw Error(f(311));
    a.lastRenderedReducer = l;
    var u = t.baseQueue,
      n = a.pending;
    if (n !== null) {
      if (u !== null) {
        var c = u.next;
        ((u.next = n.next), (n.next = c));
      }
      ((e.baseQueue = u = n), (a.pending = null));
    }
    if (((n = t.baseState), u === null)) t.memoizedState = n;
    else {
      e = u.next;
      var r = (c = null),
        m = null,
        T = e,
        _ = !1;
      do {
        var H = T.lane & -536870913;
        if (H !== T.lane ? (it & H) === H : (nl & H) === H) {
          var x = T.revertLane;
          if (x === 0)
            (m !== null &&
              (m = m.next =
                {
                  lane: 0,
                  revertLane: 0,
                  action: T.action,
                  hasEagerState: T.hasEagerState,
                  eagerState: T.eagerState,
                  next: null,
                }),
              H === ca && (_ = !0));
          else if ((nl & x) === x) {
            ((T = T.next), x === ca && (_ = !0));
            continue;
          } else
            ((H = {
              lane: 0,
              revertLane: T.revertLane,
              action: T.action,
              hasEagerState: T.hasEagerState,
              eagerState: T.eagerState,
              next: null,
            }),
              m === null ? ((r = m = H), (c = n)) : (m = m.next = H),
              (lt.lanes |= x),
              (dl |= x));
          ((H = T.action),
            ql && l(n, H),
            (n = T.hasEagerState ? T.eagerState : l(n, H)));
        } else
          ((x = {
            lane: H,
            revertLane: T.revertLane,
            action: T.action,
            hasEagerState: T.hasEagerState,
            eagerState: T.eagerState,
            next: null,
          }),
            m === null ? ((r = m = x), (c = n)) : (m = m.next = x),
            (lt.lanes |= H),
            (dl |= H));
        T = T.next;
      } while (T !== null && T !== e);
      if (
        (m === null ? (c = n) : (m.next = r),
        !ue(n, t.memoizedState) && ((Bt = !0), _ && ((l = fa), l !== null)))
      )
        throw l;
      ((t.memoizedState = n),
        (t.baseState = c),
        (t.baseQueue = m),
        (a.lastRenderedState = n));
    }
    return (u === null && (a.lanes = 0), [t.memoizedState, a.dispatch]);
  }
  function mc(t) {
    var e = Ut(),
      l = e.queue;
    if (l === null) throw Error(f(311));
    l.lastRenderedReducer = t;
    var a = l.dispatch,
      u = l.pending,
      n = e.memoizedState;
    if (u !== null) {
      l.pending = null;
      var c = (u = u.next);
      do ((n = t(n, c.action)), (c = c.next));
      while (c !== u);
      (ue(n, e.memoizedState) || (Bt = !0),
        (e.memoizedState = n),
        e.baseQueue === null && (e.baseState = n),
        (l.lastRenderedState = n));
    }
    return [n, a];
  }
  function ns(t, e, l) {
    var a = lt,
      u = Ut(),
      n = st;
    if (n) {
      if (l === void 0) throw Error(f(407));
      l = l();
    } else l = e();
    var c = !ue((ht || u).memoizedState, l);
    (c && ((u.memoizedState = l), (Bt = !0)), (u = u.queue));
    var r = fs.bind(null, a, u, t);
    if (
      (uu(2048, 8, r, [t]),
      u.getSnapshot !== e || c || (Nt !== null && Nt.memoizedState.tag & 1))
    ) {
      if (
        ((a.flags |= 2048),
        da(9, yn(), cs.bind(null, a, u, l, e), null),
        pt === null)
      )
        throw Error(f(349));
      n || (nl & 124) !== 0 || is(a, e, l);
    }
    return l;
  }
  function is(t, e, l) {
    ((t.flags |= 16384),
      (t = { getSnapshot: e, value: l }),
      (e = lt.updateQueue),
      e === null
        ? ((e = oc()), (lt.updateQueue = e), (e.stores = [t]))
        : ((l = e.stores), l === null ? (e.stores = [t]) : l.push(t)));
  }
  function cs(t, e, l, a) {
    ((e.value = l), (e.getSnapshot = a), rs(e) && ss(t));
  }
  function fs(t, e, l) {
    return l(function () {
      rs(e) && ss(t);
    });
  }
  function rs(t) {
    var e = t.getSnapshot;
    t = t.value;
    try {
      var l = e();
      return !ue(t, l);
    } catch {
      return !0;
    }
  }
  function ss(t) {
    var e = aa(t, 2);
    e !== null && se(e, t, 2);
  }
  function vc(t) {
    var e = Pt();
    if (typeof t == "function") {
      var l = t;
      if (((t = l()), ql)) {
        Pe(!0);
        try {
          l();
        } finally {
          Pe(!1);
        }
      }
    }
    return (
      (e.memoizedState = e.baseState = t),
      (e.queue = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: Le,
        lastRenderedState: t,
      }),
      e
    );
  }
  function os(t, e, l, a) {
    return ((t.baseState = l), hc(t, ht, typeof a == "function" ? a : Le));
  }
  function _m(t, e, l, a, u) {
    if (pn(t)) throw Error(f(485));
    if (((t = e.action), t !== null)) {
      var n = {
        payload: u,
        action: t,
        next: null,
        isTransition: !0,
        status: "pending",
        value: null,
        reason: null,
        listeners: [],
        then: function (c) {
          n.listeners.push(c);
        },
      };
      (N.T !== null ? l(!0) : (n.isTransition = !1),
        a(n),
        (l = e.pending),
        l === null
          ? ((n.next = e.pending = n), ds(e, n))
          : ((n.next = l.next), (e.pending = l.next = n)));
    }
  }
  function ds(t, e) {
    var l = e.action,
      a = e.payload,
      u = t.state;
    if (e.isTransition) {
      var n = N.T,
        c = {};
      N.T = c;
      try {
        var r = l(u, a),
          m = N.S;
        (m !== null && m(c, r), hs(t, e, r));
      } catch (T) {
        yc(t, e, T);
      } finally {
        N.T = n;
      }
    } else
      try {
        ((n = l(u, a)), hs(t, e, n));
      } catch (T) {
        yc(t, e, T);
      }
  }
  function hs(t, e, l) {
    l !== null && typeof l == "object" && typeof l.then == "function"
      ? l.then(
          function (a) {
            ms(t, e, a);
          },
          function (a) {
            return yc(t, e, a);
          },
        )
      : ms(t, e, l);
  }
  function ms(t, e, l) {
    ((e.status = "fulfilled"),
      (e.value = l),
      vs(e),
      (t.state = l),
      (e = t.pending),
      e !== null &&
        ((l = e.next),
        l === e ? (t.pending = null) : ((l = l.next), (e.next = l), ds(t, l))));
  }
  function yc(t, e, l) {
    var a = t.pending;
    if (((t.pending = null), a !== null)) {
      a = a.next;
      do ((e.status = "rejected"), (e.reason = l), vs(e), (e = e.next));
      while (e !== a);
    }
    t.action = null;
  }
  function vs(t) {
    t = t.listeners;
    for (var e = 0; e < t.length; e++) (0, t[e])();
  }
  function ys(t, e) {
    return e;
  }
  function gs(t, e) {
    if (st) {
      var l = pt.formState;
      if (l !== null) {
        t: {
          var a = lt;
          if (st) {
            if (Rt) {
              e: {
                for (var u = Rt, n = Oe; u.nodeType !== 8; ) {
                  if (!n) {
                    u = null;
                    break e;
                  }
                  if (((u = Ae(u.nextSibling)), u === null)) {
                    u = null;
                    break e;
                  }
                }
                ((n = u.data), (u = n === "F!" || n === "F" ? u : null));
              }
              if (u) {
                ((Rt = Ae(u.nextSibling)), (a = u.data === "F!"));
                break t;
              }
            }
            Cl(a);
          }
          a = !1;
        }
        a && (e = l[0]);
      }
    }
    return (
      (l = Pt()),
      (l.memoizedState = l.baseState = e),
      (a = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: ys,
        lastRenderedState: e,
      }),
      (l.queue = a),
      (l = Bs.bind(null, lt, a)),
      (a.dispatch = l),
      (a = vc(!1)),
      (n = Ec.bind(null, lt, !1, a.queue)),
      (a = Pt()),
      (u = { state: e, dispatch: null, action: t, pending: null }),
      (a.queue = u),
      (l = _m.bind(null, lt, u, n, l)),
      (u.dispatch = l),
      (a.memoizedState = t),
      [e, l, !1]
    );
  }
  function ps(t) {
    var e = Ut();
    return Ss(e, ht, t);
  }
  function Ss(t, e, l) {
    if (
      ((e = hc(t, e, ys)[0]),
      (t = vn(Le)[0]),
      typeof e == "object" && e !== null && typeof e.then == "function")
    )
      try {
        var a = au(e);
      } catch (c) {
        throw c === Fa ? rn : c;
      }
    else a = e;
    e = Ut();
    var u = e.queue,
      n = u.dispatch;
    return (
      l !== e.memoizedState &&
        ((lt.flags |= 2048), da(9, yn(), Nm.bind(null, u, l), null)),
      [a, n, t]
    );
  }
  function Nm(t, e) {
    t.action = e;
  }
  function bs(t) {
    var e = Ut(),
      l = ht;
    if (l !== null) return Ss(e, l, t);
    (Ut(), (e = e.memoizedState), (l = Ut()));
    var a = l.queue.dispatch;
    return ((l.memoizedState = t), [e, a, !1]);
  }
  function da(t, e, l, a) {
    return (
      (t = { tag: t, create: l, deps: a, inst: e, next: null }),
      (e = lt.updateQueue),
      e === null && ((e = oc()), (lt.updateQueue = e)),
      (l = e.lastEffect),
      l === null
        ? (e.lastEffect = t.next = t)
        : ((a = l.next), (l.next = t), (t.next = a), (e.lastEffect = t)),
      t
    );
  }
  function yn() {
    return { destroy: void 0, resource: void 0 };
  }
  function Es() {
    return Ut().memoizedState;
  }
  function gn(t, e, l, a) {
    var u = Pt();
    ((a = a === void 0 ? null : a),
      (lt.flags |= t),
      (u.memoizedState = da(1 | e, yn(), l, a)));
  }
  function uu(t, e, l, a) {
    var u = Ut();
    a = a === void 0 ? null : a;
    var n = u.memoizedState.inst;
    ht !== null && a !== null && ic(a, ht.memoizedState.deps)
      ? (u.memoizedState = da(e, n, l, a))
      : ((lt.flags |= t), (u.memoizedState = da(1 | e, n, l, a)));
  }
  function Ts(t, e) {
    gn(8390656, 8, t, e);
  }
  function As(t, e) {
    uu(2048, 8, t, e);
  }
  function xs(t, e) {
    return uu(4, 2, t, e);
  }
  function Rs(t, e) {
    return uu(4, 4, t, e);
  }
  function Os(t, e) {
    if (typeof e == "function") {
      t = t();
      var l = e(t);
      return function () {
        typeof l == "function" ? l() : e(null);
      };
    }
    if (e != null)
      return (
        (t = t()),
        (e.current = t),
        function () {
          e.current = null;
        }
      );
  }
  function zs(t, e, l) {
    ((l = l != null ? l.concat([t]) : null), uu(4, 4, Os.bind(null, e, t), l));
  }
  function gc() {}
  function Ms(t, e) {
    var l = Ut();
    e = e === void 0 ? null : e;
    var a = l.memoizedState;
    return e !== null && ic(e, a[1]) ? a[0] : ((l.memoizedState = [t, e]), t);
  }
  function Ds(t, e) {
    var l = Ut();
    e = e === void 0 ? null : e;
    var a = l.memoizedState;
    if (e !== null && ic(e, a[1])) return a[0];
    if (((a = t()), ql)) {
      Pe(!0);
      try {
        t();
      } finally {
        Pe(!1);
      }
    }
    return ((l.memoizedState = [a, e]), a);
  }
  function pc(t, e, l) {
    return l === void 0 || (nl & 1073741824) !== 0
      ? (t.memoizedState = e)
      : ((t.memoizedState = l), (t = Co()), (lt.lanes |= t), (dl |= t), l);
  }
  function _s(t, e, l, a) {
    return ue(l, e)
      ? l
      : ra.current !== null
        ? ((t = pc(t, l, a)), ue(t, e) || (Bt = !0), t)
        : (nl & 42) === 0
          ? ((Bt = !0), (t.memoizedState = l))
          : ((t = Co()), (lt.lanes |= t), (dl |= t), e);
  }
  function Ns(t, e, l, a, u) {
    var n = L.p;
    L.p = n !== 0 && 8 > n ? n : 8;
    var c = N.T,
      r = {};
    ((N.T = r), Ec(t, !1, e, l));
    try {
      var m = u(),
        T = N.S;
      if (
        (T !== null && T(r, m),
        m !== null && typeof m == "object" && typeof m.then == "function")
      ) {
        var _ = zm(m, a);
        nu(t, e, _, re(t));
      } else nu(t, e, a, re(t));
    } catch (H) {
      nu(t, e, { then: function () {}, status: "rejected", reason: H }, re());
    } finally {
      ((L.p = n), (N.T = c));
    }
  }
  function Um() {}
  function Sc(t, e, l, a) {
    if (t.tag !== 5) throw Error(f(476));
    var u = Us(t).queue;
    Ns(
      t,
      u,
      e,
      W,
      l === null
        ? Um
        : function () {
            return (Cs(t), l(a));
          },
    );
  }
  function Us(t) {
    var e = t.memoizedState;
    if (e !== null) return e;
    e = {
      memoizedState: W,
      baseState: W,
      baseQueue: null,
      queue: {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: Le,
        lastRenderedState: W,
      },
      next: null,
    };
    var l = {};
    return (
      (e.next = {
        memoizedState: l,
        baseState: l,
        baseQueue: null,
        queue: {
          pending: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: Le,
          lastRenderedState: l,
        },
        next: null,
      }),
      (t.memoizedState = e),
      (t = t.alternate),
      t !== null && (t.memoizedState = e),
      e
    );
  }
  function Cs(t) {
    var e = Us(t).next.queue;
    nu(t, e, {}, re());
  }
  function bc() {
    return Vt(Au);
  }
  function Hs() {
    return Ut().memoizedState;
  }
  function js() {
    return Ut().memoizedState;
  }
  function Cm(t) {
    for (var e = t.return; e !== null; ) {
      switch (e.tag) {
        case 24:
        case 3:
          var l = re();
          t = al(l);
          var a = ul(e, t, l);
          (a !== null && (se(a, e, l), Ia(a, e, l)),
            (e = { cache: Wi() }),
            (t.payload = e));
          return;
      }
      e = e.return;
    }
  }
  function Hm(t, e, l) {
    var a = re();
    ((l = {
      lane: a,
      revertLane: 0,
      action: l,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    }),
      pn(t)
        ? qs(e, l)
        : ((l = Li(t, e, l, a)), l !== null && (se(l, t, a), Ys(l, e, a))));
  }
  function Bs(t, e, l) {
    var a = re();
    nu(t, e, l, a);
  }
  function nu(t, e, l, a) {
    var u = {
      lane: a,
      revertLane: 0,
      action: l,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    };
    if (pn(t)) qs(e, u);
    else {
      var n = t.alternate;
      if (
        t.lanes === 0 &&
        (n === null || n.lanes === 0) &&
        ((n = e.lastRenderedReducer), n !== null)
      )
        try {
          var c = e.lastRenderedState,
            r = n(c, l);
          if (((u.hasEagerState = !0), (u.eagerState = r), ue(r, c)))
            return (tn(t, e, u, 0), pt === null && Iu(), !1);
        } catch {
        } finally {
        }
      if (((l = Li(t, e, u, a)), l !== null))
        return (se(l, t, a), Ys(l, e, a), !0);
    }
    return !1;
  }
  function Ec(t, e, l, a) {
    if (
      ((a = {
        lane: 2,
        revertLane: Ic(),
        action: a,
        hasEagerState: !1,
        eagerState: null,
        next: null,
      }),
      pn(t))
    ) {
      if (e) throw Error(f(479));
    } else ((e = Li(t, l, a, 2)), e !== null && se(e, t, 2));
  }
  function pn(t) {
    var e = t.alternate;
    return t === lt || (e !== null && e === lt);
  }
  function qs(t, e) {
    sa = dn = !0;
    var l = t.pending;
    (l === null ? (e.next = e) : ((e.next = l.next), (l.next = e)),
      (t.pending = e));
  }
  function Ys(t, e, l) {
    if ((l & 4194048) !== 0) {
      var a = e.lanes;
      ((a &= t.pendingLanes), (l |= a), (e.lanes = l), Kf(t, l));
    }
  }
  var Sn = {
      readContext: Vt,
      use: mn,
      useCallback: Mt,
      useContext: Mt,
      useEffect: Mt,
      useImperativeHandle: Mt,
      useLayoutEffect: Mt,
      useInsertionEffect: Mt,
      useMemo: Mt,
      useReducer: Mt,
      useRef: Mt,
      useState: Mt,
      useDebugValue: Mt,
      useDeferredValue: Mt,
      useTransition: Mt,
      useSyncExternalStore: Mt,
      useId: Mt,
      useHostTransitionStatus: Mt,
      useFormState: Mt,
      useActionState: Mt,
      useOptimistic: Mt,
      useMemoCache: Mt,
      useCacheRefresh: Mt,
    },
    ws = {
      readContext: Vt,
      use: mn,
      useCallback: function (t, e) {
        return ((Pt().memoizedState = [t, e === void 0 ? null : e]), t);
      },
      useContext: Vt,
      useEffect: Ts,
      useImperativeHandle: function (t, e, l) {
        ((l = l != null ? l.concat([t]) : null),
          gn(4194308, 4, Os.bind(null, e, t), l));
      },
      useLayoutEffect: function (t, e) {
        return gn(4194308, 4, t, e);
      },
      useInsertionEffect: function (t, e) {
        gn(4, 2, t, e);
      },
      useMemo: function (t, e) {
        var l = Pt();
        e = e === void 0 ? null : e;
        var a = t();
        if (ql) {
          Pe(!0);
          try {
            t();
          } finally {
            Pe(!1);
          }
        }
        return ((l.memoizedState = [a, e]), a);
      },
      useReducer: function (t, e, l) {
        var a = Pt();
        if (l !== void 0) {
          var u = l(e);
          if (ql) {
            Pe(!0);
            try {
              l(e);
            } finally {
              Pe(!1);
            }
          }
        } else u = e;
        return (
          (a.memoizedState = a.baseState = u),
          (t = {
            pending: null,
            lanes: 0,
            dispatch: null,
            lastRenderedReducer: t,
            lastRenderedState: u,
          }),
          (a.queue = t),
          (t = t.dispatch = Hm.bind(null, lt, t)),
          [a.memoizedState, t]
        );
      },
      useRef: function (t) {
        var e = Pt();
        return ((t = { current: t }), (e.memoizedState = t));
      },
      useState: function (t) {
        t = vc(t);
        var e = t.queue,
          l = Bs.bind(null, lt, e);
        return ((e.dispatch = l), [t.memoizedState, l]);
      },
      useDebugValue: gc,
      useDeferredValue: function (t, e) {
        var l = Pt();
        return pc(l, t, e);
      },
      useTransition: function () {
        var t = vc(!1);
        return (
          (t = Ns.bind(null, lt, t.queue, !0, !1)),
          (Pt().memoizedState = t),
          [!1, t]
        );
      },
      useSyncExternalStore: function (t, e, l) {
        var a = lt,
          u = Pt();
        if (st) {
          if (l === void 0) throw Error(f(407));
          l = l();
        } else {
          if (((l = e()), pt === null)) throw Error(f(349));
          (it & 124) !== 0 || is(a, e, l);
        }
        u.memoizedState = l;
        var n = { value: l, getSnapshot: e };
        return (
          (u.queue = n),
          Ts(fs.bind(null, a, n, t), [t]),
          (a.flags |= 2048),
          da(9, yn(), cs.bind(null, a, n, l, e), null),
          l
        );
      },
      useId: function () {
        var t = Pt(),
          e = pt.identifierPrefix;
        if (st) {
          var l = qe,
            a = Be;
          ((l = (a & ~(1 << (32 - ae(a) - 1))).toString(32) + l),
            (e = "«" + e + "R" + l),
            (l = hn++),
            0 < l && (e += "H" + l.toString(32)),
            (e += "»"));
        } else ((l = Mm++), (e = "«" + e + "r" + l.toString(32) + "»"));
        return (t.memoizedState = e);
      },
      useHostTransitionStatus: bc,
      useFormState: gs,
      useActionState: gs,
      useOptimistic: function (t) {
        var e = Pt();
        e.memoizedState = e.baseState = t;
        var l = {
          pending: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: null,
          lastRenderedState: null,
        };
        return (
          (e.queue = l),
          (e = Ec.bind(null, lt, !0, l)),
          (l.dispatch = e),
          [t, e]
        );
      },
      useMemoCache: dc,
      useCacheRefresh: function () {
        return (Pt().memoizedState = Cm.bind(null, lt));
      },
    },
    Ls = {
      readContext: Vt,
      use: mn,
      useCallback: Ms,
      useContext: Vt,
      useEffect: As,
      useImperativeHandle: zs,
      useInsertionEffect: xs,
      useLayoutEffect: Rs,
      useMemo: Ds,
      useReducer: vn,
      useRef: Es,
      useState: function () {
        return vn(Le);
      },
      useDebugValue: gc,
      useDeferredValue: function (t, e) {
        var l = Ut();
        return _s(l, ht.memoizedState, t, e);
      },
      useTransition: function () {
        var t = vn(Le)[0],
          e = Ut().memoizedState;
        return [typeof t == "boolean" ? t : au(t), e];
      },
      useSyncExternalStore: ns,
      useId: Hs,
      useHostTransitionStatus: bc,
      useFormState: ps,
      useActionState: ps,
      useOptimistic: function (t, e) {
        var l = Ut();
        return os(l, ht, t, e);
      },
      useMemoCache: dc,
      useCacheRefresh: js,
    },
    jm = {
      readContext: Vt,
      use: mn,
      useCallback: Ms,
      useContext: Vt,
      useEffect: As,
      useImperativeHandle: zs,
      useInsertionEffect: xs,
      useLayoutEffect: Rs,
      useMemo: Ds,
      useReducer: mc,
      useRef: Es,
      useState: function () {
        return mc(Le);
      },
      useDebugValue: gc,
      useDeferredValue: function (t, e) {
        var l = Ut();
        return ht === null ? pc(l, t, e) : _s(l, ht.memoizedState, t, e);
      },
      useTransition: function () {
        var t = mc(Le)[0],
          e = Ut().memoizedState;
        return [typeof t == "boolean" ? t : au(t), e];
      },
      useSyncExternalStore: ns,
      useId: Hs,
      useHostTransitionStatus: bc,
      useFormState: bs,
      useActionState: bs,
      useOptimistic: function (t, e) {
        var l = Ut();
        return ht !== null
          ? os(l, ht, t, e)
          : ((l.baseState = t), [t, l.queue.dispatch]);
      },
      useMemoCache: dc,
      useCacheRefresh: js,
    },
    ha = null,
    iu = 0;
  function bn(t) {
    var e = iu;
    return ((iu += 1), ha === null && (ha = []), Fr(ha, t, e));
  }
  function cu(t, e) {
    ((e = e.props.ref), (t.ref = e !== void 0 ? e : null));
  }
  function En(t, e) {
    throw e.$$typeof === j
      ? Error(f(525))
      : ((t = Object.prototype.toString.call(e)),
        Error(
          f(
            31,
            t === "[object Object]"
              ? "object with keys {" + Object.keys(e).join(", ") + "}"
              : t,
          ),
        ));
  }
  function Gs(t) {
    var e = t._init;
    return e(t._payload);
  }
  function Xs(t) {
    function e(S, g) {
      if (t) {
        var E = S.deletions;
        E === null ? ((S.deletions = [g]), (S.flags |= 16)) : E.push(g);
      }
    }
    function l(S, g) {
      if (!t) return null;
      for (; g !== null; ) (e(S, g), (g = g.sibling));
      return null;
    }
    function a(S) {
      for (var g = new Map(); S !== null; )
        (S.key !== null ? g.set(S.key, S) : g.set(S.index, S), (S = S.sibling));
      return g;
    }
    function u(S, g) {
      return ((S = je(S, g)), (S.index = 0), (S.sibling = null), S);
    }
    function n(S, g, E) {
      return (
        (S.index = E),
        t
          ? ((E = S.alternate),
            E !== null
              ? ((E = E.index), E < g ? ((S.flags |= 67108866), g) : E)
              : ((S.flags |= 67108866), g))
          : ((S.flags |= 1048576), g)
      );
    }
    function c(S) {
      return (t && S.alternate === null && (S.flags |= 67108866), S);
    }
    function r(S, g, E, U) {
      return g === null || g.tag !== 6
        ? ((g = Xi(E, S.mode, U)), (g.return = S), g)
        : ((g = u(g, E)), (g.return = S), g);
    }
    function m(S, g, E, U) {
      var Z = E.type;
      return Z === M
        ? _(S, g, E.props.children, U, E.key)
        : g !== null &&
            (g.elementType === Z ||
              (typeof Z == "object" &&
                Z !== null &&
                Z.$$typeof === zt &&
                Gs(Z) === g.type))
          ? ((g = u(g, E.props)), cu(g, E), (g.return = S), g)
          : ((g = ln(E.type, E.key, E.props, null, S.mode, U)),
            cu(g, E),
            (g.return = S),
            g);
    }
    function T(S, g, E, U) {
      return g === null ||
        g.tag !== 4 ||
        g.stateNode.containerInfo !== E.containerInfo ||
        g.stateNode.implementation !== E.implementation
        ? ((g = Qi(E, S.mode, U)), (g.return = S), g)
        : ((g = u(g, E.children || [])), (g.return = S), g);
    }
    function _(S, g, E, U, Z) {
      return g === null || g.tag !== 7
        ? ((g = Dl(E, S.mode, U, Z)), (g.return = S), g)
        : ((g = u(g, E)), (g.return = S), g);
    }
    function H(S, g, E) {
      if (
        (typeof g == "string" && g !== "") ||
        typeof g == "number" ||
        typeof g == "bigint"
      )
        return ((g = Xi("" + g, S.mode, E)), (g.return = S), g);
      if (typeof g == "object" && g !== null) {
        switch (g.$$typeof) {
          case z:
            return (
              (E = ln(g.type, g.key, g.props, null, S.mode, E)),
              cu(E, g),
              (E.return = S),
              E
            );
          case G:
            return ((g = Qi(g, S.mode, E)), (g.return = S), g);
          case zt:
            var U = g._init;
            return ((g = U(g._payload)), H(S, g, E));
        }
        if (Qt(g) || Xt(g))
          return ((g = Dl(g, S.mode, E, null)), (g.return = S), g);
        if (typeof g.then == "function") return H(S, bn(g), E);
        if (g.$$typeof === P) return H(S, cn(S, g), E);
        En(S, g);
      }
      return null;
    }
    function x(S, g, E, U) {
      var Z = g !== null ? g.key : null;
      if (
        (typeof E == "string" && E !== "") ||
        typeof E == "number" ||
        typeof E == "bigint"
      )
        return Z !== null ? null : r(S, g, "" + E, U);
      if (typeof E == "object" && E !== null) {
        switch (E.$$typeof) {
          case z:
            return E.key === Z ? m(S, g, E, U) : null;
          case G:
            return E.key === Z ? T(S, g, E, U) : null;
          case zt:
            return ((Z = E._init), (E = Z(E._payload)), x(S, g, E, U));
        }
        if (Qt(E) || Xt(E)) return Z !== null ? null : _(S, g, E, U, null);
        if (typeof E.then == "function") return x(S, g, bn(E), U);
        if (E.$$typeof === P) return x(S, g, cn(S, E), U);
        En(S, E);
      }
      return null;
    }
    function R(S, g, E, U, Z) {
      if (
        (typeof U == "string" && U !== "") ||
        typeof U == "number" ||
        typeof U == "bigint"
      )
        return ((S = S.get(E) || null), r(g, S, "" + U, Z));
      if (typeof U == "object" && U !== null) {
        switch (U.$$typeof) {
          case z:
            return (
              (S = S.get(U.key === null ? E : U.key) || null),
              m(g, S, U, Z)
            );
          case G:
            return (
              (S = S.get(U.key === null ? E : U.key) || null),
              T(g, S, U, Z)
            );
          case zt:
            var at = U._init;
            return ((U = at(U._payload)), R(S, g, E, U, Z));
        }
        if (Qt(U) || Xt(U))
          return ((S = S.get(E) || null), _(g, S, U, Z, null));
        if (typeof U.then == "function") return R(S, g, E, bn(U), Z);
        if (U.$$typeof === P) return R(S, g, E, cn(g, U), Z);
        En(g, U);
      }
      return null;
    }
    function F(S, g, E, U) {
      for (
        var Z = null, at = null, K = g, k = (g = 0), Yt = null;
        K !== null && k < E.length;
        k++
      ) {
        K.index > k ? ((Yt = K), (K = null)) : (Yt = K.sibling);
        var rt = x(S, K, E[k], U);
        if (rt === null) {
          K === null && (K = Yt);
          break;
        }
        (t && K && rt.alternate === null && e(S, K),
          (g = n(rt, g, k)),
          at === null ? (Z = rt) : (at.sibling = rt),
          (at = rt),
          (K = Yt));
      }
      if (k === E.length) return (l(S, K), st && Nl(S, k), Z);
      if (K === null) {
        for (; k < E.length; k++)
          ((K = H(S, E[k], U)),
            K !== null &&
              ((g = n(K, g, k)),
              at === null ? (Z = K) : (at.sibling = K),
              (at = K)));
        return (st && Nl(S, k), Z);
      }
      for (K = a(K); k < E.length; k++)
        ((Yt = R(K, S, k, E[k], U)),
          Yt !== null &&
            (t &&
              Yt.alternate !== null &&
              K.delete(Yt.key === null ? k : Yt.key),
            (g = n(Yt, g, k)),
            at === null ? (Z = Yt) : (at.sibling = Yt),
            (at = Yt)));
      return (
        t &&
          K.forEach(function (El) {
            return e(S, El);
          }),
        st && Nl(S, k),
        Z
      );
    }
    function $(S, g, E, U) {
      if (E == null) throw Error(f(151));
      for (
        var Z = null, at = null, K = g, k = (g = 0), Yt = null, rt = E.next();
        K !== null && !rt.done;
        k++, rt = E.next()
      ) {
        K.index > k ? ((Yt = K), (K = null)) : (Yt = K.sibling);
        var El = x(S, K, rt.value, U);
        if (El === null) {
          K === null && (K = Yt);
          break;
        }
        (t && K && El.alternate === null && e(S, K),
          (g = n(El, g, k)),
          at === null ? (Z = El) : (at.sibling = El),
          (at = El),
          (K = Yt));
      }
      if (rt.done) return (l(S, K), st && Nl(S, k), Z);
      if (K === null) {
        for (; !rt.done; k++, rt = E.next())
          ((rt = H(S, rt.value, U)),
            rt !== null &&
              ((g = n(rt, g, k)),
              at === null ? (Z = rt) : (at.sibling = rt),
              (at = rt)));
        return (st && Nl(S, k), Z);
      }
      for (K = a(K); !rt.done; k++, rt = E.next())
        ((rt = R(K, S, k, rt.value, U)),
          rt !== null &&
            (t &&
              rt.alternate !== null &&
              K.delete(rt.key === null ? k : rt.key),
            (g = n(rt, g, k)),
            at === null ? (Z = rt) : (at.sibling = rt),
            (at = rt)));
      return (
        t &&
          K.forEach(function (B0) {
            return e(S, B0);
          }),
        st && Nl(S, k),
        Z
      );
    }
    function vt(S, g, E, U) {
      if (
        (typeof E == "object" &&
          E !== null &&
          E.type === M &&
          E.key === null &&
          (E = E.props.children),
        typeof E == "object" && E !== null)
      ) {
        switch (E.$$typeof) {
          case z:
            t: {
              for (var Z = E.key; g !== null; ) {
                if (g.key === Z) {
                  if (((Z = E.type), Z === M)) {
                    if (g.tag === 7) {
                      (l(S, g.sibling),
                        (U = u(g, E.props.children)),
                        (U.return = S),
                        (S = U));
                      break t;
                    }
                  } else if (
                    g.elementType === Z ||
                    (typeof Z == "object" &&
                      Z !== null &&
                      Z.$$typeof === zt &&
                      Gs(Z) === g.type)
                  ) {
                    (l(S, g.sibling),
                      (U = u(g, E.props)),
                      cu(U, E),
                      (U.return = S),
                      (S = U));
                    break t;
                  }
                  l(S, g);
                  break;
                } else e(S, g);
                g = g.sibling;
              }
              E.type === M
                ? ((U = Dl(E.props.children, S.mode, U, E.key)),
                  (U.return = S),
                  (S = U))
                : ((U = ln(E.type, E.key, E.props, null, S.mode, U)),
                  cu(U, E),
                  (U.return = S),
                  (S = U));
            }
            return c(S);
          case G:
            t: {
              for (Z = E.key; g !== null; ) {
                if (g.key === Z)
                  if (
                    g.tag === 4 &&
                    g.stateNode.containerInfo === E.containerInfo &&
                    g.stateNode.implementation === E.implementation
                  ) {
                    (l(S, g.sibling),
                      (U = u(g, E.children || [])),
                      (U.return = S),
                      (S = U));
                    break t;
                  } else {
                    l(S, g);
                    break;
                  }
                else e(S, g);
                g = g.sibling;
              }
              ((U = Qi(E, S.mode, U)), (U.return = S), (S = U));
            }
            return c(S);
          case zt:
            return ((Z = E._init), (E = Z(E._payload)), vt(S, g, E, U));
        }
        if (Qt(E)) return F(S, g, E, U);
        if (Xt(E)) {
          if (((Z = Xt(E)), typeof Z != "function")) throw Error(f(150));
          return ((E = Z.call(E)), $(S, g, E, U));
        }
        if (typeof E.then == "function") return vt(S, g, bn(E), U);
        if (E.$$typeof === P) return vt(S, g, cn(S, E), U);
        En(S, E);
      }
      return (typeof E == "string" && E !== "") ||
        typeof E == "number" ||
        typeof E == "bigint"
        ? ((E = "" + E),
          g !== null && g.tag === 6
            ? (l(S, g.sibling), (U = u(g, E)), (U.return = S), (S = U))
            : (l(S, g), (U = Xi(E, S.mode, U)), (U.return = S), (S = U)),
          c(S))
        : l(S, g);
    }
    return function (S, g, E, U) {
      try {
        iu = 0;
        var Z = vt(S, g, E, U);
        return ((ha = null), Z);
      } catch (K) {
        if (K === Fa || K === rn) throw K;
        var at = ne(29, K, null, S.mode);
        return ((at.lanes = U), (at.return = S), at);
      } finally {
      }
    };
  }
  var ma = Xs(!0),
    Qs = Xs(!1),
    pe = B(null),
    ze = null;
  function il(t) {
    var e = t.alternate;
    (w(Ht, Ht.current & 1),
      w(pe, t),
      ze === null &&
        (e === null || ra.current !== null || e.memoizedState !== null) &&
        (ze = t));
  }
  function Zs(t) {
    if (t.tag === 22) {
      if ((w(Ht, Ht.current), w(pe, t), ze === null)) {
        var e = t.alternate;
        e !== null && e.memoizedState !== null && (ze = t);
      }
    } else cl();
  }
  function cl() {
    (w(Ht, Ht.current), w(pe, pe.current));
  }
  function Ge(t) {
    (X(pe), ze === t && (ze = null), X(Ht));
  }
  var Ht = B(0);
  function Tn(t) {
    for (var e = t; e !== null; ) {
      if (e.tag === 13) {
        var l = e.memoizedState;
        if (
          l !== null &&
          ((l = l.dehydrated), l === null || l.data === "$?" || df(l))
        )
          return e;
      } else if (e.tag === 19 && e.memoizedProps.revealOrder !== void 0) {
        if ((e.flags & 128) !== 0) return e;
      } else if (e.child !== null) {
        ((e.child.return = e), (e = e.child));
        continue;
      }
      if (e === t) break;
      for (; e.sibling === null; ) {
        if (e.return === null || e.return === t) return null;
        e = e.return;
      }
      ((e.sibling.return = e.return), (e = e.sibling));
    }
    return null;
  }
  function Tc(t, e, l, a) {
    ((e = t.memoizedState),
      (l = l(a, e)),
      (l = l == null ? e : O({}, e, l)),
      (t.memoizedState = l),
      t.lanes === 0 && (t.updateQueue.baseState = l));
  }
  var Ac = {
    enqueueSetState: function (t, e, l) {
      t = t._reactInternals;
      var a = re(),
        u = al(a);
      ((u.payload = e),
        l != null && (u.callback = l),
        (e = ul(t, u, a)),
        e !== null && (se(e, t, a), Ia(e, t, a)));
    },
    enqueueReplaceState: function (t, e, l) {
      t = t._reactInternals;
      var a = re(),
        u = al(a);
      ((u.tag = 1),
        (u.payload = e),
        l != null && (u.callback = l),
        (e = ul(t, u, a)),
        e !== null && (se(e, t, a), Ia(e, t, a)));
    },
    enqueueForceUpdate: function (t, e) {
      t = t._reactInternals;
      var l = re(),
        a = al(l);
      ((a.tag = 2),
        e != null && (a.callback = e),
        (e = ul(t, a, l)),
        e !== null && (se(e, t, l), Ia(e, t, l)));
    },
  };
  function Vs(t, e, l, a, u, n, c) {
    return (
      (t = t.stateNode),
      typeof t.shouldComponentUpdate == "function"
        ? t.shouldComponentUpdate(a, n, c)
        : e.prototype && e.prototype.isPureReactComponent
          ? !Qa(l, a) || !Qa(u, n)
          : !0
    );
  }
  function Ks(t, e, l, a) {
    ((t = e.state),
      typeof e.componentWillReceiveProps == "function" &&
        e.componentWillReceiveProps(l, a),
      typeof e.UNSAFE_componentWillReceiveProps == "function" &&
        e.UNSAFE_componentWillReceiveProps(l, a),
      e.state !== t && Ac.enqueueReplaceState(e, e.state, null));
  }
  function Yl(t, e) {
    var l = e;
    if ("ref" in e) {
      l = {};
      for (var a in e) a !== "ref" && (l[a] = e[a]);
    }
    if ((t = t.defaultProps)) {
      l === e && (l = O({}, l));
      for (var u in t) l[u] === void 0 && (l[u] = t[u]);
    }
    return l;
  }
  var An =
    typeof reportError == "function"
      ? reportError
      : function (t) {
          if (
            typeof window == "object" &&
            typeof window.ErrorEvent == "function"
          ) {
            var e = new window.ErrorEvent("error", {
              bubbles: !0,
              cancelable: !0,
              message:
                typeof t == "object" &&
                t !== null &&
                typeof t.message == "string"
                  ? String(t.message)
                  : String(t),
              error: t,
            });
            if (!window.dispatchEvent(e)) return;
          } else if (
            typeof process == "object" &&
            typeof process.emit == "function"
          ) {
            process.emit("uncaughtException", t);
            return;
          }
          console.error(t);
        };
  function Js(t) {
    An(t);
  }
  function $s(t) {
    console.error(t);
  }
  function ks(t) {
    An(t);
  }
  function xn(t, e) {
    try {
      var l = t.onUncaughtError;
      l(e.value, { componentStack: e.stack });
    } catch (a) {
      setTimeout(function () {
        throw a;
      });
    }
  }
  function Ws(t, e, l) {
    try {
      var a = t.onCaughtError;
      a(l.value, {
        componentStack: l.stack,
        errorBoundary: e.tag === 1 ? e.stateNode : null,
      });
    } catch (u) {
      setTimeout(function () {
        throw u;
      });
    }
  }
  function xc(t, e, l) {
    return (
      (l = al(l)),
      (l.tag = 3),
      (l.payload = { element: null }),
      (l.callback = function () {
        xn(t, e);
      }),
      l
    );
  }
  function Fs(t) {
    return ((t = al(t)), (t.tag = 3), t);
  }
  function Ps(t, e, l, a) {
    var u = l.type.getDerivedStateFromError;
    if (typeof u == "function") {
      var n = a.value;
      ((t.payload = function () {
        return u(n);
      }),
        (t.callback = function () {
          Ws(e, l, a);
        }));
    }
    var c = l.stateNode;
    c !== null &&
      typeof c.componentDidCatch == "function" &&
      (t.callback = function () {
        (Ws(e, l, a),
          typeof u != "function" &&
            (hl === null ? (hl = new Set([this])) : hl.add(this)));
        var r = a.stack;
        this.componentDidCatch(a.value, {
          componentStack: r !== null ? r : "",
        });
      });
  }
  function Bm(t, e, l, a, u) {
    if (
      ((l.flags |= 32768),
      a !== null && typeof a == "object" && typeof a.then == "function")
    ) {
      if (
        ((e = l.alternate),
        e !== null && $a(e, l, u, !0),
        (l = pe.current),
        l !== null)
      ) {
        switch (l.tag) {
          case 13:
            return (
              ze === null ? $c() : l.alternate === null && Ot === 0 && (Ot = 3),
              (l.flags &= -257),
              (l.flags |= 65536),
              (l.lanes = u),
              a === Ii
                ? (l.flags |= 16384)
                : ((e = l.updateQueue),
                  e === null ? (l.updateQueue = new Set([a])) : e.add(a),
                  Wc(t, a, u)),
              !1
            );
          case 22:
            return (
              (l.flags |= 65536),
              a === Ii
                ? (l.flags |= 16384)
                : ((e = l.updateQueue),
                  e === null
                    ? ((e = {
                        transitions: null,
                        markerInstances: null,
                        retryQueue: new Set([a]),
                      }),
                      (l.updateQueue = e))
                    : ((l = e.retryQueue),
                      l === null ? (e.retryQueue = new Set([a])) : l.add(a)),
                  Wc(t, a, u)),
              !1
            );
        }
        throw Error(f(435, l.tag));
      }
      return (Wc(t, a, u), $c(), !1);
    }
    if (st)
      return (
        (e = pe.current),
        e !== null
          ? ((e.flags & 65536) === 0 && (e.flags |= 256),
            (e.flags |= 65536),
            (e.lanes = u),
            a !== Ki && ((t = Error(f(422), { cause: a })), Ja(me(t, l))))
          : (a !== Ki && ((e = Error(f(423), { cause: a })), Ja(me(e, l))),
            (t = t.current.alternate),
            (t.flags |= 65536),
            (u &= -u),
            (t.lanes |= u),
            (a = me(a, l)),
            (u = xc(t.stateNode, a, u)),
            lc(t, u),
            Ot !== 4 && (Ot = 2)),
        !1
      );
    var n = Error(f(520), { cause: a });
    if (
      ((n = me(n, l)),
      mu === null ? (mu = [n]) : mu.push(n),
      Ot !== 4 && (Ot = 2),
      e === null)
    )
      return !0;
    ((a = me(a, l)), (l = e));
    do {
      switch (l.tag) {
        case 3:
          return (
            (l.flags |= 65536),
            (t = u & -u),
            (l.lanes |= t),
            (t = xc(l.stateNode, a, t)),
            lc(l, t),
            !1
          );
        case 1:
          if (
            ((e = l.type),
            (n = l.stateNode),
            (l.flags & 128) === 0 &&
              (typeof e.getDerivedStateFromError == "function" ||
                (n !== null &&
                  typeof n.componentDidCatch == "function" &&
                  (hl === null || !hl.has(n)))))
          )
            return (
              (l.flags |= 65536),
              (u &= -u),
              (l.lanes |= u),
              (u = Fs(u)),
              Ps(u, t, l, a),
              lc(l, u),
              !1
            );
      }
      l = l.return;
    } while (l !== null);
    return !1;
  }
  var Is = Error(f(461)),
    Bt = !1;
  function wt(t, e, l, a) {
    e.child = t === null ? Qs(e, null, l, a) : ma(e, t.child, l, a);
  }
  function to(t, e, l, a, u) {
    l = l.render;
    var n = e.ref;
    if ("ref" in a) {
      var c = {};
      for (var r in a) r !== "ref" && (c[r] = a[r]);
    } else c = a;
    return (
      jl(e),
      (a = cc(t, e, l, c, n, u)),
      (r = fc()),
      t !== null && !Bt
        ? (rc(t, e, u), Xe(t, e, u))
        : (st && r && Zi(e), (e.flags |= 1), wt(t, e, a, u), e.child)
    );
  }
  function eo(t, e, l, a, u) {
    if (t === null) {
      var n = l.type;
      return typeof n == "function" &&
        !Gi(n) &&
        n.defaultProps === void 0 &&
        l.compare === null
        ? ((e.tag = 15), (e.type = n), lo(t, e, n, a, u))
        : ((t = ln(l.type, null, a, e, e.mode, u)),
          (t.ref = e.ref),
          (t.return = e),
          (e.child = t));
    }
    if (((n = t.child), !Uc(t, u))) {
      var c = n.memoizedProps;
      if (
        ((l = l.compare), (l = l !== null ? l : Qa), l(c, a) && t.ref === e.ref)
      )
        return Xe(t, e, u);
    }
    return (
      (e.flags |= 1),
      (t = je(n, a)),
      (t.ref = e.ref),
      (t.return = e),
      (e.child = t)
    );
  }
  function lo(t, e, l, a, u) {
    if (t !== null) {
      var n = t.memoizedProps;
      if (Qa(n, a) && t.ref === e.ref)
        if (((Bt = !1), (e.pendingProps = a = n), Uc(t, u)))
          (t.flags & 131072) !== 0 && (Bt = !0);
        else return ((e.lanes = t.lanes), Xe(t, e, u));
    }
    return Rc(t, e, l, a, u);
  }
  function ao(t, e, l) {
    var a = e.pendingProps,
      u = a.children,
      n = t !== null ? t.memoizedState : null;
    if (a.mode === "hidden") {
      if ((e.flags & 128) !== 0) {
        if (((a = n !== null ? n.baseLanes | l : l), t !== null)) {
          for (u = e.child = t.child, n = 0; u !== null; )
            ((n = n | u.lanes | u.childLanes), (u = u.sibling));
          e.childLanes = n & ~a;
        } else ((e.childLanes = 0), (e.child = null));
        return uo(t, e, a, l);
      }
      if ((l & 536870912) !== 0)
        ((e.memoizedState = { baseLanes: 0, cachePool: null }),
          t !== null && fn(e, n !== null ? n.cachePool : null),
          n !== null ? ls(e, n) : uc(),
          Zs(e));
      else
        return (
          (e.lanes = e.childLanes = 536870912),
          uo(t, e, n !== null ? n.baseLanes | l : l, l)
        );
    } else
      n !== null
        ? (fn(e, n.cachePool), ls(e, n), cl(), (e.memoizedState = null))
        : (t !== null && fn(e, null), uc(), cl());
    return (wt(t, e, u, l), e.child);
  }
  function uo(t, e, l, a) {
    var u = Pi();
    return (
      (u = u === null ? null : { parent: Ct._currentValue, pool: u }),
      (e.memoizedState = { baseLanes: l, cachePool: u }),
      t !== null && fn(e, null),
      uc(),
      Zs(e),
      t !== null && $a(t, e, a, !0),
      null
    );
  }
  function Rn(t, e) {
    var l = e.ref;
    if (l === null) t !== null && t.ref !== null && (e.flags |= 4194816);
    else {
      if (typeof l != "function" && typeof l != "object") throw Error(f(284));
      (t === null || t.ref !== l) && (e.flags |= 4194816);
    }
  }
  function Rc(t, e, l, a, u) {
    return (
      jl(e),
      (l = cc(t, e, l, a, void 0, u)),
      (a = fc()),
      t !== null && !Bt
        ? (rc(t, e, u), Xe(t, e, u))
        : (st && a && Zi(e), (e.flags |= 1), wt(t, e, l, u), e.child)
    );
  }
  function no(t, e, l, a, u, n) {
    return (
      jl(e),
      (e.updateQueue = null),
      (l = us(e, a, l, u)),
      as(t),
      (a = fc()),
      t !== null && !Bt
        ? (rc(t, e, n), Xe(t, e, n))
        : (st && a && Zi(e), (e.flags |= 1), wt(t, e, l, n), e.child)
    );
  }
  function io(t, e, l, a, u) {
    if ((jl(e), e.stateNode === null)) {
      var n = ua,
        c = l.contextType;
      (typeof c == "object" && c !== null && (n = Vt(c)),
        (n = new l(a, n)),
        (e.memoizedState =
          n.state !== null && n.state !== void 0 ? n.state : null),
        (n.updater = Ac),
        (e.stateNode = n),
        (n._reactInternals = e),
        (n = e.stateNode),
        (n.props = a),
        (n.state = e.memoizedState),
        (n.refs = {}),
        tc(e),
        (c = l.contextType),
        (n.context = typeof c == "object" && c !== null ? Vt(c) : ua),
        (n.state = e.memoizedState),
        (c = l.getDerivedStateFromProps),
        typeof c == "function" && (Tc(e, l, c, a), (n.state = e.memoizedState)),
        typeof l.getDerivedStateFromProps == "function" ||
          typeof n.getSnapshotBeforeUpdate == "function" ||
          (typeof n.UNSAFE_componentWillMount != "function" &&
            typeof n.componentWillMount != "function") ||
          ((c = n.state),
          typeof n.componentWillMount == "function" && n.componentWillMount(),
          typeof n.UNSAFE_componentWillMount == "function" &&
            n.UNSAFE_componentWillMount(),
          c !== n.state && Ac.enqueueReplaceState(n, n.state, null),
          eu(e, a, n, u),
          tu(),
          (n.state = e.memoizedState)),
        typeof n.componentDidMount == "function" && (e.flags |= 4194308),
        (a = !0));
    } else if (t === null) {
      n = e.stateNode;
      var r = e.memoizedProps,
        m = Yl(l, r);
      n.props = m;
      var T = n.context,
        _ = l.contextType;
      ((c = ua), typeof _ == "object" && _ !== null && (c = Vt(_)));
      var H = l.getDerivedStateFromProps;
      ((_ =
        typeof H == "function" ||
        typeof n.getSnapshotBeforeUpdate == "function"),
        (r = e.pendingProps !== r),
        _ ||
          (typeof n.UNSAFE_componentWillReceiveProps != "function" &&
            typeof n.componentWillReceiveProps != "function") ||
          ((r || T !== c) && Ks(e, n, a, c)),
        (ll = !1));
      var x = e.memoizedState;
      ((n.state = x),
        eu(e, a, n, u),
        tu(),
        (T = e.memoizedState),
        r || x !== T || ll
          ? (typeof H == "function" && (Tc(e, l, H, a), (T = e.memoizedState)),
            (m = ll || Vs(e, l, m, a, x, T, c))
              ? (_ ||
                  (typeof n.UNSAFE_componentWillMount != "function" &&
                    typeof n.componentWillMount != "function") ||
                  (typeof n.componentWillMount == "function" &&
                    n.componentWillMount(),
                  typeof n.UNSAFE_componentWillMount == "function" &&
                    n.UNSAFE_componentWillMount()),
                typeof n.componentDidMount == "function" &&
                  (e.flags |= 4194308))
              : (typeof n.componentDidMount == "function" &&
                  (e.flags |= 4194308),
                (e.memoizedProps = a),
                (e.memoizedState = T)),
            (n.props = a),
            (n.state = T),
            (n.context = c),
            (a = m))
          : (typeof n.componentDidMount == "function" && (e.flags |= 4194308),
            (a = !1)));
    } else {
      ((n = e.stateNode),
        ec(t, e),
        (c = e.memoizedProps),
        (_ = Yl(l, c)),
        (n.props = _),
        (H = e.pendingProps),
        (x = n.context),
        (T = l.contextType),
        (m = ua),
        typeof T == "object" && T !== null && (m = Vt(T)),
        (r = l.getDerivedStateFromProps),
        (T =
          typeof r == "function" ||
          typeof n.getSnapshotBeforeUpdate == "function") ||
          (typeof n.UNSAFE_componentWillReceiveProps != "function" &&
            typeof n.componentWillReceiveProps != "function") ||
          ((c !== H || x !== m) && Ks(e, n, a, m)),
        (ll = !1),
        (x = e.memoizedState),
        (n.state = x),
        eu(e, a, n, u),
        tu());
      var R = e.memoizedState;
      c !== H ||
      x !== R ||
      ll ||
      (t !== null && t.dependencies !== null && nn(t.dependencies))
        ? (typeof r == "function" && (Tc(e, l, r, a), (R = e.memoizedState)),
          (_ =
            ll ||
            Vs(e, l, _, a, x, R, m) ||
            (t !== null && t.dependencies !== null && nn(t.dependencies)))
            ? (T ||
                (typeof n.UNSAFE_componentWillUpdate != "function" &&
                  typeof n.componentWillUpdate != "function") ||
                (typeof n.componentWillUpdate == "function" &&
                  n.componentWillUpdate(a, R, m),
                typeof n.UNSAFE_componentWillUpdate == "function" &&
                  n.UNSAFE_componentWillUpdate(a, R, m)),
              typeof n.componentDidUpdate == "function" && (e.flags |= 4),
              typeof n.getSnapshotBeforeUpdate == "function" &&
                (e.flags |= 1024))
            : (typeof n.componentDidUpdate != "function" ||
                (c === t.memoizedProps && x === t.memoizedState) ||
                (e.flags |= 4),
              typeof n.getSnapshotBeforeUpdate != "function" ||
                (c === t.memoizedProps && x === t.memoizedState) ||
                (e.flags |= 1024),
              (e.memoizedProps = a),
              (e.memoizedState = R)),
          (n.props = a),
          (n.state = R),
          (n.context = m),
          (a = _))
        : (typeof n.componentDidUpdate != "function" ||
            (c === t.memoizedProps && x === t.memoizedState) ||
            (e.flags |= 4),
          typeof n.getSnapshotBeforeUpdate != "function" ||
            (c === t.memoizedProps && x === t.memoizedState) ||
            (e.flags |= 1024),
          (a = !1));
    }
    return (
      (n = a),
      Rn(t, e),
      (a = (e.flags & 128) !== 0),
      n || a
        ? ((n = e.stateNode),
          (l =
            a && typeof l.getDerivedStateFromError != "function"
              ? null
              : n.render()),
          (e.flags |= 1),
          t !== null && a
            ? ((e.child = ma(e, t.child, null, u)),
              (e.child = ma(e, null, l, u)))
            : wt(t, e, l, u),
          (e.memoizedState = n.state),
          (t = e.child))
        : (t = Xe(t, e, u)),
      t
    );
  }
  function co(t, e, l, a) {
    return (Ka(), (e.flags |= 256), wt(t, e, l, a), e.child);
  }
  var Oc = {
    dehydrated: null,
    treeContext: null,
    retryLane: 0,
    hydrationErrors: null,
  };
  function zc(t) {
    return { baseLanes: t, cachePool: $r() };
  }
  function Mc(t, e, l) {
    return ((t = t !== null ? t.childLanes & ~l : 0), e && (t |= Se), t);
  }
  function fo(t, e, l) {
    var a = e.pendingProps,
      u = !1,
      n = (e.flags & 128) !== 0,
      c;
    if (
      ((c = n) ||
        (c =
          t !== null && t.memoizedState === null ? !1 : (Ht.current & 2) !== 0),
      c && ((u = !0), (e.flags &= -129)),
      (c = (e.flags & 32) !== 0),
      (e.flags &= -33),
      t === null)
    ) {
      if (st) {
        if ((u ? il(e) : cl(), st)) {
          var r = Rt,
            m;
          if ((m = r)) {
            t: {
              for (m = r, r = Oe; m.nodeType !== 8; ) {
                if (!r) {
                  r = null;
                  break t;
                }
                if (((m = Ae(m.nextSibling)), m === null)) {
                  r = null;
                  break t;
                }
              }
              r = m;
            }
            r !== null
              ? ((e.memoizedState = {
                  dehydrated: r,
                  treeContext: _l !== null ? { id: Be, overflow: qe } : null,
                  retryLane: 536870912,
                  hydrationErrors: null,
                }),
                (m = ne(18, null, null, 0)),
                (m.stateNode = r),
                (m.return = e),
                (e.child = m),
                ($t = e),
                (Rt = null),
                (m = !0))
              : (m = !1);
          }
          m || Cl(e);
        }
        if (
          ((r = e.memoizedState),
          r !== null && ((r = r.dehydrated), r !== null))
        )
          return (df(r) ? (e.lanes = 32) : (e.lanes = 536870912), null);
        Ge(e);
      }
      return (
        (r = a.children),
        (a = a.fallback),
        u
          ? (cl(),
            (u = e.mode),
            (r = On({ mode: "hidden", children: r }, u)),
            (a = Dl(a, u, l, null)),
            (r.return = e),
            (a.return = e),
            (r.sibling = a),
            (e.child = r),
            (u = e.child),
            (u.memoizedState = zc(l)),
            (u.childLanes = Mc(t, c, l)),
            (e.memoizedState = Oc),
            a)
          : (il(e), Dc(e, r))
      );
    }
    if (
      ((m = t.memoizedState), m !== null && ((r = m.dehydrated), r !== null))
    ) {
      if (n)
        e.flags & 256
          ? (il(e), (e.flags &= -257), (e = _c(t, e, l)))
          : e.memoizedState !== null
            ? (cl(), (e.child = t.child), (e.flags |= 128), (e = null))
            : (cl(),
              (u = a.fallback),
              (r = e.mode),
              (a = On({ mode: "visible", children: a.children }, r)),
              (u = Dl(u, r, l, null)),
              (u.flags |= 2),
              (a.return = e),
              (u.return = e),
              (a.sibling = u),
              (e.child = a),
              ma(e, t.child, null, l),
              (a = e.child),
              (a.memoizedState = zc(l)),
              (a.childLanes = Mc(t, c, l)),
              (e.memoizedState = Oc),
              (e = u));
      else if ((il(e), df(r))) {
        if (((c = r.nextSibling && r.nextSibling.dataset), c)) var T = c.dgst;
        ((c = T),
          (a = Error(f(419))),
          (a.stack = ""),
          (a.digest = c),
          Ja({ value: a, source: null, stack: null }),
          (e = _c(t, e, l)));
      } else if (
        (Bt || $a(t, e, l, !1), (c = (l & t.childLanes) !== 0), Bt || c)
      ) {
        if (
          ((c = pt),
          c !== null &&
            ((a = l & -l),
            (a = (a & 42) !== 0 ? 1 : di(a)),
            (a = (a & (c.suspendedLanes | l)) !== 0 ? 0 : a),
            a !== 0 && a !== m.retryLane))
        )
          throw ((m.retryLane = a), aa(t, a), se(c, t, a), Is);
        (r.data === "$?" || $c(), (e = _c(t, e, l)));
      } else
        r.data === "$?"
          ? ((e.flags |= 192), (e.child = t.child), (e = null))
          : ((t = m.treeContext),
            (Rt = Ae(r.nextSibling)),
            ($t = e),
            (st = !0),
            (Ul = null),
            (Oe = !1),
            t !== null &&
              ((ye[ge++] = Be),
              (ye[ge++] = qe),
              (ye[ge++] = _l),
              (Be = t.id),
              (qe = t.overflow),
              (_l = e)),
            (e = Dc(e, a.children)),
            (e.flags |= 4096));
      return e;
    }
    return u
      ? (cl(),
        (u = a.fallback),
        (r = e.mode),
        (m = t.child),
        (T = m.sibling),
        (a = je(m, { mode: "hidden", children: a.children })),
        (a.subtreeFlags = m.subtreeFlags & 65011712),
        T !== null ? (u = je(T, u)) : ((u = Dl(u, r, l, null)), (u.flags |= 2)),
        (u.return = e),
        (a.return = e),
        (a.sibling = u),
        (e.child = a),
        (a = u),
        (u = e.child),
        (r = t.child.memoizedState),
        r === null
          ? (r = zc(l))
          : ((m = r.cachePool),
            m !== null
              ? ((T = Ct._currentValue),
                (m = m.parent !== T ? { parent: T, pool: T } : m))
              : (m = $r()),
            (r = { baseLanes: r.baseLanes | l, cachePool: m })),
        (u.memoizedState = r),
        (u.childLanes = Mc(t, c, l)),
        (e.memoizedState = Oc),
        a)
      : (il(e),
        (l = t.child),
        (t = l.sibling),
        (l = je(l, { mode: "visible", children: a.children })),
        (l.return = e),
        (l.sibling = null),
        t !== null &&
          ((c = e.deletions),
          c === null ? ((e.deletions = [t]), (e.flags |= 16)) : c.push(t)),
        (e.child = l),
        (e.memoizedState = null),
        l);
  }
  function Dc(t, e) {
    return (
      (e = On({ mode: "visible", children: e }, t.mode)),
      (e.return = t),
      (t.child = e)
    );
  }
  function On(t, e) {
    return (
      (t = ne(22, t, null, e)),
      (t.lanes = 0),
      (t.stateNode = {
        _visibility: 1,
        _pendingMarkers: null,
        _retryCache: null,
        _transitions: null,
      }),
      t
    );
  }
  function _c(t, e, l) {
    return (
      ma(e, t.child, null, l),
      (t = Dc(e, e.pendingProps.children)),
      (t.flags |= 2),
      (e.memoizedState = null),
      t
    );
  }
  function ro(t, e, l) {
    t.lanes |= e;
    var a = t.alternate;
    (a !== null && (a.lanes |= e), $i(t.return, e, l));
  }
  function Nc(t, e, l, a, u) {
    var n = t.memoizedState;
    n === null
      ? (t.memoizedState = {
          isBackwards: e,
          rendering: null,
          renderingStartTime: 0,
          last: a,
          tail: l,
          tailMode: u,
        })
      : ((n.isBackwards = e),
        (n.rendering = null),
        (n.renderingStartTime = 0),
        (n.last = a),
        (n.tail = l),
        (n.tailMode = u));
  }
  function so(t, e, l) {
    var a = e.pendingProps,
      u = a.revealOrder,
      n = a.tail;
    if ((wt(t, e, a.children, l), (a = Ht.current), (a & 2) !== 0))
      ((a = (a & 1) | 2), (e.flags |= 128));
    else {
      if (t !== null && (t.flags & 128) !== 0)
        t: for (t = e.child; t !== null; ) {
          if (t.tag === 13) t.memoizedState !== null && ro(t, l, e);
          else if (t.tag === 19) ro(t, l, e);
          else if (t.child !== null) {
            ((t.child.return = t), (t = t.child));
            continue;
          }
          if (t === e) break t;
          for (; t.sibling === null; ) {
            if (t.return === null || t.return === e) break t;
            t = t.return;
          }
          ((t.sibling.return = t.return), (t = t.sibling));
        }
      a &= 1;
    }
    switch ((w(Ht, a), u)) {
      case "forwards":
        for (l = e.child, u = null; l !== null; )
          ((t = l.alternate),
            t !== null && Tn(t) === null && (u = l),
            (l = l.sibling));
        ((l = u),
          l === null
            ? ((u = e.child), (e.child = null))
            : ((u = l.sibling), (l.sibling = null)),
          Nc(e, !1, u, l, n));
        break;
      case "backwards":
        for (l = null, u = e.child, e.child = null; u !== null; ) {
          if (((t = u.alternate), t !== null && Tn(t) === null)) {
            e.child = u;
            break;
          }
          ((t = u.sibling), (u.sibling = l), (l = u), (u = t));
        }
        Nc(e, !0, l, null, n);
        break;
      case "together":
        Nc(e, !1, null, null, void 0);
        break;
      default:
        e.memoizedState = null;
    }
    return e.child;
  }
  function Xe(t, e, l) {
    if (
      (t !== null && (e.dependencies = t.dependencies),
      (dl |= e.lanes),
      (l & e.childLanes) === 0)
    )
      if (t !== null) {
        if (($a(t, e, l, !1), (l & e.childLanes) === 0)) return null;
      } else return null;
    if (t !== null && e.child !== t.child) throw Error(f(153));
    if (e.child !== null) {
      for (
        t = e.child, l = je(t, t.pendingProps), e.child = l, l.return = e;
        t.sibling !== null;

      )
        ((t = t.sibling),
          (l = l.sibling = je(t, t.pendingProps)),
          (l.return = e));
      l.sibling = null;
    }
    return e.child;
  }
  function Uc(t, e) {
    return (t.lanes & e) !== 0
      ? !0
      : ((t = t.dependencies), !!(t !== null && nn(t)));
  }
  function qm(t, e, l) {
    switch (e.tag) {
      case 3:
        (bt(e, e.stateNode.containerInfo),
          el(e, Ct, t.memoizedState.cache),
          Ka());
        break;
      case 27:
      case 5:
        ci(e);
        break;
      case 4:
        bt(e, e.stateNode.containerInfo);
        break;
      case 10:
        el(e, e.type, e.memoizedProps.value);
        break;
      case 13:
        var a = e.memoizedState;
        if (a !== null)
          return a.dehydrated !== null
            ? (il(e), (e.flags |= 128), null)
            : (l & e.child.childLanes) !== 0
              ? fo(t, e, l)
              : (il(e), (t = Xe(t, e, l)), t !== null ? t.sibling : null);
        il(e);
        break;
      case 19:
        var u = (t.flags & 128) !== 0;
        if (
          ((a = (l & e.childLanes) !== 0),
          a || ($a(t, e, l, !1), (a = (l & e.childLanes) !== 0)),
          u)
        ) {
          if (a) return so(t, e, l);
          e.flags |= 128;
        }
        if (
          ((u = e.memoizedState),
          u !== null &&
            ((u.rendering = null), (u.tail = null), (u.lastEffect = null)),
          w(Ht, Ht.current),
          a)
        )
          break;
        return null;
      case 22:
      case 23:
        return ((e.lanes = 0), ao(t, e, l));
      case 24:
        el(e, Ct, t.memoizedState.cache);
    }
    return Xe(t, e, l);
  }
  function oo(t, e, l) {
    if (t !== null)
      if (t.memoizedProps !== e.pendingProps) Bt = !0;
      else {
        if (!Uc(t, l) && (e.flags & 128) === 0) return ((Bt = !1), qm(t, e, l));
        Bt = (t.flags & 131072) !== 0;
      }
    else ((Bt = !1), st && (e.flags & 1048576) !== 0 && Gr(e, un, e.index));
    switch (((e.lanes = 0), e.tag)) {
      case 16:
        t: {
          t = e.pendingProps;
          var a = e.elementType,
            u = a._init;
          if (((a = u(a._payload)), (e.type = a), typeof a == "function"))
            Gi(a)
              ? ((t = Yl(a, t)), (e.tag = 1), (e = io(null, e, a, t, l)))
              : ((e.tag = 0), (e = Rc(null, e, a, t, l)));
          else {
            if (a != null) {
              if (((u = a.$$typeof), u === ct)) {
                ((e.tag = 11), (e = to(null, e, a, t, l)));
                break t;
              } else if (u === xt) {
                ((e.tag = 14), (e = eo(null, e, a, t, l)));
                break t;
              }
            }
            throw ((e = Al(a) || a), Error(f(306, e, "")));
          }
        }
        return e;
      case 0:
        return Rc(t, e, e.type, e.pendingProps, l);
      case 1:
        return ((a = e.type), (u = Yl(a, e.pendingProps)), io(t, e, a, u, l));
      case 3:
        t: {
          if ((bt(e, e.stateNode.containerInfo), t === null))
            throw Error(f(387));
          a = e.pendingProps;
          var n = e.memoizedState;
          ((u = n.element), ec(t, e), eu(e, a, null, l));
          var c = e.memoizedState;
          if (
            ((a = c.cache),
            el(e, Ct, a),
            a !== n.cache && ki(e, [Ct], l, !0),
            tu(),
            (a = c.element),
            n.isDehydrated)
          )
            if (
              ((n = { element: a, isDehydrated: !1, cache: c.cache }),
              (e.updateQueue.baseState = n),
              (e.memoizedState = n),
              e.flags & 256)
            ) {
              e = co(t, e, a, l);
              break t;
            } else if (a !== u) {
              ((u = me(Error(f(424)), e)), Ja(u), (e = co(t, e, a, l)));
              break t;
            } else {
              switch (((t = e.stateNode.containerInfo), t.nodeType)) {
                case 9:
                  t = t.body;
                  break;
                default:
                  t = t.nodeName === "HTML" ? t.ownerDocument.body : t;
              }
              for (
                Rt = Ae(t.firstChild),
                  $t = e,
                  st = !0,
                  Ul = null,
                  Oe = !0,
                  l = Qs(e, null, a, l),
                  e.child = l;
                l;

              )
                ((l.flags = (l.flags & -3) | 4096), (l = l.sibling));
            }
          else {
            if ((Ka(), a === u)) {
              e = Xe(t, e, l);
              break t;
            }
            wt(t, e, a, l);
          }
          e = e.child;
        }
        return e;
      case 26:
        return (
          Rn(t, e),
          t === null
            ? (l = yd(e.type, null, e.pendingProps, null))
              ? (e.memoizedState = l)
              : st ||
                ((l = e.type),
                (t = e.pendingProps),
                (a = Ln(tt.current).createElement(l)),
                (a[Zt] = e),
                (a[Wt] = t),
                Gt(a, l, t),
                jt(a),
                (e.stateNode = a))
            : (e.memoizedState = yd(
                e.type,
                t.memoizedProps,
                e.pendingProps,
                t.memoizedState,
              )),
          null
        );
      case 27:
        return (
          ci(e),
          t === null &&
            st &&
            ((a = e.stateNode = hd(e.type, e.pendingProps, tt.current)),
            ($t = e),
            (Oe = !0),
            (u = Rt),
            yl(e.type) ? ((hf = u), (Rt = Ae(a.firstChild))) : (Rt = u)),
          wt(t, e, e.pendingProps.children, l),
          Rn(t, e),
          t === null && (e.flags |= 4194304),
          e.child
        );
      case 5:
        return (
          t === null &&
            st &&
            ((u = a = Rt) &&
              ((a = o0(a, e.type, e.pendingProps, Oe)),
              a !== null
                ? ((e.stateNode = a),
                  ($t = e),
                  (Rt = Ae(a.firstChild)),
                  (Oe = !1),
                  (u = !0))
                : (u = !1)),
            u || Cl(e)),
          ci(e),
          (u = e.type),
          (n = e.pendingProps),
          (c = t !== null ? t.memoizedProps : null),
          (a = n.children),
          rf(u, n) ? (a = null) : c !== null && rf(u, c) && (e.flags |= 32),
          e.memoizedState !== null &&
            ((u = cc(t, e, Dm, null, null, l)), (Au._currentValue = u)),
          Rn(t, e),
          wt(t, e, a, l),
          e.child
        );
      case 6:
        return (
          t === null &&
            st &&
            ((t = l = Rt) &&
              ((l = d0(l, e.pendingProps, Oe)),
              l !== null
                ? ((e.stateNode = l), ($t = e), (Rt = null), (t = !0))
                : (t = !1)),
            t || Cl(e)),
          null
        );
      case 13:
        return fo(t, e, l);
      case 4:
        return (
          bt(e, e.stateNode.containerInfo),
          (a = e.pendingProps),
          t === null ? (e.child = ma(e, null, a, l)) : wt(t, e, a, l),
          e.child
        );
      case 11:
        return to(t, e, e.type, e.pendingProps, l);
      case 7:
        return (wt(t, e, e.pendingProps, l), e.child);
      case 8:
        return (wt(t, e, e.pendingProps.children, l), e.child);
      case 12:
        return (wt(t, e, e.pendingProps.children, l), e.child);
      case 10:
        return (
          (a = e.pendingProps),
          el(e, e.type, a.value),
          wt(t, e, a.children, l),
          e.child
        );
      case 9:
        return (
          (u = e.type._context),
          (a = e.pendingProps.children),
          jl(e),
          (u = Vt(u)),
          (a = a(u)),
          (e.flags |= 1),
          wt(t, e, a, l),
          e.child
        );
      case 14:
        return eo(t, e, e.type, e.pendingProps, l);
      case 15:
        return lo(t, e, e.type, e.pendingProps, l);
      case 19:
        return so(t, e, l);
      case 31:
        return (
          (a = e.pendingProps),
          (l = e.mode),
          (a = { mode: a.mode, children: a.children }),
          t === null
            ? ((l = On(a, l)),
              (l.ref = e.ref),
              (e.child = l),
              (l.return = e),
              (e = l))
            : ((l = je(t.child, a)),
              (l.ref = e.ref),
              (e.child = l),
              (l.return = e),
              (e = l)),
          e
        );
      case 22:
        return ao(t, e, l);
      case 24:
        return (
          jl(e),
          (a = Vt(Ct)),
          t === null
            ? ((u = Pi()),
              u === null &&
                ((u = pt),
                (n = Wi()),
                (u.pooledCache = n),
                n.refCount++,
                n !== null && (u.pooledCacheLanes |= l),
                (u = n)),
              (e.memoizedState = { parent: a, cache: u }),
              tc(e),
              el(e, Ct, u))
            : ((t.lanes & l) !== 0 && (ec(t, e), eu(e, null, null, l), tu()),
              (u = t.memoizedState),
              (n = e.memoizedState),
              u.parent !== a
                ? ((u = { parent: a, cache: a }),
                  (e.memoizedState = u),
                  e.lanes === 0 &&
                    (e.memoizedState = e.updateQueue.baseState = u),
                  el(e, Ct, a))
                : ((a = n.cache),
                  el(e, Ct, a),
                  a !== u.cache && ki(e, [Ct], l, !0))),
          wt(t, e, e.pendingProps.children, l),
          e.child
        );
      case 29:
        throw e.pendingProps;
    }
    throw Error(f(156, e.tag));
  }
  function Qe(t) {
    t.flags |= 4;
  }
  function ho(t, e) {
    if (e.type !== "stylesheet" || (e.state.loading & 4) !== 0)
      t.flags &= -16777217;
    else if (((t.flags |= 16777216), !Ed(e))) {
      if (
        ((e = pe.current),
        e !== null &&
          ((it & 4194048) === it
            ? ze !== null
            : ((it & 62914560) !== it && (it & 536870912) === 0) || e !== ze))
      )
        throw ((Pa = Ii), kr);
      t.flags |= 8192;
    }
  }
  function zn(t, e) {
    (e !== null && (t.flags |= 4),
      t.flags & 16384 &&
        ((e = t.tag !== 22 ? Zf() : 536870912), (t.lanes |= e), (pa |= e)));
  }
  function fu(t, e) {
    if (!st)
      switch (t.tailMode) {
        case "hidden":
          e = t.tail;
          for (var l = null; e !== null; )
            (e.alternate !== null && (l = e), (e = e.sibling));
          l === null ? (t.tail = null) : (l.sibling = null);
          break;
        case "collapsed":
          l = t.tail;
          for (var a = null; l !== null; )
            (l.alternate !== null && (a = l), (l = l.sibling));
          a === null
            ? e || t.tail === null
              ? (t.tail = null)
              : (t.tail.sibling = null)
            : (a.sibling = null);
      }
  }
  function Tt(t) {
    var e = t.alternate !== null && t.alternate.child === t.child,
      l = 0,
      a = 0;
    if (e)
      for (var u = t.child; u !== null; )
        ((l |= u.lanes | u.childLanes),
          (a |= u.subtreeFlags & 65011712),
          (a |= u.flags & 65011712),
          (u.return = t),
          (u = u.sibling));
    else
      for (u = t.child; u !== null; )
        ((l |= u.lanes | u.childLanes),
          (a |= u.subtreeFlags),
          (a |= u.flags),
          (u.return = t),
          (u = u.sibling));
    return ((t.subtreeFlags |= a), (t.childLanes = l), e);
  }
  function Ym(t, e, l) {
    var a = e.pendingProps;
    switch ((Vi(e), e.tag)) {
      case 31:
      case 16:
      case 15:
      case 0:
      case 11:
      case 7:
      case 8:
      case 12:
      case 9:
      case 14:
        return (Tt(e), null);
      case 1:
        return (Tt(e), null);
      case 3:
        return (
          (l = e.stateNode),
          (a = null),
          t !== null && (a = t.memoizedState.cache),
          e.memoizedState.cache !== a && (e.flags |= 2048),
          we(Ct),
          Fe(),
          l.pendingContext &&
            ((l.context = l.pendingContext), (l.pendingContext = null)),
          (t === null || t.child === null) &&
            (Va(e)
              ? Qe(e)
              : t === null ||
                (t.memoizedState.isDehydrated && (e.flags & 256) === 0) ||
                ((e.flags |= 1024), Zr())),
          Tt(e),
          null
        );
      case 26:
        return (
          (l = e.memoizedState),
          t === null
            ? (Qe(e),
              l !== null ? (Tt(e), ho(e, l)) : (Tt(e), (e.flags &= -16777217)))
            : l
              ? l !== t.memoizedState
                ? (Qe(e), Tt(e), ho(e, l))
                : (Tt(e), (e.flags &= -16777217))
              : (t.memoizedProps !== a && Qe(e), Tt(e), (e.flags &= -16777217)),
          null
        );
      case 27:
        (qu(e), (l = tt.current));
        var u = e.type;
        if (t !== null && e.stateNode != null) t.memoizedProps !== a && Qe(e);
        else {
          if (!a) {
            if (e.stateNode === null) throw Error(f(166));
            return (Tt(e), null);
          }
          ((t = J.current),
            Va(e) ? Xr(e) : ((t = hd(u, a, l)), (e.stateNode = t), Qe(e)));
        }
        return (Tt(e), null);
      case 5:
        if ((qu(e), (l = e.type), t !== null && e.stateNode != null))
          t.memoizedProps !== a && Qe(e);
        else {
          if (!a) {
            if (e.stateNode === null) throw Error(f(166));
            return (Tt(e), null);
          }
          if (((t = J.current), Va(e))) Xr(e);
          else {
            switch (((u = Ln(tt.current)), t)) {
              case 1:
                t = u.createElementNS("http://www.w3.org/2000/svg", l);
                break;
              case 2:
                t = u.createElementNS("http://www.w3.org/1998/Math/MathML", l);
                break;
              default:
                switch (l) {
                  case "svg":
                    t = u.createElementNS("http://www.w3.org/2000/svg", l);
                    break;
                  case "math":
                    t = u.createElementNS(
                      "http://www.w3.org/1998/Math/MathML",
                      l,
                    );
                    break;
                  case "script":
                    ((t = u.createElement("div")),
                      (t.innerHTML = "<script><\/script>"),
                      (t = t.removeChild(t.firstChild)));
                    break;
                  case "select":
                    ((t =
                      typeof a.is == "string"
                        ? u.createElement("select", { is: a.is })
                        : u.createElement("select")),
                      a.multiple
                        ? (t.multiple = !0)
                        : a.size && (t.size = a.size));
                    break;
                  default:
                    t =
                      typeof a.is == "string"
                        ? u.createElement(l, { is: a.is })
                        : u.createElement(l);
                }
            }
            ((t[Zt] = e), (t[Wt] = a));
            t: for (u = e.child; u !== null; ) {
              if (u.tag === 5 || u.tag === 6) t.appendChild(u.stateNode);
              else if (u.tag !== 4 && u.tag !== 27 && u.child !== null) {
                ((u.child.return = u), (u = u.child));
                continue;
              }
              if (u === e) break t;
              for (; u.sibling === null; ) {
                if (u.return === null || u.return === e) break t;
                u = u.return;
              }
              ((u.sibling.return = u.return), (u = u.sibling));
            }
            e.stateNode = t;
            t: switch ((Gt(t, l, a), l)) {
              case "button":
              case "input":
              case "select":
              case "textarea":
                t = !!a.autoFocus;
                break t;
              case "img":
                t = !0;
                break t;
              default:
                t = !1;
            }
            t && Qe(e);
          }
        }
        return (Tt(e), (e.flags &= -16777217), null);
      case 6:
        if (t && e.stateNode != null) t.memoizedProps !== a && Qe(e);
        else {
          if (typeof a != "string" && e.stateNode === null) throw Error(f(166));
          if (((t = tt.current), Va(e))) {
            if (
              ((t = e.stateNode),
              (l = e.memoizedProps),
              (a = null),
              (u = $t),
              u !== null)
            )
              switch (u.tag) {
                case 27:
                case 5:
                  a = u.memoizedProps;
              }
            ((t[Zt] = e),
              (t = !!(
                t.nodeValue === l ||
                (a !== null && a.suppressHydrationWarning === !0) ||
                id(t.nodeValue, l)
              )),
              t || Cl(e));
          } else
            ((t = Ln(t).createTextNode(a)), (t[Zt] = e), (e.stateNode = t));
        }
        return (Tt(e), null);
      case 13:
        if (
          ((a = e.memoizedState),
          t === null ||
            (t.memoizedState !== null && t.memoizedState.dehydrated !== null))
        ) {
          if (((u = Va(e)), a !== null && a.dehydrated !== null)) {
            if (t === null) {
              if (!u) throw Error(f(318));
              if (
                ((u = e.memoizedState),
                (u = u !== null ? u.dehydrated : null),
                !u)
              )
                throw Error(f(317));
              u[Zt] = e;
            } else
              (Ka(),
                (e.flags & 128) === 0 && (e.memoizedState = null),
                (e.flags |= 4));
            (Tt(e), (u = !1));
          } else
            ((u = Zr()),
              t !== null &&
                t.memoizedState !== null &&
                (t.memoizedState.hydrationErrors = u),
              (u = !0));
          if (!u) return e.flags & 256 ? (Ge(e), e) : (Ge(e), null);
        }
        if ((Ge(e), (e.flags & 128) !== 0)) return ((e.lanes = l), e);
        if (
          ((l = a !== null), (t = t !== null && t.memoizedState !== null), l)
        ) {
          ((a = e.child),
            (u = null),
            a.alternate !== null &&
              a.alternate.memoizedState !== null &&
              a.alternate.memoizedState.cachePool !== null &&
              (u = a.alternate.memoizedState.cachePool.pool));
          var n = null;
          (a.memoizedState !== null &&
            a.memoizedState.cachePool !== null &&
            (n = a.memoizedState.cachePool.pool),
            n !== u && (a.flags |= 2048));
        }
        return (
          l !== t && l && (e.child.flags |= 8192),
          zn(e, e.updateQueue),
          Tt(e),
          null
        );
      case 4:
        return (Fe(), t === null && af(e.stateNode.containerInfo), Tt(e), null);
      case 10:
        return (we(e.type), Tt(e), null);
      case 19:
        if ((X(Ht), (u = e.memoizedState), u === null)) return (Tt(e), null);
        if (((a = (e.flags & 128) !== 0), (n = u.rendering), n === null))
          if (a) fu(u, !1);
          else {
            if (Ot !== 0 || (t !== null && (t.flags & 128) !== 0))
              for (t = e.child; t !== null; ) {
                if (((n = Tn(t)), n !== null)) {
                  for (
                    e.flags |= 128,
                      fu(u, !1),
                      t = n.updateQueue,
                      e.updateQueue = t,
                      zn(e, t),
                      e.subtreeFlags = 0,
                      t = l,
                      l = e.child;
                    l !== null;

                  )
                    (Lr(l, t), (l = l.sibling));
                  return (w(Ht, (Ht.current & 1) | 2), e.child);
                }
                t = t.sibling;
              }
            u.tail !== null &&
              Re() > _n &&
              ((e.flags |= 128), (a = !0), fu(u, !1), (e.lanes = 4194304));
          }
        else {
          if (!a)
            if (((t = Tn(n)), t !== null)) {
              if (
                ((e.flags |= 128),
                (a = !0),
                (t = t.updateQueue),
                (e.updateQueue = t),
                zn(e, t),
                fu(u, !0),
                u.tail === null &&
                  u.tailMode === "hidden" &&
                  !n.alternate &&
                  !st)
              )
                return (Tt(e), null);
            } else
              2 * Re() - u.renderingStartTime > _n &&
                l !== 536870912 &&
                ((e.flags |= 128), (a = !0), fu(u, !1), (e.lanes = 4194304));
          u.isBackwards
            ? ((n.sibling = e.child), (e.child = n))
            : ((t = u.last),
              t !== null ? (t.sibling = n) : (e.child = n),
              (u.last = n));
        }
        return u.tail !== null
          ? ((e = u.tail),
            (u.rendering = e),
            (u.tail = e.sibling),
            (u.renderingStartTime = Re()),
            (e.sibling = null),
            (t = Ht.current),
            w(Ht, a ? (t & 1) | 2 : t & 1),
            e)
          : (Tt(e), null);
      case 22:
      case 23:
        return (
          Ge(e),
          nc(),
          (a = e.memoizedState !== null),
          t !== null
            ? (t.memoizedState !== null) !== a && (e.flags |= 8192)
            : a && (e.flags |= 8192),
          a
            ? (l & 536870912) !== 0 &&
              (e.flags & 128) === 0 &&
              (Tt(e), e.subtreeFlags & 6 && (e.flags |= 8192))
            : Tt(e),
          (l = e.updateQueue),
          l !== null && zn(e, l.retryQueue),
          (l = null),
          t !== null &&
            t.memoizedState !== null &&
            t.memoizedState.cachePool !== null &&
            (l = t.memoizedState.cachePool.pool),
          (a = null),
          e.memoizedState !== null &&
            e.memoizedState.cachePool !== null &&
            (a = e.memoizedState.cachePool.pool),
          a !== l && (e.flags |= 2048),
          t !== null && X(Bl),
          null
        );
      case 24:
        return (
          (l = null),
          t !== null && (l = t.memoizedState.cache),
          e.memoizedState.cache !== l && (e.flags |= 2048),
          we(Ct),
          Tt(e),
          null
        );
      case 25:
        return null;
      case 30:
        return null;
    }
    throw Error(f(156, e.tag));
  }
  function wm(t, e) {
    switch ((Vi(e), e.tag)) {
      case 1:
        return (
          (t = e.flags),
          t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 3:
        return (
          we(Ct),
          Fe(),
          (t = e.flags),
          (t & 65536) !== 0 && (t & 128) === 0
            ? ((e.flags = (t & -65537) | 128), e)
            : null
        );
      case 26:
      case 27:
      case 5:
        return (qu(e), null);
      case 13:
        if (
          (Ge(e), (t = e.memoizedState), t !== null && t.dehydrated !== null)
        ) {
          if (e.alternate === null) throw Error(f(340));
          Ka();
        }
        return (
          (t = e.flags),
          t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 19:
        return (X(Ht), null);
      case 4:
        return (Fe(), null);
      case 10:
        return (we(e.type), null);
      case 22:
      case 23:
        return (
          Ge(e),
          nc(),
          t !== null && X(Bl),
          (t = e.flags),
          t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 24:
        return (we(Ct), null);
      case 25:
        return null;
      default:
        return null;
    }
  }
  function mo(t, e) {
    switch ((Vi(e), e.tag)) {
      case 3:
        (we(Ct), Fe());
        break;
      case 26:
      case 27:
      case 5:
        qu(e);
        break;
      case 4:
        Fe();
        break;
      case 13:
        Ge(e);
        break;
      case 19:
        X(Ht);
        break;
      case 10:
        we(e.type);
        break;
      case 22:
      case 23:
        (Ge(e), nc(), t !== null && X(Bl));
        break;
      case 24:
        we(Ct);
    }
  }
  function ru(t, e) {
    try {
      var l = e.updateQueue,
        a = l !== null ? l.lastEffect : null;
      if (a !== null) {
        var u = a.next;
        l = u;
        do {
          if ((l.tag & t) === t) {
            a = void 0;
            var n = l.create,
              c = l.inst;
            ((a = n()), (c.destroy = a));
          }
          l = l.next;
        } while (l !== u);
      }
    } catch (r) {
      gt(e, e.return, r);
    }
  }
  function fl(t, e, l) {
    try {
      var a = e.updateQueue,
        u = a !== null ? a.lastEffect : null;
      if (u !== null) {
        var n = u.next;
        a = n;
        do {
          if ((a.tag & t) === t) {
            var c = a.inst,
              r = c.destroy;
            if (r !== void 0) {
              ((c.destroy = void 0), (u = e));
              var m = l,
                T = r;
              try {
                T();
              } catch (_) {
                gt(u, m, _);
              }
            }
          }
          a = a.next;
        } while (a !== n);
      }
    } catch (_) {
      gt(e, e.return, _);
    }
  }
  function vo(t) {
    var e = t.updateQueue;
    if (e !== null) {
      var l = t.stateNode;
      try {
        es(e, l);
      } catch (a) {
        gt(t, t.return, a);
      }
    }
  }
  function yo(t, e, l) {
    ((l.props = Yl(t.type, t.memoizedProps)), (l.state = t.memoizedState));
    try {
      l.componentWillUnmount();
    } catch (a) {
      gt(t, e, a);
    }
  }
  function su(t, e) {
    try {
      var l = t.ref;
      if (l !== null) {
        switch (t.tag) {
          case 26:
          case 27:
          case 5:
            var a = t.stateNode;
            break;
          case 30:
            a = t.stateNode;
            break;
          default:
            a = t.stateNode;
        }
        typeof l == "function" ? (t.refCleanup = l(a)) : (l.current = a);
      }
    } catch (u) {
      gt(t, e, u);
    }
  }
  function Me(t, e) {
    var l = t.ref,
      a = t.refCleanup;
    if (l !== null)
      if (typeof a == "function")
        try {
          a();
        } catch (u) {
          gt(t, e, u);
        } finally {
          ((t.refCleanup = null),
            (t = t.alternate),
            t != null && (t.refCleanup = null));
        }
      else if (typeof l == "function")
        try {
          l(null);
        } catch (u) {
          gt(t, e, u);
        }
      else l.current = null;
  }
  function go(t) {
    var e = t.type,
      l = t.memoizedProps,
      a = t.stateNode;
    try {
      t: switch (e) {
        case "button":
        case "input":
        case "select":
        case "textarea":
          l.autoFocus && a.focus();
          break t;
        case "img":
          l.src ? (a.src = l.src) : l.srcSet && (a.srcset = l.srcSet);
      }
    } catch (u) {
      gt(t, t.return, u);
    }
  }
  function Cc(t, e, l) {
    try {
      var a = t.stateNode;
      (i0(a, t.type, l, e), (a[Wt] = e));
    } catch (u) {
      gt(t, t.return, u);
    }
  }
  function po(t) {
    return (
      t.tag === 5 ||
      t.tag === 3 ||
      t.tag === 26 ||
      (t.tag === 27 && yl(t.type)) ||
      t.tag === 4
    );
  }
  function Hc(t) {
    t: for (;;) {
      for (; t.sibling === null; ) {
        if (t.return === null || po(t.return)) return null;
        t = t.return;
      }
      for (
        t.sibling.return = t.return, t = t.sibling;
        t.tag !== 5 && t.tag !== 6 && t.tag !== 18;

      ) {
        if (
          (t.tag === 27 && yl(t.type)) ||
          t.flags & 2 ||
          t.child === null ||
          t.tag === 4
        )
          continue t;
        ((t.child.return = t), (t = t.child));
      }
      if (!(t.flags & 2)) return t.stateNode;
    }
  }
  function jc(t, e, l) {
    var a = t.tag;
    if (a === 5 || a === 6)
      ((t = t.stateNode),
        e
          ? (l.nodeType === 9
              ? l.body
              : l.nodeName === "HTML"
                ? l.ownerDocument.body
                : l
            ).insertBefore(t, e)
          : ((e =
              l.nodeType === 9
                ? l.body
                : l.nodeName === "HTML"
                  ? l.ownerDocument.body
                  : l),
            e.appendChild(t),
            (l = l._reactRootContainer),
            l != null || e.onclick !== null || (e.onclick = wn)));
    else if (
      a !== 4 &&
      (a === 27 && yl(t.type) && ((l = t.stateNode), (e = null)),
      (t = t.child),
      t !== null)
    )
      for (jc(t, e, l), t = t.sibling; t !== null; )
        (jc(t, e, l), (t = t.sibling));
  }
  function Mn(t, e, l) {
    var a = t.tag;
    if (a === 5 || a === 6)
      ((t = t.stateNode), e ? l.insertBefore(t, e) : l.appendChild(t));
    else if (
      a !== 4 &&
      (a === 27 && yl(t.type) && (l = t.stateNode), (t = t.child), t !== null)
    )
      for (Mn(t, e, l), t = t.sibling; t !== null; )
        (Mn(t, e, l), (t = t.sibling));
  }
  function So(t) {
    var e = t.stateNode,
      l = t.memoizedProps;
    try {
      for (var a = t.type, u = e.attributes; u.length; )
        e.removeAttributeNode(u[0]);
      (Gt(e, a, l), (e[Zt] = t), (e[Wt] = l));
    } catch (n) {
      gt(t, t.return, n);
    }
  }
  var Ze = !1,
    Dt = !1,
    Bc = !1,
    bo = typeof WeakSet == "function" ? WeakSet : Set,
    qt = null;
  function Lm(t, e) {
    if (((t = t.containerInfo), (cf = Kn), (t = _r(t)), Hi(t))) {
      if ("selectionStart" in t)
        var l = { start: t.selectionStart, end: t.selectionEnd };
      else
        t: {
          l = ((l = t.ownerDocument) && l.defaultView) || window;
          var a = l.getSelection && l.getSelection();
          if (a && a.rangeCount !== 0) {
            l = a.anchorNode;
            var u = a.anchorOffset,
              n = a.focusNode;
            a = a.focusOffset;
            try {
              (l.nodeType, n.nodeType);
            } catch {
              l = null;
              break t;
            }
            var c = 0,
              r = -1,
              m = -1,
              T = 0,
              _ = 0,
              H = t,
              x = null;
            e: for (;;) {
              for (
                var R;
                H !== l || (u !== 0 && H.nodeType !== 3) || (r = c + u),
                  H !== n || (a !== 0 && H.nodeType !== 3) || (m = c + a),
                  H.nodeType === 3 && (c += H.nodeValue.length),
                  (R = H.firstChild) !== null;

              )
                ((x = H), (H = R));
              for (;;) {
                if (H === t) break e;
                if (
                  (x === l && ++T === u && (r = c),
                  x === n && ++_ === a && (m = c),
                  (R = H.nextSibling) !== null)
                )
                  break;
                ((H = x), (x = H.parentNode));
              }
              H = R;
            }
            l = r === -1 || m === -1 ? null : { start: r, end: m };
          } else l = null;
        }
      l = l || { start: 0, end: 0 };
    } else l = null;
    for (
      ff = { focusedElem: t, selectionRange: l }, Kn = !1, qt = e;
      qt !== null;

    )
      if (
        ((e = qt), (t = e.child), (e.subtreeFlags & 1024) !== 0 && t !== null)
      )
        ((t.return = e), (qt = t));
      else
        for (; qt !== null; ) {
          switch (((e = qt), (n = e.alternate), (t = e.flags), e.tag)) {
            case 0:
              break;
            case 11:
            case 15:
              break;
            case 1:
              if ((t & 1024) !== 0 && n !== null) {
                ((t = void 0),
                  (l = e),
                  (u = n.memoizedProps),
                  (n = n.memoizedState),
                  (a = l.stateNode));
                try {
                  var F = Yl(l.type, u, l.elementType === l.type);
                  ((t = a.getSnapshotBeforeUpdate(F, n)),
                    (a.__reactInternalSnapshotBeforeUpdate = t));
                } catch ($) {
                  gt(l, l.return, $);
                }
              }
              break;
            case 3:
              if ((t & 1024) !== 0) {
                if (
                  ((t = e.stateNode.containerInfo), (l = t.nodeType), l === 9)
                )
                  of(t);
                else if (l === 1)
                  switch (t.nodeName) {
                    case "HEAD":
                    case "HTML":
                    case "BODY":
                      of(t);
                      break;
                    default:
                      t.textContent = "";
                  }
              }
              break;
            case 5:
            case 26:
            case 27:
            case 6:
            case 4:
            case 17:
              break;
            default:
              if ((t & 1024) !== 0) throw Error(f(163));
          }
          if (((t = e.sibling), t !== null)) {
            ((t.return = e.return), (qt = t));
            break;
          }
          qt = e.return;
        }
  }
  function Eo(t, e, l) {
    var a = l.flags;
    switch (l.tag) {
      case 0:
      case 11:
      case 15:
        (rl(t, l), a & 4 && ru(5, l));
        break;
      case 1:
        if ((rl(t, l), a & 4))
          if (((t = l.stateNode), e === null))
            try {
              t.componentDidMount();
            } catch (c) {
              gt(l, l.return, c);
            }
          else {
            var u = Yl(l.type, e.memoizedProps);
            e = e.memoizedState;
            try {
              t.componentDidUpdate(u, e, t.__reactInternalSnapshotBeforeUpdate);
            } catch (c) {
              gt(l, l.return, c);
            }
          }
        (a & 64 && vo(l), a & 512 && su(l, l.return));
        break;
      case 3:
        if ((rl(t, l), a & 64 && ((t = l.updateQueue), t !== null))) {
          if (((e = null), l.child !== null))
            switch (l.child.tag) {
              case 27:
              case 5:
                e = l.child.stateNode;
                break;
              case 1:
                e = l.child.stateNode;
            }
          try {
            es(t, e);
          } catch (c) {
            gt(l, l.return, c);
          }
        }
        break;
      case 27:
        e === null && a & 4 && So(l);
      case 26:
      case 5:
        (rl(t, l), e === null && a & 4 && go(l), a & 512 && su(l, l.return));
        break;
      case 12:
        rl(t, l);
        break;
      case 13:
        (rl(t, l),
          a & 4 && xo(t, l),
          a & 64 &&
            ((t = l.memoizedState),
            t !== null &&
              ((t = t.dehydrated),
              t !== null && ((l = km.bind(null, l)), h0(t, l)))));
        break;
      case 22:
        if (((a = l.memoizedState !== null || Ze), !a)) {
          ((e = (e !== null && e.memoizedState !== null) || Dt), (u = Ze));
          var n = Dt;
          ((Ze = a),
            (Dt = e) && !n ? sl(t, l, (l.subtreeFlags & 8772) !== 0) : rl(t, l),
            (Ze = u),
            (Dt = n));
        }
        break;
      case 30:
        break;
      default:
        rl(t, l);
    }
  }
  function To(t) {
    var e = t.alternate;
    (e !== null && ((t.alternate = null), To(e)),
      (t.child = null),
      (t.deletions = null),
      (t.sibling = null),
      t.tag === 5 && ((e = t.stateNode), e !== null && vi(e)),
      (t.stateNode = null),
      (t.return = null),
      (t.dependencies = null),
      (t.memoizedProps = null),
      (t.memoizedState = null),
      (t.pendingProps = null),
      (t.stateNode = null),
      (t.updateQueue = null));
  }
  var Et = null,
    It = !1;
  function Ve(t, e, l) {
    for (l = l.child; l !== null; ) (Ao(t, e, l), (l = l.sibling));
  }
  function Ao(t, e, l) {
    if (le && typeof le.onCommitFiberUnmount == "function")
      try {
        le.onCommitFiberUnmount(Na, l);
      } catch {}
    switch (l.tag) {
      case 26:
        (Dt || Me(l, e),
          Ve(t, e, l),
          l.memoizedState
            ? l.memoizedState.count--
            : l.stateNode && ((l = l.stateNode), l.parentNode.removeChild(l)));
        break;
      case 27:
        Dt || Me(l, e);
        var a = Et,
          u = It;
        (yl(l.type) && ((Et = l.stateNode), (It = !1)),
          Ve(t, e, l),
          Su(l.stateNode),
          (Et = a),
          (It = u));
        break;
      case 5:
        Dt || Me(l, e);
      case 6:
        if (
          ((a = Et),
          (u = It),
          (Et = null),
          Ve(t, e, l),
          (Et = a),
          (It = u),
          Et !== null)
        )
          if (It)
            try {
              (Et.nodeType === 9
                ? Et.body
                : Et.nodeName === "HTML"
                  ? Et.ownerDocument.body
                  : Et
              ).removeChild(l.stateNode);
            } catch (n) {
              gt(l, e, n);
            }
          else
            try {
              Et.removeChild(l.stateNode);
            } catch (n) {
              gt(l, e, n);
            }
        break;
      case 18:
        Et !== null &&
          (It
            ? ((t = Et),
              od(
                t.nodeType === 9
                  ? t.body
                  : t.nodeName === "HTML"
                    ? t.ownerDocument.body
                    : t,
                l.stateNode,
              ),
              zu(t))
            : od(Et, l.stateNode));
        break;
      case 4:
        ((a = Et),
          (u = It),
          (Et = l.stateNode.containerInfo),
          (It = !0),
          Ve(t, e, l),
          (Et = a),
          (It = u));
        break;
      case 0:
      case 11:
      case 14:
      case 15:
        (Dt || fl(2, l, e), Dt || fl(4, l, e), Ve(t, e, l));
        break;
      case 1:
        (Dt ||
          (Me(l, e),
          (a = l.stateNode),
          typeof a.componentWillUnmount == "function" && yo(l, e, a)),
          Ve(t, e, l));
        break;
      case 21:
        Ve(t, e, l);
        break;
      case 22:
        ((Dt = (a = Dt) || l.memoizedState !== null), Ve(t, e, l), (Dt = a));
        break;
      default:
        Ve(t, e, l);
    }
  }
  function xo(t, e) {
    if (
      e.memoizedState === null &&
      ((t = e.alternate),
      t !== null &&
        ((t = t.memoizedState), t !== null && ((t = t.dehydrated), t !== null)))
    )
      try {
        zu(t);
      } catch (l) {
        gt(e, e.return, l);
      }
  }
  function Gm(t) {
    switch (t.tag) {
      case 13:
      case 19:
        var e = t.stateNode;
        return (e === null && (e = t.stateNode = new bo()), e);
      case 22:
        return (
          (t = t.stateNode),
          (e = t._retryCache),
          e === null && (e = t._retryCache = new bo()),
          e
        );
      default:
        throw Error(f(435, t.tag));
    }
  }
  function qc(t, e) {
    var l = Gm(t);
    e.forEach(function (a) {
      var u = Wm.bind(null, t, a);
      l.has(a) || (l.add(a), a.then(u, u));
    });
  }
  function ie(t, e) {
    var l = e.deletions;
    if (l !== null)
      for (var a = 0; a < l.length; a++) {
        var u = l[a],
          n = t,
          c = e,
          r = c;
        t: for (; r !== null; ) {
          switch (r.tag) {
            case 27:
              if (yl(r.type)) {
                ((Et = r.stateNode), (It = !1));
                break t;
              }
              break;
            case 5:
              ((Et = r.stateNode), (It = !1));
              break t;
            case 3:
            case 4:
              ((Et = r.stateNode.containerInfo), (It = !0));
              break t;
          }
          r = r.return;
        }
        if (Et === null) throw Error(f(160));
        (Ao(n, c, u),
          (Et = null),
          (It = !1),
          (n = u.alternate),
          n !== null && (n.return = null),
          (u.return = null));
      }
    if (e.subtreeFlags & 13878)
      for (e = e.child; e !== null; ) (Ro(e, t), (e = e.sibling));
  }
  var Te = null;
  function Ro(t, e) {
    var l = t.alternate,
      a = t.flags;
    switch (t.tag) {
      case 0:
      case 11:
      case 14:
      case 15:
        (ie(e, t),
          ce(t),
          a & 4 && (fl(3, t, t.return), ru(3, t), fl(5, t, t.return)));
        break;
      case 1:
        (ie(e, t),
          ce(t),
          a & 512 && (Dt || l === null || Me(l, l.return)),
          a & 64 &&
            Ze &&
            ((t = t.updateQueue),
            t !== null &&
              ((a = t.callbacks),
              a !== null &&
                ((l = t.shared.hiddenCallbacks),
                (t.shared.hiddenCallbacks = l === null ? a : l.concat(a))))));
        break;
      case 26:
        var u = Te;
        if (
          (ie(e, t),
          ce(t),
          a & 512 && (Dt || l === null || Me(l, l.return)),
          a & 4)
        ) {
          var n = l !== null ? l.memoizedState : null;
          if (((a = t.memoizedState), l === null))
            if (a === null)
              if (t.stateNode === null) {
                t: {
                  ((a = t.type),
                    (l = t.memoizedProps),
                    (u = u.ownerDocument || u));
                  e: switch (a) {
                    case "title":
                      ((n = u.getElementsByTagName("title")[0]),
                        (!n ||
                          n[Ha] ||
                          n[Zt] ||
                          n.namespaceURI === "http://www.w3.org/2000/svg" ||
                          n.hasAttribute("itemprop")) &&
                          ((n = u.createElement(a)),
                          u.head.insertBefore(
                            n,
                            u.querySelector("head > title"),
                          )),
                        Gt(n, a, l),
                        (n[Zt] = t),
                        jt(n),
                        (a = n));
                      break t;
                    case "link":
                      var c = Sd("link", "href", u).get(a + (l.href || ""));
                      if (c) {
                        for (var r = 0; r < c.length; r++)
                          if (
                            ((n = c[r]),
                            n.getAttribute("href") ===
                              (l.href == null || l.href === ""
                                ? null
                                : l.href) &&
                              n.getAttribute("rel") ===
                                (l.rel == null ? null : l.rel) &&
                              n.getAttribute("title") ===
                                (l.title == null ? null : l.title) &&
                              n.getAttribute("crossorigin") ===
                                (l.crossOrigin == null ? null : l.crossOrigin))
                          ) {
                            c.splice(r, 1);
                            break e;
                          }
                      }
                      ((n = u.createElement(a)),
                        Gt(n, a, l),
                        u.head.appendChild(n));
                      break;
                    case "meta":
                      if (
                        (c = Sd("meta", "content", u).get(
                          a + (l.content || ""),
                        ))
                      ) {
                        for (r = 0; r < c.length; r++)
                          if (
                            ((n = c[r]),
                            n.getAttribute("content") ===
                              (l.content == null ? null : "" + l.content) &&
                              n.getAttribute("name") ===
                                (l.name == null ? null : l.name) &&
                              n.getAttribute("property") ===
                                (l.property == null ? null : l.property) &&
                              n.getAttribute("http-equiv") ===
                                (l.httpEquiv == null ? null : l.httpEquiv) &&
                              n.getAttribute("charset") ===
                                (l.charSet == null ? null : l.charSet))
                          ) {
                            c.splice(r, 1);
                            break e;
                          }
                      }
                      ((n = u.createElement(a)),
                        Gt(n, a, l),
                        u.head.appendChild(n));
                      break;
                    default:
                      throw Error(f(468, a));
                  }
                  ((n[Zt] = t), jt(n), (a = n));
                }
                t.stateNode = a;
              } else bd(u, t.type, t.stateNode);
            else t.stateNode = pd(u, a, t.memoizedProps);
          else
            n !== a
              ? (n === null
                  ? l.stateNode !== null &&
                    ((l = l.stateNode), l.parentNode.removeChild(l))
                  : n.count--,
                a === null
                  ? bd(u, t.type, t.stateNode)
                  : pd(u, a, t.memoizedProps))
              : a === null &&
                t.stateNode !== null &&
                Cc(t, t.memoizedProps, l.memoizedProps);
        }
        break;
      case 27:
        (ie(e, t),
          ce(t),
          a & 512 && (Dt || l === null || Me(l, l.return)),
          l !== null && a & 4 && Cc(t, t.memoizedProps, l.memoizedProps));
        break;
      case 5:
        if (
          (ie(e, t),
          ce(t),
          a & 512 && (Dt || l === null || Me(l, l.return)),
          t.flags & 32)
        ) {
          u = t.stateNode;
          try {
            Wl(u, "");
          } catch (R) {
            gt(t, t.return, R);
          }
        }
        (a & 4 &&
          t.stateNode != null &&
          ((u = t.memoizedProps), Cc(t, u, l !== null ? l.memoizedProps : u)),
          a & 1024 && (Bc = !0));
        break;
      case 6:
        if ((ie(e, t), ce(t), a & 4)) {
          if (t.stateNode === null) throw Error(f(162));
          ((a = t.memoizedProps), (l = t.stateNode));
          try {
            l.nodeValue = a;
          } catch (R) {
            gt(t, t.return, R);
          }
        }
        break;
      case 3:
        if (
          ((Qn = null),
          (u = Te),
          (Te = Gn(e.containerInfo)),
          ie(e, t),
          (Te = u),
          ce(t),
          a & 4 && l !== null && l.memoizedState.isDehydrated)
        )
          try {
            zu(e.containerInfo);
          } catch (R) {
            gt(t, t.return, R);
          }
        Bc && ((Bc = !1), Oo(t));
        break;
      case 4:
        ((a = Te),
          (Te = Gn(t.stateNode.containerInfo)),
          ie(e, t),
          ce(t),
          (Te = a));
        break;
      case 12:
        (ie(e, t), ce(t));
        break;
      case 13:
        (ie(e, t),
          ce(t),
          t.child.flags & 8192 &&
            (t.memoizedState !== null) !=
              (l !== null && l.memoizedState !== null) &&
            (Qc = Re()),
          a & 4 &&
            ((a = t.updateQueue),
            a !== null && ((t.updateQueue = null), qc(t, a))));
        break;
      case 22:
        u = t.memoizedState !== null;
        var m = l !== null && l.memoizedState !== null,
          T = Ze,
          _ = Dt;
        if (
          ((Ze = T || u),
          (Dt = _ || m),
          ie(e, t),
          (Dt = _),
          (Ze = T),
          ce(t),
          a & 8192)
        )
          t: for (
            e = t.stateNode,
              e._visibility = u ? e._visibility & -2 : e._visibility | 1,
              u && (l === null || m || Ze || Dt || wl(t)),
              l = null,
              e = t;
            ;

          ) {
            if (e.tag === 5 || e.tag === 26) {
              if (l === null) {
                m = l = e;
                try {
                  if (((n = m.stateNode), u))
                    ((c = n.style),
                      typeof c.setProperty == "function"
                        ? c.setProperty("display", "none", "important")
                        : (c.display = "none"));
                  else {
                    r = m.stateNode;
                    var H = m.memoizedProps.style,
                      x =
                        H != null && H.hasOwnProperty("display")
                          ? H.display
                          : null;
                    r.style.display =
                      x == null || typeof x == "boolean" ? "" : ("" + x).trim();
                  }
                } catch (R) {
                  gt(m, m.return, R);
                }
              }
            } else if (e.tag === 6) {
              if (l === null) {
                m = e;
                try {
                  m.stateNode.nodeValue = u ? "" : m.memoizedProps;
                } catch (R) {
                  gt(m, m.return, R);
                }
              }
            } else if (
              ((e.tag !== 22 && e.tag !== 23) ||
                e.memoizedState === null ||
                e === t) &&
              e.child !== null
            ) {
              ((e.child.return = e), (e = e.child));
              continue;
            }
            if (e === t) break t;
            for (; e.sibling === null; ) {
              if (e.return === null || e.return === t) break t;
              (l === e && (l = null), (e = e.return));
            }
            (l === e && (l = null),
              (e.sibling.return = e.return),
              (e = e.sibling));
          }
        a & 4 &&
          ((a = t.updateQueue),
          a !== null &&
            ((l = a.retryQueue),
            l !== null && ((a.retryQueue = null), qc(t, l))));
        break;
      case 19:
        (ie(e, t),
          ce(t),
          a & 4 &&
            ((a = t.updateQueue),
            a !== null && ((t.updateQueue = null), qc(t, a))));
        break;
      case 30:
        break;
      case 21:
        break;
      default:
        (ie(e, t), ce(t));
    }
  }
  function ce(t) {
    var e = t.flags;
    if (e & 2) {
      try {
        for (var l, a = t.return; a !== null; ) {
          if (po(a)) {
            l = a;
            break;
          }
          a = a.return;
        }
        if (l == null) throw Error(f(160));
        switch (l.tag) {
          case 27:
            var u = l.stateNode,
              n = Hc(t);
            Mn(t, n, u);
            break;
          case 5:
            var c = l.stateNode;
            l.flags & 32 && (Wl(c, ""), (l.flags &= -33));
            var r = Hc(t);
            Mn(t, r, c);
            break;
          case 3:
          case 4:
            var m = l.stateNode.containerInfo,
              T = Hc(t);
            jc(t, T, m);
            break;
          default:
            throw Error(f(161));
        }
      } catch (_) {
        gt(t, t.return, _);
      }
      t.flags &= -3;
    }
    e & 4096 && (t.flags &= -4097);
  }
  function Oo(t) {
    if (t.subtreeFlags & 1024)
      for (t = t.child; t !== null; ) {
        var e = t;
        (Oo(e),
          e.tag === 5 && e.flags & 1024 && e.stateNode.reset(),
          (t = t.sibling));
      }
  }
  function rl(t, e) {
    if (e.subtreeFlags & 8772)
      for (e = e.child; e !== null; ) (Eo(t, e.alternate, e), (e = e.sibling));
  }
  function wl(t) {
    for (t = t.child; t !== null; ) {
      var e = t;
      switch (e.tag) {
        case 0:
        case 11:
        case 14:
        case 15:
          (fl(4, e, e.return), wl(e));
          break;
        case 1:
          Me(e, e.return);
          var l = e.stateNode;
          (typeof l.componentWillUnmount == "function" && yo(e, e.return, l),
            wl(e));
          break;
        case 27:
          Su(e.stateNode);
        case 26:
        case 5:
          (Me(e, e.return), wl(e));
          break;
        case 22:
          e.memoizedState === null && wl(e);
          break;
        case 30:
          wl(e);
          break;
        default:
          wl(e);
      }
      t = t.sibling;
    }
  }
  function sl(t, e, l) {
    for (l = l && (e.subtreeFlags & 8772) !== 0, e = e.child; e !== null; ) {
      var a = e.alternate,
        u = t,
        n = e,
        c = n.flags;
      switch (n.tag) {
        case 0:
        case 11:
        case 15:
          (sl(u, n, l), ru(4, n));
          break;
        case 1:
          if (
            (sl(u, n, l),
            (a = n),
            (u = a.stateNode),
            typeof u.componentDidMount == "function")
          )
            try {
              u.componentDidMount();
            } catch (T) {
              gt(a, a.return, T);
            }
          if (((a = n), (u = a.updateQueue), u !== null)) {
            var r = a.stateNode;
            try {
              var m = u.shared.hiddenCallbacks;
              if (m !== null)
                for (u.shared.hiddenCallbacks = null, u = 0; u < m.length; u++)
                  ts(m[u], r);
            } catch (T) {
              gt(a, a.return, T);
            }
          }
          (l && c & 64 && vo(n), su(n, n.return));
          break;
        case 27:
          So(n);
        case 26:
        case 5:
          (sl(u, n, l), l && a === null && c & 4 && go(n), su(n, n.return));
          break;
        case 12:
          sl(u, n, l);
          break;
        case 13:
          (sl(u, n, l), l && c & 4 && xo(u, n));
          break;
        case 22:
          (n.memoizedState === null && sl(u, n, l), su(n, n.return));
          break;
        case 30:
          break;
        default:
          sl(u, n, l);
      }
      e = e.sibling;
    }
  }
  function Yc(t, e) {
    var l = null;
    (t !== null &&
      t.memoizedState !== null &&
      t.memoizedState.cachePool !== null &&
      (l = t.memoizedState.cachePool.pool),
      (t = null),
      e.memoizedState !== null &&
        e.memoizedState.cachePool !== null &&
        (t = e.memoizedState.cachePool.pool),
      t !== l && (t != null && t.refCount++, l != null && ka(l)));
  }
  function wc(t, e) {
    ((t = null),
      e.alternate !== null && (t = e.alternate.memoizedState.cache),
      (e = e.memoizedState.cache),
      e !== t && (e.refCount++, t != null && ka(t)));
  }
  function De(t, e, l, a) {
    if (e.subtreeFlags & 10256)
      for (e = e.child; e !== null; ) (zo(t, e, l, a), (e = e.sibling));
  }
  function zo(t, e, l, a) {
    var u = e.flags;
    switch (e.tag) {
      case 0:
      case 11:
      case 15:
        (De(t, e, l, a), u & 2048 && ru(9, e));
        break;
      case 1:
        De(t, e, l, a);
        break;
      case 3:
        (De(t, e, l, a),
          u & 2048 &&
            ((t = null),
            e.alternate !== null && (t = e.alternate.memoizedState.cache),
            (e = e.memoizedState.cache),
            e !== t && (e.refCount++, t != null && ka(t))));
        break;
      case 12:
        if (u & 2048) {
          (De(t, e, l, a), (t = e.stateNode));
          try {
            var n = e.memoizedProps,
              c = n.id,
              r = n.onPostCommit;
            typeof r == "function" &&
              r(
                c,
                e.alternate === null ? "mount" : "update",
                t.passiveEffectDuration,
                -0,
              );
          } catch (m) {
            gt(e, e.return, m);
          }
        } else De(t, e, l, a);
        break;
      case 13:
        De(t, e, l, a);
        break;
      case 23:
        break;
      case 22:
        ((n = e.stateNode),
          (c = e.alternate),
          e.memoizedState !== null
            ? n._visibility & 2
              ? De(t, e, l, a)
              : ou(t, e)
            : n._visibility & 2
              ? De(t, e, l, a)
              : ((n._visibility |= 2),
                va(t, e, l, a, (e.subtreeFlags & 10256) !== 0)),
          u & 2048 && Yc(c, e));
        break;
      case 24:
        (De(t, e, l, a), u & 2048 && wc(e.alternate, e));
        break;
      default:
        De(t, e, l, a);
    }
  }
  function va(t, e, l, a, u) {
    for (u = u && (e.subtreeFlags & 10256) !== 0, e = e.child; e !== null; ) {
      var n = t,
        c = e,
        r = l,
        m = a,
        T = c.flags;
      switch (c.tag) {
        case 0:
        case 11:
        case 15:
          (va(n, c, r, m, u), ru(8, c));
          break;
        case 23:
          break;
        case 22:
          var _ = c.stateNode;
          (c.memoizedState !== null
            ? _._visibility & 2
              ? va(n, c, r, m, u)
              : ou(n, c)
            : ((_._visibility |= 2), va(n, c, r, m, u)),
            u && T & 2048 && Yc(c.alternate, c));
          break;
        case 24:
          (va(n, c, r, m, u), u && T & 2048 && wc(c.alternate, c));
          break;
        default:
          va(n, c, r, m, u);
      }
      e = e.sibling;
    }
  }
  function ou(t, e) {
    if (e.subtreeFlags & 10256)
      for (e = e.child; e !== null; ) {
        var l = t,
          a = e,
          u = a.flags;
        switch (a.tag) {
          case 22:
            (ou(l, a), u & 2048 && Yc(a.alternate, a));
            break;
          case 24:
            (ou(l, a), u & 2048 && wc(a.alternate, a));
            break;
          default:
            ou(l, a);
        }
        e = e.sibling;
      }
  }
  var du = 8192;
  function ya(t) {
    if (t.subtreeFlags & du)
      for (t = t.child; t !== null; ) (Mo(t), (t = t.sibling));
  }
  function Mo(t) {
    switch (t.tag) {
      case 26:
        (ya(t),
          t.flags & du &&
            t.memoizedState !== null &&
            O0(Te, t.memoizedState, t.memoizedProps));
        break;
      case 5:
        ya(t);
        break;
      case 3:
      case 4:
        var e = Te;
        ((Te = Gn(t.stateNode.containerInfo)), ya(t), (Te = e));
        break;
      case 22:
        t.memoizedState === null &&
          ((e = t.alternate),
          e !== null && e.memoizedState !== null
            ? ((e = du), (du = 16777216), ya(t), (du = e))
            : ya(t));
        break;
      default:
        ya(t);
    }
  }
  function Do(t) {
    var e = t.alternate;
    if (e !== null && ((t = e.child), t !== null)) {
      e.child = null;
      do ((e = t.sibling), (t.sibling = null), (t = e));
      while (t !== null);
    }
  }
  function hu(t) {
    var e = t.deletions;
    if ((t.flags & 16) !== 0) {
      if (e !== null)
        for (var l = 0; l < e.length; l++) {
          var a = e[l];
          ((qt = a), No(a, t));
        }
      Do(t);
    }
    if (t.subtreeFlags & 10256)
      for (t = t.child; t !== null; ) (_o(t), (t = t.sibling));
  }
  function _o(t) {
    switch (t.tag) {
      case 0:
      case 11:
      case 15:
        (hu(t), t.flags & 2048 && fl(9, t, t.return));
        break;
      case 3:
        hu(t);
        break;
      case 12:
        hu(t);
        break;
      case 22:
        var e = t.stateNode;
        t.memoizedState !== null &&
        e._visibility & 2 &&
        (t.return === null || t.return.tag !== 13)
          ? ((e._visibility &= -3), Dn(t))
          : hu(t);
        break;
      default:
        hu(t);
    }
  }
  function Dn(t) {
    var e = t.deletions;
    if ((t.flags & 16) !== 0) {
      if (e !== null)
        for (var l = 0; l < e.length; l++) {
          var a = e[l];
          ((qt = a), No(a, t));
        }
      Do(t);
    }
    for (t = t.child; t !== null; ) {
      switch (((e = t), e.tag)) {
        case 0:
        case 11:
        case 15:
          (fl(8, e, e.return), Dn(e));
          break;
        case 22:
          ((l = e.stateNode),
            l._visibility & 2 && ((l._visibility &= -3), Dn(e)));
          break;
        default:
          Dn(e);
      }
      t = t.sibling;
    }
  }
  function No(t, e) {
    for (; qt !== null; ) {
      var l = qt;
      switch (l.tag) {
        case 0:
        case 11:
        case 15:
          fl(8, l, e);
          break;
        case 23:
        case 22:
          if (l.memoizedState !== null && l.memoizedState.cachePool !== null) {
            var a = l.memoizedState.cachePool.pool;
            a != null && a.refCount++;
          }
          break;
        case 24:
          ka(l.memoizedState.cache);
      }
      if (((a = l.child), a !== null)) ((a.return = l), (qt = a));
      else
        t: for (l = t; qt !== null; ) {
          a = qt;
          var u = a.sibling,
            n = a.return;
          if ((To(a), a === l)) {
            qt = null;
            break t;
          }
          if (u !== null) {
            ((u.return = n), (qt = u));
            break t;
          }
          qt = n;
        }
    }
  }
  var Xm = {
      getCacheForType: function (t) {
        var e = Vt(Ct),
          l = e.data.get(t);
        return (l === void 0 && ((l = t()), e.data.set(t, l)), l);
      },
    },
    Qm = typeof WeakMap == "function" ? WeakMap : Map,
    ot = 0,
    pt = null,
    ut = null,
    it = 0,
    dt = 0,
    fe = null,
    ol = !1,
    ga = !1,
    Lc = !1,
    Ke = 0,
    Ot = 0,
    dl = 0,
    Ll = 0,
    Gc = 0,
    Se = 0,
    pa = 0,
    mu = null,
    te = null,
    Xc = !1,
    Qc = 0,
    _n = 1 / 0,
    Nn = null,
    hl = null,
    Lt = 0,
    ml = null,
    Sa = null,
    ba = 0,
    Zc = 0,
    Vc = null,
    Uo = null,
    vu = 0,
    Kc = null;
  function re() {
    if ((ot & 2) !== 0 && it !== 0) return it & -it;
    if (N.T !== null) {
      var t = ca;
      return t !== 0 ? t : Ic();
    }
    return Jf();
  }
  function Co() {
    Se === 0 && (Se = (it & 536870912) === 0 || st ? Qf() : 536870912);
    var t = pe.current;
    return (t !== null && (t.flags |= 32), Se);
  }
  function se(t, e, l) {
    (((t === pt && (dt === 2 || dt === 9)) || t.cancelPendingCommit !== null) &&
      (Ea(t, 0), vl(t, it, Se, !1)),
      Ca(t, l),
      ((ot & 2) === 0 || t !== pt) &&
        (t === pt &&
          ((ot & 2) === 0 && (Ll |= l), Ot === 4 && vl(t, it, Se, !1)),
        _e(t)));
  }
  function Ho(t, e, l) {
    if ((ot & 6) !== 0) throw Error(f(327));
    var a = (!l && (e & 124) === 0 && (e & t.expiredLanes) === 0) || Ua(t, e),
      u = a ? Km(t, e) : kc(t, e, !0),
      n = a;
    do {
      if (u === 0) {
        ga && !a && vl(t, e, 0, !1);
        break;
      } else {
        if (((l = t.current.alternate), n && !Zm(l))) {
          ((u = kc(t, e, !1)), (n = !1));
          continue;
        }
        if (u === 2) {
          if (((n = e), t.errorRecoveryDisabledLanes & n)) var c = 0;
          else
            ((c = t.pendingLanes & -536870913),
              (c = c !== 0 ? c : c & 536870912 ? 536870912 : 0));
          if (c !== 0) {
            e = c;
            t: {
              var r = t;
              u = mu;
              var m = r.current.memoizedState.isDehydrated;
              if ((m && (Ea(r, c).flags |= 256), (c = kc(r, c, !1)), c !== 2)) {
                if (Lc && !m) {
                  ((r.errorRecoveryDisabledLanes |= n), (Ll |= n), (u = 4));
                  break t;
                }
                ((n = te),
                  (te = u),
                  n !== null &&
                    (te === null ? (te = n) : te.push.apply(te, n)));
              }
              u = c;
            }
            if (((n = !1), u !== 2)) continue;
          }
        }
        if (u === 1) {
          (Ea(t, 0), vl(t, e, 0, !0));
          break;
        }
        t: {
          switch (((a = t), (n = u), n)) {
            case 0:
            case 1:
              throw Error(f(345));
            case 4:
              if ((e & 4194048) !== e) break;
            case 6:
              vl(a, e, Se, !ol);
              break t;
            case 2:
              te = null;
              break;
            case 3:
            case 5:
              break;
            default:
              throw Error(f(329));
          }
          if ((e & 62914560) === e && ((u = Qc + 300 - Re()), 10 < u)) {
            if ((vl(a, e, Se, !ol), Gu(a, 0, !0) !== 0)) break t;
            a.timeoutHandle = rd(
              jo.bind(null, a, l, te, Nn, Xc, e, Se, Ll, pa, ol, n, 2, -0, 0),
              u,
            );
            break t;
          }
          jo(a, l, te, Nn, Xc, e, Se, Ll, pa, ol, n, 0, -0, 0);
        }
      }
      break;
    } while (!0);
    _e(t);
  }
  function jo(t, e, l, a, u, n, c, r, m, T, _, H, x, R) {
    if (
      ((t.timeoutHandle = -1),
      (H = e.subtreeFlags),
      (H & 8192 || (H & 16785408) === 16785408) &&
        ((Tu = { stylesheets: null, count: 0, unsuspend: R0 }),
        Mo(e),
        (H = z0()),
        H !== null))
    ) {
      ((t.cancelPendingCommit = H(
        Xo.bind(null, t, e, n, l, a, u, c, r, m, _, 1, x, R),
      )),
        vl(t, n, c, !T));
      return;
    }
    Xo(t, e, n, l, a, u, c, r, m);
  }
  function Zm(t) {
    for (var e = t; ; ) {
      var l = e.tag;
      if (
        (l === 0 || l === 11 || l === 15) &&
        e.flags & 16384 &&
        ((l = e.updateQueue), l !== null && ((l = l.stores), l !== null))
      )
        for (var a = 0; a < l.length; a++) {
          var u = l[a],
            n = u.getSnapshot;
          u = u.value;
          try {
            if (!ue(n(), u)) return !1;
          } catch {
            return !1;
          }
        }
      if (((l = e.child), e.subtreeFlags & 16384 && l !== null))
        ((l.return = e), (e = l));
      else {
        if (e === t) break;
        for (; e.sibling === null; ) {
          if (e.return === null || e.return === t) return !0;
          e = e.return;
        }
        ((e.sibling.return = e.return), (e = e.sibling));
      }
    }
    return !0;
  }
  function vl(t, e, l, a) {
    ((e &= ~Gc),
      (e &= ~Ll),
      (t.suspendedLanes |= e),
      (t.pingedLanes &= ~e),
      a && (t.warmLanes |= e),
      (a = t.expirationTimes));
    for (var u = e; 0 < u; ) {
      var n = 31 - ae(u),
        c = 1 << n;
      ((a[n] = -1), (u &= ~c));
    }
    l !== 0 && Vf(t, l, e);
  }
  function Un() {
    return (ot & 6) === 0 ? (yu(0), !1) : !0;
  }
  function Jc() {
    if (ut !== null) {
      if (dt === 0) var t = ut.return;
      else ((t = ut), (Ye = Hl = null), sc(t), (ha = null), (iu = 0), (t = ut));
      for (; t !== null; ) (mo(t.alternate, t), (t = t.return));
      ut = null;
    }
  }
  function Ea(t, e) {
    var l = t.timeoutHandle;
    (l !== -1 && ((t.timeoutHandle = -1), f0(l)),
      (l = t.cancelPendingCommit),
      l !== null && ((t.cancelPendingCommit = null), l()),
      Jc(),
      (pt = t),
      (ut = l = je(t.current, null)),
      (it = e),
      (dt = 0),
      (fe = null),
      (ol = !1),
      (ga = Ua(t, e)),
      (Lc = !1),
      (pa = Se = Gc = Ll = dl = Ot = 0),
      (te = mu = null),
      (Xc = !1),
      (e & 8) !== 0 && (e |= e & 32));
    var a = t.entangledLanes;
    if (a !== 0)
      for (t = t.entanglements, a &= e; 0 < a; ) {
        var u = 31 - ae(a),
          n = 1 << u;
        ((e |= t[u]), (a &= ~n));
      }
    return ((Ke = e), Iu(), l);
  }
  function Bo(t, e) {
    ((lt = null),
      (N.H = Sn),
      e === Fa || e === rn
        ? ((e = Pr()), (dt = 3))
        : e === kr
          ? ((e = Pr()), (dt = 4))
          : (dt =
              e === Is
                ? 8
                : e !== null &&
                    typeof e == "object" &&
                    typeof e.then == "function"
                  ? 6
                  : 1),
      (fe = e),
      ut === null && ((Ot = 1), xn(t, me(e, t.current))));
  }
  function qo() {
    var t = N.H;
    return ((N.H = Sn), t === null ? Sn : t);
  }
  function Yo() {
    var t = N.A;
    return ((N.A = Xm), t);
  }
  function $c() {
    ((Ot = 4),
      ol || ((it & 4194048) !== it && pe.current !== null) || (ga = !0),
      ((dl & 134217727) === 0 && (Ll & 134217727) === 0) ||
        pt === null ||
        vl(pt, it, Se, !1));
  }
  function kc(t, e, l) {
    var a = ot;
    ot |= 2;
    var u = qo(),
      n = Yo();
    ((pt !== t || it !== e) && ((Nn = null), Ea(t, e)), (e = !1));
    var c = Ot;
    t: do
      try {
        if (dt !== 0 && ut !== null) {
          var r = ut,
            m = fe;
          switch (dt) {
            case 8:
              (Jc(), (c = 6));
              break t;
            case 3:
            case 2:
            case 9:
            case 6:
              pe.current === null && (e = !0);
              var T = dt;
              if (((dt = 0), (fe = null), Ta(t, r, m, T), l && ga)) {
                c = 0;
                break t;
              }
              break;
            default:
              ((T = dt), (dt = 0), (fe = null), Ta(t, r, m, T));
          }
        }
        (Vm(), (c = Ot));
        break;
      } catch (_) {
        Bo(t, _);
      }
    while (!0);
    return (
      e && t.shellSuspendCounter++,
      (Ye = Hl = null),
      (ot = a),
      (N.H = u),
      (N.A = n),
      ut === null && ((pt = null), (it = 0), Iu()),
      c
    );
  }
  function Vm() {
    for (; ut !== null; ) wo(ut);
  }
  function Km(t, e) {
    var l = ot;
    ot |= 2;
    var a = qo(),
      u = Yo();
    pt !== t || it !== e
      ? ((Nn = null), (_n = Re() + 500), Ea(t, e))
      : (ga = Ua(t, e));
    t: do
      try {
        if (dt !== 0 && ut !== null) {
          e = ut;
          var n = fe;
          e: switch (dt) {
            case 1:
              ((dt = 0), (fe = null), Ta(t, e, n, 1));
              break;
            case 2:
            case 9:
              if (Wr(n)) {
                ((dt = 0), (fe = null), Lo(e));
                break;
              }
              ((e = function () {
                ((dt !== 2 && dt !== 9) || pt !== t || (dt = 7), _e(t));
              }),
                n.then(e, e));
              break t;
            case 3:
              dt = 7;
              break t;
            case 4:
              dt = 5;
              break t;
            case 7:
              Wr(n)
                ? ((dt = 0), (fe = null), Lo(e))
                : ((dt = 0), (fe = null), Ta(t, e, n, 7));
              break;
            case 5:
              var c = null;
              switch (ut.tag) {
                case 26:
                  c = ut.memoizedState;
                case 5:
                case 27:
                  var r = ut;
                  if (!c || Ed(c)) {
                    ((dt = 0), (fe = null));
                    var m = r.sibling;
                    if (m !== null) ut = m;
                    else {
                      var T = r.return;
                      T !== null ? ((ut = T), Cn(T)) : (ut = null);
                    }
                    break e;
                  }
              }
              ((dt = 0), (fe = null), Ta(t, e, n, 5));
              break;
            case 6:
              ((dt = 0), (fe = null), Ta(t, e, n, 6));
              break;
            case 8:
              (Jc(), (Ot = 6));
              break t;
            default:
              throw Error(f(462));
          }
        }
        Jm();
        break;
      } catch (_) {
        Bo(t, _);
      }
    while (!0);
    return (
      (Ye = Hl = null),
      (N.H = a),
      (N.A = u),
      (ot = l),
      ut !== null ? 0 : ((pt = null), (it = 0), Iu(), Ot)
    );
  }
  function Jm() {
    for (; ut !== null && !vh(); ) wo(ut);
  }
  function wo(t) {
    var e = oo(t.alternate, t, Ke);
    ((t.memoizedProps = t.pendingProps), e === null ? Cn(t) : (ut = e));
  }
  function Lo(t) {
    var e = t,
      l = e.alternate;
    switch (e.tag) {
      case 15:
      case 0:
        e = no(l, e, e.pendingProps, e.type, void 0, it);
        break;
      case 11:
        e = no(l, e, e.pendingProps, e.type.render, e.ref, it);
        break;
      case 5:
        sc(e);
      default:
        (mo(l, e), (e = ut = Lr(e, Ke)), (e = oo(l, e, Ke)));
    }
    ((t.memoizedProps = t.pendingProps), e === null ? Cn(t) : (ut = e));
  }
  function Ta(t, e, l, a) {
    ((Ye = Hl = null), sc(e), (ha = null), (iu = 0));
    var u = e.return;
    try {
      if (Bm(t, u, e, l, it)) {
        ((Ot = 1), xn(t, me(l, t.current)), (ut = null));
        return;
      }
    } catch (n) {
      if (u !== null) throw ((ut = u), n);
      ((Ot = 1), xn(t, me(l, t.current)), (ut = null));
      return;
    }
    e.flags & 32768
      ? (st || a === 1
          ? (t = !0)
          : ga || (it & 536870912) !== 0
            ? (t = !1)
            : ((ol = t = !0),
              (a === 2 || a === 9 || a === 3 || a === 6) &&
                ((a = pe.current),
                a !== null && a.tag === 13 && (a.flags |= 16384))),
        Go(e, t))
      : Cn(e);
  }
  function Cn(t) {
    var e = t;
    do {
      if ((e.flags & 32768) !== 0) {
        Go(e, ol);
        return;
      }
      t = e.return;
      var l = Ym(e.alternate, e, Ke);
      if (l !== null) {
        ut = l;
        return;
      }
      if (((e = e.sibling), e !== null)) {
        ut = e;
        return;
      }
      ut = e = t;
    } while (e !== null);
    Ot === 0 && (Ot = 5);
  }
  function Go(t, e) {
    do {
      var l = wm(t.alternate, t);
      if (l !== null) {
        ((l.flags &= 32767), (ut = l));
        return;
      }
      if (
        ((l = t.return),
        l !== null &&
          ((l.flags |= 32768), (l.subtreeFlags = 0), (l.deletions = null)),
        !e && ((t = t.sibling), t !== null))
      ) {
        ut = t;
        return;
      }
      ut = t = l;
    } while (t !== null);
    ((Ot = 6), (ut = null));
  }
  function Xo(t, e, l, a, u, n, c, r, m) {
    t.cancelPendingCommit = null;
    do Hn();
    while (Lt !== 0);
    if ((ot & 6) !== 0) throw Error(f(327));
    if (e !== null) {
      if (e === t.current) throw Error(f(177));
      if (
        ((n = e.lanes | e.childLanes),
        (n |= wi),
        Rh(t, l, n, c, r, m),
        t === pt && ((ut = pt = null), (it = 0)),
        (Sa = e),
        (ml = t),
        (ba = l),
        (Zc = n),
        (Vc = u),
        (Uo = a),
        (e.subtreeFlags & 10256) !== 0 || (e.flags & 10256) !== 0
          ? ((t.callbackNode = null),
            (t.callbackPriority = 0),
            Fm(Yu, function () {
              return (Jo(), null);
            }))
          : ((t.callbackNode = null), (t.callbackPriority = 0)),
        (a = (e.flags & 13878) !== 0),
        (e.subtreeFlags & 13878) !== 0 || a)
      ) {
        ((a = N.T), (N.T = null), (u = L.p), (L.p = 2), (c = ot), (ot |= 4));
        try {
          Lm(t, e, l);
        } finally {
          ((ot = c), (L.p = u), (N.T = a));
        }
      }
      ((Lt = 1), Qo(), Zo(), Vo());
    }
  }
  function Qo() {
    if (Lt === 1) {
      Lt = 0;
      var t = ml,
        e = Sa,
        l = (e.flags & 13878) !== 0;
      if ((e.subtreeFlags & 13878) !== 0 || l) {
        ((l = N.T), (N.T = null));
        var a = L.p;
        L.p = 2;
        var u = ot;
        ot |= 4;
        try {
          Ro(e, t);
          var n = ff,
            c = _r(t.containerInfo),
            r = n.focusedElem,
            m = n.selectionRange;
          if (
            c !== r &&
            r &&
            r.ownerDocument &&
            Dr(r.ownerDocument.documentElement, r)
          ) {
            if (m !== null && Hi(r)) {
              var T = m.start,
                _ = m.end;
              if ((_ === void 0 && (_ = T), "selectionStart" in r))
                ((r.selectionStart = T),
                  (r.selectionEnd = Math.min(_, r.value.length)));
              else {
                var H = r.ownerDocument || document,
                  x = (H && H.defaultView) || window;
                if (x.getSelection) {
                  var R = x.getSelection(),
                    F = r.textContent.length,
                    $ = Math.min(m.start, F),
                    vt = m.end === void 0 ? $ : Math.min(m.end, F);
                  !R.extend && $ > vt && ((c = vt), (vt = $), ($ = c));
                  var S = Mr(r, $),
                    g = Mr(r, vt);
                  if (
                    S &&
                    g &&
                    (R.rangeCount !== 1 ||
                      R.anchorNode !== S.node ||
                      R.anchorOffset !== S.offset ||
                      R.focusNode !== g.node ||
                      R.focusOffset !== g.offset)
                  ) {
                    var E = H.createRange();
                    (E.setStart(S.node, S.offset),
                      R.removeAllRanges(),
                      $ > vt
                        ? (R.addRange(E), R.extend(g.node, g.offset))
                        : (E.setEnd(g.node, g.offset), R.addRange(E)));
                  }
                }
              }
            }
            for (H = [], R = r; (R = R.parentNode); )
              R.nodeType === 1 &&
                H.push({ element: R, left: R.scrollLeft, top: R.scrollTop });
            for (
              typeof r.focus == "function" && r.focus(), r = 0;
              r < H.length;
              r++
            ) {
              var U = H[r];
              ((U.element.scrollLeft = U.left), (U.element.scrollTop = U.top));
            }
          }
          ((Kn = !!cf), (ff = cf = null));
        } finally {
          ((ot = u), (L.p = a), (N.T = l));
        }
      }
      ((t.current = e), (Lt = 2));
    }
  }
  function Zo() {
    if (Lt === 2) {
      Lt = 0;
      var t = ml,
        e = Sa,
        l = (e.flags & 8772) !== 0;
      if ((e.subtreeFlags & 8772) !== 0 || l) {
        ((l = N.T), (N.T = null));
        var a = L.p;
        L.p = 2;
        var u = ot;
        ot |= 4;
        try {
          Eo(t, e.alternate, e);
        } finally {
          ((ot = u), (L.p = a), (N.T = l));
        }
      }
      Lt = 3;
    }
  }
  function Vo() {
    if (Lt === 4 || Lt === 3) {
      ((Lt = 0), yh());
      var t = ml,
        e = Sa,
        l = ba,
        a = Uo;
      (e.subtreeFlags & 10256) !== 0 || (e.flags & 10256) !== 0
        ? (Lt = 5)
        : ((Lt = 0), (Sa = ml = null), Ko(t, t.pendingLanes));
      var u = t.pendingLanes;
      if (
        (u === 0 && (hl = null),
        hi(l),
        (e = e.stateNode),
        le && typeof le.onCommitFiberRoot == "function")
      )
        try {
          le.onCommitFiberRoot(Na, e, void 0, (e.current.flags & 128) === 128);
        } catch {}
      if (a !== null) {
        ((e = N.T), (u = L.p), (L.p = 2), (N.T = null));
        try {
          for (var n = t.onRecoverableError, c = 0; c < a.length; c++) {
            var r = a[c];
            n(r.value, { componentStack: r.stack });
          }
        } finally {
          ((N.T = e), (L.p = u));
        }
      }
      ((ba & 3) !== 0 && Hn(),
        _e(t),
        (u = t.pendingLanes),
        (l & 4194090) !== 0 && (u & 42) !== 0
          ? t === Kc
            ? vu++
            : ((vu = 0), (Kc = t))
          : (vu = 0),
        yu(0));
    }
  }
  function Ko(t, e) {
    (t.pooledCacheLanes &= e) === 0 &&
      ((e = t.pooledCache), e != null && ((t.pooledCache = null), ka(e)));
  }
  function Hn(t) {
    return (Qo(), Zo(), Vo(), Jo());
  }
  function Jo() {
    if (Lt !== 5) return !1;
    var t = ml,
      e = Zc;
    Zc = 0;
    var l = hi(ba),
      a = N.T,
      u = L.p;
    try {
      ((L.p = 32 > l ? 32 : l), (N.T = null), (l = Vc), (Vc = null));
      var n = ml,
        c = ba;
      if (((Lt = 0), (Sa = ml = null), (ba = 0), (ot & 6) !== 0))
        throw Error(f(331));
      var r = ot;
      if (
        ((ot |= 4),
        _o(n.current),
        zo(n, n.current, c, l),
        (ot = r),
        yu(0, !1),
        le && typeof le.onPostCommitFiberRoot == "function")
      )
        try {
          le.onPostCommitFiberRoot(Na, n);
        } catch {}
      return !0;
    } finally {
      ((L.p = u), (N.T = a), Ko(t, e));
    }
  }
  function $o(t, e, l) {
    ((e = me(l, e)),
      (e = xc(t.stateNode, e, 2)),
      (t = ul(t, e, 2)),
      t !== null && (Ca(t, 2), _e(t)));
  }
  function gt(t, e, l) {
    if (t.tag === 3) $o(t, t, l);
    else
      for (; e !== null; ) {
        if (e.tag === 3) {
          $o(e, t, l);
          break;
        } else if (e.tag === 1) {
          var a = e.stateNode;
          if (
            typeof e.type.getDerivedStateFromError == "function" ||
            (typeof a.componentDidCatch == "function" &&
              (hl === null || !hl.has(a)))
          ) {
            ((t = me(l, t)),
              (l = Fs(2)),
              (a = ul(e, l, 2)),
              a !== null && (Ps(l, a, e, t), Ca(a, 2), _e(a)));
            break;
          }
        }
        e = e.return;
      }
  }
  function Wc(t, e, l) {
    var a = t.pingCache;
    if (a === null) {
      a = t.pingCache = new Qm();
      var u = new Set();
      a.set(e, u);
    } else ((u = a.get(e)), u === void 0 && ((u = new Set()), a.set(e, u)));
    u.has(l) ||
      ((Lc = !0), u.add(l), (t = $m.bind(null, t, e, l)), e.then(t, t));
  }
  function $m(t, e, l) {
    var a = t.pingCache;
    (a !== null && a.delete(e),
      (t.pingedLanes |= t.suspendedLanes & l),
      (t.warmLanes &= ~l),
      pt === t &&
        (it & l) === l &&
        (Ot === 4 || (Ot === 3 && (it & 62914560) === it && 300 > Re() - Qc)
          ? (ot & 2) === 0 && Ea(t, 0)
          : (Gc |= l),
        pa === it && (pa = 0)),
      _e(t));
  }
  function ko(t, e) {
    (e === 0 && (e = Zf()), (t = aa(t, e)), t !== null && (Ca(t, e), _e(t)));
  }
  function km(t) {
    var e = t.memoizedState,
      l = 0;
    (e !== null && (l = e.retryLane), ko(t, l));
  }
  function Wm(t, e) {
    var l = 0;
    switch (t.tag) {
      case 13:
        var a = t.stateNode,
          u = t.memoizedState;
        u !== null && (l = u.retryLane);
        break;
      case 19:
        a = t.stateNode;
        break;
      case 22:
        a = t.stateNode._retryCache;
        break;
      default:
        throw Error(f(314));
    }
    (a !== null && a.delete(e), ko(t, l));
  }
  function Fm(t, e) {
    return ri(t, e);
  }
  var jn = null,
    Aa = null,
    Fc = !1,
    Bn = !1,
    Pc = !1,
    Gl = 0;
  function _e(t) {
    (t !== Aa &&
      t.next === null &&
      (Aa === null ? (jn = Aa = t) : (Aa = Aa.next = t)),
      (Bn = !0),
      Fc || ((Fc = !0), Im()));
  }
  function yu(t, e) {
    if (!Pc && Bn) {
      Pc = !0;
      do
        for (var l = !1, a = jn; a !== null; ) {
          if (t !== 0) {
            var u = a.pendingLanes;
            if (u === 0) var n = 0;
            else {
              var c = a.suspendedLanes,
                r = a.pingedLanes;
              ((n = (1 << (31 - ae(42 | t) + 1)) - 1),
                (n &= u & ~(c & ~r)),
                (n = n & 201326741 ? (n & 201326741) | 1 : n ? n | 2 : 0));
            }
            n !== 0 && ((l = !0), Io(a, n));
          } else
            ((n = it),
              (n = Gu(
                a,
                a === pt ? n : 0,
                a.cancelPendingCommit !== null || a.timeoutHandle !== -1,
              )),
              (n & 3) === 0 || Ua(a, n) || ((l = !0), Io(a, n)));
          a = a.next;
        }
      while (l);
      Pc = !1;
    }
  }
  function Pm() {
    Wo();
  }
  function Wo() {
    Bn = Fc = !1;
    var t = 0;
    Gl !== 0 && (c0() && (t = Gl), (Gl = 0));
    for (var e = Re(), l = null, a = jn; a !== null; ) {
      var u = a.next,
        n = Fo(a, e);
      (n === 0
        ? ((a.next = null),
          l === null ? (jn = u) : (l.next = u),
          u === null && (Aa = l))
        : ((l = a), (t !== 0 || (n & 3) !== 0) && (Bn = !0)),
        (a = u));
    }
    yu(t);
  }
  function Fo(t, e) {
    for (
      var l = t.suspendedLanes,
        a = t.pingedLanes,
        u = t.expirationTimes,
        n = t.pendingLanes & -62914561;
      0 < n;

    ) {
      var c = 31 - ae(n),
        r = 1 << c,
        m = u[c];
      (m === -1
        ? ((r & l) === 0 || (r & a) !== 0) && (u[c] = xh(r, e))
        : m <= e && (t.expiredLanes |= r),
        (n &= ~r));
    }
    if (
      ((e = pt),
      (l = it),
      (l = Gu(
        t,
        t === e ? l : 0,
        t.cancelPendingCommit !== null || t.timeoutHandle !== -1,
      )),
      (a = t.callbackNode),
      l === 0 ||
        (t === e && (dt === 2 || dt === 9)) ||
        t.cancelPendingCommit !== null)
    )
      return (
        a !== null && a !== null && si(a),
        (t.callbackNode = null),
        (t.callbackPriority = 0)
      );
    if ((l & 3) === 0 || Ua(t, l)) {
      if (((e = l & -l), e === t.callbackPriority)) return e;
      switch ((a !== null && si(a), hi(l))) {
        case 2:
        case 8:
          l = Gf;
          break;
        case 32:
          l = Yu;
          break;
        case 268435456:
          l = Xf;
          break;
        default:
          l = Yu;
      }
      return (
        (a = Po.bind(null, t)),
        (l = ri(l, a)),
        (t.callbackPriority = e),
        (t.callbackNode = l),
        e
      );
    }
    return (
      a !== null && a !== null && si(a),
      (t.callbackPriority = 2),
      (t.callbackNode = null),
      2
    );
  }
  function Po(t, e) {
    if (Lt !== 0 && Lt !== 5)
      return ((t.callbackNode = null), (t.callbackPriority = 0), null);
    var l = t.callbackNode;
    if (Hn() && t.callbackNode !== l) return null;
    var a = it;
    return (
      (a = Gu(
        t,
        t === pt ? a : 0,
        t.cancelPendingCommit !== null || t.timeoutHandle !== -1,
      )),
      a === 0
        ? null
        : (Ho(t, a, e),
          Fo(t, Re()),
          t.callbackNode != null && t.callbackNode === l
            ? Po.bind(null, t)
            : null)
    );
  }
  function Io(t, e) {
    if (Hn()) return null;
    Ho(t, e, !0);
  }
  function Im() {
    r0(function () {
      (ot & 6) !== 0 ? ri(Lf, Pm) : Wo();
    });
  }
  function Ic() {
    return (Gl === 0 && (Gl = Qf()), Gl);
  }
  function td(t) {
    return t == null || typeof t == "symbol" || typeof t == "boolean"
      ? null
      : typeof t == "function"
        ? t
        : Ku("" + t);
  }
  function ed(t, e) {
    var l = e.ownerDocument.createElement("input");
    return (
      (l.name = e.name),
      (l.value = e.value),
      t.id && l.setAttribute("form", t.id),
      e.parentNode.insertBefore(l, e),
      (t = new FormData(t)),
      l.parentNode.removeChild(l),
      t
    );
  }
  function t0(t, e, l, a, u) {
    if (e === "submit" && l && l.stateNode === u) {
      var n = td((u[Wt] || null).action),
        c = a.submitter;
      c &&
        ((e = (e = c[Wt] || null)
          ? td(e.formAction)
          : c.getAttribute("formAction")),
        e !== null && ((n = e), (c = null)));
      var r = new Wu("action", "action", null, a, u);
      t.push({
        event: r,
        listeners: [
          {
            instance: null,
            listener: function () {
              if (a.defaultPrevented) {
                if (Gl !== 0) {
                  var m = c ? ed(u, c) : new FormData(u);
                  Sc(
                    l,
                    { pending: !0, data: m, method: u.method, action: n },
                    null,
                    m,
                  );
                }
              } else
                typeof n == "function" &&
                  (r.preventDefault(),
                  (m = c ? ed(u, c) : new FormData(u)),
                  Sc(
                    l,
                    { pending: !0, data: m, method: u.method, action: n },
                    n,
                    m,
                  ));
            },
            currentTarget: u,
          },
        ],
      });
    }
  }
  for (var tf = 0; tf < Yi.length; tf++) {
    var ef = Yi[tf],
      e0 = ef.toLowerCase(),
      l0 = ef[0].toUpperCase() + ef.slice(1);
    Ee(e0, "on" + l0);
  }
  (Ee(Cr, "onAnimationEnd"),
    Ee(Hr, "onAnimationIteration"),
    Ee(jr, "onAnimationStart"),
    Ee("dblclick", "onDoubleClick"),
    Ee("focusin", "onFocus"),
    Ee("focusout", "onBlur"),
    Ee(Sm, "onTransitionRun"),
    Ee(bm, "onTransitionStart"),
    Ee(Em, "onTransitionCancel"),
    Ee(Br, "onTransitionEnd"),
    Jl("onMouseEnter", ["mouseout", "mouseover"]),
    Jl("onMouseLeave", ["mouseout", "mouseover"]),
    Jl("onPointerEnter", ["pointerout", "pointerover"]),
    Jl("onPointerLeave", ["pointerout", "pointerover"]),
    Rl(
      "onChange",
      "change click focusin focusout input keydown keyup selectionchange".split(
        " ",
      ),
    ),
    Rl(
      "onSelect",
      "focusout contextmenu dragend focusin keydown keyup mousedown mouseup selectionchange".split(
        " ",
      ),
    ),
    Rl("onBeforeInput", ["compositionend", "keypress", "textInput", "paste"]),
    Rl(
      "onCompositionEnd",
      "compositionend focusout keydown keypress keyup mousedown".split(" "),
    ),
    Rl(
      "onCompositionStart",
      "compositionstart focusout keydown keypress keyup mousedown".split(" "),
    ),
    Rl(
      "onCompositionUpdate",
      "compositionupdate focusout keydown keypress keyup mousedown".split(" "),
    ));
  var gu =
      "abort canplay canplaythrough durationchange emptied encrypted ended error loadeddata loadedmetadata loadstart pause play playing progress ratechange resize seeked seeking stalled suspend timeupdate volumechange waiting".split(
        " ",
      ),
    a0 = new Set(
      "beforetoggle cancel close invalid load scroll scrollend toggle"
        .split(" ")
        .concat(gu),
    );
  function ld(t, e) {
    e = (e & 4) !== 0;
    for (var l = 0; l < t.length; l++) {
      var a = t[l],
        u = a.event;
      a = a.listeners;
      t: {
        var n = void 0;
        if (e)
          for (var c = a.length - 1; 0 <= c; c--) {
            var r = a[c],
              m = r.instance,
              T = r.currentTarget;
            if (((r = r.listener), m !== n && u.isPropagationStopped()))
              break t;
            ((n = r), (u.currentTarget = T));
            try {
              n(u);
            } catch (_) {
              An(_);
            }
            ((u.currentTarget = null), (n = m));
          }
        else
          for (c = 0; c < a.length; c++) {
            if (
              ((r = a[c]),
              (m = r.instance),
              (T = r.currentTarget),
              (r = r.listener),
              m !== n && u.isPropagationStopped())
            )
              break t;
            ((n = r), (u.currentTarget = T));
            try {
              n(u);
            } catch (_) {
              An(_);
            }
            ((u.currentTarget = null), (n = m));
          }
      }
    }
  }
  function nt(t, e) {
    var l = e[mi];
    l === void 0 && (l = e[mi] = new Set());
    var a = t + "__bubble";
    l.has(a) || (ad(e, t, 2, !1), l.add(a));
  }
  function lf(t, e, l) {
    var a = 0;
    (e && (a |= 4), ad(l, t, a, e));
  }
  var qn = "_reactListening" + Math.random().toString(36).slice(2);
  function af(t) {
    if (!t[qn]) {
      ((t[qn] = !0),
        kf.forEach(function (l) {
          l !== "selectionchange" && (a0.has(l) || lf(l, !1, t), lf(l, !0, t));
        }));
      var e = t.nodeType === 9 ? t : t.ownerDocument;
      e === null || e[qn] || ((e[qn] = !0), lf("selectionchange", !1, e));
    }
  }
  function ad(t, e, l, a) {
    switch (zd(e)) {
      case 2:
        var u = _0;
        break;
      case 8:
        u = N0;
        break;
      default:
        u = pf;
    }
    ((l = u.bind(null, e, l, t)),
      (u = void 0),
      !Ri ||
        (e !== "touchstart" && e !== "touchmove" && e !== "wheel") ||
        (u = !0),
      a
        ? u !== void 0
          ? t.addEventListener(e, l, { capture: !0, passive: u })
          : t.addEventListener(e, l, !0)
        : u !== void 0
          ? t.addEventListener(e, l, { passive: u })
          : t.addEventListener(e, l, !1));
  }
  function uf(t, e, l, a, u) {
    var n = a;
    if ((e & 1) === 0 && (e & 2) === 0 && a !== null)
      t: for (;;) {
        if (a === null) return;
        var c = a.tag;
        if (c === 3 || c === 4) {
          var r = a.stateNode.containerInfo;
          if (r === u) break;
          if (c === 4)
            for (c = a.return; c !== null; ) {
              var m = c.tag;
              if ((m === 3 || m === 4) && c.stateNode.containerInfo === u)
                return;
              c = c.return;
            }
          for (; r !== null; ) {
            if (((c = Zl(r)), c === null)) return;
            if (((m = c.tag), m === 5 || m === 6 || m === 26 || m === 27)) {
              a = n = c;
              continue t;
            }
            r = r.parentNode;
          }
        }
        a = a.return;
      }
    rr(function () {
      var T = n,
        _ = Ai(l),
        H = [];
      t: {
        var x = qr.get(t);
        if (x !== void 0) {
          var R = Wu,
            F = t;
          switch (t) {
            case "keypress":
              if ($u(l) === 0) break t;
            case "keydown":
            case "keyup":
              R = Fh;
              break;
            case "focusin":
              ((F = "focus"), (R = Di));
              break;
            case "focusout":
              ((F = "blur"), (R = Di));
              break;
            case "beforeblur":
            case "afterblur":
              R = Di;
              break;
            case "click":
              if (l.button === 2) break t;
            case "auxclick":
            case "dblclick":
            case "mousedown":
            case "mousemove":
            case "mouseup":
            case "mouseout":
            case "mouseover":
            case "contextmenu":
              R = dr;
              break;
            case "drag":
            case "dragend":
            case "dragenter":
            case "dragexit":
            case "dragleave":
            case "dragover":
            case "dragstart":
            case "drop":
              R = wh;
              break;
            case "touchcancel":
            case "touchend":
            case "touchmove":
            case "touchstart":
              R = tm;
              break;
            case Cr:
            case Hr:
            case jr:
              R = Xh;
              break;
            case Br:
              R = lm;
              break;
            case "scroll":
            case "scrollend":
              R = qh;
              break;
            case "wheel":
              R = um;
              break;
            case "copy":
            case "cut":
            case "paste":
              R = Zh;
              break;
            case "gotpointercapture":
            case "lostpointercapture":
            case "pointercancel":
            case "pointerdown":
            case "pointermove":
            case "pointerout":
            case "pointerover":
            case "pointerup":
              R = mr;
              break;
            case "toggle":
            case "beforetoggle":
              R = im;
          }
          var $ = (e & 4) !== 0,
            vt = !$ && (t === "scroll" || t === "scrollend"),
            S = $ ? (x !== null ? x + "Capture" : null) : x;
          $ = [];
          for (var g = T, E; g !== null; ) {
            var U = g;
            if (
              ((E = U.stateNode),
              (U = U.tag),
              (U !== 5 && U !== 26 && U !== 27) ||
                E === null ||
                S === null ||
                ((U = Ba(g, S)), U != null && $.push(pu(g, U, E))),
              vt)
            )
              break;
            g = g.return;
          }
          0 < $.length &&
            ((x = new R(x, F, null, l, _)), H.push({ event: x, listeners: $ }));
        }
      }
      if ((e & 7) === 0) {
        t: {
          if (
            ((x = t === "mouseover" || t === "pointerover"),
            (R = t === "mouseout" || t === "pointerout"),
            x &&
              l !== Ti &&
              (F = l.relatedTarget || l.fromElement) &&
              (Zl(F) || F[Ql]))
          )
            break t;
          if (
            (R || x) &&
            ((x =
              _.window === _
                ? _
                : (x = _.ownerDocument)
                  ? x.defaultView || x.parentWindow
                  : window),
            R
              ? ((F = l.relatedTarget || l.toElement),
                (R = T),
                (F = F ? Zl(F) : null),
                F !== null &&
                  ((vt = v(F)),
                  ($ = F.tag),
                  F !== vt || ($ !== 5 && $ !== 27 && $ !== 6)) &&
                  (F = null))
              : ((R = null), (F = T)),
            R !== F)
          ) {
            if (
              (($ = dr),
              (U = "onMouseLeave"),
              (S = "onMouseEnter"),
              (g = "mouse"),
              (t === "pointerout" || t === "pointerover") &&
                (($ = mr),
                (U = "onPointerLeave"),
                (S = "onPointerEnter"),
                (g = "pointer")),
              (vt = R == null ? x : ja(R)),
              (E = F == null ? x : ja(F)),
              (x = new $(U, g + "leave", R, l, _)),
              (x.target = vt),
              (x.relatedTarget = E),
              (U = null),
              Zl(_) === T &&
                (($ = new $(S, g + "enter", F, l, _)),
                ($.target = E),
                ($.relatedTarget = vt),
                (U = $)),
              (vt = U),
              R && F)
            )
              e: {
                for ($ = R, S = F, g = 0, E = $; E; E = xa(E)) g++;
                for (E = 0, U = S; U; U = xa(U)) E++;
                for (; 0 < g - E; ) (($ = xa($)), g--);
                for (; 0 < E - g; ) ((S = xa(S)), E--);
                for (; g--; ) {
                  if ($ === S || (S !== null && $ === S.alternate)) break e;
                  (($ = xa($)), (S = xa(S)));
                }
                $ = null;
              }
            else $ = null;
            (R !== null && ud(H, x, R, $, !1),
              F !== null && vt !== null && ud(H, vt, F, $, !0));
          }
        }
        t: {
          if (
            ((x = T ? ja(T) : window),
            (R = x.nodeName && x.nodeName.toLowerCase()),
            R === "select" || (R === "input" && x.type === "file"))
          )
            var Z = Tr;
          else if (br(x))
            if (Ar) Z = ym;
            else {
              Z = mm;
              var at = hm;
            }
          else
            ((R = x.nodeName),
              !R ||
              R.toLowerCase() !== "input" ||
              (x.type !== "checkbox" && x.type !== "radio")
                ? T && Ei(T.elementType) && (Z = Tr)
                : (Z = vm));
          if (Z && (Z = Z(t, T))) {
            Er(H, Z, l, _);
            break t;
          }
          (at && at(t, x, T),
            t === "focusout" &&
              T &&
              x.type === "number" &&
              T.memoizedProps.value != null &&
              bi(x, "number", x.value));
        }
        switch (((at = T ? ja(T) : window), t)) {
          case "focusin":
            (br(at) || at.contentEditable === "true") &&
              ((ta = at), (ji = T), (Za = null));
            break;
          case "focusout":
            Za = ji = ta = null;
            break;
          case "mousedown":
            Bi = !0;
            break;
          case "contextmenu":
          case "mouseup":
          case "dragend":
            ((Bi = !1), Nr(H, l, _));
            break;
          case "selectionchange":
            if (pm) break;
          case "keydown":
          case "keyup":
            Nr(H, l, _);
        }
        var K;
        if (Ni)
          t: {
            switch (t) {
              case "compositionstart":
                var k = "onCompositionStart";
                break t;
              case "compositionend":
                k = "onCompositionEnd";
                break t;
              case "compositionupdate":
                k = "onCompositionUpdate";
                break t;
            }
            k = void 0;
          }
        else
          Il
            ? pr(t, l) && (k = "onCompositionEnd")
            : t === "keydown" &&
              l.keyCode === 229 &&
              (k = "onCompositionStart");
        (k &&
          (vr &&
            l.locale !== "ko" &&
            (Il || k !== "onCompositionStart"
              ? k === "onCompositionEnd" && Il && (K = sr())
              : ((tl = _),
                (Oi = "value" in tl ? tl.value : tl.textContent),
                (Il = !0))),
          (at = Yn(T, k)),
          0 < at.length &&
            ((k = new hr(k, t, null, l, _)),
            H.push({ event: k, listeners: at }),
            K ? (k.data = K) : ((K = Sr(l)), K !== null && (k.data = K)))),
          (K = fm ? rm(t, l) : sm(t, l)) &&
            ((k = Yn(T, "onBeforeInput")),
            0 < k.length &&
              ((at = new hr("onBeforeInput", "beforeinput", null, l, _)),
              H.push({ event: at, listeners: k }),
              (at.data = K))),
          t0(H, t, T, l, _));
      }
      ld(H, e);
    });
  }
  function pu(t, e, l) {
    return { instance: t, listener: e, currentTarget: l };
  }
  function Yn(t, e) {
    for (var l = e + "Capture", a = []; t !== null; ) {
      var u = t,
        n = u.stateNode;
      if (
        ((u = u.tag),
        (u !== 5 && u !== 26 && u !== 27) ||
          n === null ||
          ((u = Ba(t, l)),
          u != null && a.unshift(pu(t, u, n)),
          (u = Ba(t, e)),
          u != null && a.push(pu(t, u, n))),
        t.tag === 3)
      )
        return a;
      t = t.return;
    }
    return [];
  }
  function xa(t) {
    if (t === null) return null;
    do t = t.return;
    while (t && t.tag !== 5 && t.tag !== 27);
    return t || null;
  }
  function ud(t, e, l, a, u) {
    for (var n = e._reactName, c = []; l !== null && l !== a; ) {
      var r = l,
        m = r.alternate,
        T = r.stateNode;
      if (((r = r.tag), m !== null && m === a)) break;
      ((r !== 5 && r !== 26 && r !== 27) ||
        T === null ||
        ((m = T),
        u
          ? ((T = Ba(l, n)), T != null && c.unshift(pu(l, T, m)))
          : u || ((T = Ba(l, n)), T != null && c.push(pu(l, T, m)))),
        (l = l.return));
    }
    c.length !== 0 && t.push({ event: e, listeners: c });
  }
  var u0 = /\r\n?/g,
    n0 = /\u0000|\uFFFD/g;
  function nd(t) {
    return (typeof t == "string" ? t : "" + t)
      .replace(
        u0,
        `
`,
      )
      .replace(n0, "");
  }
  function id(t, e) {
    return ((e = nd(e)), nd(t) === e);
  }
  function wn() {}
  function mt(t, e, l, a, u, n) {
    switch (l) {
      case "children":
        typeof a == "string"
          ? e === "body" || (e === "textarea" && a === "") || Wl(t, a)
          : (typeof a == "number" || typeof a == "bigint") &&
            e !== "body" &&
            Wl(t, "" + a);
        break;
      case "className":
        Qu(t, "class", a);
        break;
      case "tabIndex":
        Qu(t, "tabindex", a);
        break;
      case "dir":
      case "role":
      case "viewBox":
      case "width":
      case "height":
        Qu(t, l, a);
        break;
      case "style":
        cr(t, a, n);
        break;
      case "data":
        if (e !== "object") {
          Qu(t, "data", a);
          break;
        }
      case "src":
      case "href":
        if (a === "" && (e !== "a" || l !== "href")) {
          t.removeAttribute(l);
          break;
        }
        if (
          a == null ||
          typeof a == "function" ||
          typeof a == "symbol" ||
          typeof a == "boolean"
        ) {
          t.removeAttribute(l);
          break;
        }
        ((a = Ku("" + a)), t.setAttribute(l, a));
        break;
      case "action":
      case "formAction":
        if (typeof a == "function") {
          t.setAttribute(
            l,
            "javascript:throw new Error('A React form was unexpectedly submitted. If you called form.submit() manually, consider using form.requestSubmit() instead. If you\\'re trying to use event.stopPropagation() in a submit event handler, consider also calling event.preventDefault().')",
          );
          break;
        } else
          typeof n == "function" &&
            (l === "formAction"
              ? (e !== "input" && mt(t, e, "name", u.name, u, null),
                mt(t, e, "formEncType", u.formEncType, u, null),
                mt(t, e, "formMethod", u.formMethod, u, null),
                mt(t, e, "formTarget", u.formTarget, u, null))
              : (mt(t, e, "encType", u.encType, u, null),
                mt(t, e, "method", u.method, u, null),
                mt(t, e, "target", u.target, u, null)));
        if (a == null || typeof a == "symbol" || typeof a == "boolean") {
          t.removeAttribute(l);
          break;
        }
        ((a = Ku("" + a)), t.setAttribute(l, a));
        break;
      case "onClick":
        a != null && (t.onclick = wn);
        break;
      case "onScroll":
        a != null && nt("scroll", t);
        break;
      case "onScrollEnd":
        a != null && nt("scrollend", t);
        break;
      case "dangerouslySetInnerHTML":
        if (a != null) {
          if (typeof a != "object" || !("__html" in a)) throw Error(f(61));
          if (((l = a.__html), l != null)) {
            if (u.children != null) throw Error(f(60));
            t.innerHTML = l;
          }
        }
        break;
      case "multiple":
        t.multiple = a && typeof a != "function" && typeof a != "symbol";
        break;
      case "muted":
        t.muted = a && typeof a != "function" && typeof a != "symbol";
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "defaultValue":
      case "defaultChecked":
      case "innerHTML":
      case "ref":
        break;
      case "autoFocus":
        break;
      case "xlinkHref":
        if (
          a == null ||
          typeof a == "function" ||
          typeof a == "boolean" ||
          typeof a == "symbol"
        ) {
          t.removeAttribute("xlink:href");
          break;
        }
        ((l = Ku("" + a)),
          t.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", l));
        break;
      case "contentEditable":
      case "spellCheck":
      case "draggable":
      case "value":
      case "autoReverse":
      case "externalResourcesRequired":
      case "focusable":
      case "preserveAlpha":
        a != null && typeof a != "function" && typeof a != "symbol"
          ? t.setAttribute(l, "" + a)
          : t.removeAttribute(l);
        break;
      case "inert":
      case "allowFullScreen":
      case "async":
      case "autoPlay":
      case "controls":
      case "default":
      case "defer":
      case "disabled":
      case "disablePictureInPicture":
      case "disableRemotePlayback":
      case "formNoValidate":
      case "hidden":
      case "loop":
      case "noModule":
      case "noValidate":
      case "open":
      case "playsInline":
      case "readOnly":
      case "required":
      case "reversed":
      case "scoped":
      case "seamless":
      case "itemScope":
        a && typeof a != "function" && typeof a != "symbol"
          ? t.setAttribute(l, "")
          : t.removeAttribute(l);
        break;
      case "capture":
      case "download":
        a === !0
          ? t.setAttribute(l, "")
          : a !== !1 &&
              a != null &&
              typeof a != "function" &&
              typeof a != "symbol"
            ? t.setAttribute(l, a)
            : t.removeAttribute(l);
        break;
      case "cols":
      case "rows":
      case "size":
      case "span":
        a != null &&
        typeof a != "function" &&
        typeof a != "symbol" &&
        !isNaN(a) &&
        1 <= a
          ? t.setAttribute(l, a)
          : t.removeAttribute(l);
        break;
      case "rowSpan":
      case "start":
        a == null || typeof a == "function" || typeof a == "symbol" || isNaN(a)
          ? t.removeAttribute(l)
          : t.setAttribute(l, a);
        break;
      case "popover":
        (nt("beforetoggle", t), nt("toggle", t), Xu(t, "popover", a));
        break;
      case "xlinkActuate":
        Ce(t, "http://www.w3.org/1999/xlink", "xlink:actuate", a);
        break;
      case "xlinkArcrole":
        Ce(t, "http://www.w3.org/1999/xlink", "xlink:arcrole", a);
        break;
      case "xlinkRole":
        Ce(t, "http://www.w3.org/1999/xlink", "xlink:role", a);
        break;
      case "xlinkShow":
        Ce(t, "http://www.w3.org/1999/xlink", "xlink:show", a);
        break;
      case "xlinkTitle":
        Ce(t, "http://www.w3.org/1999/xlink", "xlink:title", a);
        break;
      case "xlinkType":
        Ce(t, "http://www.w3.org/1999/xlink", "xlink:type", a);
        break;
      case "xmlBase":
        Ce(t, "http://www.w3.org/XML/1998/namespace", "xml:base", a);
        break;
      case "xmlLang":
        Ce(t, "http://www.w3.org/XML/1998/namespace", "xml:lang", a);
        break;
      case "xmlSpace":
        Ce(t, "http://www.w3.org/XML/1998/namespace", "xml:space", a);
        break;
      case "is":
        Xu(t, "is", a);
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        (!(2 < l.length) ||
          (l[0] !== "o" && l[0] !== "O") ||
          (l[1] !== "n" && l[1] !== "N")) &&
          ((l = jh.get(l) || l), Xu(t, l, a));
    }
  }
  function nf(t, e, l, a, u, n) {
    switch (l) {
      case "style":
        cr(t, a, n);
        break;
      case "dangerouslySetInnerHTML":
        if (a != null) {
          if (typeof a != "object" || !("__html" in a)) throw Error(f(61));
          if (((l = a.__html), l != null)) {
            if (u.children != null) throw Error(f(60));
            t.innerHTML = l;
          }
        }
        break;
      case "children":
        typeof a == "string"
          ? Wl(t, a)
          : (typeof a == "number" || typeof a == "bigint") && Wl(t, "" + a);
        break;
      case "onScroll":
        a != null && nt("scroll", t);
        break;
      case "onScrollEnd":
        a != null && nt("scrollend", t);
        break;
      case "onClick":
        a != null && (t.onclick = wn);
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "innerHTML":
      case "ref":
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        if (!Wf.hasOwnProperty(l))
          t: {
            if (
              l[0] === "o" &&
              l[1] === "n" &&
              ((u = l.endsWith("Capture")),
              (e = l.slice(2, u ? l.length - 7 : void 0)),
              (n = t[Wt] || null),
              (n = n != null ? n[l] : null),
              typeof n == "function" && t.removeEventListener(e, n, u),
              typeof a == "function")
            ) {
              (typeof n != "function" &&
                n !== null &&
                (l in t
                  ? (t[l] = null)
                  : t.hasAttribute(l) && t.removeAttribute(l)),
                t.addEventListener(e, a, u));
              break t;
            }
            l in t
              ? (t[l] = a)
              : a === !0
                ? t.setAttribute(l, "")
                : Xu(t, l, a);
          }
    }
  }
  function Gt(t, e, l) {
    switch (e) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "img":
        (nt("error", t), nt("load", t));
        var a = !1,
          u = !1,
          n;
        for (n in l)
          if (l.hasOwnProperty(n)) {
            var c = l[n];
            if (c != null)
              switch (n) {
                case "src":
                  a = !0;
                  break;
                case "srcSet":
                  u = !0;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  throw Error(f(137, e));
                default:
                  mt(t, e, n, c, l, null);
              }
          }
        (u && mt(t, e, "srcSet", l.srcSet, l, null),
          a && mt(t, e, "src", l.src, l, null));
        return;
      case "input":
        nt("invalid", t);
        var r = (n = c = u = null),
          m = null,
          T = null;
        for (a in l)
          if (l.hasOwnProperty(a)) {
            var _ = l[a];
            if (_ != null)
              switch (a) {
                case "name":
                  u = _;
                  break;
                case "type":
                  c = _;
                  break;
                case "checked":
                  m = _;
                  break;
                case "defaultChecked":
                  T = _;
                  break;
                case "value":
                  n = _;
                  break;
                case "defaultValue":
                  r = _;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  if (_ != null) throw Error(f(137, e));
                  break;
                default:
                  mt(t, e, a, _, l, null);
              }
          }
        (ar(t, n, r, m, T, c, u, !1), Zu(t));
        return;
      case "select":
        (nt("invalid", t), (a = c = n = null));
        for (u in l)
          if (l.hasOwnProperty(u) && ((r = l[u]), r != null))
            switch (u) {
              case "value":
                n = r;
                break;
              case "defaultValue":
                c = r;
                break;
              case "multiple":
                a = r;
              default:
                mt(t, e, u, r, l, null);
            }
        ((e = n),
          (l = c),
          (t.multiple = !!a),
          e != null ? kl(t, !!a, e, !1) : l != null && kl(t, !!a, l, !0));
        return;
      case "textarea":
        (nt("invalid", t), (n = u = a = null));
        for (c in l)
          if (l.hasOwnProperty(c) && ((r = l[c]), r != null))
            switch (c) {
              case "value":
                a = r;
                break;
              case "defaultValue":
                u = r;
                break;
              case "children":
                n = r;
                break;
              case "dangerouslySetInnerHTML":
                if (r != null) throw Error(f(91));
                break;
              default:
                mt(t, e, c, r, l, null);
            }
        (nr(t, a, u, n), Zu(t));
        return;
      case "option":
        for (m in l)
          if (l.hasOwnProperty(m) && ((a = l[m]), a != null))
            switch (m) {
              case "selected":
                t.selected =
                  a && typeof a != "function" && typeof a != "symbol";
                break;
              default:
                mt(t, e, m, a, l, null);
            }
        return;
      case "dialog":
        (nt("beforetoggle", t),
          nt("toggle", t),
          nt("cancel", t),
          nt("close", t));
        break;
      case "iframe":
      case "object":
        nt("load", t);
        break;
      case "video":
      case "audio":
        for (a = 0; a < gu.length; a++) nt(gu[a], t);
        break;
      case "image":
        (nt("error", t), nt("load", t));
        break;
      case "details":
        nt("toggle", t);
        break;
      case "embed":
      case "source":
      case "link":
        (nt("error", t), nt("load", t));
      case "area":
      case "base":
      case "br":
      case "col":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "track":
      case "wbr":
      case "menuitem":
        for (T in l)
          if (l.hasOwnProperty(T) && ((a = l[T]), a != null))
            switch (T) {
              case "children":
              case "dangerouslySetInnerHTML":
                throw Error(f(137, e));
              default:
                mt(t, e, T, a, l, null);
            }
        return;
      default:
        if (Ei(e)) {
          for (_ in l)
            l.hasOwnProperty(_) &&
              ((a = l[_]), a !== void 0 && nf(t, e, _, a, l, void 0));
          return;
        }
    }
    for (r in l)
      l.hasOwnProperty(r) && ((a = l[r]), a != null && mt(t, e, r, a, l, null));
  }
  function i0(t, e, l, a) {
    switch (e) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "input":
        var u = null,
          n = null,
          c = null,
          r = null,
          m = null,
          T = null,
          _ = null;
        for (R in l) {
          var H = l[R];
          if (l.hasOwnProperty(R) && H != null)
            switch (R) {
              case "checked":
                break;
              case "value":
                break;
              case "defaultValue":
                m = H;
              default:
                a.hasOwnProperty(R) || mt(t, e, R, null, a, H);
            }
        }
        for (var x in a) {
          var R = a[x];
          if (((H = l[x]), a.hasOwnProperty(x) && (R != null || H != null)))
            switch (x) {
              case "type":
                n = R;
                break;
              case "name":
                u = R;
                break;
              case "checked":
                T = R;
                break;
              case "defaultChecked":
                _ = R;
                break;
              case "value":
                c = R;
                break;
              case "defaultValue":
                r = R;
                break;
              case "children":
              case "dangerouslySetInnerHTML":
                if (R != null) throw Error(f(137, e));
                break;
              default:
                R !== H && mt(t, e, x, R, a, H);
            }
        }
        Si(t, c, r, m, T, _, n, u);
        return;
      case "select":
        R = c = r = x = null;
        for (n in l)
          if (((m = l[n]), l.hasOwnProperty(n) && m != null))
            switch (n) {
              case "value":
                break;
              case "multiple":
                R = m;
              default:
                a.hasOwnProperty(n) || mt(t, e, n, null, a, m);
            }
        for (u in a)
          if (
            ((n = a[u]),
            (m = l[u]),
            a.hasOwnProperty(u) && (n != null || m != null))
          )
            switch (u) {
              case "value":
                x = n;
                break;
              case "defaultValue":
                r = n;
                break;
              case "multiple":
                c = n;
              default:
                n !== m && mt(t, e, u, n, a, m);
            }
        ((e = r),
          (l = c),
          (a = R),
          x != null
            ? kl(t, !!l, x, !1)
            : !!a != !!l &&
              (e != null ? kl(t, !!l, e, !0) : kl(t, !!l, l ? [] : "", !1)));
        return;
      case "textarea":
        R = x = null;
        for (r in l)
          if (
            ((u = l[r]),
            l.hasOwnProperty(r) && u != null && !a.hasOwnProperty(r))
          )
            switch (r) {
              case "value":
                break;
              case "children":
                break;
              default:
                mt(t, e, r, null, a, u);
            }
        for (c in a)
          if (
            ((u = a[c]),
            (n = l[c]),
            a.hasOwnProperty(c) && (u != null || n != null))
          )
            switch (c) {
              case "value":
                x = u;
                break;
              case "defaultValue":
                R = u;
                break;
              case "children":
                break;
              case "dangerouslySetInnerHTML":
                if (u != null) throw Error(f(91));
                break;
              default:
                u !== n && mt(t, e, c, u, a, n);
            }
        ur(t, x, R);
        return;
      case "option":
        for (var F in l)
          if (
            ((x = l[F]),
            l.hasOwnProperty(F) && x != null && !a.hasOwnProperty(F))
          )
            switch (F) {
              case "selected":
                t.selected = !1;
                break;
              default:
                mt(t, e, F, null, a, x);
            }
        for (m in a)
          if (
            ((x = a[m]),
            (R = l[m]),
            a.hasOwnProperty(m) && x !== R && (x != null || R != null))
          )
            switch (m) {
              case "selected":
                t.selected =
                  x && typeof x != "function" && typeof x != "symbol";
                break;
              default:
                mt(t, e, m, x, a, R);
            }
        return;
      case "img":
      case "link":
      case "area":
      case "base":
      case "br":
      case "col":
      case "embed":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "source":
      case "track":
      case "wbr":
      case "menuitem":
        for (var $ in l)
          ((x = l[$]),
            l.hasOwnProperty($) &&
              x != null &&
              !a.hasOwnProperty($) &&
              mt(t, e, $, null, a, x));
        for (T in a)
          if (
            ((x = a[T]),
            (R = l[T]),
            a.hasOwnProperty(T) && x !== R && (x != null || R != null))
          )
            switch (T) {
              case "children":
              case "dangerouslySetInnerHTML":
                if (x != null) throw Error(f(137, e));
                break;
              default:
                mt(t, e, T, x, a, R);
            }
        return;
      default:
        if (Ei(e)) {
          for (var vt in l)
            ((x = l[vt]),
              l.hasOwnProperty(vt) &&
                x !== void 0 &&
                !a.hasOwnProperty(vt) &&
                nf(t, e, vt, void 0, a, x));
          for (_ in a)
            ((x = a[_]),
              (R = l[_]),
              !a.hasOwnProperty(_) ||
                x === R ||
                (x === void 0 && R === void 0) ||
                nf(t, e, _, x, a, R));
          return;
        }
    }
    for (var S in l)
      ((x = l[S]),
        l.hasOwnProperty(S) &&
          x != null &&
          !a.hasOwnProperty(S) &&
          mt(t, e, S, null, a, x));
    for (H in a)
      ((x = a[H]),
        (R = l[H]),
        !a.hasOwnProperty(H) ||
          x === R ||
          (x == null && R == null) ||
          mt(t, e, H, x, a, R));
  }
  var cf = null,
    ff = null;
  function Ln(t) {
    return t.nodeType === 9 ? t : t.ownerDocument;
  }
  function cd(t) {
    switch (t) {
      case "http://www.w3.org/2000/svg":
        return 1;
      case "http://www.w3.org/1998/Math/MathML":
        return 2;
      default:
        return 0;
    }
  }
  function fd(t, e) {
    if (t === 0)
      switch (e) {
        case "svg":
          return 1;
        case "math":
          return 2;
        default:
          return 0;
      }
    return t === 1 && e === "foreignObject" ? 0 : t;
  }
  function rf(t, e) {
    return (
      t === "textarea" ||
      t === "noscript" ||
      typeof e.children == "string" ||
      typeof e.children == "number" ||
      typeof e.children == "bigint" ||
      (typeof e.dangerouslySetInnerHTML == "object" &&
        e.dangerouslySetInnerHTML !== null &&
        e.dangerouslySetInnerHTML.__html != null)
    );
  }
  var sf = null;
  function c0() {
    var t = window.event;
    return t && t.type === "popstate"
      ? t === sf
        ? !1
        : ((sf = t), !0)
      : ((sf = null), !1);
  }
  var rd = typeof setTimeout == "function" ? setTimeout : void 0,
    f0 = typeof clearTimeout == "function" ? clearTimeout : void 0,
    sd = typeof Promise == "function" ? Promise : void 0,
    r0 =
      typeof queueMicrotask == "function"
        ? queueMicrotask
        : typeof sd < "u"
          ? function (t) {
              return sd.resolve(null).then(t).catch(s0);
            }
          : rd;
  function s0(t) {
    setTimeout(function () {
      throw t;
    });
  }
  function yl(t) {
    return t === "head";
  }
  function od(t, e) {
    var l = e,
      a = 0,
      u = 0;
    do {
      var n = l.nextSibling;
      if ((t.removeChild(l), n && n.nodeType === 8))
        if (((l = n.data), l === "/$")) {
          if (0 < a && 8 > a) {
            l = a;
            var c = t.ownerDocument;
            if ((l & 1 && Su(c.documentElement), l & 2 && Su(c.body), l & 4))
              for (l = c.head, Su(l), c = l.firstChild; c; ) {
                var r = c.nextSibling,
                  m = c.nodeName;
                (c[Ha] ||
                  m === "SCRIPT" ||
                  m === "STYLE" ||
                  (m === "LINK" && c.rel.toLowerCase() === "stylesheet") ||
                  l.removeChild(c),
                  (c = r));
              }
          }
          if (u === 0) {
            (t.removeChild(n), zu(e));
            return;
          }
          u--;
        } else
          l === "$" || l === "$?" || l === "$!"
            ? u++
            : (a = l.charCodeAt(0) - 48);
      else a = 0;
      l = n;
    } while (l);
    zu(e);
  }
  function of(t) {
    var e = t.firstChild;
    for (e && e.nodeType === 10 && (e = e.nextSibling); e; ) {
      var l = e;
      switch (((e = e.nextSibling), l.nodeName)) {
        case "HTML":
        case "HEAD":
        case "BODY":
          (of(l), vi(l));
          continue;
        case "SCRIPT":
        case "STYLE":
          continue;
        case "LINK":
          if (l.rel.toLowerCase() === "stylesheet") continue;
      }
      t.removeChild(l);
    }
  }
  function o0(t, e, l, a) {
    for (; t.nodeType === 1; ) {
      var u = l;
      if (t.nodeName.toLowerCase() !== e.toLowerCase()) {
        if (!a && (t.nodeName !== "INPUT" || t.type !== "hidden")) break;
      } else if (a) {
        if (!t[Ha])
          switch (e) {
            case "meta":
              if (!t.hasAttribute("itemprop")) break;
              return t;
            case "link":
              if (
                ((n = t.getAttribute("rel")),
                n === "stylesheet" && t.hasAttribute("data-precedence"))
              )
                break;
              if (
                n !== u.rel ||
                t.getAttribute("href") !==
                  (u.href == null || u.href === "" ? null : u.href) ||
                t.getAttribute("crossorigin") !==
                  (u.crossOrigin == null ? null : u.crossOrigin) ||
                t.getAttribute("title") !== (u.title == null ? null : u.title)
              )
                break;
              return t;
            case "style":
              if (t.hasAttribute("data-precedence")) break;
              return t;
            case "script":
              if (
                ((n = t.getAttribute("src")),
                (n !== (u.src == null ? null : u.src) ||
                  t.getAttribute("type") !== (u.type == null ? null : u.type) ||
                  t.getAttribute("crossorigin") !==
                    (u.crossOrigin == null ? null : u.crossOrigin)) &&
                  n &&
                  t.hasAttribute("async") &&
                  !t.hasAttribute("itemprop"))
              )
                break;
              return t;
            default:
              return t;
          }
      } else if (e === "input" && t.type === "hidden") {
        var n = u.name == null ? null : "" + u.name;
        if (u.type === "hidden" && t.getAttribute("name") === n) return t;
      } else return t;
      if (((t = Ae(t.nextSibling)), t === null)) break;
    }
    return null;
  }
  function d0(t, e, l) {
    if (e === "") return null;
    for (; t.nodeType !== 3; )
      if (
        ((t.nodeType !== 1 || t.nodeName !== "INPUT" || t.type !== "hidden") &&
          !l) ||
        ((t = Ae(t.nextSibling)), t === null)
      )
        return null;
    return t;
  }
  function df(t) {
    return (
      t.data === "$!" ||
      (t.data === "$?" && t.ownerDocument.readyState === "complete")
    );
  }
  function h0(t, e) {
    var l = t.ownerDocument;
    if (t.data !== "$?" || l.readyState === "complete") e();
    else {
      var a = function () {
        (e(), l.removeEventListener("DOMContentLoaded", a));
      };
      (l.addEventListener("DOMContentLoaded", a), (t._reactRetry = a));
    }
  }
  function Ae(t) {
    for (; t != null; t = t.nextSibling) {
      var e = t.nodeType;
      if (e === 1 || e === 3) break;
      if (e === 8) {
        if (
          ((e = t.data),
          e === "$" || e === "$!" || e === "$?" || e === "F!" || e === "F")
        )
          break;
        if (e === "/$") return null;
      }
    }
    return t;
  }
  var hf = null;
  function dd(t) {
    t = t.previousSibling;
    for (var e = 0; t; ) {
      if (t.nodeType === 8) {
        var l = t.data;
        if (l === "$" || l === "$!" || l === "$?") {
          if (e === 0) return t;
          e--;
        } else l === "/$" && e++;
      }
      t = t.previousSibling;
    }
    return null;
  }
  function hd(t, e, l) {
    switch (((e = Ln(l)), t)) {
      case "html":
        if (((t = e.documentElement), !t)) throw Error(f(452));
        return t;
      case "head":
        if (((t = e.head), !t)) throw Error(f(453));
        return t;
      case "body":
        if (((t = e.body), !t)) throw Error(f(454));
        return t;
      default:
        throw Error(f(451));
    }
  }
  function Su(t) {
    for (var e = t.attributes; e.length; ) t.removeAttributeNode(e[0]);
    vi(t);
  }
  var be = new Map(),
    md = new Set();
  function Gn(t) {
    return typeof t.getRootNode == "function"
      ? t.getRootNode()
      : t.nodeType === 9
        ? t
        : t.ownerDocument;
  }
  var Je = L.d;
  L.d = { f: m0, r: v0, D: y0, C: g0, L: p0, m: S0, X: E0, S: b0, M: T0 };
  function m0() {
    var t = Je.f(),
      e = Un();
    return t || e;
  }
  function v0(t) {
    var e = Vl(t);
    e !== null && e.tag === 5 && e.type === "form" ? Cs(e) : Je.r(t);
  }
  var Ra = typeof document > "u" ? null : document;
  function vd(t, e, l) {
    var a = Ra;
    if (a && typeof e == "string" && e) {
      var u = he(e);
      ((u = 'link[rel="' + t + '"][href="' + u + '"]'),
        typeof l == "string" && (u += '[crossorigin="' + l + '"]'),
        md.has(u) ||
          (md.add(u),
          (t = { rel: t, crossOrigin: l, href: e }),
          a.querySelector(u) === null &&
            ((e = a.createElement("link")),
            Gt(e, "link", t),
            jt(e),
            a.head.appendChild(e))));
    }
  }
  function y0(t) {
    (Je.D(t), vd("dns-prefetch", t, null));
  }
  function g0(t, e) {
    (Je.C(t, e), vd("preconnect", t, e));
  }
  function p0(t, e, l) {
    Je.L(t, e, l);
    var a = Ra;
    if (a && t && e) {
      var u = 'link[rel="preload"][as="' + he(e) + '"]';
      e === "image" && l && l.imageSrcSet
        ? ((u += '[imagesrcset="' + he(l.imageSrcSet) + '"]'),
          typeof l.imageSizes == "string" &&
            (u += '[imagesizes="' + he(l.imageSizes) + '"]'))
        : (u += '[href="' + he(t) + '"]');
      var n = u;
      switch (e) {
        case "style":
          n = Oa(t);
          break;
        case "script":
          n = za(t);
      }
      be.has(n) ||
        ((t = O(
          {
            rel: "preload",
            href: e === "image" && l && l.imageSrcSet ? void 0 : t,
            as: e,
          },
          l,
        )),
        be.set(n, t),
        a.querySelector(u) !== null ||
          (e === "style" && a.querySelector(bu(n))) ||
          (e === "script" && a.querySelector(Eu(n))) ||
          ((e = a.createElement("link")),
          Gt(e, "link", t),
          jt(e),
          a.head.appendChild(e)));
    }
  }
  function S0(t, e) {
    Je.m(t, e);
    var l = Ra;
    if (l && t) {
      var a = e && typeof e.as == "string" ? e.as : "script",
        u =
          'link[rel="modulepreload"][as="' + he(a) + '"][href="' + he(t) + '"]',
        n = u;
      switch (a) {
        case "audioworklet":
        case "paintworklet":
        case "serviceworker":
        case "sharedworker":
        case "worker":
        case "script":
          n = za(t);
      }
      if (
        !be.has(n) &&
        ((t = O({ rel: "modulepreload", href: t }, e)),
        be.set(n, t),
        l.querySelector(u) === null)
      ) {
        switch (a) {
          case "audioworklet":
          case "paintworklet":
          case "serviceworker":
          case "sharedworker":
          case "worker":
          case "script":
            if (l.querySelector(Eu(n))) return;
        }
        ((a = l.createElement("link")),
          Gt(a, "link", t),
          jt(a),
          l.head.appendChild(a));
      }
    }
  }
  function b0(t, e, l) {
    Je.S(t, e, l);
    var a = Ra;
    if (a && t) {
      var u = Kl(a).hoistableStyles,
        n = Oa(t);
      e = e || "default";
      var c = u.get(n);
      if (!c) {
        var r = { loading: 0, preload: null };
        if ((c = a.querySelector(bu(n)))) r.loading = 5;
        else {
          ((t = O({ rel: "stylesheet", href: t, "data-precedence": e }, l)),
            (l = be.get(n)) && mf(t, l));
          var m = (c = a.createElement("link"));
          (jt(m),
            Gt(m, "link", t),
            (m._p = new Promise(function (T, _) {
              ((m.onload = T), (m.onerror = _));
            })),
            m.addEventListener("load", function () {
              r.loading |= 1;
            }),
            m.addEventListener("error", function () {
              r.loading |= 2;
            }),
            (r.loading |= 4),
            Xn(c, e, a));
        }
        ((c = { type: "stylesheet", instance: c, count: 1, state: r }),
          u.set(n, c));
      }
    }
  }
  function E0(t, e) {
    Je.X(t, e);
    var l = Ra;
    if (l && t) {
      var a = Kl(l).hoistableScripts,
        u = za(t),
        n = a.get(u);
      n ||
        ((n = l.querySelector(Eu(u))),
        n ||
          ((t = O({ src: t, async: !0 }, e)),
          (e = be.get(u)) && vf(t, e),
          (n = l.createElement("script")),
          jt(n),
          Gt(n, "link", t),
          l.head.appendChild(n)),
        (n = { type: "script", instance: n, count: 1, state: null }),
        a.set(u, n));
    }
  }
  function T0(t, e) {
    Je.M(t, e);
    var l = Ra;
    if (l && t) {
      var a = Kl(l).hoistableScripts,
        u = za(t),
        n = a.get(u);
      n ||
        ((n = l.querySelector(Eu(u))),
        n ||
          ((t = O({ src: t, async: !0, type: "module" }, e)),
          (e = be.get(u)) && vf(t, e),
          (n = l.createElement("script")),
          jt(n),
          Gt(n, "link", t),
          l.head.appendChild(n)),
        (n = { type: "script", instance: n, count: 1, state: null }),
        a.set(u, n));
    }
  }
  function yd(t, e, l, a) {
    var u = (u = tt.current) ? Gn(u) : null;
    if (!u) throw Error(f(446));
    switch (t) {
      case "meta":
      case "title":
        return null;
      case "style":
        return typeof l.precedence == "string" && typeof l.href == "string"
          ? ((e = Oa(l.href)),
            (l = Kl(u).hoistableStyles),
            (a = l.get(e)),
            a ||
              ((a = { type: "style", instance: null, count: 0, state: null }),
              l.set(e, a)),
            a)
          : { type: "void", instance: null, count: 0, state: null };
      case "link":
        if (
          l.rel === "stylesheet" &&
          typeof l.href == "string" &&
          typeof l.precedence == "string"
        ) {
          t = Oa(l.href);
          var n = Kl(u).hoistableStyles,
            c = n.get(t);
          if (
            (c ||
              ((u = u.ownerDocument || u),
              (c = {
                type: "stylesheet",
                instance: null,
                count: 0,
                state: { loading: 0, preload: null },
              }),
              n.set(t, c),
              (n = u.querySelector(bu(t))) &&
                !n._p &&
                ((c.instance = n), (c.state.loading = 5)),
              be.has(t) ||
                ((l = {
                  rel: "preload",
                  as: "style",
                  href: l.href,
                  crossOrigin: l.crossOrigin,
                  integrity: l.integrity,
                  media: l.media,
                  hrefLang: l.hrefLang,
                  referrerPolicy: l.referrerPolicy,
                }),
                be.set(t, l),
                n || A0(u, t, l, c.state))),
            e && a === null)
          )
            throw Error(f(528, ""));
          return c;
        }
        if (e && a !== null) throw Error(f(529, ""));
        return null;
      case "script":
        return (
          (e = l.async),
          (l = l.src),
          typeof l == "string" &&
          e &&
          typeof e != "function" &&
          typeof e != "symbol"
            ? ((e = za(l)),
              (l = Kl(u).hoistableScripts),
              (a = l.get(e)),
              a ||
                ((a = {
                  type: "script",
                  instance: null,
                  count: 0,
                  state: null,
                }),
                l.set(e, a)),
              a)
            : { type: "void", instance: null, count: 0, state: null }
        );
      default:
        throw Error(f(444, t));
    }
  }
  function Oa(t) {
    return 'href="' + he(t) + '"';
  }
  function bu(t) {
    return 'link[rel="stylesheet"][' + t + "]";
  }
  function gd(t) {
    return O({}, t, { "data-precedence": t.precedence, precedence: null });
  }
  function A0(t, e, l, a) {
    t.querySelector('link[rel="preload"][as="style"][' + e + "]")
      ? (a.loading = 1)
      : ((e = t.createElement("link")),
        (a.preload = e),
        e.addEventListener("load", function () {
          return (a.loading |= 1);
        }),
        e.addEventListener("error", function () {
          return (a.loading |= 2);
        }),
        Gt(e, "link", l),
        jt(e),
        t.head.appendChild(e));
  }
  function za(t) {
    return '[src="' + he(t) + '"]';
  }
  function Eu(t) {
    return "script[async]" + t;
  }
  function pd(t, e, l) {
    if ((e.count++, e.instance === null))
      switch (e.type) {
        case "style":
          var a = t.querySelector('style[data-href~="' + he(l.href) + '"]');
          if (a) return ((e.instance = a), jt(a), a);
          var u = O({}, l, {
            "data-href": l.href,
            "data-precedence": l.precedence,
            href: null,
            precedence: null,
          });
          return (
            (a = (t.ownerDocument || t).createElement("style")),
            jt(a),
            Gt(a, "style", u),
            Xn(a, l.precedence, t),
            (e.instance = a)
          );
        case "stylesheet":
          u = Oa(l.href);
          var n = t.querySelector(bu(u));
          if (n) return ((e.state.loading |= 4), (e.instance = n), jt(n), n);
          ((a = gd(l)),
            (u = be.get(u)) && mf(a, u),
            (n = (t.ownerDocument || t).createElement("link")),
            jt(n));
          var c = n;
          return (
            (c._p = new Promise(function (r, m) {
              ((c.onload = r), (c.onerror = m));
            })),
            Gt(n, "link", a),
            (e.state.loading |= 4),
            Xn(n, l.precedence, t),
            (e.instance = n)
          );
        case "script":
          return (
            (n = za(l.src)),
            (u = t.querySelector(Eu(n)))
              ? ((e.instance = u), jt(u), u)
              : ((a = l),
                (u = be.get(n)) && ((a = O({}, l)), vf(a, u)),
                (t = t.ownerDocument || t),
                (u = t.createElement("script")),
                jt(u),
                Gt(u, "link", a),
                t.head.appendChild(u),
                (e.instance = u))
          );
        case "void":
          return null;
        default:
          throw Error(f(443, e.type));
      }
    else
      e.type === "stylesheet" &&
        (e.state.loading & 4) === 0 &&
        ((a = e.instance), (e.state.loading |= 4), Xn(a, l.precedence, t));
    return e.instance;
  }
  function Xn(t, e, l) {
    for (
      var a = l.querySelectorAll(
          'link[rel="stylesheet"][data-precedence],style[data-precedence]',
        ),
        u = a.length ? a[a.length - 1] : null,
        n = u,
        c = 0;
      c < a.length;
      c++
    ) {
      var r = a[c];
      if (r.dataset.precedence === e) n = r;
      else if (n !== u) break;
    }
    n
      ? n.parentNode.insertBefore(t, n.nextSibling)
      : ((e = l.nodeType === 9 ? l.head : l), e.insertBefore(t, e.firstChild));
  }
  function mf(t, e) {
    (t.crossOrigin == null && (t.crossOrigin = e.crossOrigin),
      t.referrerPolicy == null && (t.referrerPolicy = e.referrerPolicy),
      t.title == null && (t.title = e.title));
  }
  function vf(t, e) {
    (t.crossOrigin == null && (t.crossOrigin = e.crossOrigin),
      t.referrerPolicy == null && (t.referrerPolicy = e.referrerPolicy),
      t.integrity == null && (t.integrity = e.integrity));
  }
  var Qn = null;
  function Sd(t, e, l) {
    if (Qn === null) {
      var a = new Map(),
        u = (Qn = new Map());
      u.set(l, a);
    } else ((u = Qn), (a = u.get(l)), a || ((a = new Map()), u.set(l, a)));
    if (a.has(t)) return a;
    for (
      a.set(t, null), l = l.getElementsByTagName(t), u = 0;
      u < l.length;
      u++
    ) {
      var n = l[u];
      if (
        !(
          n[Ha] ||
          n[Zt] ||
          (t === "link" && n.getAttribute("rel") === "stylesheet")
        ) &&
        n.namespaceURI !== "http://www.w3.org/2000/svg"
      ) {
        var c = n.getAttribute(e) || "";
        c = t + c;
        var r = a.get(c);
        r ? r.push(n) : a.set(c, [n]);
      }
    }
    return a;
  }
  function bd(t, e, l) {
    ((t = t.ownerDocument || t),
      t.head.insertBefore(
        l,
        e === "title" ? t.querySelector("head > title") : null,
      ));
  }
  function x0(t, e, l) {
    if (l === 1 || e.itemProp != null) return !1;
    switch (t) {
      case "meta":
      case "title":
        return !0;
      case "style":
        if (
          typeof e.precedence != "string" ||
          typeof e.href != "string" ||
          e.href === ""
        )
          break;
        return !0;
      case "link":
        if (
          typeof e.rel != "string" ||
          typeof e.href != "string" ||
          e.href === "" ||
          e.onLoad ||
          e.onError
        )
          break;
        switch (e.rel) {
          case "stylesheet":
            return (
              (t = e.disabled),
              typeof e.precedence == "string" && t == null
            );
          default:
            return !0;
        }
      case "script":
        if (
          e.async &&
          typeof e.async != "function" &&
          typeof e.async != "symbol" &&
          !e.onLoad &&
          !e.onError &&
          e.src &&
          typeof e.src == "string"
        )
          return !0;
    }
    return !1;
  }
  function Ed(t) {
    return !(t.type === "stylesheet" && (t.state.loading & 3) === 0);
  }
  var Tu = null;
  function R0() {}
  function O0(t, e, l) {
    if (Tu === null) throw Error(f(475));
    var a = Tu;
    if (
      e.type === "stylesheet" &&
      (typeof l.media != "string" || matchMedia(l.media).matches !== !1) &&
      (e.state.loading & 4) === 0
    ) {
      if (e.instance === null) {
        var u = Oa(l.href),
          n = t.querySelector(bu(u));
        if (n) {
          ((t = n._p),
            t !== null &&
              typeof t == "object" &&
              typeof t.then == "function" &&
              (a.count++, (a = Zn.bind(a)), t.then(a, a)),
            (e.state.loading |= 4),
            (e.instance = n),
            jt(n));
          return;
        }
        ((n = t.ownerDocument || t),
          (l = gd(l)),
          (u = be.get(u)) && mf(l, u),
          (n = n.createElement("link")),
          jt(n));
        var c = n;
        ((c._p = new Promise(function (r, m) {
          ((c.onload = r), (c.onerror = m));
        })),
          Gt(n, "link", l),
          (e.instance = n));
      }
      (a.stylesheets === null && (a.stylesheets = new Map()),
        a.stylesheets.set(e, t),
        (t = e.state.preload) &&
          (e.state.loading & 3) === 0 &&
          (a.count++,
          (e = Zn.bind(a)),
          t.addEventListener("load", e),
          t.addEventListener("error", e)));
    }
  }
  function z0() {
    if (Tu === null) throw Error(f(475));
    var t = Tu;
    return (
      t.stylesheets && t.count === 0 && yf(t, t.stylesheets),
      0 < t.count
        ? function (e) {
            var l = setTimeout(function () {
              if ((t.stylesheets && yf(t, t.stylesheets), t.unsuspend)) {
                var a = t.unsuspend;
                ((t.unsuspend = null), a());
              }
            }, 6e4);
            return (
              (t.unsuspend = e),
              function () {
                ((t.unsuspend = null), clearTimeout(l));
              }
            );
          }
        : null
    );
  }
  function Zn() {
    if ((this.count--, this.count === 0)) {
      if (this.stylesheets) yf(this, this.stylesheets);
      else if (this.unsuspend) {
        var t = this.unsuspend;
        ((this.unsuspend = null), t());
      }
    }
  }
  var Vn = null;
  function yf(t, e) {
    ((t.stylesheets = null),
      t.unsuspend !== null &&
        (t.count++,
        (Vn = new Map()),
        e.forEach(M0, t),
        (Vn = null),
        Zn.call(t)));
  }
  function M0(t, e) {
    if (!(e.state.loading & 4)) {
      var l = Vn.get(t);
      if (l) var a = l.get(null);
      else {
        ((l = new Map()), Vn.set(t, l));
        for (
          var u = t.querySelectorAll(
              "link[data-precedence],style[data-precedence]",
            ),
            n = 0;
          n < u.length;
          n++
        ) {
          var c = u[n];
          (c.nodeName === "LINK" || c.getAttribute("media") !== "not all") &&
            (l.set(c.dataset.precedence, c), (a = c));
        }
        a && l.set(null, a);
      }
      ((u = e.instance),
        (c = u.getAttribute("data-precedence")),
        (n = l.get(c) || a),
        n === a && l.set(null, u),
        l.set(c, u),
        this.count++,
        (a = Zn.bind(this)),
        u.addEventListener("load", a),
        u.addEventListener("error", a),
        n
          ? n.parentNode.insertBefore(u, n.nextSibling)
          : ((t = t.nodeType === 9 ? t.head : t),
            t.insertBefore(u, t.firstChild)),
        (e.state.loading |= 4));
    }
  }
  var Au = {
    $$typeof: P,
    Provider: null,
    Consumer: null,
    _currentValue: W,
    _currentValue2: W,
    _threadCount: 0,
  };
  function D0(t, e, l, a, u, n, c, r) {
    ((this.tag = 1),
      (this.containerInfo = t),
      (this.pingCache = this.current = this.pendingChildren = null),
      (this.timeoutHandle = -1),
      (this.callbackNode =
        this.next =
        this.pendingContext =
        this.context =
        this.cancelPendingCommit =
          null),
      (this.callbackPriority = 0),
      (this.expirationTimes = oi(-1)),
      (this.entangledLanes =
        this.shellSuspendCounter =
        this.errorRecoveryDisabledLanes =
        this.expiredLanes =
        this.warmLanes =
        this.pingedLanes =
        this.suspendedLanes =
        this.pendingLanes =
          0),
      (this.entanglements = oi(0)),
      (this.hiddenUpdates = oi(null)),
      (this.identifierPrefix = a),
      (this.onUncaughtError = u),
      (this.onCaughtError = n),
      (this.onRecoverableError = c),
      (this.pooledCache = null),
      (this.pooledCacheLanes = 0),
      (this.formState = r),
      (this.incompleteTransitions = new Map()));
  }
  function Td(t, e, l, a, u, n, c, r, m, T, _, H) {
    return (
      (t = new D0(t, e, l, c, r, m, T, H)),
      (e = 1),
      n === !0 && (e |= 24),
      (n = ne(3, null, null, e)),
      (t.current = n),
      (n.stateNode = t),
      (e = Wi()),
      e.refCount++,
      (t.pooledCache = e),
      e.refCount++,
      (n.memoizedState = { element: a, isDehydrated: l, cache: e }),
      tc(n),
      t
    );
  }
  function Ad(t) {
    return t ? ((t = ua), t) : ua;
  }
  function xd(t, e, l, a, u, n) {
    ((u = Ad(u)),
      a.context === null ? (a.context = u) : (a.pendingContext = u),
      (a = al(e)),
      (a.payload = { element: l }),
      (n = n === void 0 ? null : n),
      n !== null && (a.callback = n),
      (l = ul(t, a, e)),
      l !== null && (se(l, t, e), Ia(l, t, e)));
  }
  function Rd(t, e) {
    if (((t = t.memoizedState), t !== null && t.dehydrated !== null)) {
      var l = t.retryLane;
      t.retryLane = l !== 0 && l < e ? l : e;
    }
  }
  function gf(t, e) {
    (Rd(t, e), (t = t.alternate) && Rd(t, e));
  }
  function Od(t) {
    if (t.tag === 13) {
      var e = aa(t, 67108864);
      (e !== null && se(e, t, 67108864), gf(t, 67108864));
    }
  }
  var Kn = !0;
  function _0(t, e, l, a) {
    var u = N.T;
    N.T = null;
    var n = L.p;
    try {
      ((L.p = 2), pf(t, e, l, a));
    } finally {
      ((L.p = n), (N.T = u));
    }
  }
  function N0(t, e, l, a) {
    var u = N.T;
    N.T = null;
    var n = L.p;
    try {
      ((L.p = 8), pf(t, e, l, a));
    } finally {
      ((L.p = n), (N.T = u));
    }
  }
  function pf(t, e, l, a) {
    if (Kn) {
      var u = Sf(a);
      if (u === null) (uf(t, e, a, Jn, l), Md(t, a));
      else if (C0(u, t, e, l, a)) a.stopPropagation();
      else if ((Md(t, a), e & 4 && -1 < U0.indexOf(t))) {
        for (; u !== null; ) {
          var n = Vl(u);
          if (n !== null)
            switch (n.tag) {
              case 3:
                if (((n = n.stateNode), n.current.memoizedState.isDehydrated)) {
                  var c = xl(n.pendingLanes);
                  if (c !== 0) {
                    var r = n;
                    for (r.pendingLanes |= 2, r.entangledLanes |= 2; c; ) {
                      var m = 1 << (31 - ae(c));
                      ((r.entanglements[1] |= m), (c &= ~m));
                    }
                    (_e(n), (ot & 6) === 0 && ((_n = Re() + 500), yu(0)));
                  }
                }
                break;
              case 13:
                ((r = aa(n, 2)), r !== null && se(r, n, 2), Un(), gf(n, 2));
            }
          if (((n = Sf(a)), n === null && uf(t, e, a, Jn, l), n === u)) break;
          u = n;
        }
        u !== null && a.stopPropagation();
      } else uf(t, e, a, null, l);
    }
  }
  function Sf(t) {
    return ((t = Ai(t)), bf(t));
  }
  var Jn = null;
  function bf(t) {
    if (((Jn = null), (t = Zl(t)), t !== null)) {
      var e = v(t);
      if (e === null) t = null;
      else {
        var l = e.tag;
        if (l === 13) {
          if (((t = A(e)), t !== null)) return t;
          t = null;
        } else if (l === 3) {
          if (e.stateNode.current.memoizedState.isDehydrated)
            return e.tag === 3 ? e.stateNode.containerInfo : null;
          t = null;
        } else e !== t && (t = null);
      }
    }
    return ((Jn = t), null);
  }
  function zd(t) {
    switch (t) {
      case "beforetoggle":
      case "cancel":
      case "click":
      case "close":
      case "contextmenu":
      case "copy":
      case "cut":
      case "auxclick":
      case "dblclick":
      case "dragend":
      case "dragstart":
      case "drop":
      case "focusin":
      case "focusout":
      case "input":
      case "invalid":
      case "keydown":
      case "keypress":
      case "keyup":
      case "mousedown":
      case "mouseup":
      case "paste":
      case "pause":
      case "play":
      case "pointercancel":
      case "pointerdown":
      case "pointerup":
      case "ratechange":
      case "reset":
      case "resize":
      case "seeked":
      case "submit":
      case "toggle":
      case "touchcancel":
      case "touchend":
      case "touchstart":
      case "volumechange":
      case "change":
      case "selectionchange":
      case "textInput":
      case "compositionstart":
      case "compositionend":
      case "compositionupdate":
      case "beforeblur":
      case "afterblur":
      case "beforeinput":
      case "blur":
      case "fullscreenchange":
      case "focus":
      case "hashchange":
      case "popstate":
      case "select":
      case "selectstart":
        return 2;
      case "drag":
      case "dragenter":
      case "dragexit":
      case "dragleave":
      case "dragover":
      case "mousemove":
      case "mouseout":
      case "mouseover":
      case "pointermove":
      case "pointerout":
      case "pointerover":
      case "scroll":
      case "touchmove":
      case "wheel":
      case "mouseenter":
      case "mouseleave":
      case "pointerenter":
      case "pointerleave":
        return 8;
      case "message":
        switch (gh()) {
          case Lf:
            return 2;
          case Gf:
            return 8;
          case Yu:
          case ph:
            return 32;
          case Xf:
            return 268435456;
          default:
            return 32;
        }
      default:
        return 32;
    }
  }
  var Ef = !1,
    gl = null,
    pl = null,
    Sl = null,
    xu = new Map(),
    Ru = new Map(),
    bl = [],
    U0 =
      "mousedown mouseup touchcancel touchend touchstart auxclick dblclick pointercancel pointerdown pointerup dragend dragstart drop compositionend compositionstart keydown keypress keyup input textInput copy cut paste click change contextmenu reset".split(
        " ",
      );
  function Md(t, e) {
    switch (t) {
      case "focusin":
      case "focusout":
        gl = null;
        break;
      case "dragenter":
      case "dragleave":
        pl = null;
        break;
      case "mouseover":
      case "mouseout":
        Sl = null;
        break;
      case "pointerover":
      case "pointerout":
        xu.delete(e.pointerId);
        break;
      case "gotpointercapture":
      case "lostpointercapture":
        Ru.delete(e.pointerId);
    }
  }
  function Ou(t, e, l, a, u, n) {
    return t === null || t.nativeEvent !== n
      ? ((t = {
          blockedOn: e,
          domEventName: l,
          eventSystemFlags: a,
          nativeEvent: n,
          targetContainers: [u],
        }),
        e !== null && ((e = Vl(e)), e !== null && Od(e)),
        t)
      : ((t.eventSystemFlags |= a),
        (e = t.targetContainers),
        u !== null && e.indexOf(u) === -1 && e.push(u),
        t);
  }
  function C0(t, e, l, a, u) {
    switch (e) {
      case "focusin":
        return ((gl = Ou(gl, t, e, l, a, u)), !0);
      case "dragenter":
        return ((pl = Ou(pl, t, e, l, a, u)), !0);
      case "mouseover":
        return ((Sl = Ou(Sl, t, e, l, a, u)), !0);
      case "pointerover":
        var n = u.pointerId;
        return (xu.set(n, Ou(xu.get(n) || null, t, e, l, a, u)), !0);
      case "gotpointercapture":
        return (
          (n = u.pointerId),
          Ru.set(n, Ou(Ru.get(n) || null, t, e, l, a, u)),
          !0
        );
    }
    return !1;
  }
  function Dd(t) {
    var e = Zl(t.target);
    if (e !== null) {
      var l = v(e);
      if (l !== null) {
        if (((e = l.tag), e === 13)) {
          if (((e = A(l)), e !== null)) {
            ((t.blockedOn = e),
              Oh(t.priority, function () {
                if (l.tag === 13) {
                  var a = re();
                  a = di(a);
                  var u = aa(l, a);
                  (u !== null && se(u, l, a), gf(l, a));
                }
              }));
            return;
          }
        } else if (e === 3 && l.stateNode.current.memoizedState.isDehydrated) {
          t.blockedOn = l.tag === 3 ? l.stateNode.containerInfo : null;
          return;
        }
      }
    }
    t.blockedOn = null;
  }
  function $n(t) {
    if (t.blockedOn !== null) return !1;
    for (var e = t.targetContainers; 0 < e.length; ) {
      var l = Sf(t.nativeEvent);
      if (l === null) {
        l = t.nativeEvent;
        var a = new l.constructor(l.type, l);
        ((Ti = a), l.target.dispatchEvent(a), (Ti = null));
      } else return ((e = Vl(l)), e !== null && Od(e), (t.blockedOn = l), !1);
      e.shift();
    }
    return !0;
  }
  function _d(t, e, l) {
    $n(t) && l.delete(e);
  }
  function H0() {
    ((Ef = !1),
      gl !== null && $n(gl) && (gl = null),
      pl !== null && $n(pl) && (pl = null),
      Sl !== null && $n(Sl) && (Sl = null),
      xu.forEach(_d),
      Ru.forEach(_d));
  }
  function kn(t, e) {
    t.blockedOn === e &&
      ((t.blockedOn = null),
      Ef ||
        ((Ef = !0),
        i.unstable_scheduleCallback(i.unstable_NormalPriority, H0)));
  }
  var Wn = null;
  function Nd(t) {
    Wn !== t &&
      ((Wn = t),
      i.unstable_scheduleCallback(i.unstable_NormalPriority, function () {
        Wn === t && (Wn = null);
        for (var e = 0; e < t.length; e += 3) {
          var l = t[e],
            a = t[e + 1],
            u = t[e + 2];
          if (typeof a != "function") {
            if (bf(a || l) === null) continue;
            break;
          }
          var n = Vl(l);
          n !== null &&
            (t.splice(e, 3),
            (e -= 3),
            Sc(n, { pending: !0, data: u, method: l.method, action: a }, a, u));
        }
      }));
  }
  function zu(t) {
    function e(m) {
      return kn(m, t);
    }
    (gl !== null && kn(gl, t),
      pl !== null && kn(pl, t),
      Sl !== null && kn(Sl, t),
      xu.forEach(e),
      Ru.forEach(e));
    for (var l = 0; l < bl.length; l++) {
      var a = bl[l];
      a.blockedOn === t && (a.blockedOn = null);
    }
    for (; 0 < bl.length && ((l = bl[0]), l.blockedOn === null); )
      (Dd(l), l.blockedOn === null && bl.shift());
    if (((l = (t.ownerDocument || t).$$reactFormReplay), l != null))
      for (a = 0; a < l.length; a += 3) {
        var u = l[a],
          n = l[a + 1],
          c = u[Wt] || null;
        if (typeof n == "function") c || Nd(l);
        else if (c) {
          var r = null;
          if (n && n.hasAttribute("formAction")) {
            if (((u = n), (c = n[Wt] || null))) r = c.formAction;
            else if (bf(u) !== null) continue;
          } else r = c.action;
          (typeof r == "function" ? (l[a + 1] = r) : (l.splice(a, 3), (a -= 3)),
            Nd(l));
        }
      }
  }
  function Tf(t) {
    this._internalRoot = t;
  }
  ((Fn.prototype.render = Tf.prototype.render =
    function (t) {
      var e = this._internalRoot;
      if (e === null) throw Error(f(409));
      var l = e.current,
        a = re();
      xd(l, a, t, e, null, null);
    }),
    (Fn.prototype.unmount = Tf.prototype.unmount =
      function () {
        var t = this._internalRoot;
        if (t !== null) {
          this._internalRoot = null;
          var e = t.containerInfo;
          (xd(t.current, 2, null, t, null, null), Un(), (e[Ql] = null));
        }
      }));
  function Fn(t) {
    this._internalRoot = t;
  }
  Fn.prototype.unstable_scheduleHydration = function (t) {
    if (t) {
      var e = Jf();
      t = { blockedOn: null, target: t, priority: e };
      for (var l = 0; l < bl.length && e !== 0 && e < bl[l].priority; l++);
      (bl.splice(l, 0, t), l === 0 && Dd(t));
    }
  };
  var Ud = s.version;
  if (Ud !== "19.1.0") throw Error(f(527, Ud, "19.1.0"));
  L.findDOMNode = function (t) {
    var e = t._reactInternals;
    if (e === void 0)
      throw typeof t.render == "function"
        ? Error(f(188))
        : ((t = Object.keys(t).join(",")), Error(f(268, t)));
    return (
      (t = p(e)),
      (t = t !== null ? h(t) : null),
      (t = t === null ? null : t.stateNode),
      t
    );
  };
  var j0 = {
    bundleType: 0,
    version: "19.1.0",
    rendererPackageName: "react-dom",
    currentDispatcherRef: N,
    reconcilerVersion: "19.1.0",
  };
  if (typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ < "u") {
    var Pn = __REACT_DEVTOOLS_GLOBAL_HOOK__;
    if (!Pn.isDisabled && Pn.supportsFiber)
      try {
        ((Na = Pn.inject(j0)), (le = Pn));
      } catch {}
  }
  return (
    (Du.createRoot = function (t, e) {
      if (!d(t)) throw Error(f(299));
      var l = !1,
        a = "",
        u = Js,
        n = $s,
        c = ks,
        r = null;
      return (
        e != null &&
          (e.unstable_strictMode === !0 && (l = !0),
          e.identifierPrefix !== void 0 && (a = e.identifierPrefix),
          e.onUncaughtError !== void 0 && (u = e.onUncaughtError),
          e.onCaughtError !== void 0 && (n = e.onCaughtError),
          e.onRecoverableError !== void 0 && (c = e.onRecoverableError),
          e.unstable_transitionCallbacks !== void 0 &&
            (r = e.unstable_transitionCallbacks)),
        (e = Td(t, 1, !1, null, null, l, a, u, n, c, r, null)),
        (t[Ql] = e.current),
        af(t),
        new Tf(e)
      );
    }),
    (Du.hydrateRoot = function (t, e, l) {
      if (!d(t)) throw Error(f(299));
      var a = !1,
        u = "",
        n = Js,
        c = $s,
        r = ks,
        m = null,
        T = null;
      return (
        l != null &&
          (l.unstable_strictMode === !0 && (a = !0),
          l.identifierPrefix !== void 0 && (u = l.identifierPrefix),
          l.onUncaughtError !== void 0 && (n = l.onUncaughtError),
          l.onCaughtError !== void 0 && (c = l.onCaughtError),
          l.onRecoverableError !== void 0 && (r = l.onRecoverableError),
          l.unstable_transitionCallbacks !== void 0 &&
            (m = l.unstable_transitionCallbacks),
          l.formState !== void 0 && (T = l.formState)),
        (e = Td(t, 1, !0, e, l ?? null, a, u, n, c, r, m, T)),
        (e.context = Ad(null)),
        (l = e.current),
        (a = re()),
        (a = di(a)),
        (u = al(a)),
        (u.callback = null),
        ul(l, u, a),
        (l = a),
        (e.current.lanes = l),
        Ca(e, l),
        _e(e),
        (t[Ql] = e.current),
        af(t),
        new Fn(e)
      );
    }),
    (Du.version = "19.1.0"),
    Du
  );
}
var Xd;
function V0() {
  if (Xd) return Rf.exports;
  Xd = 1;
  function i() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(i);
      } catch (s) {
        console.error(s);
      }
  }
  return (i(), (Rf.exports = Z0()), Rf.exports);
}
var K0 = V0(),
  _u = {},
  Qd;
function J0() {
  if (Qd) return _u;
  ((Qd = 1),
    Object.defineProperty(_u, "__esModule", { value: !0 }),
    (_u.parse = A),
    (_u.serialize = h));
  const i = /^[\u0021-\u003A\u003C\u003E-\u007E]+$/,
    s = /^[\u0021-\u003A\u003C-\u007E]*$/,
    o =
      /^([.]?[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?)([.][a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?)*$/i,
    f = /^[\u0020-\u003A\u003D-\u007E]*$/,
    d = Object.prototype.toString,
    v = (() => {
      const z = function () {};
      return ((z.prototype = Object.create(null)), z);
    })();
  function A(z, G) {
    const M = new v(),
      Y = z.length;
    if (Y < 2) return M;
    const Q = (G == null ? void 0 : G.decode) || O;
    let q = 0;
    do {
      const V = z.indexOf("=", q);
      if (V === -1) break;
      const P = z.indexOf(";", q),
        ct = P === -1 ? Y : P;
      if (V > ct) {
        q = z.lastIndexOf(";", V - 1) + 1;
        continue;
      }
      const I = b(z, q, V),
        _t = p(z, V, I),
        xt = z.slice(I, _t);
      if (M[xt] === void 0) {
        let zt = b(z, V + 1, ct),
          St = p(z, ct, zt);
        const Jt = Q(z.slice(zt, St));
        M[xt] = Jt;
      }
      q = ct + 1;
    } while (q < Y);
    return M;
  }
  function b(z, G, M) {
    do {
      const Y = z.charCodeAt(G);
      if (Y !== 32 && Y !== 9) return G;
    } while (++G < M);
    return M;
  }
  function p(z, G, M) {
    for (; G > M; ) {
      const Y = z.charCodeAt(--G);
      if (Y !== 32 && Y !== 9) return G + 1;
    }
    return M;
  }
  function h(z, G, M) {
    const Y = (M == null ? void 0 : M.encode) || encodeURIComponent;
    if (!i.test(z)) throw new TypeError(`argument name is invalid: ${z}`);
    const Q = Y(G);
    if (!s.test(Q)) throw new TypeError(`argument val is invalid: ${G}`);
    let q = z + "=" + Q;
    if (!M) return q;
    if (M.maxAge !== void 0) {
      if (!Number.isInteger(M.maxAge))
        throw new TypeError(`option maxAge is invalid: ${M.maxAge}`);
      q += "; Max-Age=" + M.maxAge;
    }
    if (M.domain) {
      if (!o.test(M.domain))
        throw new TypeError(`option domain is invalid: ${M.domain}`);
      q += "; Domain=" + M.domain;
    }
    if (M.path) {
      if (!f.test(M.path))
        throw new TypeError(`option path is invalid: ${M.path}`);
      q += "; Path=" + M.path;
    }
    if (M.expires) {
      if (!j(M.expires) || !Number.isFinite(M.expires.valueOf()))
        throw new TypeError(`option expires is invalid: ${M.expires}`);
      q += "; Expires=" + M.expires.toUTCString();
    }
    if (
      (M.httpOnly && (q += "; HttpOnly"),
      M.secure && (q += "; Secure"),
      M.partitioned && (q += "; Partitioned"),
      M.priority)
    )
      switch (
        typeof M.priority == "string" ? M.priority.toLowerCase() : void 0
      ) {
        case "low":
          q += "; Priority=Low";
          break;
        case "medium":
          q += "; Priority=Medium";
          break;
        case "high":
          q += "; Priority=High";
          break;
        default:
          throw new TypeError(`option priority is invalid: ${M.priority}`);
      }
    if (M.sameSite)
      switch (
        typeof M.sameSite == "string" ? M.sameSite.toLowerCase() : M.sameSite
      ) {
        case !0:
        case "strict":
          q += "; SameSite=Strict";
          break;
        case "lax":
          q += "; SameSite=Lax";
          break;
        case "none":
          q += "; SameSite=None";
          break;
        default:
          throw new TypeError(`option sameSite is invalid: ${M.sameSite}`);
      }
    return q;
  }
  function O(z) {
    if (z.indexOf("%") === -1) return z;
    try {
      return decodeURIComponent(z);
    } catch {
      return z;
    }
  }
  function j(z) {
    return d.call(z) === "[object Date]";
  }
  return _u;
}
J0();
var Zd = "popstate";
function $0(i = {}) {
  function s(f, d) {
    let { pathname: v, search: A, hash: b } = f.location;
    return Uf(
      "",
      { pathname: v, search: A, hash: b },
      (d.state && d.state.usr) || null,
      (d.state && d.state.key) || "default",
    );
  }
  function o(f, d) {
    return typeof d == "string" ? d : Uu(d);
  }
  return W0(s, o, null, i);
}
function At(i, s) {
  if (i === !1 || i === null || typeof i > "u") throw new Error(s);
}
function Ne(i, s) {
  if (!i) {
    typeof console < "u" && console.warn(s);
    try {
      throw new Error(s);
    } catch {}
  }
}
function k0() {
  return Math.random().toString(36).substring(2, 10);
}
function Vd(i, s) {
  return { usr: i.state, key: i.key, idx: s };
}
function Uf(i, s, o = null, f) {
  return {
    pathname: typeof i == "string" ? i : i.pathname,
    search: "",
    hash: "",
    ...(typeof s == "string" ? Da(s) : s),
    state: o,
    key: (s && s.key) || f || k0(),
  };
}
function Uu({ pathname: i = "/", search: s = "", hash: o = "" }) {
  return (
    s && s !== "?" && (i += s.charAt(0) === "?" ? s : "?" + s),
    o && o !== "#" && (i += o.charAt(0) === "#" ? o : "#" + o),
    i
  );
}
function Da(i) {
  let s = {};
  if (i) {
    let o = i.indexOf("#");
    o >= 0 && ((s.hash = i.substring(o)), (i = i.substring(0, o)));
    let f = i.indexOf("?");
    (f >= 0 && ((s.search = i.substring(f)), (i = i.substring(0, f))),
      i && (s.pathname = i));
  }
  return s;
}
function W0(i, s, o, f = {}) {
  let { window: d = document.defaultView, v5Compat: v = !1 } = f,
    A = d.history,
    b = "POP",
    p = null,
    h = O();
  h == null && ((h = 0), A.replaceState({ ...A.state, idx: h }, ""));
  function O() {
    return (A.state || { idx: null }).idx;
  }
  function j() {
    b = "POP";
    let Q = O(),
      q = Q == null ? null : Q - h;
    ((h = Q), p && p({ action: b, location: Y.location, delta: q }));
  }
  function z(Q, q) {
    b = "PUSH";
    let V = Uf(Y.location, Q, q);
    h = O() + 1;
    let P = Vd(V, h),
      ct = Y.createHref(V);
    try {
      A.pushState(P, "", ct);
    } catch (I) {
      if (I instanceof DOMException && I.name === "DataCloneError") throw I;
      d.location.assign(ct);
    }
    v && p && p({ action: b, location: Y.location, delta: 1 });
  }
  function G(Q, q) {
    b = "REPLACE";
    let V = Uf(Y.location, Q, q);
    h = O();
    let P = Vd(V, h),
      ct = Y.createHref(V);
    (A.replaceState(P, "", ct),
      v && p && p({ action: b, location: Y.location, delta: 0 }));
  }
  function M(Q) {
    return F0(Q);
  }
  let Y = {
    get action() {
      return b;
    },
    get location() {
      return i(d, A);
    },
    listen(Q) {
      if (p) throw new Error("A history only accepts one active listener");
      return (
        d.addEventListener(Zd, j),
        (p = Q),
        () => {
          (d.removeEventListener(Zd, j), (p = null));
        }
      );
    },
    createHref(Q) {
      return s(d, Q);
    },
    createURL: M,
    encodeLocation(Q) {
      let q = M(Q);
      return { pathname: q.pathname, search: q.search, hash: q.hash };
    },
    push: z,
    replace: G,
    go(Q) {
      return A.go(Q);
    },
  };
  return Y;
}
function F0(i, s = !1) {
  let o = "http://localhost";
  (typeof window < "u" &&
    (o =
      window.location.origin !== "null"
        ? window.location.origin
        : window.location.href),
    At(o, "No window.location.(origin|href) available to create URL"));
  let f = typeof i == "string" ? i : Uu(i);
  return (
    (f = f.replace(/ $/, "%20")),
    !s && f.startsWith("//") && (f = o + f),
    new URL(f, o)
  );
}
function Fd(i, s, o = "/") {
  return P0(i, s, o, !1);
}
function P0(i, s, o, f) {
  let d = typeof s == "string" ? Da(s) : s,
    v = ke(d.pathname || "/", o);
  if (v == null) return null;
  let A = Pd(i);
  I0(A);
  let b = null;
  for (let p = 0; b == null && p < A.length; ++p) {
    let h = sv(v);
    b = fv(A[p], h, f);
  }
  return b;
}
function Pd(i, s = [], o = [], f = "") {
  let d = (v, A, b) => {
    let p = {
      relativePath: b === void 0 ? v.path || "" : b,
      caseSensitive: v.caseSensitive === !0,
      childrenIndex: A,
      route: v,
    };
    p.relativePath.startsWith("/") &&
      (At(
        p.relativePath.startsWith(f),
        `Absolute route path "${p.relativePath}" nested under path "${f}" is not valid. An absolute child route path must start with the combined path of all its parent routes.`,
      ),
      (p.relativePath = p.relativePath.slice(f.length)));
    let h = $e([f, p.relativePath]),
      O = o.concat(p);
    (v.children &&
      v.children.length > 0 &&
      (At(
        v.index !== !0,
        `Index routes must not have child routes. Please remove all child routes from route path "${h}".`,
      ),
      Pd(v.children, s, O, h)),
      !(v.path == null && !v.index) &&
        s.push({ path: h, score: iv(h, v.index), routesMeta: O }));
  };
  return (
    i.forEach((v, A) => {
      var b;
      if (v.path === "" || !((b = v.path) != null && b.includes("?"))) d(v, A);
      else for (let p of Id(v.path)) d(v, A, p);
    }),
    s
  );
}
function Id(i) {
  let s = i.split("/");
  if (s.length === 0) return [];
  let [o, ...f] = s,
    d = o.endsWith("?"),
    v = o.replace(/\?$/, "");
  if (f.length === 0) return d ? [v, ""] : [v];
  let A = Id(f.join("/")),
    b = [];
  return (
    b.push(...A.map((p) => (p === "" ? v : [v, p].join("/")))),
    d && b.push(...A),
    b.map((p) => (i.startsWith("/") && p === "" ? "/" : p))
  );
}
function I0(i) {
  i.sort((s, o) =>
    s.score !== o.score
      ? o.score - s.score
      : cv(
          s.routesMeta.map((f) => f.childrenIndex),
          o.routesMeta.map((f) => f.childrenIndex),
        ),
  );
}
var tv = /^:[\w-]+$/,
  ev = 3,
  lv = 2,
  av = 1,
  uv = 10,
  nv = -2,
  Kd = (i) => i === "*";
function iv(i, s) {
  let o = i.split("/"),
    f = o.length;
  return (
    o.some(Kd) && (f += nv),
    s && (f += lv),
    o
      .filter((d) => !Kd(d))
      .reduce((d, v) => d + (tv.test(v) ? ev : v === "" ? av : uv), f)
  );
}
function cv(i, s) {
  return i.length === s.length && i.slice(0, -1).every((f, d) => f === s[d])
    ? i[i.length - 1] - s[s.length - 1]
    : 0;
}
function fv(i, s, o = !1) {
  let { routesMeta: f } = i,
    d = {},
    v = "/",
    A = [];
  for (let b = 0; b < f.length; ++b) {
    let p = f[b],
      h = b === f.length - 1,
      O = v === "/" ? s : s.slice(v.length) || "/",
      j = li(
        { path: p.relativePath, caseSensitive: p.caseSensitive, end: h },
        O,
      ),
      z = p.route;
    if (
      (!j &&
        h &&
        o &&
        !f[f.length - 1].route.index &&
        (j = li(
          { path: p.relativePath, caseSensitive: p.caseSensitive, end: !1 },
          O,
        )),
      !j)
    )
      return null;
    (Object.assign(d, j.params),
      A.push({
        params: d,
        pathname: $e([v, j.pathname]),
        pathnameBase: mv($e([v, j.pathnameBase])),
        route: z,
      }),
      j.pathnameBase !== "/" && (v = $e([v, j.pathnameBase])));
  }
  return A;
}
function li(i, s) {
  typeof i == "string" && (i = { path: i, caseSensitive: !1, end: !0 });
  let [o, f] = rv(i.path, i.caseSensitive, i.end),
    d = s.match(o);
  if (!d) return null;
  let v = d[0],
    A = v.replace(/(.)\/+$/, "$1"),
    b = d.slice(1);
  return {
    params: f.reduce((h, { paramName: O, isOptional: j }, z) => {
      if (O === "*") {
        let M = b[z] || "";
        A = v.slice(0, v.length - M.length).replace(/(.)\/+$/, "$1");
      }
      const G = b[z];
      return (
        j && !G ? (h[O] = void 0) : (h[O] = (G || "").replace(/%2F/g, "/")),
        h
      );
    }, {}),
    pathname: v,
    pathnameBase: A,
    pattern: i,
  };
}
function rv(i, s = !1, o = !0) {
  Ne(
    i === "*" || !i.endsWith("*") || i.endsWith("/*"),
    `Route path "${i}" will be treated as if it were "${i.replace(/\*$/, "/*")}" because the \`*\` character must always follow a \`/\` in the pattern. To get rid of this warning, please change the route path to "${i.replace(/\*$/, "/*")}".`,
  );
  let f = [],
    d =
      "^" +
      i
        .replace(/\/*\*?$/, "")
        .replace(/^\/*/, "/")
        .replace(/[\\.*+^${}|()[\]]/g, "\\$&")
        .replace(
          /\/:([\w-]+)(\?)?/g,
          (A, b, p) => (
            f.push({ paramName: b, isOptional: p != null }),
            p ? "/?([^\\/]+)?" : "/([^\\/]+)"
          ),
        );
  return (
    i.endsWith("*")
      ? (f.push({ paramName: "*" }),
        (d += i === "*" || i === "/*" ? "(.*)$" : "(?:\\/(.+)|\\/*)$"))
      : o
        ? (d += "\\/*$")
        : i !== "" && i !== "/" && (d += "(?:(?=\\/|$))"),
    [new RegExp(d, s ? void 0 : "i"), f]
  );
}
function sv(i) {
  try {
    return i
      .split("/")
      .map((s) => decodeURIComponent(s).replace(/\//g, "%2F"))
      .join("/");
  } catch (s) {
    return (
      Ne(
        !1,
        `The URL path "${i}" could not be decoded because it is a malformed URL segment. This is probably due to a bad percent encoding (${s}).`,
      ),
      i
    );
  }
}
function ke(i, s) {
  if (s === "/") return i;
  if (!i.toLowerCase().startsWith(s.toLowerCase())) return null;
  let o = s.endsWith("/") ? s.length - 1 : s.length,
    f = i.charAt(o);
  return f && f !== "/" ? null : i.slice(o) || "/";
}
function ov(i, s = "/") {
  let {
    pathname: o,
    search: f = "",
    hash: d = "",
  } = typeof i == "string" ? Da(i) : i;
  return {
    pathname: o ? (o.startsWith("/") ? o : dv(o, s)) : s,
    search: vv(f),
    hash: yv(d),
  };
}
function dv(i, s) {
  let o = s.replace(/\/+$/, "").split("/");
  return (
    i.split("/").forEach((d) => {
      d === ".." ? o.length > 1 && o.pop() : d !== "." && o.push(d);
    }),
    o.length > 1 ? o.join("/") : "/"
  );
}
function Df(i, s, o, f) {
  return `Cannot include a '${i}' character in a manually specified \`to.${s}\` field [${JSON.stringify(f)}].  Please separate it out to the \`to.${o}\` field. Alternatively you may provide the full path as a string in <Link to="..."> and the router will parse it for you.`;
}
function hv(i) {
  return i.filter(
    (s, o) => o === 0 || (s.route.path && s.route.path.length > 0),
  );
}
function th(i) {
  let s = hv(i);
  return s.map((o, f) => (f === s.length - 1 ? o.pathname : o.pathnameBase));
}
function eh(i, s, o, f = !1) {
  let d;
  typeof i == "string"
    ? (d = Da(i))
    : ((d = { ...i }),
      At(
        !d.pathname || !d.pathname.includes("?"),
        Df("?", "pathname", "search", d),
      ),
      At(
        !d.pathname || !d.pathname.includes("#"),
        Df("#", "pathname", "hash", d),
      ),
      At(!d.search || !d.search.includes("#"), Df("#", "search", "hash", d)));
  let v = i === "" || d.pathname === "",
    A = v ? "/" : d.pathname,
    b;
  if (A == null) b = o;
  else {
    let j = s.length - 1;
    if (!f && A.startsWith("..")) {
      let z = A.split("/");
      for (; z[0] === ".."; ) (z.shift(), (j -= 1));
      d.pathname = z.join("/");
    }
    b = j >= 0 ? s[j] : "/";
  }
  let p = ov(d, b),
    h = A && A !== "/" && A.endsWith("/"),
    O = (v || A === ".") && o.endsWith("/");
  return (!p.pathname.endsWith("/") && (h || O) && (p.pathname += "/"), p);
}
var $e = (i) => i.join("/").replace(/\/\/+/g, "/"),
  mv = (i) => i.replace(/\/+$/, "").replace(/^\/*/, "/"),
  vv = (i) => (!i || i === "?" ? "" : i.startsWith("?") ? i : "?" + i),
  yv = (i) => (!i || i === "#" ? "" : i.startsWith("#") ? i : "#" + i);
function gv(i) {
  return (
    i != null &&
    typeof i.status == "number" &&
    typeof i.statusText == "string" &&
    typeof i.internal == "boolean" &&
    "data" in i
  );
}
var lh = ["POST", "PUT", "PATCH", "DELETE"];
new Set(lh);
var pv = ["GET", ...lh];
new Set(pv);
var _a = D.createContext(null);
_a.displayName = "DataRouter";
var ni = D.createContext(null);
ni.displayName = "DataRouterState";
var ah = D.createContext({ isTransitioning: !1 });
ah.displayName = "ViewTransition";
var Sv = D.createContext(new Map());
Sv.displayName = "Fetchers";
var bv = D.createContext(null);
bv.displayName = "Await";
var Ue = D.createContext(null);
Ue.displayName = "Navigation";
var Hu = D.createContext(null);
Hu.displayName = "Location";
var We = D.createContext({ outlet: null, matches: [], isDataRoute: !1 });
We.displayName = "Route";
var jf = D.createContext(null);
jf.displayName = "RouteError";
function Ev(i, { relative: s } = {}) {
  At(
    ju(),
    "useHref() may be used only in the context of a <Router> component.",
  );
  let { basename: o, navigator: f } = D.useContext(Ue),
    { hash: d, pathname: v, search: A } = Bu(i, { relative: s }),
    b = v;
  return (
    o !== "/" && (b = v === "/" ? o : $e([o, v])),
    f.createHref({ pathname: b, search: A, hash: d })
  );
}
function ju() {
  return D.useContext(Hu) != null;
}
function Xl() {
  return (
    At(
      ju(),
      "useLocation() may be used only in the context of a <Router> component.",
    ),
    D.useContext(Hu).location
  );
}
var uh =
  "You should call navigate() in a React.useEffect(), not when your component is first rendered.";
function nh(i) {
  D.useContext(Ue).static || D.useLayoutEffect(i);
}
function Tv() {
  let { isDataRoute: i } = D.useContext(We);
  return i ? jv() : Av();
}
function Av() {
  At(
    ju(),
    "useNavigate() may be used only in the context of a <Router> component.",
  );
  let i = D.useContext(_a),
    { basename: s, navigator: o } = D.useContext(Ue),
    { matches: f } = D.useContext(We),
    { pathname: d } = Xl(),
    v = JSON.stringify(th(f)),
    A = D.useRef(!1);
  return (
    nh(() => {
      A.current = !0;
    }),
    D.useCallback(
      (p, h = {}) => {
        if ((Ne(A.current, uh), !A.current)) return;
        if (typeof p == "number") {
          o.go(p);
          return;
        }
        let O = eh(p, JSON.parse(v), d, h.relative === "path");
        (i == null &&
          s !== "/" &&
          (O.pathname = O.pathname === "/" ? s : $e([s, O.pathname])),
          (h.replace ? o.replace : o.push)(O, h.state, h));
      },
      [s, o, v, d, i],
    )
  );
}
D.createContext(null);
function Bu(i, { relative: s } = {}) {
  let { matches: o } = D.useContext(We),
    { pathname: f } = Xl(),
    d = JSON.stringify(th(o));
  return D.useMemo(() => eh(i, JSON.parse(d), f, s === "path"), [i, d, f, s]);
}
function xv(i, s) {
  return ih(i, s);
}
function ih(i, s, o, f) {
  var q;
  At(
    ju(),
    "useRoutes() may be used only in the context of a <Router> component.",
  );
  let { navigator: d } = D.useContext(Ue),
    { matches: v } = D.useContext(We),
    A = v[v.length - 1],
    b = A ? A.params : {},
    p = A ? A.pathname : "/",
    h = A ? A.pathnameBase : "/",
    O = A && A.route;
  {
    let V = (O && O.path) || "";
    ch(
      p,
      !O || V.endsWith("*") || V.endsWith("*?"),
      `You rendered descendant <Routes> (or called \`useRoutes()\`) at "${p}" (under <Route path="${V}">) but the parent route path has no trailing "*". This means if you navigate deeper, the parent won't match anymore and therefore the child routes will never render.

Please change the parent <Route path="${V}"> to <Route path="${V === "/" ? "*" : `${V}/*`}">.`,
    );
  }
  let j = Xl(),
    z;
  if (s) {
    let V = typeof s == "string" ? Da(s) : s;
    (At(
      h === "/" || ((q = V.pathname) == null ? void 0 : q.startsWith(h)),
      `When overriding the location using \`<Routes location>\` or \`useRoutes(routes, location)\`, the location pathname must begin with the portion of the URL pathname that was matched by all parent routes. The current pathname base is "${h}" but pathname "${V.pathname}" was given in the \`location\` prop.`,
    ),
      (z = V));
  } else z = j;
  let G = z.pathname || "/",
    M = G;
  if (h !== "/") {
    let V = h.replace(/^\//, "").split("/");
    M = "/" + G.replace(/^\//, "").split("/").slice(V.length).join("/");
  }
  let Y = Fd(i, { pathname: M });
  (Ne(
    O || Y != null,
    `No routes matched location "${z.pathname}${z.search}${z.hash}" `,
  ),
    Ne(
      Y == null ||
        Y[Y.length - 1].route.element !== void 0 ||
        Y[Y.length - 1].route.Component !== void 0 ||
        Y[Y.length - 1].route.lazy !== void 0,
      `Matched leaf route at location "${z.pathname}${z.search}${z.hash}" does not have an element or Component. This means it will render an <Outlet /> with a null value by default resulting in an "empty" page.`,
    ));
  let Q = Dv(
    Y &&
      Y.map((V) =>
        Object.assign({}, V, {
          params: Object.assign({}, b, V.params),
          pathname: $e([
            h,
            d.encodeLocation
              ? d.encodeLocation(V.pathname).pathname
              : V.pathname,
          ]),
          pathnameBase:
            V.pathnameBase === "/"
              ? h
              : $e([
                  h,
                  d.encodeLocation
                    ? d.encodeLocation(V.pathnameBase).pathname
                    : V.pathnameBase,
                ]),
        }),
      ),
    v,
    o,
    f,
  );
  return s && Q
    ? D.createElement(
        Hu.Provider,
        {
          value: {
            location: {
              pathname: "/",
              search: "",
              hash: "",
              state: null,
              key: "default",
              ...z,
            },
            navigationType: "POP",
          },
        },
        Q,
      )
    : Q;
}
function Rv() {
  let i = Hv(),
    s = gv(i)
      ? `${i.status} ${i.statusText}`
      : i instanceof Error
        ? i.message
        : JSON.stringify(i),
    o = i instanceof Error ? i.stack : null,
    f = "rgba(200,200,200, 0.5)",
    d = { padding: "0.5rem", backgroundColor: f },
    v = { padding: "2px 4px", backgroundColor: f },
    A = null;
  return (
    console.error("Error handled by React Router default ErrorBoundary:", i),
    (A = D.createElement(
      D.Fragment,
      null,
      D.createElement("p", null, "💿 Hey developer 👋"),
      D.createElement(
        "p",
        null,
        "You can provide a way better UX than this when your app throws errors by providing your own ",
        D.createElement("code", { style: v }, "ErrorBoundary"),
        " or",
        " ",
        D.createElement("code", { style: v }, "errorElement"),
        " prop on your route.",
      ),
    )),
    D.createElement(
      D.Fragment,
      null,
      D.createElement("h2", null, "Unexpected Application Error!"),
      D.createElement("h3", { style: { fontStyle: "italic" } }, s),
      o ? D.createElement("pre", { style: d }, o) : null,
      A,
    )
  );
}
var Ov = D.createElement(Rv, null),
  zv = class extends D.Component {
    constructor(i) {
      (super(i),
        (this.state = {
          location: i.location,
          revalidation: i.revalidation,
          error: i.error,
        }));
    }
    static getDerivedStateFromError(i) {
      return { error: i };
    }
    static getDerivedStateFromProps(i, s) {
      return s.location !== i.location ||
        (s.revalidation !== "idle" && i.revalidation === "idle")
        ? { error: i.error, location: i.location, revalidation: i.revalidation }
        : {
            error: i.error !== void 0 ? i.error : s.error,
            location: s.location,
            revalidation: i.revalidation || s.revalidation,
          };
    }
    componentDidCatch(i, s) {
      console.error(
        "React Router caught the following error during render",
        i,
        s,
      );
    }
    render() {
      return this.state.error !== void 0
        ? D.createElement(
            We.Provider,
            { value: this.props.routeContext },
            D.createElement(jf.Provider, {
              value: this.state.error,
              children: this.props.component,
            }),
          )
        : this.props.children;
    }
  };
function Mv({ routeContext: i, match: s, children: o }) {
  let f = D.useContext(_a);
  return (
    f &&
      f.static &&
      f.staticContext &&
      (s.route.errorElement || s.route.ErrorBoundary) &&
      (f.staticContext._deepestRenderedBoundaryId = s.route.id),
    D.createElement(We.Provider, { value: i }, o)
  );
}
function Dv(i, s = [], o = null, f = null) {
  if (i == null) {
    if (!o) return null;
    if (o.errors) i = o.matches;
    else if (s.length === 0 && !o.initialized && o.matches.length > 0)
      i = o.matches;
    else return null;
  }
  let d = i,
    v = o == null ? void 0 : o.errors;
  if (v != null) {
    let p = d.findIndex(
      (h) => h.route.id && (v == null ? void 0 : v[h.route.id]) !== void 0,
    );
    (At(
      p >= 0,
      `Could not find a matching route for errors on route IDs: ${Object.keys(v).join(",")}`,
    ),
      (d = d.slice(0, Math.min(d.length, p + 1))));
  }
  let A = !1,
    b = -1;
  if (o)
    for (let p = 0; p < d.length; p++) {
      let h = d[p];
      if (
        ((h.route.HydrateFallback || h.route.hydrateFallbackElement) && (b = p),
        h.route.id)
      ) {
        let { loaderData: O, errors: j } = o,
          z =
            h.route.loader &&
            !O.hasOwnProperty(h.route.id) &&
            (!j || j[h.route.id] === void 0);
        if (h.route.lazy || z) {
          ((A = !0), b >= 0 ? (d = d.slice(0, b + 1)) : (d = [d[0]]));
          break;
        }
      }
    }
  return d.reduceRight((p, h, O) => {
    let j,
      z = !1,
      G = null,
      M = null;
    o &&
      ((j = v && h.route.id ? v[h.route.id] : void 0),
      (G = h.route.errorElement || Ov),
      A &&
        (b < 0 && O === 0
          ? (ch(
              "route-fallback",
              !1,
              "No `HydrateFallback` element provided to render during initial hydration",
            ),
            (z = !0),
            (M = null))
          : b === O &&
            ((z = !0), (M = h.route.hydrateFallbackElement || null))));
    let Y = s.concat(d.slice(0, O + 1)),
      Q = () => {
        let q;
        return (
          j
            ? (q = G)
            : z
              ? (q = M)
              : h.route.Component
                ? (q = D.createElement(h.route.Component, null))
                : h.route.element
                  ? (q = h.route.element)
                  : (q = p),
          D.createElement(Mv, {
            match: h,
            routeContext: { outlet: p, matches: Y, isDataRoute: o != null },
            children: q,
          })
        );
      };
    return o && (h.route.ErrorBoundary || h.route.errorElement || O === 0)
      ? D.createElement(zv, {
          location: o.location,
          revalidation: o.revalidation,
          component: G,
          error: j,
          children: Q(),
          routeContext: { outlet: null, matches: Y, isDataRoute: !0 },
        })
      : Q();
  }, null);
}
function Bf(i) {
  return `${i} must be used within a data router.  See https://reactrouter.com/en/main/routers/picking-a-router.`;
}
function _v(i) {
  let s = D.useContext(_a);
  return (At(s, Bf(i)), s);
}
function Nv(i) {
  let s = D.useContext(ni);
  return (At(s, Bf(i)), s);
}
function Uv(i) {
  let s = D.useContext(We);
  return (At(s, Bf(i)), s);
}
function qf(i) {
  let s = Uv(i),
    o = s.matches[s.matches.length - 1];
  return (
    At(
      o.route.id,
      `${i} can only be used on routes that contain a unique "id"`,
    ),
    o.route.id
  );
}
function Cv() {
  return qf("useRouteId");
}
function Hv() {
  var f;
  let i = D.useContext(jf),
    s = Nv("useRouteError"),
    o = qf("useRouteError");
  return i !== void 0 ? i : (f = s.errors) == null ? void 0 : f[o];
}
function jv() {
  let { router: i } = _v("useNavigate"),
    s = qf("useNavigate"),
    o = D.useRef(!1);
  return (
    nh(() => {
      o.current = !0;
    }),
    D.useCallback(
      async (d, v = {}) => {
        (Ne(o.current, uh),
          o.current &&
            (typeof d == "number"
              ? i.navigate(d)
              : await i.navigate(d, { fromRouteId: s, ...v })));
      },
      [i, s],
    )
  );
}
var Jd = {};
function ch(i, s, o) {
  !s && !Jd[i] && ((Jd[i] = !0), Ne(!1, o));
}
D.memo(Bv);
function Bv({ routes: i, future: s, state: o }) {
  return ih(i, void 0, o, s);
}
function Ma(i) {
  At(
    !1,
    "A <Route> is only ever to be used as the child of <Routes> element, never rendered directly. Please wrap your <Route> in a <Routes>.",
  );
}
function qv({
  basename: i = "/",
  children: s = null,
  location: o,
  navigationType: f = "POP",
  navigator: d,
  static: v = !1,
}) {
  At(
    !ju(),
    "You cannot render a <Router> inside another <Router>. You should never have more than one in your app.",
  );
  let A = i.replace(/^\/*/, "/"),
    b = D.useMemo(
      () => ({ basename: A, navigator: d, static: v, future: {} }),
      [A, d, v],
    );
  typeof o == "string" && (o = Da(o));
  let {
      pathname: p = "/",
      search: h = "",
      hash: O = "",
      state: j = null,
      key: z = "default",
    } = o,
    G = D.useMemo(() => {
      let M = ke(p, A);
      return M == null
        ? null
        : {
            location: { pathname: M, search: h, hash: O, state: j, key: z },
            navigationType: f,
          };
    }, [A, p, h, O, j, z, f]);
  return (
    Ne(
      G != null,
      `<Router basename="${A}"> is not able to match the URL "${p}${h}${O}" because it does not start with the basename, so the <Router> won't render anything.`,
    ),
    G == null
      ? null
      : D.createElement(
          Ue.Provider,
          { value: b },
          D.createElement(Hu.Provider, { children: s, value: G }),
        )
  );
}
function Yv({ children: i, location: s }) {
  return xv(Cf(i), s);
}
function Cf(i, s = []) {
  let o = [];
  return (
    D.Children.forEach(i, (f, d) => {
      if (!D.isValidElement(f)) return;
      let v = [...s, d];
      if (f.type === D.Fragment) {
        o.push.apply(o, Cf(f.props.children, v));
        return;
      }
      (At(
        f.type === Ma,
        `[${typeof f.type == "string" ? f.type : f.type.name}] is not a <Route> component. All component children of <Routes> must be a <Route> or <React.Fragment>`,
      ),
        At(
          !f.props.index || !f.props.children,
          "An index route cannot have child routes.",
        ));
      let A = {
        id: f.props.id || v.join("-"),
        caseSensitive: f.props.caseSensitive,
        element: f.props.element,
        Component: f.props.Component,
        index: f.props.index,
        path: f.props.path,
        loader: f.props.loader,
        action: f.props.action,
        hydrateFallbackElement: f.props.hydrateFallbackElement,
        HydrateFallback: f.props.HydrateFallback,
        errorElement: f.props.errorElement,
        ErrorBoundary: f.props.ErrorBoundary,
        hasErrorBoundary:
          f.props.hasErrorBoundary === !0 ||
          f.props.ErrorBoundary != null ||
          f.props.errorElement != null,
        shouldRevalidate: f.props.shouldRevalidate,
        handle: f.props.handle,
        lazy: f.props.lazy,
      };
      (f.props.children && (A.children = Cf(f.props.children, v)), o.push(A));
    }),
    o
  );
}
var ti = "get",
  ei = "application/x-www-form-urlencoded";
function ii(i) {
  return i != null && typeof i.tagName == "string";
}
function wv(i) {
  return ii(i) && i.tagName.toLowerCase() === "button";
}
function Lv(i) {
  return ii(i) && i.tagName.toLowerCase() === "form";
}
function Gv(i) {
  return ii(i) && i.tagName.toLowerCase() === "input";
}
function Xv(i) {
  return !!(i.metaKey || i.altKey || i.ctrlKey || i.shiftKey);
}
function Qv(i, s) {
  return i.button === 0 && (!s || s === "_self") && !Xv(i);
}
var In = null;
function Zv() {
  if (In === null)
    try {
      (new FormData(document.createElement("form"), 0), (In = !1));
    } catch {
      In = !0;
    }
  return In;
}
var Vv = new Set([
  "application/x-www-form-urlencoded",
  "multipart/form-data",
  "text/plain",
]);
function _f(i) {
  return i != null && !Vv.has(i)
    ? (Ne(
        !1,
        `"${i}" is not a valid \`encType\` for \`<Form>\`/\`<fetcher.Form>\` and will default to "${ei}"`,
      ),
      null)
    : i;
}
function Kv(i, s) {
  let o, f, d, v, A;
  if (Lv(i)) {
    let b = i.getAttribute("action");
    ((f = b ? ke(b, s) : null),
      (o = i.getAttribute("method") || ti),
      (d = _f(i.getAttribute("enctype")) || ei),
      (v = new FormData(i)));
  } else if (wv(i) || (Gv(i) && (i.type === "submit" || i.type === "image"))) {
    let b = i.form;
    if (b == null)
      throw new Error(
        'Cannot submit a <button> or <input type="submit"> without a <form>',
      );
    let p = i.getAttribute("formaction") || b.getAttribute("action");
    if (
      ((f = p ? ke(p, s) : null),
      (o = i.getAttribute("formmethod") || b.getAttribute("method") || ti),
      (d =
        _f(i.getAttribute("formenctype")) ||
        _f(b.getAttribute("enctype")) ||
        ei),
      (v = new FormData(b, i)),
      !Zv())
    ) {
      let { name: h, type: O, value: j } = i;
      if (O === "image") {
        let z = h ? `${h}.` : "";
        (v.append(`${z}x`, "0"), v.append(`${z}y`, "0"));
      } else h && v.append(h, j);
    }
  } else {
    if (ii(i))
      throw new Error(
        'Cannot submit element that is not <form>, <button>, or <input type="submit|image">',
      );
    ((o = ti), (f = null), (d = ei), (A = i));
  }
  return (
    v && d === "text/plain" && ((A = v), (v = void 0)),
    { action: f, method: o.toLowerCase(), encType: d, formData: v, body: A }
  );
}
function Yf(i, s) {
  if (i === !1 || i === null || typeof i > "u") throw new Error(s);
}
async function Jv(i, s) {
  if (i.id in s) return s[i.id];
  try {
    let o = await import(i.module);
    return ((s[i.id] = o), o);
  } catch (o) {
    return (
      console.error(
        `Error loading route module \`${i.module}\`, reloading page...`,
      ),
      console.error(o),
      window.__reactRouterContext && window.__reactRouterContext.isSpaMode,
      window.location.reload(),
      new Promise(() => {})
    );
  }
}
function $v(i) {
  return i == null
    ? !1
    : i.href == null
      ? i.rel === "preload" &&
        typeof i.imageSrcSet == "string" &&
        typeof i.imageSizes == "string"
      : typeof i.rel == "string" && typeof i.href == "string";
}
async function kv(i, s, o) {
  let f = await Promise.all(
    i.map(async (d) => {
      let v = s.routes[d.route.id];
      if (v) {
        let A = await Jv(v, o);
        return A.links ? A.links() : [];
      }
      return [];
    }),
  );
  return Iv(
    f
      .flat(1)
      .filter($v)
      .filter((d) => d.rel === "stylesheet" || d.rel === "preload")
      .map((d) =>
        d.rel === "stylesheet"
          ? { ...d, rel: "prefetch", as: "style" }
          : { ...d, rel: "prefetch" },
      ),
  );
}
function $d(i, s, o, f, d, v) {
  let A = (p, h) => (o[h] ? p.route.id !== o[h].route.id : !0),
    b = (p, h) => {
      var O;
      return (
        o[h].pathname !== p.pathname ||
        (((O = o[h].route.path) == null ? void 0 : O.endsWith("*")) &&
          o[h].params["*"] !== p.params["*"])
      );
    };
  return v === "assets"
    ? s.filter((p, h) => A(p, h) || b(p, h))
    : v === "data"
      ? s.filter((p, h) => {
          var j;
          let O = f.routes[p.route.id];
          if (!O || !O.hasLoader) return !1;
          if (A(p, h) || b(p, h)) return !0;
          if (p.route.shouldRevalidate) {
            let z = p.route.shouldRevalidate({
              currentUrl: new URL(
                d.pathname + d.search + d.hash,
                window.origin,
              ),
              currentParams: ((j = o[0]) == null ? void 0 : j.params) || {},
              nextUrl: new URL(i, window.origin),
              nextParams: p.params,
              defaultShouldRevalidate: !0,
            });
            if (typeof z == "boolean") return z;
          }
          return !0;
        })
      : [];
}
function Wv(i, s, { includeHydrateFallback: o } = {}) {
  return Fv(
    i
      .map((f) => {
        let d = s.routes[f.route.id];
        if (!d) return [];
        let v = [d.module];
        return (
          d.clientActionModule && (v = v.concat(d.clientActionModule)),
          d.clientLoaderModule && (v = v.concat(d.clientLoaderModule)),
          o &&
            d.hydrateFallbackModule &&
            (v = v.concat(d.hydrateFallbackModule)),
          d.imports && (v = v.concat(d.imports)),
          v
        );
      })
      .flat(1),
  );
}
function Fv(i) {
  return [...new Set(i)];
}
function Pv(i) {
  let s = {},
    o = Object.keys(i).sort();
  for (let f of o) s[f] = i[f];
  return s;
}
function Iv(i, s) {
  let o = new Set();
  return (
    new Set(s),
    i.reduce((f, d) => {
      let v = JSON.stringify(Pv(d));
      return (o.has(v) || (o.add(v), f.push({ key: v, link: d })), f);
    }, [])
  );
}
Object.getOwnPropertyNames(Object.prototype).sort().join("\0");
var ty = new Set([100, 101, 204, 205]);
function ey(i, s) {
  let o =
    typeof i == "string"
      ? new URL(
          i,
          typeof window > "u"
            ? "server://singlefetch/"
            : window.location.origin,
        )
      : i;
  return (
    o.pathname === "/"
      ? (o.pathname = "_root.data")
      : s && ke(o.pathname, s) === "/"
        ? (o.pathname = `${s.replace(/\/$/, "")}/_root.data`)
        : (o.pathname = `${o.pathname.replace(/\/$/, "")}.data`),
    o
  );
}
function fh() {
  let i = D.useContext(_a);
  return (
    Yf(
      i,
      "You must render this element inside a <DataRouterContext.Provider> element",
    ),
    i
  );
}
function ly() {
  let i = D.useContext(ni);
  return (
    Yf(
      i,
      "You must render this element inside a <DataRouterStateContext.Provider> element",
    ),
    i
  );
}
var wf = D.createContext(void 0);
wf.displayName = "FrameworkContext";
function rh() {
  let i = D.useContext(wf);
  return (
    Yf(i, "You must render this element inside a <HydratedRouter> element"),
    i
  );
}
function ay(i, s) {
  let o = D.useContext(wf),
    [f, d] = D.useState(!1),
    [v, A] = D.useState(!1),
    {
      onFocus: b,
      onBlur: p,
      onMouseEnter: h,
      onMouseLeave: O,
      onTouchStart: j,
    } = s,
    z = D.useRef(null);
  (D.useEffect(() => {
    if ((i === "render" && A(!0), i === "viewport")) {
      let Y = (q) => {
          q.forEach((V) => {
            A(V.isIntersecting);
          });
        },
        Q = new IntersectionObserver(Y, { threshold: 0.5 });
      return (
        z.current && Q.observe(z.current),
        () => {
          Q.disconnect();
        }
      );
    }
  }, [i]),
    D.useEffect(() => {
      if (f) {
        let Y = setTimeout(() => {
          A(!0);
        }, 100);
        return () => {
          clearTimeout(Y);
        };
      }
    }, [f]));
  let G = () => {
      d(!0);
    },
    M = () => {
      (d(!1), A(!1));
    };
  return o
    ? i !== "intent"
      ? [v, z, {}]
      : [
          v,
          z,
          {
            onFocus: Nu(b, G),
            onBlur: Nu(p, M),
            onMouseEnter: Nu(h, G),
            onMouseLeave: Nu(O, M),
            onTouchStart: Nu(j, G),
          },
        ]
    : [!1, z, {}];
}
function Nu(i, s) {
  return (o) => {
    (i && i(o), o.defaultPrevented || s(o));
  };
}
function uy({ page: i, ...s }) {
  let { router: o } = fh(),
    f = D.useMemo(() => Fd(o.routes, i, o.basename), [o.routes, i, o.basename]);
  return f ? D.createElement(iy, { page: i, matches: f, ...s }) : null;
}
function ny(i) {
  let { manifest: s, routeModules: o } = rh(),
    [f, d] = D.useState([]);
  return (
    D.useEffect(() => {
      let v = !1;
      return (
        kv(i, s, o).then((A) => {
          v || d(A);
        }),
        () => {
          v = !0;
        }
      );
    }, [i, s, o]),
    f
  );
}
function iy({ page: i, matches: s, ...o }) {
  let f = Xl(),
    { manifest: d, routeModules: v } = rh(),
    { basename: A } = fh(),
    { loaderData: b, matches: p } = ly(),
    h = D.useMemo(() => $d(i, s, p, d, f, "data"), [i, s, p, d, f]),
    O = D.useMemo(() => $d(i, s, p, d, f, "assets"), [i, s, p, d, f]),
    j = D.useMemo(() => {
      if (i === f.pathname + f.search + f.hash) return [];
      let M = new Set(),
        Y = !1;
      if (
        (s.forEach((q) => {
          var P;
          let V = d.routes[q.route.id];
          !V ||
            !V.hasLoader ||
            ((!h.some((ct) => ct.route.id === q.route.id) &&
              q.route.id in b &&
              (P = v[q.route.id]) != null &&
              P.shouldRevalidate) ||
            V.hasClientLoader
              ? (Y = !0)
              : M.add(q.route.id));
        }),
        M.size === 0)
      )
        return [];
      let Q = ey(i, A);
      return (
        Y &&
          M.size > 0 &&
          Q.searchParams.set(
            "_routes",
            s
              .filter((q) => M.has(q.route.id))
              .map((q) => q.route.id)
              .join(","),
          ),
        [Q.pathname + Q.search]
      );
    }, [A, b, f, d, h, s, i, v]),
    z = D.useMemo(() => Wv(O, d), [O, d]),
    G = ny(O);
  return D.createElement(
    D.Fragment,
    null,
    j.map((M) =>
      D.createElement("link", {
        key: M,
        rel: "prefetch",
        as: "fetch",
        href: M,
        ...o,
      }),
    ),
    z.map((M) =>
      D.createElement("link", { key: M, rel: "modulepreload", href: M, ...o }),
    ),
    G.map(({ key: M, link: Y }) => D.createElement("link", { key: M, ...Y })),
  );
}
function cy(...i) {
  return (s) => {
    i.forEach((o) => {
      typeof o == "function" ? o(s) : o != null && (o.current = s);
    });
  };
}
var sh =
  typeof window < "u" &&
  typeof window.document < "u" &&
  typeof window.document.createElement < "u";
try {
  sh && (window.__reactRouterVersion = "7.6.2");
} catch {}
function fy({ basename: i, children: s, window: o }) {
  let f = D.useRef();
  f.current == null && (f.current = $0({ window: o, v5Compat: !0 }));
  let d = f.current,
    [v, A] = D.useState({ action: d.action, location: d.location }),
    b = D.useCallback(
      (p) => {
        D.startTransition(() => A(p));
      },
      [A],
    );
  return (
    D.useLayoutEffect(() => d.listen(b), [d, b]),
    D.createElement(qv, {
      basename: i,
      children: s,
      location: v.location,
      navigationType: v.action,
      navigator: d,
    })
  );
}
var oh = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i,
  ai = D.forwardRef(function (
    {
      onClick: s,
      discover: o = "render",
      prefetch: f = "none",
      relative: d,
      reloadDocument: v,
      replace: A,
      state: b,
      target: p,
      to: h,
      preventScrollReset: O,
      viewTransition: j,
      ...z
    },
    G,
  ) {
    let { basename: M } = D.useContext(Ue),
      Y = typeof h == "string" && oh.test(h),
      Q,
      q = !1;
    if (typeof h == "string" && Y && ((Q = h), sh))
      try {
        let St = new URL(window.location.href),
          Jt = h.startsWith("//") ? new URL(St.protocol + h) : new URL(h),
          oe = ke(Jt.pathname, M);
        Jt.origin === St.origin && oe != null
          ? (h = oe + Jt.search + Jt.hash)
          : (q = !0);
      } catch {
        Ne(
          !1,
          `<Link to="${h}"> contains an invalid URL which will probably break when clicked - please update to a valid URL path.`,
        );
      }
    let V = Ev(h, { relative: d }),
      [P, ct, I] = ay(f, z),
      _t = oy(h, {
        replace: A,
        state: b,
        target: p,
        preventScrollReset: O,
        relative: d,
        viewTransition: j,
      });
    function xt(St) {
      (s && s(St), St.defaultPrevented || _t(St));
    }
    let zt = D.createElement("a", {
      ...z,
      ...I,
      href: Q || V,
      onClick: q || v ? s : xt,
      ref: cy(G, ct),
      target: p,
      "data-discover": !Y && o === "render" ? "true" : void 0,
    });
    return P && !Y
      ? D.createElement(D.Fragment, null, zt, D.createElement(uy, { page: V }))
      : zt;
  });
ai.displayName = "Link";
var xe = D.forwardRef(function (
  {
    "aria-current": s = "page",
    caseSensitive: o = !1,
    className: f = "",
    end: d = !1,
    style: v,
    to: A,
    viewTransition: b,
    children: p,
    ...h
  },
  O,
) {
  let j = Bu(A, { relative: h.relative }),
    z = Xl(),
    G = D.useContext(ni),
    { navigator: M, basename: Y } = D.useContext(Ue),
    Q = G != null && yy(j) && b === !0,
    q = M.encodeLocation ? M.encodeLocation(j).pathname : j.pathname,
    V = z.pathname,
    P =
      G && G.navigation && G.navigation.location
        ? G.navigation.location.pathname
        : null;
  (o ||
    ((V = V.toLowerCase()),
    (P = P ? P.toLowerCase() : null),
    (q = q.toLowerCase())),
    P && Y && (P = ke(P, Y) || P));
  const ct = q !== "/" && q.endsWith("/") ? q.length - 1 : q.length;
  let I = V === q || (!d && V.startsWith(q) && V.charAt(ct) === "/"),
    _t =
      P != null &&
      (P === q || (!d && P.startsWith(q) && P.charAt(q.length) === "/")),
    xt = { isActive: I, isPending: _t, isTransitioning: Q },
    zt = I ? s : void 0,
    St;
  typeof f == "function"
    ? (St = f(xt))
    : (St = [
        f,
        I ? "active" : null,
        _t ? "pending" : null,
        Q ? "transitioning" : null,
      ]
        .filter(Boolean)
        .join(" "));
  let Jt = typeof v == "function" ? v(xt) : v;
  return D.createElement(
    ai,
    {
      ...h,
      "aria-current": zt,
      className: St,
      ref: O,
      style: Jt,
      to: A,
      viewTransition: b,
    },
    typeof p == "function" ? p(xt) : p,
  );
});
xe.displayName = "NavLink";
var ry = D.forwardRef(
  (
    {
      discover: i = "render",
      fetcherKey: s,
      navigate: o,
      reloadDocument: f,
      replace: d,
      state: v,
      method: A = ti,
      action: b,
      onSubmit: p,
      relative: h,
      preventScrollReset: O,
      viewTransition: j,
      ...z
    },
    G,
  ) => {
    let M = my(),
      Y = vy(b, { relative: h }),
      Q = A.toLowerCase() === "get" ? "get" : "post",
      q = typeof b == "string" && oh.test(b),
      V = (P) => {
        if ((p && p(P), P.defaultPrevented)) return;
        P.preventDefault();
        let ct = P.nativeEvent.submitter,
          I = (ct == null ? void 0 : ct.getAttribute("formmethod")) || A;
        M(ct || P.currentTarget, {
          fetcherKey: s,
          method: I,
          navigate: o,
          replace: d,
          state: v,
          relative: h,
          preventScrollReset: O,
          viewTransition: j,
        });
      };
    return D.createElement("form", {
      ref: G,
      method: Q,
      action: Y,
      onSubmit: f ? p : V,
      ...z,
      "data-discover": !q && i === "render" ? "true" : void 0,
    });
  },
);
ry.displayName = "Form";
function sy(i) {
  return `${i} must be used within a data router.  See https://reactrouter.com/en/main/routers/picking-a-router.`;
}
function dh(i) {
  let s = D.useContext(_a);
  return (At(s, sy(i)), s);
}
function oy(
  i,
  {
    target: s,
    replace: o,
    state: f,
    preventScrollReset: d,
    relative: v,
    viewTransition: A,
  } = {},
) {
  let b = Tv(),
    p = Xl(),
    h = Bu(i, { relative: v });
  return D.useCallback(
    (O) => {
      if (Qv(O, s)) {
        O.preventDefault();
        let j = o !== void 0 ? o : Uu(p) === Uu(h);
        b(i, {
          replace: j,
          state: f,
          preventScrollReset: d,
          relative: v,
          viewTransition: A,
        });
      }
    },
    [p, b, h, o, f, s, i, d, v, A],
  );
}
var dy = 0,
  hy = () => `__${String(++dy)}__`;
function my() {
  let { router: i } = dh("useSubmit"),
    { basename: s } = D.useContext(Ue),
    o = Cv();
  return D.useCallback(
    async (f, d = {}) => {
      let { action: v, method: A, encType: b, formData: p, body: h } = Kv(f, s);
      if (d.navigate === !1) {
        let O = d.fetcherKey || hy();
        await i.fetch(O, o, d.action || v, {
          preventScrollReset: d.preventScrollReset,
          formData: p,
          body: h,
          formMethod: d.method || A,
          formEncType: d.encType || b,
          flushSync: d.flushSync,
        });
      } else
        await i.navigate(d.action || v, {
          preventScrollReset: d.preventScrollReset,
          formData: p,
          body: h,
          formMethod: d.method || A,
          formEncType: d.encType || b,
          replace: d.replace,
          state: d.state,
          fromRouteId: o,
          flushSync: d.flushSync,
          viewTransition: d.viewTransition,
        });
    },
    [i, s, o],
  );
}
function vy(i, { relative: s } = {}) {
  let { basename: o } = D.useContext(Ue),
    f = D.useContext(We);
  At(f, "useFormAction must be used inside a RouteContext");
  let [d] = f.matches.slice(-1),
    v = { ...Bu(i || ".", { relative: s }) },
    A = Xl();
  if (i == null) {
    v.search = A.search;
    let b = new URLSearchParams(v.search),
      p = b.getAll("index");
    if (p.some((O) => O === "")) {
      (b.delete("index"),
        p.filter((j) => j).forEach((j) => b.append("index", j)));
      let O = b.toString();
      v.search = O ? `?${O}` : "";
    }
  }
  return (
    (!i || i === ".") &&
      d.route.index &&
      (v.search = v.search ? v.search.replace(/^\?/, "?index&") : "?index"),
    o !== "/" && (v.pathname = v.pathname === "/" ? o : $e([o, v.pathname])),
    Uu(v)
  );
}
function yy(i, s = {}) {
  let o = D.useContext(ah);
  At(
    o != null,
    "`useViewTransitionState` must be used within `react-router-dom`'s `RouterProvider`.  Did you accidentally import `RouterProvider` from `react-router`?",
  );
  let { basename: f } = dh("useViewTransitionState"),
    d = Bu(i, { relative: s.relative });
  if (!o.isTransitioning) return !1;
  let v = ke(o.currentLocation.pathname, f) || o.currentLocation.pathname,
    A = ke(o.nextLocation.pathname, f) || o.nextLocation.pathname;
  return li(d.pathname, A) != null || li(d.pathname, v) != null;
}
[...ty];
function hh(i) {
  var s,
    o,
    f = "";
  if (typeof i == "string" || typeof i == "number") f += i;
  else if (typeof i == "object")
    if (Array.isArray(i)) {
      var d = i.length;
      for (s = 0; s < d; s++)
        i[s] && (o = hh(i[s])) && (f && (f += " "), (f += o));
    } else for (o in i) i[o] && (f && (f += " "), (f += o));
  return f;
}
function kt() {
  for (var i, s, o = 0, f = "", d = arguments.length; o < d; o++)
    (i = arguments[o]) && (s = hh(i)) && (f && (f += " "), (f += s));
  return f;
}
const mh = ({ variant: i, onClick: s, className: o, withShadow: f, ...d }) =>
    C.jsx("button", {
      className: `sw-button sw-button-${i || "default"} ${f && "sw-button-shadow"} ${o}`,
      onClick: s,
      children: d.children,
    }),
  kd = ({
    autoComplete: i,
    autoFocus: s,
    className: o,
    defaultValue: f,
    helperText: d,
    label: v,
    placeholder: A,
    type: b,
    value: p,
    error: h,
    InputRef: O,
    InputProps: j,
    ...z
  }) =>
    C.jsxs("div", {
      className: kt("sw-input", o),
      ...z,
      children: [
        v && C.jsx("p", { className: "sw-input-label", children: v }),
        C.jsx("input", {
          placeholder: A,
          type: b,
          autoComplete: i,
          autoFocus: s,
          defaultValue: f,
          value: p,
          className: kt("sw-input-input", !!h && "sw-input-shake"),
          ref: O,
          ...j,
        }),
        h && C.jsx("p", { className: "sw-input-error", children: h }),
        d && C.jsx("p", { className: "sw-input-helper", children: d }),
      ],
    }),
  gy = ({
    autoComplete: i,
    autoFocus: s,
    className: o,
    defaultValue: f,
    helperText: d,
    label: v,
    placeholder: A,
    rows: b,
    value: p,
    error: h,
    InputRef: O,
    InputProps: j,
    ...z
  }) =>
    C.jsxs("div", {
      className: kt("sw-input", o),
      ...z,
      children: [
        v && C.jsx("p", { className: "sw-input-label", children: v }),
        C.jsx("textarea", {
          placeholder: A,
          autoComplete: i,
          autoFocus: s,
          defaultValue: f,
          value: p,
          className: kt("sw-input-input", !!h && "sw-input-shake"),
          ref: O,
          rows: b,
          ...j,
        }),
        h && C.jsx("p", { className: "sw-input-error", children: h }),
        d && C.jsx("p", { className: "sw-input-helper", children: d }),
      ],
    }),
  Wd = ({
    className: i,
    defaultValue: s,
    helperText: o,
    label: f,
    value: d,
    error: v,
    InputRef: A,
    InputProps: b,
    ...p
  }) =>
    C.jsxs("div", {
      className: kt("sw-input", i),
      ...p,
      children: [
        f && C.jsx("p", { className: "sw-input-label", children: f }),
        C.jsx("input", {
          type: "datetime-local",
          defaultValue: s,
          value: d,
          className: kt("sw-input-input", !!v && "sw-input-shake"),
          ref: A,
          ...b,
        }),
        v && C.jsx("p", { className: "sw-input-error", children: v }),
        o && C.jsx("p", { className: "sw-input-helper", children: o }),
      ],
    }),
  Nf = "http://localhost:1978",
  Cu = {
    get: async (i) => {
      const s = await fetch(`${Nf}${i}`);
      if (!s.ok) throw new Error(`GET ${i} failed: ${s.status}`);
      return s.json();
    },
    post: async (i, s) => {
      const o = await fetch(`${Nf}${i}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(s),
      });
      if (!o.ok) throw new Error(`POST ${i} failed: ${o.status}`);
      return o.json();
    },
    delete: async (i) => {
      const s = await fetch(`${Nf}${i}`, { method: "DELETE" });
      if (!s.ok) throw new Error(`DELETE ${i} failed: ${s.status}`);
      return s.json();
    },
  },
  py = ({
    imageURLSrc: i,
    className: s,
    updateBanner: o,
    error: f,
    label: d,
    ...v
  }) => {
    const [A, b] = D.useState(!1),
      p = (h) => {
        b(!0);
        const O = h.target.files[0],
          j = new FormData();
        (j.append("image", O),
          Cu.post("/uploadImage/", j, {
            headers: { "content-type": "multipart/form-data" },
          })
            .then((z) => o(z))
            .catch((z) => alert(`Failed to add - ${z}`))
            .finally(() => b(!1)));
      };
    return i
      ? C.jsxs("div", {
          className: kt("sw-input sw-input-image sw-input-image-with", s),
          ...v,
          children: [
            d && C.jsx("p", { className: "sw-input-label", children: d }),
            C.jsx("label", {
              htmlFor: "image-upload",
              className: kt(!!f && "sw-input-error"),
              children: A ? "upload in process..." : "Click to upload image",
            }),
            C.jsx("input", {
              id: "image-upload",
              accept: "image/*",
              type: "file",
              hidden: !0,
              onChange: p,
              className: kt("sw-input-input", !!f && "sw-input-error"),
            }),
            C.jsx("img", { src: i, alt: d }),
            f && C.jsx("p", { className: "sw-input-error", children: f }),
          ],
        })
      : C.jsxs("div", {
          className: kt("sw-input sw-input-image sw-input-image-empty", s),
          ...v,
          children: [
            d && C.jsx("p", { className: "sw-input-label", children: d }),
            C.jsx("label", {
              htmlFor: "image-upload",
              className: kt(!!f && "sw-input-error"),
              children: A ? "upload in process..." : "Click to upload image",
            }),
            C.jsx("input", {
              id: "image-upload",
              accept: "image/*",
              type: "file",
              hidden: !0,
              onChange: p,
              className: kt("sw-input-input", !!f && "sw-input-error"),
            }),
            f && C.jsx("p", { className: "sw-input-error", children: f }),
          ],
        });
  },
  ui = ({
    justify: i,
    justifyBetween: s,
    justifyEnd: o,
    align: f,
    alignEnd: d,
    column: v,
    className: A,
    noShrink: b,
    fullWidth: p,
    autoWrap: h,
    ...O
  }) =>
    C.jsx("div", {
      className: kt(
        "sw-flexbox",
        v && "sw-flexbox-column",
        h && "sw-flexbox-wrap",
        i && "sw-flexbox-justify-center",
        s && "sw-flexbox-justify-between",
        o && "sw-flexbox-justify-end",
        f && "sw-flexbox-align-center",
        d && "sw-flexbox-align-end",
        p && "sw-flexbox-full-width",
        b && "sw-flexbox-no-shrink",
        A,
      ),
      ...O,
    });
ui.defaultProps = {
  className: "",
  column: !1,
  justify: !1,
  align: !1,
  noShrink: !1,
  justifyBetween: !1,
  fullWidth: !1,
  justifyEnd: !1,
  alignEnd: !1,
};
const Sy = () => {
    const [i, s] = D.useState(!1),
      o = (f) => {
        (f.target == f.currentTarget && s(!1), f.target.href != null && s(!1));
      };
    return C.jsxs("nav", {
      children: [
        C.jsx(ai, { to: "/", className: "icon", children: "Sparkonics" }),
        C.jsxs(ui, {
          className: "options-desktop",
          children: [
            C.jsx(xe, {
              to: "/about/",
              className: "option",
              children: "About us",
            }),
            C.jsx(xe, {
              to: "/events/",
              className: "option",
              children: "Events",
            }),
            C.jsx(xe, {
              to: "/projects/",
              className: "option",
              children: "Projects",
            }),
            C.jsx(xe, {
              to: "/team/",
              className: "option",
              children: "The Team",
            }),
            C.jsx(xe, {
              to: "/connect/",
              className: "option",
              children: "Join us",
            }),
          ],
        }),
        C.jsxs("div", {
          className: kt("hamburger", i && "hamburger-toggled"),
          "data-menu": "1",
          onClick: () => s(!i),
          children: [
            C.jsx("div", { className: "icon-left" }),
            C.jsx("div", { className: "icon-right" }),
          ],
        }),
        C.jsx("div", {
          className: kt("options-mobile-wrapper", i && "options-mobile-shown"),
          onClick: o,
          children: C.jsxs("div", {
            className: "options-mobile",
            children: [
              C.jsx(ai, { to: "/", className: "icon", children: "Sparkonics" }),
              C.jsxs(ui, {
                column: !0,
                className: "options-",
                children: [
                  C.jsx(xe, {
                    to: "/about/",
                    className: "option",
                    children: "About us",
                  }),
                  C.jsx(xe, {
                    to: "/events/",
                    className: "option",
                    children: "Events",
                  }),
                  C.jsx(xe, {
                    to: "/projects/",
                    className: "option",
                    children: "Projects",
                  }),
                  C.jsx(xe, {
                    to: "/team/",
                    className: "option",
                    children: "The Team",
                  }),
                  C.jsx(xe, {
                    to: "/connect/",
                    className: "option",
                    children: "Join us",
                  }),
                ],
              }),
            ],
          }),
        }),
      ],
    });
  },
  by = ({ show: i, onClose: s, contentClass: o, title: f, ...d }) => {
    const v = ({ key: b }) => {
      switch (b) {
        case "Escape":
          s();
          break;
      }
    };
    D.useEffect(() => {
      if (i) document.addEventListener("keydown", v);
      else return () => document.removeEventListener("keydown", v);
    }, [i]);
    const A = (b) => {
      b.currentTarget === b.target && s();
    };
    return C.jsx("div", {
      className: `modal_wrapper ${i ? "show" : ""}`,
      onClick: A,
      children: C.jsxs("div", {
        className: "modal_container",
        children: [
          C.jsxs("div", {
            className: "modal_title",
            children: [
              C.jsx("div", { className: "title", children: f }),
              C.jsx("div", {
                className: "close_icon",
                onClick: s,
                children: "Close",
              }),
            ],
          }),
          C.jsx("div", {
            className: `modal_content ${o}`,
            children: d.children,
          }),
        ],
      }),
    });
  },
  Ey = ({ show: i, onClose: s, onCreate: o, event: f, onModify: d }) => {
    const [v, A] = D.useState({}),
      [b, p] = D.useState({}),
      [h, O] = D.useState(!1);
    D.useEffect(() => {
      f && A(f);
    }, [i]);
    const j = (M) => (Y) => {
        A((Q) => ({ ...Q, [M]: Y.target.value }));
      },
      z = (M) => (Y) => {
        p((Q) => ({ ...Q, [M]: Y }));
      },
      G = () => {
        let M = !0;
        ((v.banner === void 0 || v.banner.trim() === "") &&
          (z("banner")("Missing event banner"), (M = !1)),
          (v.name === void 0 || v.name.trim() === "") &&
            (z("name")("Missing event name"), (M = !1)),
          (v.description === void 0 || v.description.trim() === "") &&
            (z("description")("Missing event description"), (M = !1)),
          (v.startTime === void 0 || v.startTime.trim() === "") &&
            (z("startTime")("Missing event start time"), (M = !1)),
          (v.endTime === void 0 || v.endTime.trim() === "") &&
            (z("endTime")("Missing event end time"), (M = !1)),
          (v.password === void 0 || v.password.trim() === "") &&
            (z("password")("Missing admin password"), (M = !1)),
          M !== !1 &&
            (O(!0),
            f
              ? Cu.post("/events/modify/", v)
                  .then((Y) => d(Y))
                  .catch((Y) => alert(`Failed to modify - ${Y}`))
                  .finally(() => O(!1))
              : Cu.post("/events/", v)
                  .then((Y) => o(Y))
                  .catch((Y) => alert(`Failed to add - ${Y}`))
                  .finally(() => O(!1))));
      };
    return C.jsxs(by, {
      show: i,
      onClose: () => {
        (A({}), p({}), s());
      },
      title: f ? "Modify event" : "Create event",
      children: [
        C.jsx(py, {
          label: "Event banner",
          imageURLSrc: v.banner,
          updateBanner: (M) => j("banner")({ target: { value: M } }),
          error: b.banner,
        }),
        C.jsx(kd, {
          label: "Event name",
          value: v.name,
          placeholder: "Event name here",
          type: "text",
          error: b.name,
          autoFocus: !0,
          onChange: (M) => {
            (b.name && M.target.value.trim() !== "" && z("name")(""),
              j("name")(M));
          },
        }),
        C.jsx(gy, {
          label: "Event description",
          value: v.description,
          placeholder: "Event description here",
          rows: 5,
          error: b.description,
          onChange: (M) => {
            (b.description &&
              M.target.value.trim() !== "" &&
              z("description")(""),
              j("description")(M));
          },
        }),
        C.jsx(Wd, {
          label: "Start time",
          value: v.startTime,
          error: b.startTime,
          onChange: (M) => {
            (b.startTime && M.target.value.trim() !== "" && z("startTime")(""),
              j("startTime")(M));
          },
        }),
        C.jsx(Wd, {
          label: "End time",
          value: v.endTime,
          onChange: (M) => {
            (b.endTime && M.target.value.trim() !== "" && z("endTime")(""),
              j("endTime")(M));
          },
          error: b.endTime,
          InputProps: { min: v.startTime, disabled: v.startTime === void 0 },
        }),
        C.jsx(kd, {
          label: "Admin password",
          value: v.password,
          placeholder: "The secret password",
          type: "password",
          error: b.password,
          onChange: (M) => {
            (b.password && M.target.value.trim() !== "" && z("password")(""),
              j("password")(M));
          },
        }),
        C.jsx(mh, {
          onClick: G,
          variant: "filled",
          children: h
            ? f
              ? "Modifying event"
              : "Adding event..."
            : f
              ? "Modify event"
              : "Create event",
        }),
      ],
    });
  },
  Ty = () => C.jsx("h1", { children: "Landing" }),
  Ay = () => C.jsx("h1", { children: "About us" }),
  xy = () => {
    const [i, s] = D.useState([]),
      [o, f] = D.useState([]),
      [d, v] = D.useState([]);
    return (
      D.useEffect(() => {
        BackendAPI.get("/events/")
          .then((j) => setEvents(j))
          .catch((j) => console.error(j));
        let A = [
          {
            id: 1,
            name: "Summer Kickoff",
            banner:
              "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee",
            description: "A festival to start the summer.",
            startTime: "2025-06-20T14:00",
            endTime: "2025-06-20T18:00",
          },
          {
            id: 2,
            name: "AI Webinar",
            banner: "https://images.unsplash.com/photo-1551434678-e076c223a692",
            description: "Deep dive into AI ethics.",
            startTime: "2025-06-21T16:00",
            endTime: "2025-06-21T17:00",
          },
          {
            id: 3,
            name: "Beach Clean-up Drive",
            banner:
              "https://images.unsplash.com/photo-1522199755839-a2bacb67c546",
            description: "Volunteers gather to clean up the local beaches.",
            startTime: "2025-06-22T08:00",
            endTime: "2025-06-22T11:30",
          },
          {
            id: 4,
            name: "Startup Showcase",
            banner:
              "https://images.unsplash.com/photo-1531058020387-3be344556be6",
            description: "Local startups pitch live to investors.",
            startTime: "2025-06-25T16:00",
            endTime: "2025-06-25T18:30",
          },
          {
            id: 5,
            name: "Open Mic Poetry",
            banner:
              "https://images.unsplash.com/photo-1481277542470-605612bd2d61",
            description: "An evening of expression and spoken word.",
            startTime: "2025-06-25T15:30",
            endTime: "2025-06-25T17:30",
          },
          {
            id: 6,
            name: "Live Coding Jam",
            banner:
              "https://images.unsplash.com/photo-1505238680356-667803448bb6",
            description: "Watch devs solve challenges live.",
            startTime: "2025-06-25T16:45",
            endTime: "2025-06-25T19:00",
          },
          {
            id: 7,
            name: "Music Night",
            banner:
              "https://images.unsplash.com/photo-1504384764586-bb4cdc1707b0",
            description: "Groove to indie and jazz performances.",
            startTime: "2025-06-25T19:30",
            endTime: "2025-06-25T22:00",
          },
          {
            id: 8,
            name: "3D Art Workshop",
            banner:
              "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40",
            description: "Learn sculpting in virtual space.",
            startTime: "2025-06-26T10:00",
            endTime: "2025-06-26T13:00",
          },
          {
            id: 9,
            name: "Night Sky Photography",
            banner:
              "https://images.unsplash.com/photo-1515165562835-c1c4e1387a5b",
            description: "Capture the Milky Way with guidance from pros.",
            startTime: "2025-06-27T21:00",
            endTime: "2025-06-27T23:59",
          },
        ];
        const b = new Date(),
          p = [],
          h = [],
          O = [];
        (A.map((j) => {
          const z = new Date(j.startTime),
            G = new Date(j.endTime);
          z > b ? p.push(j) : z <= b && b <= G ? h.push(j) : O.push(j);
        }),
          s(p),
          f(h),
          v(O));
      }, []),
      C.jsxs("div", {
        className: "events",
        children: [
          "Ongoing events: ",
          o.length,
          C.jsx("br", {}),
          "Upcoming events: ",
          i.length,
          C.jsx("br", {}),
          "Past events: ",
          d.length,
        ],
      })
    );
  },
  Ry = () => {
    const [i, s] = D.useState([]),
      [o, f] = D.useState(!1),
      [d, v] = D.useState(null);
    D.useEffect(() => {
      Cu.get("/api/events")
        .then((b) => s(b))
        .catch((b) => console.error(b));
    }, []);
    const A = (b) => {
      const p = {
        hour: "numeric",
        minute: "2-digit",
        hour12: !0,
        day: "2-digit",
        month: "long",
        year: "2-digit",
        timeZone: "Asia/Kolkata",
      };
      return new Date(b).toLocaleString("en-IN", p).replace(",", "");
    };
    return C.jsxs("div", {
      children: [
        C.jsx(ui, {
          justifyEnd: !0,
          fullWidth: !0,
          children: C.jsx(mh, {
            onClick: () => f(!0),
            variant: "filled",
            children: "Add event",
          }),
        }),
        C.jsx(Ey, {
          show: o || d,
          event: d,
          onClose: () => {
            (o && f(!1), d && v(null));
          },
          onCreate: (b) => s((p) => [b, ...p]),
          onModify: (b) => {
            s((p) => p.map((h) => (h.id === b.id ? b : h)));
          },
        }),
        C.jsxs("table", {
          className: "modify-events-table",
          children: [
            C.jsx("thead", {
              children: C.jsxs("tr", {
                children: [
                  C.jsx("td", { children: "ID" }),
                  C.jsx("td", { children: "Name" }),
                  C.jsx("td", { children: "Description" }),
                  C.jsx("td", { children: "Domains" }),
                  C.jsx("td", { children: "Status" }),
                  C.jsx("td", { children: "Start Time" }),
                  C.jsx("td", { children: "End Time" }),
                  C.jsx("td", { children: "Register Link" }),
                  C.jsx("td", { children: "Modify" }),
                  C.jsx("td", { children: "Delete" }),
                ],
              }),
            }),
            C.jsx("tbody", {
              children: i.map((b) =>
                C.jsxs(
                  "tr",
                  {
                    children: [
                      C.jsx("td", { children: b.EventId }),
                      C.jsx("td", { children: b.EventName }),
                      C.jsx("td", { children: b.EventDescription }),
                      C.jsx("td", { children: b.EventDomains }),
                      C.jsx("td", { children: b.EventStatus }),
                      C.jsx("td", { children: A(b.EventStartTime) }),
                      C.jsx("td", { children: A(b.EventEndTime) }),
                      C.jsx("td", {
                        children: C.jsx("a", {
                          href: b.EventRegisterLink,
                          target: "_blank",
                          rel: "noreferrer",
                          children: "Link",
                        }),
                      }),
                      C.jsx("td", {
                        className: "table-control",
                        onClick: () => v(b),
                        children: "[modify]",
                      }),
                      C.jsx("td", {
                        className: "table-control",
                        onClick: async () => {
                          if (window.confirm(`Delete event "${b.EventName}"?`))
                            try {
                              (await Cu.delete(`/events/${b.EventId}`),
                                s((h) =>
                                  h.filter((O) => O.EventId !== b.EventId),
                                ));
                            } catch (h) {
                              (console.error("Failed to delete event:", h),
                                alert(
                                  "Failed to delete the event. Please try again.",
                                ));
                            }
                        },
                        children: "[delete]",
                      }),
                    ],
                  },
                  b.EventId,
                ),
              ),
            }),
          ],
        }),
      ],
    });
  };
function Oy() {
  return C.jsxs(C.Fragment, {
    children: [
      C.jsx(Sy, {}),
      C.jsx("div", {
        className: "main",
        children: C.jsxs(Yv, {
          children: [
            C.jsx(Ma, { index: !0, element: C.jsx(Ty, {}) }),
            C.jsx(Ma, { path: "about", element: C.jsx(Ay, {}) }),
            C.jsxs(Ma, {
              path: "events",
              children: [
                C.jsx(Ma, { index: !0, element: C.jsx(xy, {}) }),
                C.jsx(Ma, { path: "modify", element: C.jsx(Ry, {}) }),
              ],
            }),
          ],
        }),
      }),
    ],
  });
}
K0.createRoot(document.getElementById("root")).render(
  C.jsx(D.StrictMode, { children: C.jsx(fy, { children: C.jsx(Oy, {}) }) }),
);
